...
WebRTC (Web Real-Time Communication) वेब ब्राउज़र को बिना किसी प्लगइन और बिना किसी बाहरी softphone के कॉल करने और प्राप्त करने की सुविधा देता है — बस JavaScript, एक माइक्रोफ़ोन और Asterisk के साथ एक सुरक्षित कनेक्शन की आवश्यकता होती है। Asterisk 11 के बाद से Asterisk एक WebRTC सर्वर के रूप में कार्य करने में सक्षम है, और Asterisk 12 में PJSIP स्टैक (`res_pjsip`) के आने के बाद से यह ऐसा करने का अनुशंसित तरीका रहा है; Asterisk 22 में कॉन्फ़िगरेशन कुछ चुनिंदा और अच्छी तरह से समझे गए विकल्पों तक सीमित हो गया है। यह अध्याय दिखाता है कि कैसे एक PJSIP endpoint को ब्राउज़र फ़ोन में बदला जाए, सुरक्षित मीडिया पाथ कैसे काम करता है, और आपको कब Asterisk के इन-बिल्ट WebRTC सपोर्ट का उपयोग करना चाहिए और कब एक समर्पित गेटवे का।

इस अध्याय की हर चीज़ को पुस्तक की Asterisk 22 लैब के विरुद्ध सत्यापित किया गया है; दिखाया गया कॉन्फ़िगरेशन `lab/asterisk/etc` में दिए गए कॉन्फ़िगरेशन के समान है।
...
- वेबसाइट पर **Click-to-call** — एक विज़िटर वेब पेज से किसी कतार (queue) या extension पर कॉल करता है।
...
- आपके अपने वेब एप्लिकेशन में **Embedded calling** — उदाहरण के लिए, Asterisk से बात करने वाला SipPulse वेब softphone।
- **Zero-install internal phones** — कर्मचारी हार्डवेयर फ़ोन या इंस्टॉल किए गए क्लाइंट के बजाय ब्राउज़र टैब का उपयोग करते हैं।

इसका सबसे बड़ा लाभ इसकी पहुँच है: हर आधुनिक ब्राउज़र पहले से ही WebRTC का समर्थन करता है। इसकी कीमत यह है कि WebRTC सख्त है — इसे एन्क्रिप्टेड मीडिया और एक सुरक्षित ट्रांसपोर्ट की *आवश्यकता* होती है, इसलिए सामान्य UDP SIP फ़ोन की तुलना में कॉन्फ़िगर करने के लिए इसमें अधिक चीजें होती हैं।

## WebRTC, सामान्य SIP से कैसे भिन्न है

एक सामान्य SIP फ़ोन UDP/TCP पर सिग्नलिंग करता है और आमतौर पर ऑडियो को सामान्य RTP के रूप में ले जाता है। एक WebRTC ब्राउज़र क्लाइंट तीन महत्वपूर्ण तरीकों से अलग है, और Asterisk को प्रत्येक से मेल खाना चाहिए:
...
- **मीडिया हमेशा DTLS-SRTP के साथ एन्क्रिप्टेड होता है।** ब्राउज़र सामान्य RTP को अस्वीकार कर देते हैं। दोनों पक्ष एक DTLS हैंडशेक (SDP में एक्सचेंज किए गए सर्टिफिकेट फिंगरप्रिंट द्वारा प्रमाणित) करते हैं और उससे SRTP कुंजियाँ प्राप्त करते हैं।
...
> **[2nd-ed note]** लैब `lab/make-certs.sh` के साथ आती है, जो `CN=localhost` के साथ एक सेल्फ-साइन्ड सर्टिफिकेट उत्पन्न करती है। सार्वजनिक परिनियोजन के लिए, Let's Encrypt प्रवाह का दस्तावेजीकरण करें और `tlscertfile`/`tlsprivatekey` को जारी की गई फ़ाइलों पर इंगित करें।

यह सर्टिफिकेट मीडिया को एन्क्रिप्ट करने के लिए उपयोग किए जाने वाले DTLS सर्टिफिकेट के समान **नहीं** है — Asterisk उसे स्वचालित रूप से उत्पन्न करता है, जैसा कि हम देखेंगे।
...
> **[2nd-ed note]** `wss` ट्रांसपोर्ट एक `0.0.0.0:5060` बाइंड एड्रेस प्रिंट करता है, भले ही वास्तविक WebSocket पोर्ट 8089 पर HTTP सर्वर द्वारा परोसा जाता हो। यह अपेक्षित है: `wss` ट्रांसपोर्ट `res_http_websocket` पर एक पतली परत है; इस पर `bind` लाइन प्रभावी रूप से कॉस्मेटिक है। अंतिम पाठ में एक वाक्य जोड़ने योग्य है ताकि पाठक पोर्ट से भ्रमित न हों।

## चरण 3 — WebRTC endpoint

अब endpoint स्वयं। कुंजी `webrtc=yes` है:
...
`webrtc=yes` एक सुविधा स्विच है। यह हाथ से WebRTC-आवश्यक सभी विकल्पों को सेट करने के बराबर है। आप पुष्टि कर सकते हैं कि इसने वास्तव में क्या चालू किया है:
...
- `use_avpf: true` — AVPF RTP प्रोफ़ाइल (फीडबैक), जो WebRTC के लिए आवश्यक है।

`allow=opus` की सिफारिश की जाती है — Opus वह कोडेक है जिसे ब्राउज़र पसंद करते हैं। Asterisk 22 कोर में Opus *passthrough* (`res_format_attr_opus` मॉड्यूल) के साथ आता है, जो बिना री-एनकोडिंग के दो Opus-सक्षम लेग्स के बीच Opus को रिले करने के लिए पर्याप्त है। Opus को किसी अन्य कोडेक में *ट्रांसकोड* करने के लिए अलग `codec_opus` मॉड्यूल की आवश्यकता होती है, जिसे आधिकारिक WebRTC गाइड वैकल्पिक लेकिन अत्यधिक अनुशंसित के रूप में सूचीबद्ध करता है और जिसे आप बेस बिल्ड के ऊपर इंस्टॉल करते हैं; *Designing a VoIP network* में कोडेक चर्चा देखें। गैर-WebRTC लेग्स के लिए ब्रिजिंग के लिए `ulaw` को फॉलबैक के रूप में रखें जो Opus नहीं बोल सकते।

## चरण 4 — ICE, STUN और TURN

एक फ्लैट LAN पर, होस्ट उम्मीदवारों के साथ ICE पर्याप्त है और किसी और चीज की आवश्यकता नहीं है। इंटरनेट पर आप आमतौर पर एक STUN सर्वर जोड़ते हैं ताकि Asterisk और ब्राउज़र अपने सार्वजनिक पते खोज सकें, और उन मामलों के लिए एक TURN सर्वर जहाँ सीधा मीडिया असंभव है (symmetric NAT, प्रतिबंधात्मक फ़ायरवॉल)। उन्हें `rtp.conf` में Asterisk पर इंगित करें:
...
ब्राउज़र को JavaScript (`RTCPeerConnection` `iceServers` सूची) में अपने स्वयं के ICE सर्वर के साथ कॉन्फ़िगर किया गया है। पूरी तरह से आंतरिक परिनियोजन के लिए आप STUN/TURN को पूरी तरह से छोड़ सकते हैं।

> **[2nd-ed note]** सत्यापित करें कि क्या प्रोडक्शन के लिए सेल्फ-होस्टेड coturn की सिफारिश करनी है (अधिकांश वास्तविक परिनियोजन को TURN की आवश्यकता होती है)। यदि ऐसा है, तो एक छोटा coturn कॉन्फ़िगरेशन उदाहरण जोड़ें।
...
- **सुरक्षित संदर्भ।** `getUserMedia` (माइक्रोफ़ोन एक्सेस) केवल `https://` पेजों या `http://localhost` पर काम करता है। प्रोडक्शन में HTTPS पर पेज सर्व करें।
...
SipPulse वेब softphone इन समान प्रिमिटिव्स पर निर्मित एक प्रोडक्शन-ग्रेड संदर्भ क्लाइंट है।
...
यदि ऑडियो वन-वे है या अनुपस्थित है, तो यह लगभग हमेशा ICE या सर्टिफिकेट होता है — नीचे ट्रबलशूटिंग देखें।

## Asterisk WebRTC बनाम मीडिया गेटवे

Asterisk सीधे WebRTC को समाप्त कर सकता है, लेकिन यह हमेशा सही उपकरण नहीं होता है:

- **Asterisk-native WebRTC का उपयोग करें** जब ब्राउज़र आपके PBX पर एक *फ़ोन* हो — एक एजेंट, एक आंतरिक extension, एक क्लिक-टू-कॉल जो आपके dialplan में आता है। ब्राउज़र सिर्फ एक और endpoint है और सब कुछ (कतारें, voicemail, IVR) काम करता है।
- **एक समर्पित गेटवे (जैसे Janus) का उपयोग करें** जब आपको कॉल कंट्रोल से स्वतंत्र रूप से कई ब्राउज़र सत्रों को स्केल करने, बड़े सम्मेलनों/स्ट्रीमिंग के लिए चयनात्मक अग्रेषण करने, या मीडिया प्लेन को PBX से अलग रखने की आवश्यकता हो। एक गेटवे WebRTC को सामान्य SIP से जोड़ता है, और Asterisk तब एक सामान्य SIP लेग देखता है।

कई वास्तविक सिस्टम दोनों को जोड़ते हैं: कॉल कंट्रोल के लिए Asterisk, ब्राउज़र-साइड मीडिया स्केलिंग के लिए एक गेटवे। (यह SipPulse के अपने स्टैक के पीछे का आर्किटेक्चर है।)

## ट्रबलशूटिंग

- **WebSocket कनेक्ट नहीं होगा:** ब्राउज़र ने TLS सर्टिफिकेट को अस्वीकार कर दिया। सीधे `https://host:8089/ws` खोलें और इसे स्वीकार करें, या एक विश्वसनीय सर्टिफिकेट इंस्टॉल करें।
...
- **उत्तर देने पर कॉल ड्रॉप:** DTLS हैंडशेक विफल रहा; पुष्टि करें कि `dtls_auto_generate_cert`, `Yes` है और सिस्टम घड़ी सही है (सर्टिफिकेट समय-संवेदनशील होते हैं)।
...
3. `webrtc-1000` के रूप में रजिस्टर करें और `600` (इको टेस्ट) पर कॉल करें — आपको खुद को सुनना चाहिए।
...
WebRTC एक ब्राउज़र को प्रथम श्रेणी के Asterisk endpoint में बदल देता है। नुस्खा छोटा लेकिन सख्त है: TLS के साथ HTTP सर्वर को सक्षम करें ताकि ब्राउज़र एक सुरक्षित WebSocket खोल सके, एक `wss` PJSIP ट्रांसपोर्ट जोड़ें, और endpoint पर `webrtc=yes` सेट करें — जो DTLS-SRTP (स्वचालित रूप से उत्पन्न सर्टिफिकेट के साथ), ICE, RTP/RTCP मल्टीप्लेक्सिंग और AVPF प्रोफ़ाइल को चालू करता है। NAT को पार करते समय STUN/TURN जोड़ें, अपने पेज को HTTPS पर सर्व करें, और एक SIP.js (या JsSIP) क्लाइंट को `wss://asterisk:8089/ws` पर इंगित करें। अपने PBX पर ब्राउज़र फ़ोन के लिए, Asterisk-native WebRTC सबसे सरल रास्ता है; बड़े पैमाने पर मीडिया के लिए, इसे एक गेटवे के साथ जोड़ें।
...
5. निम्नलिखित में से कौन सा `webrtc=yes` डिफ़ॉल्ट रूप से चालू करता है? (सभी लागू विकल्प चुनें।)
...
7. `rtp.conf` में, कौन सी दो सेटिंग्स Asterisk को एक बाहरी सर्वर पर इंगित करती हैं ताकि वह अपना सार्वजनिक पता खोज सके और सीधे रास्ते विफल होने पर मीडिया को रिले कर सके? (सभी लागू विकल्प चुनें।)
   - A. `icesupport=yes`
   - B. `stunaddr=`
   - C. `turnaddr=`
   - D. `tlsbindaddr=`

8. वह URL पाथ जिसे Asterisk का `res_http_websocket` WebRTC सिग्नलिंग के लिए एक्सपोज़ करता है, ________ है।

9. अध्याय के अनुसार, आपको Asterisk-native WebRTC के बजाय एक समर्पित मीडिया गेटवे (जैसे Janus) के लिए कब जाना चाहिए?
   - A. जब भी किसी ब्राउज़र को कॉल करने की आवश्यकता हो
   - B. जब आपको कॉल कंट्रोल से स्वतंत्र रूप से कई ब्राउज़र मीडिया सत्रों को स्केल करना हो, बड़े सम्मेलनों के लिए चयनात्मक अग्रेषण करना हो, या मीडिया प्लेन को PBX से अलग रखना हो
...
**उत्तर:** 1 — B · 2 — B · 3 — गलत (Asterisk स्वचालित रूप से DTLS सर्टिफिकेट उत्पन्न करता है; `dtls_auto_generate_cert: Yes`) · 4 — D · 5 — A, B, C, D · 6 — ICE · 7 — B, C · 8 — `/ws` · 9 — B · 10 — गलत (सुरक्षित संदर्भ आवश्यक है: `getUserMedia` केवल `https://` या `http://localhost` पर काम करता है)

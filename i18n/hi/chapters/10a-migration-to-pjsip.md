...
- यह समझाने में कि Asterisk 22 में `chan_sip` क्यों चला गया है और उसकी जगह क्या आया है
- `sip.conf` पीयर/यूजर/फ्रेंड मॉडल को PJSIP ऑब्जेक्ट मॉडल (endpoint + aor + auth + identify + transport + registration) पर मैप करने में
- `sip_to_pjsip.py` कन्वर्जन स्क्रिप्ट चलाने और उसके आउटपुट की समीक्षा करने में
- सामान्य डिवाइस प्रकारों (रजिस्टरिंग फोन, इनबाउंड trunk, आउटबाउंड रजिस्ट्रेशन) को `sip.conf` से `pjsip.conf` में मैन्युअल रूप से अनुवाद करने में
- NAT, मीडिया, DTMF, codec, और ऑथेंटिकेशन सेटिंग्स को विकल्प-दर-विकल्प माइग्रेट करने में
- dialplan (`SIP/` → `PJSIP/`) और CLI (`sip show` → `pjsip show`) को अपडेट करने में
- realtime/ARA डिप्लॉयमेंट को `sippeers`/`sipregs` से Sorcery `ps_*` टेबल्स में माइग्रेट करने में
- माइग्रेशन चेकलिस्ट के माध्यम से काम करने और क्लासिक नुकसानों से बचने में

## Why migrate at all

`chan_sip` ने लगभग दो दशकों तक Asterisk की सेवा की, लेकिन इसमें आर्किटेक्चरल ऋण था: एक अखंड मॉड्यूल, प्रति डिवाइस एक सिंगल कॉन्फ़िगरेशन ब्लॉक, कमजोर मल्टी-ट्रांसपोर्ट सपोर्ट, और एक SIP स्टैक जो RFCs से पीछे रह गया था। **PJSIP** — जिसे Teluu के परिपक्व pjproject स्टैक पर बनाया गया और Asterisk 12 में पेश किया गया — इसका पूरी तरह से प्रतिस्थापन था। Asterisk 21 तक Asterisk प्रोजेक्ट ने काम पूरा कर लिया और `chan_sip` को ट्री से हटा दिया।
...
`chan_sip` *0 modules loaded* लौटाता है — यह वहां मौजूद ही नहीं है। PJSIP के अलावा माइग्रेट करने के लिए कुछ भी नहीं है, इसलिए एकमात्र वास्तविक सवाल *कैसे* है, *क्या* नहीं।

## The conceptual mapping: there is no single "peer"

मानसिक बदलाव जो `sip.conf` से आने वाले हर व्यक्ति को परेशान करता है, वह यह है: **PJSIP में कोई `[peer]` नहीं है।** `sip.conf` में एक ब्रैकेटेड ब्लॉक — एक `peer`, एक `user`, या एक `friend` — डिवाइस के बारे में *सब कुछ* वर्णित करता था: उसके क्रेडेंशियल्स, उसे कहाँ तक पहुँचना है, उसके codecs, उसका NAT व्यवहार, उसका dialplan context। PJSIP जानबूझकर उस सिंगल ब्लॉक को कई छोटे, एकल-उद्देश्य वाले ऑब्जेक्ट्स में तोड़ देता है, जिनमें से प्रत्येक को एक `type=` के साथ टैग किया जाता है, जो *एक-दूसरे को नाम से संदर्भित करते हैं*:
...
| `aor` (Address of Record) | डिवाइस तक *कहाँ* पहुँचना है — रजिस्टर्ड या स्टैटिक कॉन्टैक्ट्स, `max_contacts`, qualify |
| `auth` | इनबाउंड और/या आउटबाउंड ऑथेंटिकेशन के लिए क्रेडेंशियल्स (यूजरनेम/पासवर्ड) |
| `identify` | इनबाउंड रिक्वेस्ट को `From` यूजर के बजाय **सोर्स IP** द्वारा endpoint से मैच करना |
...
`friend`/`peer`/`user` का अंतर पूरी तरह से गायब हो जाता है — PJSIP में सब कुछ एक `endpoint` है। इसलिए एक सिंगल `sip.conf` फ्रेंड, आमतौर पर, तीन ऑब्जेक्ट्स (`endpoint` + `auth` + `aor`) बन जाता है जो एक नाम साझा करते हैं और एक-दूसरे की ओर इशारा करते हैं:
...
Endpoint गोंद (glue) है। यह एक `transport` (या डिफ़ॉल्ट को इनहेरिट करता है), एक `auth` ऑब्जेक्ट, और एक या अधिक `aors` को नाम देता है। ऑब्जेक्ट मॉडल को *SIP & PJSIP in depth* में गहराई से कवर किया गया है; यहाँ हमें बस हर अनुवाद के लक्ष्य के रूप में इसकी आवश्यकता है।

## The `sip_to_pjsip.py` conversion tool

Asterisk एक Python स्क्रिप्ट प्रदान करता है जो एक मौजूदा `sip.conf` को पढ़ती है और एक `pjsip.conf` लिखती है। यह CLI कमांड के रूप में नहीं चलती है — यह **Asterisk सोर्स ट्री** में रहती है, न कि इंस्टॉल की गई बाइनरी में:

```
${ASTERISK_SRC}/contrib/scripts/sip_to_pjsip/sip_to_pjsip.py
```

लैब के Asterisk 22.10.0 पर पूरा पथ, उदाहरण के लिए, `/usr/src/asterisk-22.10.0/contrib/scripts/sip_to_pjsip/sip_to_pjsip.py` है। उसी डायरेक्टरी में `sip_to_pjsql.py` (realtime/SQL वैरिएंट, बाद में कवर किया गया) और हेल्पर मॉड्यूल `astconfigparser.py`, `astdicts.py`, और `sqlconfigparser.py` होते हैं।
...
यह इनपुट पढ़ती है, `Converting to PJSIP...` प्रिंट करती है, और आउटपुट फाइल लिखती है। आंतरिक रूप से यह हर `sip.conf` सेक्शन में चलती है और, प्रति डिवाइस, मिलान वाले `endpoint`, `auth`, `aor`, `registration`, और (जहाँ यह उनका अनुमान लगा सकती है) `transport` ऑब्जेक्ट्स उत्सर्जित करती है, जो अगले सेक्शन में विकल्प मैपिंग को स्वचालित रूप से लागू करती है।

### What it does — and its limits

आउटपुट को **पहला ड्राफ्ट मानें, तैयार फाइल नहीं।** स्क्रिप्ट अपने स्वयं के अंतराल के बारे में ईमानदार है: जो कुछ भी यह स्पष्ट रूप से मैप नहीं कर सकती है, उसे आउटपुट फाइल के शीर्ष पर स्पष्ट रूप से फेन्स्ड ब्लॉक में लिखा जाता है:
...
उस वास्तविक टुकड़े में ध्यान दें कि `qualify = yes` एक `sip.conf` पीयर से *non-mapped* ब्लॉक में चला गया — क्योंकि PJSIP `qualify_frequency` (सेकंड) के साथ **aor** पर क्वालिफाई करता है, न कि डिवाइस पर बूलियन के साथ, स्क्रिप्ट इसे आपके द्वारा जानबूझकर सेट करने के लिए छोड़ देती है। योजना बनाने के लिए व्यावहारिक सीमाएँ:

- **ट्रांसपोर्ट का अनुमान लगाया जाता है, डिज़ाइन नहीं किया जाता।** स्क्रिप्ट `bindport`/`bindaddr` से एक बेसिक `transport-udp` उत्सर्जित करती है, लेकिन यह आपके TLS certs, आपके TCP आवश्यकताओं, या आपके मल्टी-बाइंड लेआउट को नहीं जान सकती है। ट्रांसपोर्ट की समीक्षा करें और उसे फिर से लिखें।
...
- **`qualify`, कस्टम टाइमर, और मुट्ठी भर विकल्प "non-mapped" में चले जाते हैं।** उस ब्लॉक को ऊपर से नीचे तक पढ़ें और प्रत्येक पर निर्णय लें।
...
इसलिए वर्कफ़्लो यह है: स्क्रिप्ट को एक *स्क्रैच* फाइल में चलाएं, इसे डिफ (diff) और रिव्यू करें, अच्छे हिस्सों को अपने वास्तविक `pjsip.conf` में डालें, फिर प्रोडक्शन से पहले पूरी तरह से परीक्षण करें।
...
सबसे सामान्य डिवाइस: एक डेस्क फोन या softphone जो एक सीक्रेट के साथ लॉग इन करता है और अपना स्थान रजिस्टर करता है।
...
एक प्रोवाइडर जो आपको ज्ञात IP एड्रेस से कॉल भेजता है। यहाँ कोई रजिस्ट्रेशन नहीं है — आप `identify` का उपयोग करके *कैरियर के ट्रैफ़िक को उसके सोर्स IP द्वारा* ऑथेंटिकेट करते हैं।
...
महत्वपूर्ण अनुवाद **`insecure=invite` → `identify`** है। `chan_sip` में, `insecure=invite` ने Asterisk को बताया "ऑथेंटिकेशन के लिए इस पीयर से इनबाउंड INVITEs को चुनौती न दें।" PJSIP उसी प्रभाव को `type=identify`/`match=` के साथ *सोर्स IP को endpoint से मैच करके* प्राप्त करता है, जो अधिक स्पष्ट और अधिक सुरक्षित दोनों है। स्टैटिक `host=` `aor` पर एक स्थायी `contact=` बन जाता है ताकि आप कैरियर को *आउट* भी डायल कर सकें। `match=` एक IP, एक CIDR रेंज, या एक होस्टनेम स्वीकार करता है (कॉन्फ़िग-लोड समय पर रिज़ॉल्व किया गया — यदि प्रोवाइडर का IP बदलता है तो रिलोड करें)।

### An outbound registration (`register =>`)

जब प्रोवाइडर चाहता है कि *आप* उनके पास लॉग इन करें, तो `chan_sip` ने `[general]` में एक सिंगल `register =>` लाइन का उपयोग किया। PJSIP इसे एक समर्पित `type=registration` ऑब्जेक्ट और एक `outbound_auth` के साथ बदल देता है।
...
`register =>` फ़ील्ड्स को एक-से-एक मैप करें: `1020:supersecret` क्रेडेंशियल्स `auth` ऑब्जेक्ट (`outbound_auth` के रूप में संदर्भित) बन जाते हैं; `@sip.example.com:5600` `server_uri` बन जाता है; `/9999` सफ़िक्स — वह यूजर पार्ट जिसे प्रोवाइडर इनबाउंड कॉल डिलीवर करता है — `contact_user=9999` बन जाता है। `defaultuser`/`fromuser` और `fromdomain` endpoint पर `from_user` और `from_domain` बन जाते हैं। ध्यान दें कि `outbound_auth` *दो बार* दिखाई देता है: रजिस्ट्रेशन इसे REGISTER के लिए उपयोग करता है, endpoint इसे आउटबाउंड INVITEs पर `407` चुनौती का उत्तर देने के लिए उपयोग करता है।
...
| `insecure=invite` | ऑथ छोड़ें; `type=identify` + `match=` का उपयोग करें | identify |
...
`chan_sip` का `secret=` एक `type=auth` ऑब्जेक्ट का `password=` फ़ील्ड बन जाता है। **ऑथेंटिकेशन मेथड** को `auth_type` के साथ सेट किया जाता है। `auth_type=digest` का उपयोग करें। पुराने मान `userpass` और `md5` अभी भी काम करते हैं लेकिन **deprecated हैं और चुपचाप `digest` में बदल दिए जाते हैं** — सीधे लैब से सत्यापित:
...
आप पुराने कॉन्फ़िग्स और कन्वर्जन स्क्रिप्ट के आउटपुट (और इस पुस्तक के पिछले अध्यायों) में `auth_type=userpass` देखेंगे। यह हानिरहित है, लेकिन किसी भी नई चीज़ में `digest` लिखें।

### NAT, media, and DTMF in detail

ये तीन वे हैं जहाँ से अधिकांश माइग्रेशन-बाद की "यह रजिस्टर होता है लेकिन ऑडियो नहीं है" टिकटें आती हैं। `chan_sip` शॉर्टहैंड `nat=force_rport,comedia` ने तीन व्यवहारों को एक विकल्प में पैक किया; PJSIP उन्हें विभाजित करता है ताकि आप प्रत्येक के बारे में तर्क कर सकें:
...
**मीडिया** के लिए, `directmedia` `direct_media` बन जाता है (अंडरस्कोर ही पूरा बदलाव है); जब भी कॉल को Asterisk पर एंकर किया जाना चाहिए — NAT के पार, या रिकॉर्ड/ट्रांसकोड/ट्रांसफर करने के लिए — `direct_media=no` रखें। **DTMF** के लिए, RFC को रीनंबर किया गया था: `chan_sip` का `dtmfmode=rfc2833` PJSIP का `dtmf_mode=rfc4733` है (समान आउट-ऑफ-बैंड टेलीफोन-इवेंट मैकेनिज्म, वर्तमान RFC नंबर)। लैब पुष्टि करती है कि वैध `dtmf_mode` मान `rfc4733`, `inband`, `info`, `auto`, और `auto_info` हैं, जो डिफ़ॉल्ट रूप से `rfc4733` होते हैं।
...
माइग्रेशन `pjsip.conf` पर नहीं रुकता है। दैनिक उपयोग में दो चीजें बदलती हैं।
...
यदि आपने डेटाबेस (Asterisk Realtime Architecture) से `chan_sip` चलाया, तो आपके डिवाइस `sippeers` टेबल में और रजिस्ट्रेशन `sipregs` में रहते थे। PJSIP एक पूरी तरह से अलग स्टोरेज लेयर का उपयोग करता है — **Sorcery** — जिसमें *प्रति ऑब्जेक्ट प्रकार* एक टेबल होती है। मैपिंग:
...
| — (IP मैचिंग) | `ps_endpoint_id_ips` (`identify` ऑब्जेक्ट्स) |
| — (डोमेन उपनाम) | `ps_domain_aliases` |

वैचारिक विभाजन फ्लैट-फाइल मामले के समान है: एक `sippeers` पंक्ति तीन तालिकाओं (`ps_endpoints` + `ps_aors` + `ps_auths`) में *तीन* पंक्तियाँ बन जाती है जो endpoint नाम द्वारा एक-दूसरे को संदर्भित करती हैं।
...
- **एक SQL कन्वर्जन स्क्रिप्ट है।** `sip_to_pjsip.py` के साथ उसी `contrib/scripts/sip_to_pjsip/` डायरेक्टरी में **`sip_to_pjsql.py`** स्थित है; यह उसी `convert()` लॉजिक का पुन: उपयोग करती है लेकिन फ्लैट कॉन्फ़िग फाइल के बजाय `ps_*` टेबल्स के लिए `INSERT` स्टेटमेंट्स की एक `pjsip.sql` फाइल उत्सर्जित करती है। फ्लैट-फाइल टूल की तरह, इसे लोड करने से पहले आउटपुट की समीक्षा करें।

अंत में, `sorcery.conf` को अपने डेटाबेस की ओर इंगित करें ताकि PJSIP `ps_*` टेबल्स से (`res_config_odbc` / `res_pjsip_realtime` के माध्यम से) endpoints, aors, auths, और कॉन्टैक्ट्स को पढ़े, ठीक वैसे ही जैसे `extconfig.conf` ने कभी `chan_sip` के लिए डेटाबेस पर `sippeers` को इंगित किया था। Realtime मैकेनिक्स को *Realtime* अध्याय में कवर किया गया है; माइग्रेशन-विशिष्ट बिंदु केवल यह है कि *कौन सी टेबल किसमें मैप होती है*।
...
2. **कनवर्टर को स्क्रैच फाइल में चलाएं।** `sip_to_pjsip.py sip.conf pjsip_generated.conf`। इसे अपने लाइव `pjsip.conf` पर इंगित **न करें**।
...
5. **ऑथ सत्यापित करें।** हर `auth` ऑब्जेक्ट पर `auth_type=digest`, यूजरनेम, और पासवर्ड की पुष्टि करें।
...
- **`alwaysauthreject` अब इन-बिल्ट है — इसे न खोजें।** `chan_sip` को `alwaysauthreject=yes` की आवश्यकता थी ताकि यह खराब यूजरनेम का अलग तरह से जवाब देकर यह लीक न करे कि कौन से एक्सटेंशन मौजूद थे। PJSIP डिज़ाइन द्वारा सुरक्षित काम करता है: यह कभी नहीं बताता कि कोई endpoint मौजूद है या नहीं। सेट करने के लिए कोई `alwaysauthreject` विकल्प नहीं है। संबंधित सुरक्षा — अज्ञात प्रेषकों को थ्रॉटल करना — डिफ़ॉल्ट रूप से चालू, ग्लोबल `unidentified_request_count` / `unidentified_request_period` है।

- **`insecure=invite` एक PJSIP विकल्प नहीं है — `identify` का उपयोग करें।** `pjsip.conf` में कोई `insecure=` नहीं है। ज्ञात कैरियर से अनऑथेंटिकेटेड INVITEs स्वीकार करने का तरीका यह है कि `type=identify` / `match=` के साथ *endpoint को सोर्स IP द्वारा पहचाना जाए*। जितना संभव हो उतना संकीर्ण रूप से मैच करें (विशिष्ट होस्ट IPs, न कि विस्तृत CIDRs), और इसे एक `type=acl` के साथ बैक करें — बिना ऑथ वाला IP-मैच्ड trunk टोल-फ्रॉड का लक्ष्य है।

- **प्रति IP/पोर्ट एक ट्रांसपोर्ट।** आप एक ही IP:पोर्ट पर दो ट्रांसपोर्ट बाइंड नहीं कर सकते, और आप एक ही IP वर्जन के कई TCP या TLS ट्रांसपोर्ट बाइंड नहीं कर सकते। कन्वर्जन स्क्रिप्ट एक ऐसा ट्रांसपोर्ट उत्सर्जित कर सकती है जो आपके पास पहले से मौजूद किसी ट्रांसपोर्ट से टकराता है — एक एकल, जानबूझकर डिज़ाइन किए गए ट्रांसपोर्ट लेयर पर समेकित करें।
...
2. एक सिंगल `sip.conf` `type=friend` ब्लॉक सबसे अधिक बार PJSIP ऑब्जेक्ट्स का कौन सा सेट बन जाता है?
   - A. एक सिंगल `type=peer`
   - B. केवल `type=endpoint`
   - C. `type=endpoint` + `type=auth` + `type=aor`
   - D. `type=transport` + `type=registration`

3. `sip.conf` में, `host=dynamic` (डिवाइस अपना स्थान स्वयं रजिस्टर करता है) किसमें मैप होता है:
   - A. `type=identify` के साथ `match=dynamic`
   - B. `type=aor` के साथ `max_contacts` (डिवाइस REGISTERs)
   - C. endpoint पर `direct_media=yes`
   - D. `type=registration`

4. `sip_to_pjsip.py` कन्वर्जन स्क्रिप्ट है:
   - A. एक CLI कमांड: `asterisk -rx 'sip_to_pjsip'`
   - B. `contrib/scripts/sip_to_pjsip/` के तहत Asterisk सोर्स ट्री में एक Python स्क्रिप्ट
...
8. Asterisk 22 पर, एक `auth` ऑब्जेक्ट को किस `auth_type` का उपयोग करना चाहिए, और `userpass` की स्थिति क्या है?
   ...
   - B. `auth_type=digest`; `userpass` deprecated है और `digest` में बदल दिया गया है
   - C. `auth_type=md5`; `digest` deprecated है
   - D. `auth_type=plaintext`; `digest` को हटा दिया गया था

9. `chan_sip` प्रोवाइडर पीयर जिसे `insecure=invite` (ज्ञात IP से अनऑथेंटिकेटेड INVITEs स्वीकार करें) के साथ PJSIP में माइग्रेट किया जाता है:
...
10. Realtime माइग्रेशन में, `chan_sip` `sippeers` टेबल को किन PJSIP/Sorcery टेबल्स द्वारा प्रतिस्थापित किया जाता है?
    - A. एक सिंगल `pjsip_peers` टेबल

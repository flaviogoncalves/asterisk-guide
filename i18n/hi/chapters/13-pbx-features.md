...
SIP सिस्टम में, अधिकांश फोन सुविधाएं एंडपॉइंट (endpoint) में कार्यान्वित की जाती हैं। कई तरह के SIP फोन और निर्माता मौजूद हैं, और इंटरऑपरेबिलिटी (interoperability) की गारंटी नहीं होती है। Asterisk डेवलपमेंट टीम ने PBX में ही अधिकांश सुविधाओं को लागू करने का अद्भुत काम किया है, जिससे Asterisk लगभग एंडपॉइंट स्वतंत्र हो गया है। हालाँकि, कभी-कभी आप पाएंगे कि फोन और Asterisk दोनों द्वारा एक ही कार्य किया जा रहा है। फोन और PBX का एकीकरण उपयोगिता के मामले में अगली सीमा है और यही वह जगह है जहाँ प्रोप्रायटरी सिस्टम अभी ध्यान केंद्रित कर रहे हैं। इस अध्याय में, आप सीखेंगे कि इन अधिकांश सुविधाओं का उपयोग कैसे किया जाए।
...
सबसे पहले और सबसे महत्वपूर्ण, यह समझना जरूरी है कि PBX सुविधाएं कब निष्पादित हो रही हैं और कब फोन सारा काम कर रहा है। उदाहरण के लिए, आप फोन पर TRANSFER बटन का उपयोग करके या # डायल करके (PBX द्वारा निष्पादित अनकंडीशनल ट्रांसफर) कॉल ट्रांसफर कर सकते हैं।
...
Asterisk 12 के बाद से, कॉल पार्किंग को `features.conf` से बाहर निकालकर इसके अपने मॉड्यूल, `res_parking` में स्थानांतरित कर दिया गया है, जिसका कॉन्फ़िगरेशन `res_parking.conf` में है। नीचे दिया गया पार्किंग-लॉट ब्लॉक (`parkext`, `parkpos`, `context`, `parkingtime`, आदि) `res_parking.conf` में रहता है। `[featuremap]` अनुभाग (DTMF फीचर कोड, जिसमें `parkcall` शामिल है) `features.conf` में रहता है।
...
1. PJSIP एंडपॉइंट के लिए, सुनिश्चित करें कि विकल्प `direct_media` को `no` पर सेट किया गया है (ताकि मीडिया Asterisk के माध्यम से प्रवाहित हो और फीचर कोड का पता चल सके), या `Dial()` एप्लिकेशन में `t`/`T` विकल्प का उपयोग करें।
...
डिफ़ॉल्ट रूप से, 700 एक्सटेंशन का उपयोग कॉल पार्क करने के लिए किया जाता है। बातचीत के बीच में, कॉल को 700 एक्सटेंशन पर ट्रांसफर करने के लिए # दबाएं। अब Asterisk आपके पार्किंग एक्सटेंशन की घोषणा करेगा, जैसे 701 या 702। फोन रख दें, और कॉलर को होल्ड पर रखा जाएगा। अपने डेस्क फोन पर जाएं और कॉल पुनः प्राप्त करने के लिए घोषित पार्किंग एक्सटेंशन डायल करें। यदि कॉलर को लंबे समय तक पार्क किया जाता है, तो टाइमआउट सुविधा ट्रिगर हो जाएगी और मूल रूप से डायल किया गया एक्सटेंशन फिर से बजने लगेगा।
...
में संशोधित किया जा सकता है।

![कॉल पिकअप: सदस्य केवल अपने समूह के भीतर कॉल कैप्चर कर सकते हैं; ऑपरेटर (pickupgroup=1,2,3) हर समूह से कॉल पिक अप कर सकता है](../images/13-pbx-features-fig05.png)

### कॉन्फ़िगरेशन कार्य सूची

कॉल पिकअप सुविधा को कॉन्फ़िगर करने के लिए नीचे दिए गए चरणों का पालन करें। चरण 1: अपने एक्सटेंशन के लिए एक कॉल ग्रुप कॉन्फ़िगर करें। यह चैनल कॉन्फ़िगरेशन फ़ाइल (pjsip.conf, iax.conf, chan_dahdi.conf) में किया जाता है। PJSIP एंडपॉइंट के लिए, `pjsip.conf` के एंडपॉइंट अनुभाग में `call_group` और `pickup_group` सेट करें (pjsip.conf snake_case विकल्प नामों का उपयोग करता है)। यह कार्य आवश्यक है।
...
![`core show application confbridge` का आउटपुट, जिसमें सिनोप्सिस, सिंटैक्स, और bridge_profile, user_profile, और menu तर्क दिखाए गए हैं](../images/13-pbx-features-fig06.png)

![कई PJSIP एंडपॉइंट एक नामित ConfBridge कॉन्फ्रेंस (101) में शामिल होते हैं; एक प्रतिभागी एडमिन है। मिक्सिंग और टाइमिंग को `res_confbridge` और इन-बिल्ट `res_timing_*` टाइमर द्वारा नियंत्रित किया जाता है — किसी DAHDI की आवश्यकता नहीं है।](../images/13-pbx-features-fig09.png)

जैसा कि आप ऊपर देख सकते हैं, तीन महत्वपूर्ण तर्क हैं, जिनमें से प्रत्येक `confbridge.conf` में एक अनुभाग प्रकार से मैप होता है। **bridge_profile** (एक `type=bridge` अनुभाग): यहाँ आप प्रतिभागियों की अधिकतम संख्या (`max_members`), रिकॉर्डिंग (`record_conference`), `video_mode`, और कई अन्य ब्रिज-वाइड पैरामीटर चुनते हैं।

यहाँ पूरी उदाहरण फ़ाइल को पुन: प्रस्तुत करने का कोई मतलब नहीं है, इसलिए मुझे आपको confbridge.conf फ़ाइल में bridge_profile को कॉन्फ़िगर करने का एक सरल उदाहरण देने दें।
...
**user_profile** (एक `type=user` अनुभाग): यहाँ आप उन विकल्पों को परिभाषित करते हैं जो प्रति उपयोगकर्ता विशिष्ट हैं, जैसे कि क्या उपयोगकर्ता एक एडमिनिस्ट्रेटर है (`admin=yes`), क्या वे म्यूट होकर शुरू करते हैं (`startmuted=yes`), म्यूजिक ऑन होल्ड, और कई अन्य प्रति-उपयोगकर्ता विकल्प। उदाहरण:
...
**menu** (एक `type=menu` अनुभाग): यहाँ आप कॉन्फ्रेंस के लिए कीपैड (DTMF) मैपिंग को परिभाषित करते हैं — उदाहरण के लिए कौन सी कुंजी म्यूट को टॉगल करती है, वॉल्यूम समायोजित करती है, या कॉन्फ्रेंस छोड़ती है। सभी उपलब्ध क्रियाओं को देखने के लिए `confbridge.conf.sample` फ़ाइल देखें। उदाहरण:
...
यदि आप MeetMe से आ रहे हैं, तो जिन एडमिन फ़ंक्शंस का उपयोग आपने `MeetMeAdmin()` और `a` (एडमिन) विकल्प के माध्यम से किया था, वे अब **एडमिन यूजर प्रोफ़ाइल** (`admin=yes`) और **मेनू** क्रियाओं के माध्यम से व्यक्त किए जाते हैं। एक एडमिनिस्ट्रेटर जो एडमिन प्रोफ़ाइल और एडमिन क्रियाओं वाले मेनू के साथ जुड़ता है, वह कीपैड से लाइव रूम को लॉक कर सकता है, उपयोगकर्ताओं को किक कर सकता है, और प्रतिभागियों को म्यूट कर सकता है। `confbridge.conf` में प्रासंगिक मेनू क्रियाएं हैं:

- `admin_kick_last` -- अंतिम शामिल होने वाले उपयोगकर्ता को किक करें
...
ये MeetMe `MeetMe()` विकल्प फ़्लैग्स (`a`, `A`, `m`, `M`, `l`, `x`, …) और `MeetMeAdmin()` कमांड (`k`, `K`, `L`, `M`, `N`, …) की जगह लेते हैं। Asterisk 22 में कोई `meetme.conf` नहीं है; सभी कॉन्फ्रेंस कॉन्फ़िगरेशन `confbridge.conf` में रहते हैं, और परिवर्तन `module reload res_confbridge.so` के साथ लागू किए जाते हैं।

### ConfBridge उदाहरण

एक्सटेंशन 500 पर पहुंच योग्य कॉन्फ्रेंस रूम बनाने के लिए, `extensions.conf` में:
...
500 डायल करने वाला पहला कॉलर कॉन्फ्रेंस `101` बनाता है; बाद के कॉलर इसमें शामिल हो जाते हैं। यहाँ संदर्भित प्रोफ़ाइल और मेनू (`default_bridge`, `default_user`, `sample_user_menu`) `confbridge.conf` में परिभाषित हैं। PIN की आवश्यकता के लिए, यूजर प्रोफ़ाइल में `pin=` सेट करें; प्रतिभागी को कॉन्फ्रेंस एडमिनिस्ट्रेटर बनाने के लिए, उन्हें `admin=yes` के साथ एक यूजर प्रोफ़ाइल दें।
...
![MixMonitor() एप्लिकेशन: एक चैनल के ऑडियो को फ़ाइल में रिकॉर्ड और मिक्स करता है, जिसमें अपेंड, केवल-ब्रिज्ड, और वॉल्यूम समायोजन के विकल्प हैं](../images/13-pbx-features-fig09.png)

### MixMonitor()

कॉल रिकॉर्ड करें और रिकॉर्डिंग के दौरान ऑडियो को मिक्स करें। सिंटैक्स: `MixMonitor(filename.extension[,options[,command]])`। वर्तमान चैनल पर ऑडियो को निर्दिष्ट फ़ाइल में रिकॉर्ड करता है। मान्य विकल्प:

- a - ओवरराइट करने के बजाय फ़ाइल में जोड़ता है।
- b - केवल तब फ़ाइल में ऑडियो सहेजता है जब चैनल ब्रिज्ड हो।
- नोट: इसमें कॉन्फ्रेंस शामिल नहीं हैं।
- v(<x>) - श्रव्य वॉल्यूम को <x> के कारक द्वारा समायोजित करता है (-4 से 4 तक)
- V(<x>) - बोले गए वॉल्यूम को <x> के कारक द्वारा समायोजित करता है (-4 से 4 तक)
- W(<x>) - श्रव्य और बोले गए दोनों वॉल्यूम को <x> के कारक द्वारा समायोजित करता है (-4 से 4 तक)
...
एक दिलचस्प संसाधन वन-टच रिकॉर्डिंग सुविधा `automixmon` है, जो एक पार्टी को कॉल के दौरान DTMF कोड (डिफ़ॉल्ट `*3`) डायल करने देती है ताकि तुरंत रिकॉर्डिंग शुरू (और बंद) की जा सके। यह MixMonitor पर आधारित है, इसलिए यह एक एकल मिश्रित फ़ाइल लिखता है। उदाहरण:
...
`X` और `x` विकल्प क्रमशः कॉलर और कैली के लिए वन-टच MixMonitor सुविधा को सक्षम करते हैं। चूंकि MixMonitor एक एकल मिश्रित फ़ाइल रिकॉर्ड करता है, इसलिए बाद में अलग IN/OUT फ़ाइलों को संयोजित करने की कोई आवश्यकता नहीं है (पुराना `automon`/`Monitor` दृष्टिकोण, जिसने `soxmix` के लिए दो फ़ाइलें बनाईं, को `Monitor` एप्लिकेशन के साथ हटा दिया गया था)।
...
डायल प्लान में, आप `StartMusicOnHold` के साथ एक चैनल पर म्यूजिक ऑन होल्ड शुरू कर सकते हैं (और `StopMusicOnHold` के साथ इसे रोक सकते हैं):
...
1. कॉल पार्किंग के बारे में कौन से कथन सत्य हैं?
   - A. डिफ़ॉल्ट रूप से, कॉल पार्किंग के लिए एक्सटेंशन 800 का उपयोग किया जाता है।
...
6. ConfBridge में, एक प्रतिभागी को एडमिनिस्ट्रेटर विशेषाधिकार (किक, दूसरों को म्यूट करना, रूम लॉक करना) उनकी यूजर प्रोफ़ाइल (`confbridge.conf`) में ___ सेट करके दिए जाते हैं:
   - A. admin=yes
   - B. marked=yes
   - C. moderator=yes
   - D. type=admin
...
9. आप MixMonitor() एप्लिकेशन या वन-टच रिकॉर्डिंग (`automixmon`) सुविधा के साथ कॉल रिकॉर्ड कर सकते हैं। डिफ़ॉल्ट रूप से, `automixmon` ___ DTMF अनुक्रम का उपयोग करता है।
   ...
   - C. *3
   - D. #1
10. ConfBridge में, कौन सा `confbridge.conf` यूजर-प्रोफ़ाइल विकल्प प्रतिभागी को म्यूट होकर शामिल होने के लिए बनाता है (वे कॉन्फ्रेंस सुन सकते हैं लेकिन अनम्यूट होने तक उन्हें सुना नहीं जा सकता)?
    - A. startmuted=yes
    - B. listen=only
    - C. muteall=yes
    - D. quiet=yes

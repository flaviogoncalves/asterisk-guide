...
अध्याय 3 में हमने dial plan की मूल बातें सीखीं। शैक्षणिक कारणों से, हमने सभी सुविधाओं के बारे में नहीं बताया, केवल कुछ सबसे महत्वपूर्ण सुविधाओं पर चर्चा की। यह अध्याय dial plan में गहराई से जाएगा, जिसमें उन्नत तकनीकों, नए अनुप्रयोगों और अवधारणाओं का वर्णन किया गया है।
...
- किसी अन्य PBX में स्थानांतरित करने के लिए switch कमांड का उपयोग करना
- गोपनीयता प्रबंधक (privacy manager) लागू करना
- voicemail लागू करना
- कॉर्पोरेट निर्देशिका (corporate directory) लागू करना

## अपने Dial Plan को सरल बनाना

आप एक extension को परिभाषित करने के लिए “same” कीवर्ड का उपयोग करके अपने dial plan को सरल बना सकते हैं। इससे dial plan में टाइपिंग की गलतियों की संख्या कम होनी चाहिए। नीचे दिए गए उदाहरण को देखें:
...
Asterisk dial plan में एक खामी खोजी गई थी जो उपयोगकर्ता को आपके dial plan में एक नया चैनल और डायल नंबर इंजेक्ट करने की अनुमति देती है। मान लीजिए कि आपके सर्वर `exten=>_X.,1,Dial(PJSIP/${EXTEN})` में निम्नलिखित पंक्ति है और किसी दुर्भावनापूर्ण उपयोगकर्ता ने softphone में नंबर `3000&DAHDI/1/011551123456789` डायल किया है। SIP प्रोटोकॉल, डिफ़ॉल्ट रूप से, किसी भी अल्फ़ान्यूमेरिक वर्ण को स्वीकार करता है, इसलिए डायल की गई extension वास्तव में दो कॉल ट्रिगर करेगी: एक चैनल PJSIP/3000 के लिए और दूसरी चैनल DAHDI/011551123456789 के लिए, जो एक अंतरराष्ट्रीय नंबर है। इस प्रकार, extension तक पहुंच रखने वाला कोई भी उपयोगकर्ता वास्तव में दुनिया में कहीं भी डायल कर सकता है। इस व्यवहार से बचने का सबसे आसान तरीका dial एप्लिकेशन को कॉल करने से पहले नंबरों को फ़िल्टर करना है। FILTER() फ़ंक्शन इसके लिए बहुत उपयोगी है। उदाहरण:

```
exten=>_X.,1,DIAL(PJSIP/${FILTER(0-9,${EXTEN})})
```

एप्लिकेशन फ़िल्टर आपको डायल किए गए नंबर से 0 से 9 तक के नंबरों को छोड़कर सभी वर्णों को फ़िल्टर करने की अनुमति देगा। अधिक जानकारी Asterisk से उपलब्ध फ़ाइल README-SERIOUSLY.bestpractices.txt में पाई जा सकती है।

## IVR मेनू का उपयोग करके कॉल प्राप्त करना।

पिछले अनुभाग में, आपने DID का उपयोग करके या ऑपरेटर को फ़ॉरवर्ड करके सभी कॉल प्राप्त कीं। अब आप सीखेंगे कि IVR मेनू कैसे लागू किया जाए और साथ ही एक ऑटो-अटेंडेंट सेवा कैसे बनाई जाए। विशिष्टताओं में जाने से पहले, आइए कुछ नए अनुप्रयोगों की जांच करें। हमने पाठकों के लिए इसे आसान बनाने के लिए show application कमांड का आउटपुट नीचे दिया है। आप show application application_name का उपयोग करके ये विवरण प्राप्त कर सकते हैं। 1 http://downloads.asterisk.org/pub/security/AST-2010-002.pdf

### Background() एप्लिकेशन

यह एप्लिकेशन कॉलिंग चैनल द्वारा extension डायल किए जाने की प्रतीक्षा करते समय दी गई फ़ाइलों की सूची चलाएगा। इस एप्लिकेशन द्वारा फ़ाइलों को चलाना समाप्त करने के बाद अंकों की प्रतीक्षा जारी रखने के लिए, WaitExten एप्लिकेशन का उपयोग किया जाना चाहिए। langoverride विकल्प स्पष्ट रूप से निर्दिष्ट करता है कि अनुरोधित ध्वनि फ़ाइलों के लिए किस भाषा का उपयोग करने का प्रयास करना है। निर्दिष्ट कोई भी context वह dial plan context होगा जिसका उपयोग यह एप्लिकेशन डायल की गई extension पर बाहर निकलते समय करता है। यदि अनुरोधित ध्वनि फ़ाइलों में से कोई मौजूद नहीं है, तो कॉल प्रोसेसिंग समाप्त कर दी जाएगी। विकल्प:
...
- m - केवल तभी ब्रेक करें यदि कोई अंक गंतव्य context में एक-अंक वाली extension से मेल खाता हो।

### Record() एप्लिकेशन

यह एप्लिकेशन चैनल से एक दिए गए फ़ाइल नाम में रिकॉर्ड करता है। यदि फ़ाइल मौजूद है, तो उसे अधिलेखित (overwrite) कर दिया जाएगा।
...
यह एप्लिकेशन दिए गए फ़ाइल नाम (extension शामिल न करें) को वापस चलाता है। विकल्प भी पाइप प्रतीक के बाद शामिल किए जा सकते हैं। 'skip' विकल्प संदेश के प्लेबैक को छोड़ने का कारण बनता है यदि चैनल 'up' स्थिति में नहीं है (अर्थात, अभी तक उत्तर नहीं दिया गया है)।
...
यह एप्लिकेशन उपयोगकर्ता से दिए गए वेरिएबल में, निर्धारित संख्या में स्ट्रिंग अंक, कई बार पढ़ता है।

- filename -- अंकों या टोन को पढ़ने से पहले चलाने के लिए फ़ाइल, विकल्प i के साथ
- maxdigits -- अंकों की अधिकतम स्वीकार्य संख्या। maxdigits दर्ज होने के बाद पढ़ना बंद कर देता है (उपयोगकर्ता को # कुंजी दबाने की आवश्यकता के बिना)। डिफ़ॉल्ट 0 है - कोई सीमा नहीं - उपयोगकर्ता के # कुंजी दबाने की प्रतीक्षा करने के लिए। 0 से नीचे का कोई भी मान समान अर्थ रखता है। अधिकतम स्वीकार्य मान 255 है।
...
आइए निम्नलिखित कार्यक्षमता के साथ एक IVR मेनू बनाएं। डायल किए जाने पर, IVR संदेश के साथ एक ऑडियो फ़ाइल चलाता है “XYZ कॉर्पोरेशन में आपका स्वागत है; सेल्स के लिए 1 दबाएं, तकनीकी सहायता के लिए 2, प्रशिक्षण के लिए 3, या प्रतिनिधि से बात करने के लिए प्रतीक्षा करें।” अंक कॉलर को इस प्रकार रूट करते हैं:

- `1` — सेल्स (PJSIP/4001) पर स्थानांतरित करें
- `2` — तकनीकी सहायता (PJSIP/4002) पर स्थानांतरित करें
- `3` — प्रशिक्षण (PJSIP/4003) पर स्थानांतरित करें
- कोई अंक नहीं दबाया गया — ऑपरेटर (PJSIP/4000) पर स्थानांतरित करें
...
यह कॉल प्राप्त करने के लिए एक कंपनी सेटअप मेनू है। background एप्लिकेशन वर्तमान context को पढ़ता है और किसी भी संभावित संयोजन के लिए प्रत्येक नंबर की अधिकतम लंबाई को परिभाषित करता है।
...
जब आप इस कंपनी को डायल करते हैं, तो स्वागत संदेश पहले चलाया जाता है। उसके बाद, Asterisk डायल किए जाने वाले अंक की प्रतीक्षा करता है। डायल किया गया नंबर Asterisk क्रिया तुरंत Dial(DAHDI/1) कॉल करता है टाइमआउट की प्रतीक्षा करता है, फिर Dial(DAHDI/2) पर जाता है तुरंत (DAHDI/3) कॉल करता है तुरंत (DAHDI/4) कॉल करता है टाइमआउट की प्रतीक्षा करता है, फिर डिस्कनेक्ट करता है तुरंत Dial(DAHDI/5) कॉल करता है तुरंत Dial(DAHDI/6) कॉल करता है तुरंत डिस्कनेक्ट करता है मेनू में अस्पष्टता से बचना महत्वपूर्ण है। हर कोई चाहता है कि उसे जल्दी उत्तर मिले। इस कारण से, आपको 2, 21, या 22 नंबरों का उपयोग नहीं करना चाहिए।
...
एक context में दूसरे context की सामग्री शामिल हो सकती है। ऊपर दिए गए उदाहरण में, कोई भी चैनल internal context में किसी भी extension को डायल कर सकता है, लेकिन केवल 4003 चैनल ही अंतरराष्ट्रीय extensions को डायल कर सकता है। आप dial plan निर्माण को आसान बनाने के लिए context inclusion का उपयोग कर सकते हैं। Context inclusion का उपयोग करके, आप नियंत्रित कर सकते हैं कि किसके पास किन extensions तक पहुंच है।

### “number not found” संदेश का निवारण

“number not found” संदेश प्राप्त करना बहुत आम है। अधिकांश लोग शामिल context की अवधारणा को भ्रमित करते हैं क्योंकि यह वास्तव में सहज नहीं है। अंगूठे के नियम के रूप में, पहले आने वाले चैनल कॉन्फ़िगरेशन फ़ाइल पर जाएं, जैसे `pjsip.conf`, `chan_dahdi.conf`, और `iax.conf`, और वर्तमान context निर्धारित करें। फिर, फ़ाइल extensions.conf में dial plan पर जाएं और जांचें कि क्या डायल किया गया नंबर उस context में पाया जा सकता है। यदि नहीं, तो आपके dial plan में कुछ गड़बड़ है। Context के स्वर्ण नियम हैं: 1. एक चैनल केवल उसी context के भीतर नंबर डायल कर सकता है जिसमें चैनल है। 2. वह context जहां कॉल संसाधित होती है, आने वाले चैनल कॉन्फ़िगरेशन फ़ाइल (`chan_dahdi.conf`, `iax.conf`, `pjsip.conf`) में परिभाषित होती है।

## Switch स्टेटमेंट का उपयोग करना

आप switch कमांड का उपयोग करके dial plan प्रोसेसिंग को दूसरे सर्वर पर भेज सकते हैं। आपको दूसरे सर्वर के नाम और कुंजी की आवश्यकता होगी। context गंतव्य context है।

![10-dialplan-advanced-features figure 6](../images/10-dialplan-advanced-features-img06.png)

## Dial plan प्रोसेसिंग क्रम

जब Asterisk को आने वाली कॉल प्राप्त होती है, तो यह चैनल द्वारा परिभाषित context में देखता है। कुछ मामलों में, यदि एक से अधिक पैटर्न डायल किए गए नंबर से मेल खाते हैं, तो Asterisk कॉल को उस सटीक तरीके से संसाधित नहीं कर सकता है जैसा आप सोचते हैं कि उसे करना चाहिए। आप dialplan show CLI कमांड का उपयोग करके मिलान क्रम देख सकते हैं। उदाहरण: मान लीजिए कि आप एनालॉग trunk (DAHDI/1) पर रूट करने के लिए 912 डायल करना चाहते हैं और 9 से शुरू होने वाले अन्य सभी नंबरों को दूसरे एनालॉग trunk (DAHDI/2) पर रूट करना चाहते हैं। आप कुछ इस तरह लिखेंगे:
...
Asterisk के पुराने संस्करणों में आपके पास Macro कमांड थी। यह कमांड GOSUB के पक्ष में बहुत पहले ही हटा दी गई थी। हम यहां प्रदर्शित करेंगे कि voicemail प्रोसेसिंग के लिए आसान और व्यवस्थित तरीके से सबरूटीन कैसे बनाएं। कमांड प्रारूप:

```
gosub([[context,]exten,]priority[(arg1[,...][,argN])])
```

कमांड GOSUB Asterisk 1.6 से उपलब्ध है और तर्कों को पारित करने का समर्थन करता है (सबरूटीन के अंदर `${ARG1}`, `${ARG2}`, आदि के रूप में उपलब्ध)। तर्कों के साथ, अब पुराने Macro कमांड को पूरी तरह से बदलना संभव है। Macros (`app_macro`) को Asterisk 21 में हटा दिया गया था; आपको सबरूटीन के लिए GOSUB का उपयोग करना होगा।

### सबरूटीन बनाना

परिभाषा बहुत समान है। voicemail के लिए stdexten नाम (जो नाम आप पसंद करें चुनें) के साथ परिभाषित नीचे दिए गए सबरूटीन को देखें। पहले तर्क (चैनल का नाम) के साथ कमांड Dial को कॉल करने के बाद हम कॉल लॉजिक को अगले चरण पर भेजने के लिए ${DIALSTATUS} की जांच करते हैं।
...
कॉल फ़ॉरवर्ड और ब्लैक लिस्ट लागू करने के लिए, हमें डेटा को स्टोर और पुनर्स्थापित करने के लिए किसी तरीके की आवश्यकता है। सौभाग्य से, Asterisk AstDB नामक एक अंतर्निहित डेटाबेस से डेटा स्टोर और पुनर्प्राप्त करने के लिए एक तंत्र प्रदान करता है। आधुनिक Asterisk (Asterisk 22 सहित) में AstDB **SQLite3** (फ़ाइल `/var/lib/asterisk/astdb.sqlite3`) द्वारा समर्थित है; Asterisk 1.8 और उससे पहले के संस्करण Berkeley DB v1 का उपयोग करते थे। यह परिवार और कुंजियों की पदानुक्रमित अवधारणा का उपयोग करके विंडोज रजिस्ट्री डेटाबेस के समान है। डेटा Asterisk पुनरारंभ के बीच बना रहता है। परिवार/कुंजी API पुराने बैकएंड से अपरिवर्तित है; केवल ऑन-डिस्क स्टोरेज प्रारूप बदल गया है।
...
- DB_DELETE(<family/key>) — फ़ंक्शन जो एक एकल कुंजी को लौटाता और हटाता है
- DBdeltree(<family>) — एप्लिकेशन जो पूरे परिवार/सबट्री को हटा देता है

पुराना `DBdel()` एप्लिकेशन Asterisk 22 में अब मौजूद नहीं है। `DB_DELETE()` dialplan फ़ंक्शन के साथ एक एकल कुंजी हटाएं — उदा. `Set(x=${DB_DELETE(family/key)})` या, एक लेखन ऑपरेशन के रूप में, `Set(DB_DELETE(family/key)=)`। `DBdeltree()` (पूरे परिवार/सबट्री को हटाना) अभी भी एक एप्लिकेशन है।
...
डायल करके डेटाबेस को पॉप्युलेट करने का प्रयास करें:

- *21* (कॉल फ़ॉरवर्डिंग इमीडिएट के लिए गंतव्य extension)
- *61* (कॉल फ़ॉरवर्डिंग ऑन बिजी स्थिति के लिए गंतव्य extension)
...
उपरोक्त सबरूटीन सत्यापित करता है कि क्या डेटाबेस में CFIM, CFBS, या DND के अनुरूप key:value जोड़े शामिल हैं, और फिर उन्हें उचित रूप से संभालता है। फॉलो सबरूटीन डायलिंग रूटीन को कॉल करता है:
...
पुराना `LookupBlacklist()` एप्लिकेशन Asterisk से **हटा दिया गया** था (यह विरासत "priority+101 जंप" तंत्र के साथ गायब हो गया)। Asterisk 22 में आप सीधे `DB_EXISTS()` फ़ंक्शन के साथ एक ब्लैकलिस्ट बनाते हैं (जो एक कुंजी के लिए परीक्षण करता है और, जब मिल जाता है, तो `${DB_RESULT}` में इसका मान उजागर करता है) प्लस `GotoIf`। प्रत्येक अवरुद्ध नंबर को `blacklist` परिवार में एक कुंजी के रूप में स्टोर करें, फिर अपने आने वाले context के शीर्ष पर कॉलर आईडी की जांच करें:
...
`DB_EXISTS(blacklist/${CALLERID(num)})` `1` लौटाता है जब कॉलर का नंबर डेटाबेस में मौजूद होता है (कॉल को `blocked` context में भेजता है) और अन्यथा `0`, इसलिए कॉल सामान्य `Dial()` पर आगे बढ़ती है।

ब्लैकलिस्ट में एक नंबर डालने के लिए, हम पहले वाले संसाधन का उपयोग कर सकते हैं, *31* का उपयोग करके उसके बाद ब्लैकलिस्ट की जाने वाली extensions। ब्लैकलिस्ट से एक नंबर हटाने के लिए, आपको #31# का उपयोग करना चाहिए उसके बाद हटाया जाने वाला नंबर।
...
नोट: कुंजी के साथ कोई भी मान संबद्ध किया जा सकता है। `DB_EXISTS()` परीक्षण कुंजी को खोजता है, मान को नहीं। ब्लैकलिस्ट से नंबर मिटाने के लिए, आप उपयोग कर सकते हैं:
...
निम्नलिखित आकृति में, हमारे पास तीन context वाला एक dial plan है। [incoming] context वह है जहां कॉल आमतौर पर प्राप्त होती हैं। हमने चार पंक्तियाँ शामिल की हैं जो सिस्टम समय के आधार पर व्यवहार बदलती हैं, जैसा कि नीचे उदाहरण दिया गया है:

```
include => context,<times>,<weekdays>,<mdays>,<months>
```

आधुनिक Asterisk (22 सहित) समय-शामिल फ़ील्ड को **अल्पविराम (commas)** से अलग करता है, पाइप से नहीं। विरासत पाइप रूप (`include => context|times|weekdays|mdays|months`) को एक सादे शाब्दिक context नाम के रूप में पार्स किया जाता है और चुपचाप किसी भी समय स्थिति को लागू करने में विफल रहता है।

नियमित काम के घंटों के दौरान, प्रोसेसिंग को mainmenu पर पुनर्निर्देशित किया जाएगा, जहां यह संभवतः आने वाली कॉल को संभालने के लिए IVR को कॉल करेगा। यदि कॉल काम के घंटों के बाद होती है, तो यह ${SECURITY} वेरिएबल में परिभाषित सुरक्षा extension को कॉल करेगी। यदि सुरक्षा extension कॉल का उत्तर नहीं देती है, तो इसे ऑपरेटर के voicemail पर भेज दिया जाएगा।
...
Asterisk 22 में फ़ील्ड विभाजक एक **अल्पविराम** है, पाइप नहीं (पाइप रूप को Asterisk 1.6 में हटा दिया गया था)। एक वैकल्पिक `timezone` फ़ील्ड समर्थित है, और प्रत्येक शाखा लेबल सामान्य `[[context,]extension,]priority` रूप का उपयोग करता है।
...
पिछले स्टेटमेंट का उपयोग करके, उपयोगकर्ता PBX को डायल करता है और—बिना किसी पासवर्ड की आवश्यकता के—एक डायल टोन प्राप्त करता है। DISA का उपयोग करने वाली कोई भी कॉल `default` context का उपयोग करके संसाधित की जाएगी। इस एप्लिकेशन के तर्कों में एक वैश्विक पासवर्ड या फ़ाइल के भीतर व्यक्तिगत पासवर्ड शामिल है। यदि कोई context निर्दिष्ट नहीं है, तो `disa` context माना जाता है। यदि आप पासवर्ड फ़ाइल का उपयोग करते हैं, तो पूरा पथ निर्दिष्ट किया जाना चाहिए। DISA बाहरी डायलिंग के लिए भी एक कॉलर आईडी निर्दिष्ट की जा सकती है। उदाहरण:

```
exten => s,1,DISA(numeric-passcode,default,"Flavio" <4830258590>)
```

Asterisk 22 तर्क विभाजक के रूप में अल्पविराम का उपयोग करता है (पाइप रूप 1.6 में हटा दिया गया था)। पहला तर्क या तो एक एकल पासकोड है या पासकोड फ़ाइल का पथ, और जब कोई नहीं दिया जाता है तो डिफ़ॉल्ट context `disa` है।

## एक साथ कॉल सीमित करना

GROUP() फ़ंक्शन आपको यह गिनने की अनुमति देता है कि आपके पास एक ही समय में एक समूह में कितने सक्रिय चैनल हैं। उदाहरण: रियो डी जनेरियो में आपकी एक शाखा है, जहां फोन “_214X” पैटर्न का पालन करते हैं। यह स्थान एक लीज्ड लाइन द्वारा सेवित है, जिसमें वॉयस बैंडविड्थ के लिए 64K आरक्षित है। इस मामले में, अनुमत कॉल की अधिकतम संख्या 2 है (G.729, प्रति कॉल 30r.2K)। रियो के लिए कॉल को दो तक सीमित करने के लिए:
...
Voicemailmain() एप्लिकेशन का उपयोग voicemail मेलबॉक्स को कॉन्फ़िगर करने के लिए किया जाता है। उपयोगकर्ता एप्लिकेशन डायल कर सकते हैं, अपनी ग्रीटिंग रिकॉर्ड कर सकते हैं, और अपना voicemail सुन सकते हैं। dial plan में एप्लिकेशन को कॉल करने के लिए, उपयोग करें:
...
यह एप्लिकेशन कॉलिंग पार्टी को निर्दिष्ट मेलबॉक्स की सूची के लिए संदेश छोड़ने की अनुमति देता है। जब कई मेलबॉक्स निर्दिष्ट किए जाते हैं, तो ग्रीटिंग पहले निर्दिष्ट मेलबॉक्स से ली जाएगी। यदि निर्दिष्ट मेलबॉक्स मौजूद नहीं है तो dial plan निष्पादन रुक जाएगा। सिंटैक्स नीचे दिखाया गया है:
...
आप कुछ संदेशों को “तत्काल” (urgent) के रूप में टैग कर सकते हैं। इसके लिए दो तरीके उपलब्ध हैं:

- एप्लिकेशन voicemail() में विकल्प ‘U’ पास करें
- फ़ाइल voicemail.conf में review=yes निर्दिष्ट करें। यदि इस विकल्प का उपयोग कर रहे हैं, तो उपयोगकर्ता वॉयस निर्देशों को रिकॉर्ड करने के बाद संदेश को तत्काल के रूप में टैग करने में सक्षम होगा।

## ईमेल पर voicemail भेजना
...
ईमेल पर voicemail भेजने के लिए, voicemail एप्लिकेशन आपके ऑपरेटिंग सिस्टम के एक घटक, संदेश स्थानांतरण एजेंट (MTA) का उपयोग करता है। डेबियन MTA के रूप में Exim का उपयोग करता है। ईमेल भेजने वाला एप्लिकेशन ‘mailcmd’ पैरामीटर में परिभाषित होता है।
...
ईमेल बॉडी और विषय एक टेम्प्लेट से बनाए जाते हैं जिसे आप `voicemail.conf` के `[general]` अनुभाग में सेट करते हैं। आप बॉडी और विषय दोनों को संशोधित कर सकते हैं, लेकिन संदेश की आकार सीमा 512 बाइट्स है। टेम्प्लेट में, `\n` एक नई पंक्ति सम्मिलित करता है और `\t` एक टैब सम्मिलित करता है।
...
## Voicemail वेब इंटरफ़ेस

स्रोत वितरण में `vmail.cgi` नामक एक पर्ल स्क्रिप्ट है, जो Asterisk स्रोत ट्री में `contrib/scripts/vmail.cgi` पर स्थित है (यह अभी भी Asterisk 22 के साथ आता है)। कमांड `make install` इस इंटरफ़ेस को स्थापित नहीं करती है; आपको स्रोत निर्देशिका से `make webvmail` चलाना होगा। इस स्क्रिप्ट के लिए पर्ल कमांड इंटरप्रेटर और सर्वर पर एक वेब सर्वर (जैसे Apache) स्थापित होने की आवश्यकता है।

```
make webvmail
```

`make webvmail` लक्ष्य स्क्रिप्ट (setuid root) को आपके वेब सर्वर की CGI निर्देशिका (`HTTP_CGIDIR`) में स्थापित करता है और सहायक छवियों को `images/*.gif` से `HTTP_DOCSDIR/_asterisk` (डिफ़ॉल्ट रूप से `/var/www/html/_asterisk`) में कॉपी करता है। यदि वे पथ आपके वेब सर्वर लेआउट से मेल नहीं खाते हैं, तो लक्ष्य चलाने से पहले शीर्ष-स्तरीय `Makefile` में `HTTP_CGIDIR` और `HTTP_DOCSDIR` वेरिएबल को संपादित करें।

## Voicemail अधिसूचना

आप voicemail को अपने फोन पर एक अधिसूचना संदेश भेजने के लिए कॉन्फ़िगर कर सकते हैं जब आपके पास नया voicemail हो। Asterisk 22 में, मैसेज वेटिंग इंडिकेशन (MWI) PJSIP और SIP फोन के साथ-साथ DAHDI फोन के साथ काम करता है। अनसुने voicemail को इंगित करने के लिए, एक संकेतक प्रकाश चमक सकता है या फोन शटर टोन बजा सकता है। आपको संबंधित चैनल कॉन्फ़िगरेशन फ़ाइल में मेलबॉक्स को कॉन्फ़िगर करने की आवश्यकता है। उदाहरण: `pjsip.conf` (endpoint अनुभाग में):

```
mailboxes=8590
```

PJSIP में मेलबॉक्स संकेत `pjsip.conf` के endpoint अनुभाग के अंदर `mailboxes` विकल्प के साथ सेट किया जाता है, न कि `sip.conf` के पुराने `mailbox=` के साथ। MWI सदस्यताएं `res_pjsip_mwi` मॉड्यूल द्वारा संभाली जाती हैं।
...
### लैब: फोन में संदेश अधिसूचना

यह लैब SIP सॉफ्टफोन का उपयोग करके परीक्षण की गई थी। 1. `pjsip.conf` संपादित करें और 4401 नामक डिवाइस के लिए endpoint अनुभाग में `mailboxes=4401` जोड़ें। 2. extensions.conf संपादित करें और 4401 extensions पर voicemail रिकॉर्ड करने के लिए एक extension बनाएं।

```
exten=9008,n,voicemail(b4401)
```

3. CLI > कंसोल पर जाएं और पुनः लोड करें। 4. SipPulse सॉफ्टफोन में, SIP खाता सेटिंग्स खोलें और खाते के लिए voicemail (message-waiting) जांच सक्षम करें। 5. 9008 डायल करें और एक संदेश छोड़ें। 6. फोन पर संदेश आइकन देखें।
...
3. कंसोल पर जाएं और पुनः लोड करें 4. 9006 डायल करें और प्रत्येक extension (4400, 4401) के लिए एक नाम रिकॉर्ड करें 5. 9007 डायल करें और एक extension के लिए अंतिम नाम के तीन अक्षर चुनें (Eas=327)। यदि यह सही विकल्प है, तो नाम पर स्थानांतरित करने के लिए ‘1’ दबाएं।

## लैब: सब कुछ एक साथ रखना

अब तक, आपने कई dial plan अवधारणाएं सीखी हैं। आइए सभी अनुप्रयोगों, कार्यों और अवधारणाओं को एक dial plan उदाहरण में रखें ताकि आप समझ सकें कि उनका एक साथ उपयोग कैसे किया जाता है। आइए नीचे दिए गए परिदृश्य के लिए आपको पूरे PBX कॉन्फ़िगरेशन के माध्यम से मार्गदर्शन करें।
...
एनालॉग trunks (chan_dahdi.conf) सबसे पहले, हम DAHDI चैनल कॉन्फ़िगरेशन फ़ाइल chan_dahdi.conf में एनालॉग trunks को कॉन्फ़िगर करेंगे। इस मामले में, हम 4 FXO इंटरफेस के साथ T400P Digium कार्ड का उपयोग करेंगे। मान लें कि ड्राइवर पहले से ही लोड है और ड्राइवर कॉन्फ़िगरेशन फ़ाइल (/etc/dahdi/system.conf) सही ढंग से कॉन्फ़िगर की गई है।
...
SIP चैनल (pjsip.conf) हमने 2000 से 2099 तक dial plan नंबरिंग चुनी है। दो कोडेक का उपयोग किया जाएगा: G.729 और G.711 ulaw। पहले वाले का उपयोग इंटरनेट या WAN पर Asterisk का उपयोग करने वाले फोन के लिए किया जाएगा जबकि दूसरे वाले का उपयोग स्थानीय नेटवर्क का उपयोग करने वाले फोन के लिए किया जाएगा। `pjsip.conf` में, हम मध्यस्थता करेंगे कि कौन से उपकरण सेवा के प्रत्येक वर्ग (restrict, ld, ldi) से संबंधित होंगे। ब्रूट फ़ोर्स हमलों के प्रति भेद्यता को कम करने के लिए, हम डिवाइस नामों के रूप में फोन के MAC पते का उपयोग करेंगे। मैं दृढ़ता से सलाह देता हूं कि आप ब्रूट फ़ोर्स हमलों से बचने के लिए मजबूत पासवर्ड का उपयोग करें!

हम एक ट्रांसपोर्ट और तीन पुन: प्रयोज्य टेम्प्लेट परिभाषित करते हैं — साझा कोडेक के साथ एक endpoint आधार, एक userpass प्रमाणीकरण, और एक एकल-संपर्क AOR — फिर प्रत्येक डिवाइस को टेम्प्लेट से जोड़ते हैं और केवल वही ओवरराइड करते हैं जो भिन्न है (इसका सेवा-वर्ग context और क्रेडेंशियल्स)। `host=dynamic` एक AOR बन जाता है जिसके खिलाफ फोन रजिस्टर होता है, और `directmedia` `direct_media` बन जाता है:
...
कॉल प्राप्त करने के लिए, दो context का उपयोग करें। पहला सामान्य-घंटों के संचालन के लिए है, जहां कॉल एक ऑटो-अटेंडेंट द्वारा प्राप्त की जाएगी। दूसरा काम के घंटों के बाद के लिए है, जहां कॉलर को “आपने कंपनी XYZ को कॉल किया है, हमारे सामान्य घंटे सुबह 08:00 बजे से शाम 06:00 बजे तक हैं; यदि आप गंतव्य extension नंबर जानते हैं तो आप अभी इसे डायल करने का प्रयास कर सकते हैं या हैंग अप कर सकते हैं” जैसा संदेश प्राप्त होगा। मेनू: सामान्य-घंटे, काम के घंटों के बाद नीचे दिए गए मेनू में, सिस्टम कॉलर को चेतावनी देने वाला एक संदेश चलाएगा कि कंपनी तक काम के नियमित घंटों के बाद पहुंचा गया है, जिससे कॉलर को गंतव्य extension नंबर डायल करने की अनुमति मिलती है (कोई काम के नियमित घंटों के बाद काम कर रहा हो सकता है)।
...
मेनू: मुख्य और सेल्स काम के नियमित घंटों के दौरान, कॉल का उत्तर एक ऑटो-अटेंडेंट मेनू द्वारा दिया जाता है, जिसे “XYZ कंपनी में आपका स्वागत है; सेल्स के लिए 1, तकनीकी सहायता के लिए 2, प्रशिक्षण के लिए 3, या वांछित extension नंबर डायल करें” जैसा संदेश प्राप्त होता है।
...
इस अध्याय में, आपने सीखा है कि IVR या ऑटो-अटेंडेंट का उपयोग करके कॉल कैसे प्राप्त की जाए। आपने context inclusion की अवधारणा का अध्ययन किया है और कुछ उदाहरण लागू किए हैं। दोहराव वाली टाइपिंग से बचने के लिए सबरूटीन का उपयोग किया गया था, और Asterisk डेटाबेस (AstDB, Asterisk 22 में SQLite3 द्वारा समर्थित) का उपयोग उन कार्यों के लिए किया गया था जिन्हें डेटा स्टोरेज की आवश्यकता होती है (उदा., कॉल फ़ॉरवर्ड, डू नॉट डिस्टर्ब, ब्लैकलिस्ट)। अंत में, आपने सीखा है कि काम के घंटों के बाद के व्यवहार को कैसे लागू किया जाए और इन अवधारणाओं का उपयोग करके एक पूर्ण dial plan लागू किया है।

## प्रश्नोत्तरी

1. एक समय-निर्भर context include `include => context,<times>,<weekdays>,<mdays>,<months>` रूप का उपयोग करता है। `include => normalhours,08:00-18:00,mon-fri,*,*` क्या करता है?
...
2. आधुनिक Asterisk (Asterisk 22 सहित) में, समय-आधारित `include =>` और `GotoIfTime()` के फ़ील्ड किस वर्ण से अलग होते हैं?
   - A. पाइप `|`
   - B. अल्पविराम `,`
   - C. अर्धविराम `;`
   - D. स्लैश `/`
3. एक साथ कई चैनलों को डायल करने के लिए (उन्हें एक साथ रिंग करना), आप उन्हें `Dial()` के अंदर ___ वर्ण के साथ अलग करते हैं।
4. एक वॉयस मेनू जो कॉलर के extension डायल करने की प्रतीक्षा करते समय एक प्रॉम्प्ट चलाता है, आमतौर पर ___ एप्लिकेशन के साथ बनाया जाता है।
...
7. जब आप `Dial(type1/identifier1&type2/identifier2)` का उपयोग करते हैं, तो Asterisk प्रत्येक चैनल को अनुक्रम में डायल करता है, उनके बीच 20 सेकंड प्रतीक्षा करता है।
   - A. गलत
   - B. सही
8. Background() एप्लिकेशन के साथ, आपको संदेश समाप्त होने तक प्रतीक्षा करनी होगी इससे पहले कि आप विकल्प चुनने के लिए DTMF अंक दबा सकें।
   - A. गलत
   - B. सही
9. सिंटैक्स `Goto([[context,]extension,]priority)` को देखते हुए, Goto() एप्लिकेशन के निम्नलिखित में से कौन से वैध आह्वान हैं? (सभी लागू होने वाले को चिह्नित करें)
   - A. Goto(context,extension)
   - B. Goto(context,extension,priority)
   - C. Goto(extension,priority)
   - D. Goto(priority)
10. Asterisk 22 dial plan में AstDB से एक एकल कुंजी हटाने के लिए, आप उपयोग करते हैं:
    - A. `DBdel()` एप्लिकेशन
    - B. `DB_DELETE()` फ़ंक्शन

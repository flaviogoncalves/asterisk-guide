...
- Composer des numéros entre des extensions ;
- Composer des numéros entre des téléphones et des destinations externes ; et
- Configurer un standard automatique (auto attendant).

## Comprendre les fichiers de configuration

Asterisk est contrôlé par des fichiers de configuration texte situés dans /etc/asterisk. Le format de fichier est similaire aux fichiers « .ini » de Windows. Un point-virgule est utilisé comme caractère de remarque, les signes « = » et « => » sont équivalents, et les espaces sont ignorés.
...
Les deux premières lignes configurent la valeur des options op1 et op2 sur « bas » et « adv », respectivement. Lorsque l'objet 1 est instancié, il est créé en utilisant l'option 1 comme « bas » et l'option 2 comme « adv ». Après avoir défini l'objet 1, nous changeons l'option 1 sur « int ». Ensuite, nous créons l'objet 2 avec l'option 1 sur « int » et l'option 2 sur « adv ».
...
Pour configurer un PBX, vous aurez besoin de matériel de base. Ce n'est ni difficile ni coûteux, mais il y a quelques options à considérer. Tout ce dont vous aurez besoin, ce sont deux téléphones et une connexion au réseau public. Certaines options et combinaisons sont possibles lors de la création de votre lab, dont nous discuterons ci-dessous.
...
- Un adaptateur de téléphone analogique SIP (ATA)
- Un téléphone IP
...
- Un compte avec un fournisseur VoIP
...
Après avoir terminé la section de transport, il est temps de configurer les clients SIP. Je tiens à rappeler une fois de plus au lecteur que nous aurons un chapitre entier sur SIP/PJSIP plus tard dans le livre. Pour l'instant, concentrons-nous sur les bases et laissons les détails pour plus tard.

Dans PJSIP, un client SIP est construit à partir d'un ensemble d'objets liés, reliés par une référence de nom :

- `endpoint` : Le comportement d'appel — codecs (`allow`/`disallow`), le dialplan `context`, et quels `auth` et `aors` il utilise.
...
`chan_iax2` est toujours fourni dans Asterisk 22 mais est maintenant considéré comme hérité (legacy) ; SIP/PJSIP est le protocole préféré pour les nouveaux déploiements.
...
- bindaddr : Adresse à lier à l'écouteur SIP d'Asterisk. Si vous la configurez sur 0.0.0.0 (par défaut), elle se liera à toutes les interfaces.
...
## Configuration des appareils SIP
...
![Un SipPulse Softphone enregistré — le point vert et la ligne de compte (`1001@softphone.sippulse.com.br`) confirment l'enregistrement ; passez un appel depuis le clavier ou les boutons d'appel/vidéo.](../images/softphone/sipphone-registered.png){width=35%}

## Configuration des appareils IAX

IAX2 est un protocole hérité (voir le chapitre *Legacy channels*), et le SipPulse Softphone est uniquement SIP, il ne peut donc pas enregistrer un compte IAX. Si vous devez tester IAX2, utilisez un softphone qui le prend toujours en charge. Créez un nouveau compte IAX,
...
Le fichier extensions.conf est séparé en sections. La première est la section [general] suivie de la section [globals]. Le début de chaque section commence par sa définition de nom (c'est-à-dire [default]) et se termine lorsqu'une autre section est créée.
...
- clearglobalvars : Si clearglobalvars est défini, les variables globales seront effacées et reparsées lors d'un rechargement du dialplan ou d'un rechargement d'Asterisk. Si clearglobalvars n'est pas défini, alors les variables globales persisteront à travers les rechargements et — même si elles sont supprimées du fichier extensions.conf ou de l'un de ses fichiers inclus — elles resteront définies à la valeur précédente.
...
- userscontext : C'est le context où les entrées du fichier users.conf sont enregistrées.
...
Un nom de variable peut être une chaîne alphanumérique commençant par une lettre. Les noms de variables globales ne sont pas sensibles à la casse. Cependant, les variables système (définies par Asterisk sont définies par le canal) sont sensibles à la casse. Ainsi, la variable ${EXTEN} est différente de ${exten}.
...
[Synopsis] Répond à un canal s'il sonne [Description] Answer([delay]) : Si l'appel n'a pas reçu de réponse, l'application y répondra. Sinon, elle n'a aucun effet sur l'appel. Si un délai est spécifié, Asterisk attendra le nombre de millisecondes spécifié dans « delay » avant de répondre à l'appel.
...
| `d` | Permet à l'utilisateur appelant de composer une extension à 1 chiffre en attendant que l'appel reçoive une réponse. Sort vers cette extension si elle existe dans le context actuel, ou vers le context défini dans la variable `EXITCONTEXT`, si elle existe. |
...
| `G(context^exten^pri)` | Si l'appel reçoit une réponse, transfère la partie appelante vers la priorité spécifiée et la partie appelée vers priorité+1. Optionnellement, une extension (ou extension et context) peut être spécifiée ; sinon l'extension actuelle est utilisée. |
...
Dans l'exemple ci-dessus, l'application composera le canal PJSIP correspondant. L'appelant et l'appelé pourraient tous deux transférer l'appel (Tt). La musique d'attente sera entendue au lieu de la tonalité de retour. Si personne ne répond dans les 20 secondes, l'extension passera à la priorité suivante.
...
Pour permettre la numérotation entre extensions, nous pourrions utiliser la variable de canal ${EXTEN}, qui fait référence à l'extension composée. Par exemple, si la plage d'extensions est comprise entre 4000 et 4999 et que toutes les extensions utilisent SIP, nous pourrions adopter la commande suivante :
...
Si vous n'avez aucune restriction à la numérotation externe, vous pourriez simplifier et utiliser ce qui suit :
...
### Réception d'un appel utilisant la numérotation directe entrante (DID)
...
Vous pouvez configurer Asterisk pour composer une extension et, si elle ne reçoit pas de réponse, pour composer plusieurs autres extensions simultanément, comme indiqué dans l'exemple suivant :
...
Dans cet exemple, lorsque quelqu'un compose l'opérateur, le canal DAHDI/1 est initialement essayé. Si personne ne répond après 15 secondes (délai d'attente), les canaux DAHDI/1, DAHDI/2 et DAHDI/3 sonneront simultanément pendant 15 secondes supplémentaires.
...
### Réception des appels dans un réceptionniste numérique
...
Les extensions SIP utilisent `PJSIP/` et les extensions IAX utilisent `IAX2/` — les deux pilotes sont fournis dans Asterisk 22, bien que `chan_iax2` soit maintenant considéré comme hérité et que SIP/PJSIP soit préféré.

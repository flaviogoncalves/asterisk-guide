...
Asterisk interprète « = » et « => » de la même manière. Les différences de syntaxe sont utilisées pour distinguer les objets des variables. Utilisez « = » lorsque vous voulez déclarer une variable et « => » pour désigner un objet. La syntaxe est la même entre tous les fichiers, mais trois types de grammaire sont utilisés, comme discuté ci-dessous.
...
Avec le LAB complet, il est possible de tester tous les scénarios disponibles et de comparer des solutions telles que les ATA, les téléphones IP et les softphones. Vous pouvez également apprendre à utiliser les trunks analogiques et SIP. Vous aurez besoin de :
...
Le troisième LAB utilise un serveur virtualisé sur l'ordinateur portable de l'étudiant. Le problème avec ce modèle est les conflits générés par le port UDP. Parfois, le serveur Asterisk et le softphone essaient d'accéder au même port, empêchant Asterisk de lier le port d'adresse. Un autre problème est la qualité des appels ; les environnements virtuels ne sont pas indiqués pour les applications temps réel telles qu'Asterisk. Utilisez un softphone gratuit pour le serveur et la station de travail et une connexion trunk vers un fournisseur SIP. Vous aurez besoin de :
...
Pour vous aider à comprendre la séquence d'installation, nous avons décrit la séquence d'étapes nécessaires pour installer et configurer Asterisk.
...
La sélection de codec (`disallow`/`allow`) et le `context` par défaut sont configurés sur chaque `endpoint` (montré ci-dessous), et non sur le transport. Les appels anonymes/invités sont gérés par un `endpoint` nommé `anonymous`. Les minuteurs d'enregistrement sont contrôlés par AOR via `maximum_expiration`/`default_expiration`.
...
Dans PJSIP, un client SIP est construit à partir d'un ensemble d'objets liés, attachés ensemble par référence de nom :
...
Avertissement : Utilisez des mots de passe forts, d'au moins 8 caractères, alphanumériques et numériques, et au moins un symbole. Des rapports de serveurs piratés sont apparus dans les listes de diffusion, et des craqueurs de mots de passe par force brute pour SIP sont facilement disponibles pour les script kiddies. La fraude téléphonique coûte des milliers de dollars aux consommateurs et aux fournisseurs.
...
Vous pouvez également créer des extensions IAX. Ce protocole est natif à Asterisk, et nous aurons une section entière consacrée à celui-ci plus tard dans ce livre. Pour l'instant, créons quelques extensions en utilisant le protocole. En tant que première section à configurer, la section [general] a certains paramètres à configurer. Les principales options sont :

- allow/disallow : Définit quels codecs vont être utilisés.
- bindaddr : Adresse à lier à l'écouteur SIP d'Asterisk. Si vous le configurez à 0.0.0.0 (par défaut), il se liera à toutes les interfaces.
- context : Définit le context par défaut pour tous les clients, sauf s'il est modifié dans la section client. Nous avons utilisé dummy pour des raisons de sécurité. Les utilisateurs non authentifiés entrent dans ce context lorsque l'option allowguest est définie sur yes.
...
- bandwidth : Lorsqu'il est défini sur high, il permet la sélection de codecs à large bande passante, tels que le g711 dans leurs variantes ulaw et alaw.
...
Avertissement : Utilisez des mots de passe forts avec au moins 8 caractères, alphanumériques et numériques, et au moins un symbole. Des rapports de serveurs piratés sont apparus dans les listes de diffusion, et des craqueurs de mots de passe par force brute pour les hashs md5 SIP sont disponibles pour les script kiddies. La fraude téléphonique coûte des milliers de dollars aux consommateurs et aux fournisseurs. Exemple :
...
Après avoir défini les téléphones dans le fichier de configuration d'Asterisk, il est temps de configurer le téléphone lui-même. Dans cet exemple, nous montrerons comment configurer un softphone gratuit. La 1ère édition utilisait X-Lite de CounterPath ; ce produit a été abandonné, utilisez donc n'importe quel softphone SIP gratuit moderne (par exemple Zoiper, Linphone ou MicroSIP). Consultez le manuel de votre appareil pour comprendre les paramètres de votre téléphone. Étape 1 : Configurez le téléphone pour utiliser l'extension 6000. Exécutez le programme d'installation. Après l'exécution, ouvrez les paramètres de compte/SIP et ajoutez un nouveau compte SIP. Remplissez les informations requises.

![L'écran de compte du softphone SipPulse — entrez le serveur (votre IP ou domaine Asterisk), le nom d'utilisateur, le mot de passe et le nom d'affichage, puis choisissez le transport (UDP, TCP ou TLS).](../images/softphone/sipphone-account.png){width=35%}
...
![Un softphone SipPulse enregistré — le point vert et la ligne de compte (`1001@softphone.sippulse.com.br`) confirment l'enregistrement ; passez un appel depuis le clavier ou les boutons d'appel/vidéo.](../images/softphone/sipphone-registered.png){width=35%}

## Configuration des périphériques IAX

IAX2 est un protocole hérité (voir le chapitre *Legacy channels*), et le softphone SipPulse est uniquement SIP, il ne peut donc pas enregistrer de compte IAX. Si vous avez besoin de tester IAX2, utilisez un client qui le prend toujours en charge (par exemple Zoiper, qui proposait historiquement IAX). Créez un nouveau compte IAX,
...
Pour accéder à ce trunk, nous utiliserons le nom de canal `PJSIP/siptrunk`. Le paramètre `dtmf_mode=rfc4733` transporte le DTMF hors bande (RFC 4733 remplace l'ancienne RFC 2833 ; la charge utile est identique). L'option `identify`/`match` accepte des adresses IP, des CIDR ou des noms d'hôte, mais les noms d'hôte sont résolus une fois au moment du chargement de la configuration, donc pour un fournisseur avec des IP changeantes, listez explicitement la ou les IP de signalisation. Confirmez l'enregistrement avec `pjsip show registrations`.

## Introduction au dialplan

Le dialplan est comme le cœur d'Asterisk. Il définit comment Asterisk gère chaque appel vers le PBX. Il consiste en des extensions qui créent une liste d'instructions qu'Asterisk doit suivre. Les instructions sont déclenchées par des chiffres reçus du canal ou de l'application. Afin de configurer Asterisk avec succès, il est crucial de comprendre le dialplan. La majeure partie du dialplan est contenue dans le fichier extensions.conf dans le répertoire /etc/asterisk. Ce fichier utilise la grammaire de groupe simple et possède quatre concepts majeurs :
...
Avertissement : Si vous émettez une commande save dialplan depuis la CLI, vous finirez par perdre toutes les remarques et commentaires dans le fichier.
...
- clearglobalvars : Si clearglobalvars est défini, les variables globales seront effacées et réanalysées lors d'un dialplan reload ou Asterisk reload. Si clearglobalvars n'est pas défini, alors les variables globales persisteront à travers les rechargements et — même si elles sont supprimées du extensions.conf ou de l'un de ses fichiers inclus — elles resteront définies à la valeur précédente.
...
- userscontext : C'est le context où les entrées du users.conf sont enregistrées.
...
Un context est la partition nommée du dialplan. Après les sections [general] et [globals], le dialplan est un ensemble de contexts dans lesquels chaque context a plusieurs extensions, chaque extension a plusieurs priorités, et chaque priorité appelle une application avec plusieurs arguments.

![Flux d'appel Asterisk : chaque appel arrive sur un canal (IAX, SIP et autres) en tant que branche d'appel entrante ; le context du canal — défini globalement ou par canal dans le fichier de configuration du canal — décide quel context dans extensions.conf traite l'appel avant qu'il ne parte sur la branche sortante.](../images/04-first-pbx-fig03.png)

![Traitement des appels : le `context=` défini pour un canal (dans chan_dahdi.conf ou pjsip.conf) nomme le context correspondant dans extensions.conf où le dialplan gère l'appel.](../images/04-first-pbx-fig04.png)
...
- ${CONTEXT} Context actuel
- ${CALLERID(name)}
- ${CALLERID(num)}
- ${CALLERID(all)} Identifiant de l'appelant actuel
- ${PRIORITY} Priorité actuelle

Les autres variables spécifiques au canal sont toutes en majuscules. Vous pouvez voir le contenu de plusieurs variables en utilisant l'application dumpchan(). Voici un simple extrait des variables dump-channel.
...
La disposition des champs ci-dessus est la sortie `DumpChan` d'Asterisk 22 (un vrai nom de canal `PJSIP/...`, les champs `CallerIDNum`/`ConnectedLineID` et les lignes `Raw*`/`Transcode`/`BridgeID` que les canaux PJSIP peuplent). Contrairement à l'ancien pilote, un canal PJSIP ne définit pas automatiquement les variables de canal `SIPCALLID`/`SIPUSERAGENT` ; les détails SIP équivalents sont lus à la demande avec les fonctions de dialplan `PJSIP_HEADER()` et `CHANNEL()` — par exemple `${CHANNEL(pjsip,call-id)}`, `${PJSIP_HEADER(read,User-Agent)}` et `${CHANNEL(rtp,dest)}` pour l'adresse RTP distante.
...
- ${DIALEDTIME} -> C'est le temps écoulé depuis la composition d'un canal jusqu'à sa déconnexion.
...
Exemple n°3 : Prend tous les chiffres de la variable ${EXTEN}, sauf l'indicatif régional.
...
La description suivante peut être obtenue en émettant show application dial dans le dialplan. Pour une recherche facile, elle est reproduite ci-dessous. La syntaxe pour l'application Dial est également montrée ci-dessous :
...
| `d` | Permet à l'utilisateur appelant de composer une extension à 1 chiffre en attendant que l'appel reçoive une réponse. Sort vers cette extension si elle existe dans le context actuel, ou vers le context défini dans la variable `EXITCONTEXT`, s'il existe. |
...
| `f` | Force l'identifiant de l'appelant du canal appelant à être défini sur l'extension associée au canal via un `hint` de dialplan. Utile là où le PSTN ne permet pas un identifiant d'appelant arbitraire. |
| `g` | Procède à l'exécution du dialplan à l'extension actuelle si le canal de destination raccroche. |
| `G(context^exten^pri)` | Si l'appel reçoit une réponse, transfère la partie appelante à la priorité spécifiée et la partie appelée à priorité+1. Optionnellement, une extension (ou extension et context) peut être spécifiée ; sinon l'extension actuelle est utilisée. |
...
| `r` | Indique la sonnerie à la partie appelante et ne passe aucun audio jusqu'à ce que le canal appelé réponde. |
...
Dans l'exemple ci-dessus, l'application appellera le canal PJSIP correspondant. L'appelant et l'appelé pourraient tous deux transférer l'appel (Tt). De la musique d'attente sera entendue au lieu de la tonalité de retour. Si personne ne répond dans les 20 secondes, l'extension passera à la priorité suivante.
...
Sauter à une priorité, une extension ou un context particulier [Description] Goto([[context|]extension|]priority) : Cette application fera en sorte que le canal appelant continue l'exécution du dialplan à la priorité spécifiée. Si aucune extension spécifique (ou extension et context) n'est spécifiée, cette application sautera à la priorité spécifiée de l'extension actuelle. Si la tentative de sauter à un autre emplacement dans le dialplan n'est pas réussie, le canal continuera à la priorité suivante de l'extension actuelle.

## Construire un dialplan

Pour construire un dialplan simple, vous devez traiter tous les appels entrants et sortants en créant des contexts et des extensions. Dans cette section, nous vous montrerons comment construire les extensions les plus courantes.

### Appeler entre les extensions

Pour permettre d'appeler entre les extensions, nous pourrions utiliser la variable de canal ${EXTEN}, qui fait référence à l'extension composée. Par exemple, si la plage d'extensions est comprise entre 4000 et 4999 et que toutes les extensions utilisent SIP, nous pourrions adopter la commande suivante :
...
### Recevoir un appel sur l'extension opérateur

Dans l'exemple suivant, l'extension opérateur est 4000. La ligne PSTN est connectée à une interface FXO. Dans le fichier chan_dahdi.conf, le context spécifié est from-pstn. Tout appel provenant du PSTN sera routé vers le context from-pstn dans le dialplan. Cette ligne n'a pas de numérotation directe entrante (DID) ; en tant que tel, nous devrons recevoir l'appel via l'extension « s ». Si vous recevez depuis le trunk SIP, utilisez le context [from-sip].
...
Vous pouvez configurer Asterisk pour appeler une extension et, si elle ne reçoit pas de réponse, pour appeler plusieurs autres extensions simultanément, comme indiqué dans l'exemple suivant :
...
Maintenant que nous avons quelques exemples simples, développons notre apprentissage sur les applications background() et goto(). La clé pour les systèmes interactifs dans Asterisk est l'application background(), qui vous permet d'exécuter un fichier audio qui, lorsque l'appelant appuie sur une touche, est interrompu afin d'envoyer l'appel vers l'extension composée. Syntaxe de l'application background() :

```
exten=>extension, priority, background(filename)
```

Une autre application très utile est goto(). Comme son nom l'indique, elle saute au context, à l'extension et à la priorité indiqués. Syntaxe de l'application goto() :
...
Dans l'exemple suivant, nous créerons un réceptionniste numérique. Il est très simple de modifier le fichier extensions.conf et de configurer les extensions suivantes :
...
4. Les principales différences entre les applications `Playback()` et `Background()` sont (choisissez deux) :
   ...
   - C. Background joue un message et attend que des chiffres soient composés.
   - D. Playback joue un message et attend que des chiffres soient composés.
5. Lorsqu'un appel entre dans Asterisk via une carte d'interface de téléphonie (FXO) sans DID, il est géré dans l'extension spéciale :
   - A. `0`
   - B. `9`
   - C. `s`
   - D. `i`

...
Enfin, les standards ouverts sur lesquels la VoIP est construite offrent la liberté de choisir parmi différents fournisseurs. Cet avantage unique fait du client le roi plutôt qu'un subordonné des opérateurs télécoms et des fabricants de PBX.
...
- **IAX2** est inhabituel : il multiplexe à la fois la signalisation et les médias sur un seul port UDP (4569), ce qui simplifie le passage à travers NAT et pare-feu.
...
H.323 est largement utilisé dans la VoIP. C'est l'un des premiers protocoles VoIP et il est essentiel pour connecter des infrastructures VoIP plus anciennes basées sur des passerelles. H.323 est toujours le standard sur le marché des passerelles, bien que le marché migre lentement vers SIP. Les forces de H.323 incluent sa large adoption sur le marché et sa maturité. Les faiblesses de H.323 sont liées à la complexité de l'implémentation et aux coûts associés aux organismes de normalisation.
...
| IAX2 | RFC 5456 (Informationnel) | `chan_iax2` (cœur ; toujours fourni, considéré comme hérité) | Trunks Asterisk-à-Asterisk ; téléphones IAX2 ; fournisseurs de services IAX |
...
La référence PSTN est **G.711** — c'est la référence pour la qualité « toll » et il se transcode gratuitement dans Asterisk. **G.722** offre une voix large bande (HD) aux mêmes 64 Kbps et est un bon choix LAN/interne. **Opus** est le défaut moderne pour WebRTC et les endpoints SIP capables : il adapte son débit binaire, possède une correction d'erreur directe intégrée et résiste bien à la perte de paquets ; il est fourni sous forme de binaire externe `codec_opus` (téléchargement gratuit). **G.729** reste utile sur les trunks WAN à faible bande passante, mais une utilisation légale nécessite soit le `codec_g729` sous licence de Sangoma (téléchargement gratuit, licence par canal pour l'utiliser), soit l'implémentation open-source **bcg729** comme alternative.

¹ Le binaire `codec_g729` de Sangoma est gratuit à télécharger mais nécessite l'achat d'une licence par canal pour être utilisé légalement. L'open-source `bcg729` est une alternative sans licence.
...
![Mode trunk IAX2 sur Ethernet : un seul appel g.729 a besoin de sa pile d'en-têtes complète (31,2 Kbps), mais un deuxième appel partage ces en-têtes et n'ajoute qu'une petite mini-trame IAX2, ajoutant en moyenne environ 9,6 Kbps de bande passante supplémentaire par appel additionnel.](../images/06-voip-network-fig08.png)
...
1. Lesquels des éléments suivants sont des avantages de la VoIP décrits dans ce chapitre (cochez toutes les cases qui s'appliquent) ?
   ...
   - E. Tarifs d'appel à la minute garantis inférieurs à ceux de n'importe quelle compagnie téléphonique
...
6. SIP est le protocole le plus adopté pour les téléphones IP et est un standard ouvert largement défini par l'IETF dans la RFC 3261.
   - A. Faux
   - B. Vrai
7. IAX2 transporte à la fois la signalisation et les médias sur un seul port UDP, ce qui le rend efficace et facile à traverser NAT. Quel port UDP IAX2 utilise-t-il ?
   - A. 5060
   - B. 1720
   - C. 4569
   - D. 5061
8. IAX a été développé à l'origine par Digium (maintenant Sangoma). Malgré une adoption limitée par les fournisseurs de téléphones, IAX est excellent lorsque vous avez besoin de (cochez toutes les cases qui s'appliquent) :
...
10. Concernant les codecs dans Asterisk 22, cochez toutes les affirmations vraies :
    ...
    - B. Le module codec_g729 de Sangoma est gratuit à télécharger, mais une utilisation légale nécessite l'achat d'une licence par canal.

...
Dans un monde 100 % VoIP en 2026, les types de canaux présentés dans ce chapitre sont de plus en plus rares : la plupart des nouveaux déploiements utilisent des trunks SIP et des endpoints PJSIP sur Ethernet, sans aucun matériel de téléphonie. Asterisk 22 prend néanmoins toujours pleinement en charge la plupart d'entre eux. La connectivité analogique (FXO/FXS) et numérique TDM (E1/T1/ISDN PRI/BRI) est fournie via DAHDI — la pile de pilotes développée à l'origine par Digium, acquise par Sangoma en 2018, après que les anciens pilotes Zaptel ont été renommés suite à un litige sur la marque. La connectivité serveur à serveur via IAX2 est assurée par `chan_iax2`, qui est toujours fourni et pris en charge, mais qui est désormais fermement considéré comme un protocole hérité. Ce chapitre rassemble également le matériel sur le **SIP hérité** : l'ancien pilote `chan_sip` et sa configuration `sip.conf` — supprimés dans Asterisk 21 et absents dans Asterisk 22 — ainsi qu'un guide complet pour migrer un système `sip.conf` existant vers PJSIP. Si vous gérez une boutique 100 % SIP sur PJSIP sans cartes de téléphonie, sans trunks IAX2 et sans `sip.conf` hérité à convertir, vous pouvez ignorer ce chapitre en toute sécurité.

## Canaux analogiques (FXO/FXS)

> **[Note de la 2e éd.]** Mettre à jour les dates/ISBN de la préface pour la 2e édition avant publication.

> **[Note de la 2e éd. — contexte de déploiement]** Depuis Asterisk 22, DAHDI et les cartes de téléphonie analogique restent entièrement pris en charge et DAHDI se compile toujours avec les noyaux actuels. Cependant, la majorité des nouveaux déploiements sont 100 % VoIP (trunks SIP, PJSIP). Le matériel analogique/TDM est désormais un choix de niche, principalement présent dans les environnements hérités, la connectivité PSTN rurale ou les marchés réglementés. Le contenu ci-dessous reste exact pour ces scénarios.
...
Vous pouvez utiliser deux types de signalisation pour la numérotation. La première et la plus courante est la double tonalité multifréquence (dtmf), tandis que l'autre est la numérotation par impulsions (utilisée dans les anciens téléphones à cadran). Les téléphones disposent d'un clavier pour la numérotation, et chaque bouton est associé à deux fréquences : une haute et une basse. Dans le cas de la signalisation dtmf, la combinaison de ces tonalités indique quel chiffre est pressé. MFC/R2 utilise une tonalité multifréquence différente du dtmf.
...
- Numérique : Utilisée lorsque de nombreuses lignes sont nécessaires. Une ligne numérique est généralement fournie par un CSU/DSU ou un multiplexeur fibre. Le connecteur côté utilisateur final est généralement un RJ45. Dans certains pays, les lignes E1 sont fournies à l'aide de deux connecteurs coaxiaux BNC ; dans ce cas, vous aurez besoin d'un balun pour connecter la prise RJ45 à la carte de téléphonie.
- SIP : Cette option a été développée récemment. La ligne téléphonique est fournie à l'aide d'une connexion de données avec signalisation SIP (VoIP). C'est une bonne option à utiliser avec Asterisk car vous n'aurez pas besoin d'acheter une carte de téléphonie. Les appels téléphoniques seront transmis directement au port Ethernet. Un autre avantage est que vous pourriez libérer des ressources de votre CPU en évitant le transcodage de codec.
...
- Loopstart : Utilisé dans la plupart des lignes analogiques, il permet au téléphone d'indiquer « on-hook » et « off-hook » et au commutateur d'indiquer « ring » et « no-ring ». C'est probablement ce que la plupart des gens ont à la maison. Le nom vient du fait que la ligne est toujours ouverte. Lorsque vous fermez la boucle, le commutateur vous fournit une tonalité. Un appel entrant est signalé par une tension de sonnerie de 100 V sur la paire ouverte.

![Asterisk fonctionnant comme une passerelle VoIP : un port FXO se connecte à une extension de PBX hérité tandis qu'un Asterisk distant fournit cette ligne à un téléphone analogique sur IP via un port FXS (une extension hors site, ou OPX).](../images/10-legacy-fig02.png)
...
![Un Xorcom Astribank : une banque de canaux USB montée en rack 19 pouces qui expose des dizaines de ports FXS/FXO (ici une unité de 32 ports) sans consommer d'emplacements PCI dans l'hôte.](../images/10-legacy-fig03.png)

#### Exemple 1 : Une installation FXO, une FXS

Dans cet exemple, nous utiliserons une carte d'interface de téléphonie Sangoma TDM400 (anciennement vendue sous le nom de Digium TDM400) avec un module FXS et un module FXO. Les étapes requises sont listées ci-dessous :
...
![Une carte analogique Sangoma/Digium TDM404P : jusqu'à quatre modules FXS ou FXO se branchent dans les ports numérotés, avec une carte fille d'annulation d'écho matérielle en option et un connecteur d'alimentation 12 V dédié pour les modules FXS.](../images/10-legacy-fig04.png)
...
Attention : Il est nécessaire que vous configuriez au moins l'algorithme d'annulation d'écho pour les canaux. Le paramètre base_exten définit le dialplan de base pour les extensions FXS. Dans ce cas, le premier canal FXS recevra le numéro d'extension 4000, le deuxième 4001, et ainsi de suite. Le contexte dans lequel les lignes (context_phones) et les trunks (context_lines) sont créés est très important. Après avoir généré les fichiers, vous devez inclure le fichier `/etc/asterisk/dahdi-channels.conf` dans le fichier `/etc/asterisk/chan_dahdi.conf` :

```
#include dahdi-channels.conf
```

Note : La signalisation analogique est un peu déroutante ; elle est toujours l'inverse de la carte. Les cartes FXS sont signalées avec FXO tandis que les cartes FXO sont signalées avec FXS. Asterisk parle à ces appareils comme s'il était du côté opposé.

##### Étape 3 : Charger les pilotes du noyau

Maintenant, vous devez charger le module chan_dahdi et le pilote de noyau de carte associé. Utilisez dahdi_hardware pour détecter votre carte et le nom du pilote. Par exemple :
...
Le fxotune est un utilitaire utilisé pour affiner certains paramètres pour les modules FXO. Cet affinement est nécessaire pour ajuster le déséquilibre d'impédance causé par l'hybride. L'utilitaire a trois modes de fonctionnement :
...
> **[Note de la 2e éd.]** La page du projet OSLEC (http://www.rowetel.com/ucasterisk/oslec.html) peut ne plus être à jour ; vérifiez la disponibilité et le statut d'intégration au noyau pour les noyaux modernes avant de la référencer. Pour changer l'algorithme d'annulation d'écho, modifiez le paramètre echo_can dans /etc/dahdi/system.conf. Par exemple :
...
Ces options sont utilisées pour les téléphones connectés aux interfaces FXS. Toutes les fonctionnalités fournies aux téléphones analogiques connectés directement aux interfaces DAHDI sont contrôlées par Asterisk. Adsi (Analog Display Services Interface) : Il s'agit d'un ensemble de normes télécoms utilisées par certaines compagnies de téléphone pour offrir des services tels que l'achat de billets. cancallforward : Active ou désactive le renvoi d'appel (*72 pour activer et *73 pour désactiver). calleridcallwaiting : Active le callerid reçu lors d'une indication d'appel en attente (Yes/No). immediate : En mode immédiat, au lieu de fournir une tonalité, le canal saute immédiatement à l'extension « s » dans le contexte défini. Ceci est utilisé pour créer des lignes directes. threewaycalling : Active ou désactive la conférence à trois. mailbox : Avertit l'utilisateur des messages vocaux disponibles. Il peut s'agir d'un signal sonore ou d'un indicateur visuel (si le téléphone prend en charge cette fonctionnalité). L'argument est le numéro de la boîte vocale. callgroup : Groupe les téléphones pour appeler ou pour décrocher. pickupgroup : Groupe de téléphones pour la prise d'appel.
...
> **[Note de la 2e éd.]** Depuis Asterisk 22, DAHDI et libpri restent entièrement pris en charge, mais les trunks numériques TDM (E1/T1/ISDN PRI) sont de plus en plus remplacés par des trunks SIP dans les nouveaux déploiements. Ce chapitre reste entièrement applicable là où une connectivité TDM est requise ; les lecteurs dans des environnements vierges peuvent préférer le trunking SIP (Chapitre 3) pour une densité de canaux similaire.

Les canaux numériques sont extrêmement courants, vous devrez donc apprendre à implémenter ces canaux si vous souhaitez vous concentrer sur les gros clients. Lorsque le nombre de canaux est élevé — généralement plus de 8 — il est assez courant d'utiliser des interfaces numériques telles que T1/E1/J1. T1 est très courant aux États-Unis, tandis que E1 est courant en Europe et J1 au Japon. Ces types de canaux permettent une bonne densité de circuits — 24 par canal T1 et 30 pour les canaux E1. En Amérique latine, en Chine et en Afrique, il est courant d'utiliser un type de signalisation associée par canal (CAS) connue sous le nom de MFC/R2. Ce chapitre examinera comment implémenter MFC/R2 en utilisant la bibliothèque OpenR2. Aux États-Unis et en Europe, l'Integrated Services Digital Networks (ISDN) PRI est la signalisation la plus courante. Le chapitre discutera également de l'ISDN Basic Rate Interface (BRI), qui est très courant en Europe dans les applications de milieu de gamme. Tous les exemples du livre se concentrent sur les canaux DAHDI. Certaines cartes sont implémentées en utilisant des canaux propriétaires, veuillez donc vérifier auprès de votre fabricant pour plus de détails sur la façon de configurer votre carte spécifique.
...
Les lignes numériques E1/T1 sont une option chaque fois que vous devez implémenter un grand nombre de canaux. Un seul circuit E1 est capable de 30 appels simultanés, et vous pouvez avoir des fonctionnalités telles que la numérotation directe entrante (DID), l'identification de l'appelant (Caller ID) et une signalisation avancée. La ligne E1/T1 peut arriver dans votre entreprise de plusieurs manières en utilisant des paires torsadées, de la fibre et des micro-ondes, selon votre pays. Les lignes numériques sont fournies à votre entreprise en utilisant UTP, fibre ou micro-ondes. Des modems et des multiplexeurs (MUX) sont utilisés pour fournir la ligne physique. La connexion à une ligne T1 est toujours basée sur un connecteur RJ45. Cependant, les lignes E1 peuvent également être fournies en utilisant BNC. Il est très important de connaître à l'avance le type de connecteur que vous allez recevoir, principalement dans les lignes E1. Habituellement, tout l'équipement jusqu'au RJ45 est fourni par la compagnie de téléphone (TELCO).

![Comment les circuits E1/T1 sont fournis : la TELCO peut fournir le trunk sur cuivre UTP (modem HDSL pour E1, ou connexion directe par carte pour T1), sur fibre optique via un multiplexeur optique, ou sur une liaison radio micro-ondes.](../images/10-legacy-fig05.png)

![UTP ou BNC ? La plupart des cartes numériques utilisent des connecteurs RJ45 (UTP), mais certaines lignes E1 sont fournies sur coax BNC double, auquel cas un balun est nécessaire pour adapter la paire coaxiale à la prise RJ45 de la carte.](../images/10-legacy-fig06.png)
...
![Modulation par impulsions codées (PCM) : le signal vocal analogique de 4 kHz est échantillonné 8 000 fois par seconde (Nyquist) et codé en un flux de bits numérique de 64 Kbps.](../images/10-legacy-fig07.png)
...
L'ISDN est souvent utilisé en Europe et aux États-Unis. C'est un réseau vocal numérique, standardisé par l'Union internationale des télécommunications (UIT) en 1984. L'ISDN fournit deux types de canaux :

- Canaux porteurs (Bearer) o Voix o Données
...
Il existe plusieurs types de bus sur votre PC. Il est très important que vous ayez la bonne carte pour votre serveur. L'aperçu suivant décrit les cartes les plus fréquemment utilisées :
...
> **[Note de la 2e éd.]** Sangoma a acquis Digium en 2018. Les cartes de marque Digium sont désormais vendues et prises en charge sous la marque Sangoma. Vérifiez la disponibilité actuelle des modèles sur le site Web de Sangoma (www.sangoma.com) car certaines anciennes références peuvent être abandonnées.
...
L'annulation d'écho matérielle réduit la charge sur le CPU hôte. Pour les cartes avec plus d'une seule interface E1, l'annulation d'écho matérielle peut aider à soulager votre processeur. Les nouveaux annulateurs d'écho logiciels améliorés tels que l'OSLEC réduisent le besoin d'un annulateur d'écho matériel. Pour choisir entre les annulateurs d'écho matériels et logiciels, vous devez tenir compte de la puissance de traitement disponible sur votre serveur et du nombre de circuits E1. Un processus d'annulation d'écho peut utiliser jusqu'à neuf MIPS (millions d'instructions par seconde) par canal vocal avec 128 taps d'amplitude en utilisant OSLEC (Référence : Xorcom Ltd.). Si vous considérez 1 cycle CPU par instruction (ce qui n'est pas toujours correct selon le processeur et l'implémentation logicielle elle-même), nous parlons de 1,080 GHz pour quatre E1.

#### Type de signalisation

Sélectionner le type de signalisation (par exemple, T1 CAS, T1 PRI, E1 CAS R2 ou E1 CAS ISDN) n'est pas une tâche facile. Cela dépend vraiment de ce que vous avez à disposition dans votre région et à quel prix. La signalisation par canal commun (CCS) est souvent meilleure que la signalisation associée par canal (CAS). Cependant, elle n'est souvent pas disponible. Aux États-Unis, vous pouvez généralement choisir, car la plupart des TELCO proposent T1 CAS pour les utilisateurs réguliers et T1 PRI pour les utilisateurs avancés (par exemple, les centres d'appels). En Amérique latine, E1 CAS R2 est prédominant, mais l'ISDN PRI est disponible dans certaines villes.
...
La configuration d'une carte d'interface de téléphonie implique plusieurs étapes nécessaires. Dans ce chapitre, nous vous montrerons trois des scénarios les plus courants :

- Connexion numérique utilisant ISDN PRI
- Connexion numérique utilisant ISDN BRI
- Connexion numérique utilisant MFC/R2

Il existe deux façons de configurer les canaux DAHDI. La première consiste à le configurer manuellement avec un contrôle total de tous les paramètres. La deuxième façon consiste à utiliser l'utilitaire dahdi_genconf pour détecter et configurer les cartes.

#### Détection et configuration automatiques

Grâce à l'équipe de développement DAHDI, nous disposons désormais de la détection et de la configuration automatiques des cartes. Étape 1 : Pour générer la configuration automatiquement, utilisez l'utilitaire dahdi_genconf, qui détectera la carte et générera les fichiers /etc/dahdi/system.conf et dahdi-channels.conf.
...
Étape 4 : En utilisant dahdi_test, vérifiez les interruptions manquées Vous pouvez vérifier le nombre d'interruptions manquées en utilisant l'utilitaire dahdi_test compilé avec les cartes DAHDI. Un nombre inférieur à 99,987 % indique des problèmes possibles. Vous trouverez dahdi_test dans
...
Après avoir configuré les pilotes, vous pouvez simplement redémarrer le serveur. Si vous avez installé DAHDI avec make config, vous n'aurez rien à faire de plus. Le pilote du noyau sera automatiquement chargé et configuré. Cependant, il est parfois utile de charger et de décharger les pilotes manuellement. Exemple :
...
Les alarmes indiquent des problèmes avec le port : Alarme rouge : Impossible de maintenir la synchronisation avec le commutateur distant. Il s'agit généralement d'un problème physique, tel qu'un déséquilibre de code de ligne ou de trame. Alarme jaune : Signale que le commutateur distant est en alarme rouge. Cela indique que le commutateur distant ne reçoit pas vos transmissions. Alarme bleue : Reçoit tous les 1 non tramés sur tous les timeslots ; dahdi_tool ne détecte actuellement pas d'alarme bleue. Loopback : Le port est soit en loopback local, soit en loopback distant
...
language : Active l'internationalisation et configure une langue. Cette fonctionnalité configurera les messages système pour une langue spécifique. L'anglais est la seule langue avec des invites complètes disponibles à partir de l'installation standard. musiconhold : Sélectionnez la classe de musique d'attente.
...
pridialplan : Requis pour certains commutateurs qui ont besoin d'une spécification de dialplan. Cette option est ignorée par de nombreux commutateurs. Les options valides sont private, national, international et unknown.
...
- bri_cpe : Utilisé lorsque Asterisk est connecté comme un CPE à un trunk ISDN BRI
...
De nombreuses options Caller ID sont disponibles. Certaines peuvent être désactivées, bien que la plupart soient activées par défaut. usecallerid : Active ou désactive la transmission du Caller ID pour les canaux suivants (Yes/No). Note : Si votre système nécessite deux sonneries avant de répondre, essayez de désactiver cette fonctionnalité afin qu'il réponde immédiatement. hidecallerid : Masque le Caller ID (Yes/No). calleridcallwaiting : Active la réception du Caller ID lors d'une indication d'appel en attente (Yes/No). callerid : Configure une chaîne Caller ID pour un canal spécifique. L'appelant peut être configuré avec « asreceived » dans les interfaces de trunk pour transmettre le Caller ID.

```
callerid = "Flavio Eduardo Gonçalves" <48 30258500>
```

Note : La plupart des TELCO exigent que vous configuriez votre Caller ID correct. Si vous ne transmettez pas le bon Caller ID, vous ne devriez pas pouvoir composer vers l'extérieur via la TELCO. D'un autre côté, vous pourrez recevoir des appels même sans configurer le Caller ID.

#### Options de qualité audio

Ces options ajustent certains paramètres d'Asterisk qui affectent la qualité audio dans les canaux DAHDI. echocancel : Désactive ou active l'annulation d'écho. Vous devez garder cette fonctionnalité activée. Elle accepte « yes » ou le nombre de taps. Explication : Comment fonctionne l'annulation d'écho ? La plupart des algorithmes d'annulation d'écho fonctionnent en générant plusieurs copies d'un signal reçu, chacune étant retardée d'un petit intervalle. Ce petit flux est nommé « tap ». Le nombre de taps détermine le retard d'écho qui peut être annulé. Ces copies sont retardées, ajustées et soustraites du signal original. L'astuce consiste à ajuster le signal retardé exactement à ce qui est nécessaire pour supprimer l'écho. echocancelwhenbridged : Active ou désactive l'annulateur d'écho pendant un appel TDM pur. Ce n'est généralement pas requis. rxgain : Ajuste le gain de réception audio pour augmenter ou diminuer le volume de réception (-100 % à 100 %). txgain : Ajuste le gain de transmission audio pour augmenter ou diminuer le volume de transmission (- 100 % à 100 %). Exemple :
...
Ces options modifient la façon dont les informations d'appel sont enregistrées dans la base de données des enregistrements détaillés des appels (CDR). amaflags : Affecte la catégorisation CDR. Il accepte ces valeurs :
...
accountcode : Il configure un code de compte pour un canal spécifique. Il peut contenir n'importe quelle valeur alphanumérique, généralement le département ou le nom de l'utilisateur.
...
La carte utilisée pour signaler MFC/R2 est la même que celle utilisée pour signaler l'ISDN. Il est possible d'utiliser MFC/R2 sur les canaux DAHDI en utilisant la bibliothèque appelée libopenR2 (www.libopenr2.com). Cette bibliothèque ne faisait pas partie des versions d'Asterisk antérieures à 1.6.2.
...
| --- | --- |
| Libre | B-1 |
| Envoyer la tonalité d'information spéciale | B-2 |
...
| Ligne d'abonné libre, pas de facturation | B-7 |
| Ligne d'abonné hors service | B-8 |
| Libre | B-9 |
| Libre | B-10 |
| Libre | B-11 |
| Libre | B-12 |
...
![Un flux d'appel MFC/R2 complet entre Asterisk et la TELCO : la signalisation de ligne (Inactif, Saisi, Accusé de réception de saisie, Répondu, Clearback, Clear Forward) est échangée dans le timeslot 16, les chiffres composés et les signaux arrière "envoyer le chiffre suivant" (groupes I/A/B) voyagent intrabande, et les tonalités audibles atteignent l'abonné.](../images/10-legacy-fig11.png)
...
> **[Note de la 2e éd.]** L'arborescence Asterisk 1.4 patchée de la 1ère édition est obsolète ; pour Asterisk 22, la prise en charge de MFC/R2 via libopenr2 est intégrée dans l'arborescence source principale, et les étapes ci-dessous utilisent désormais les dépôts Git actuels au lieu du `svn.digium.com` retiré. Envisagez de condenser ces étapes de construction historiques pour l'édition finale.

Étape 1 : Vérifiez les patchs pour la version d'Asterisk que vous souhaitez installer.

```
apt-get install git
```

Étape 2 : Téléchargez le code Asterisk modifié avec le patch installé.

> **[Note de la 2e éd.]** L'arborescence de patch SVN originale d'Asterisk 1.4 a été remplacée ci-dessous par les dépôts Git actuels. Sur Asterisk 22, le patch MFC/R2 est inutile — `chan_dahdi` construit la prise en charge R2 directement avec libopenr2 — vous n'avez donc besoin que de la bibliothèque openr2 plus une construction Asterisk normale.
...
mfcr2_max_ani : Quantité maximale de chiffres ANI à demander mfcr2_max_dnis : Quantité maximale de chiffres DNIS à demander mfcr2_get_ani_first : S'il faut obtenir l'ANI avant le DNIS (requis par certaines TELCO) mfcr2_category : Catégorie de l'appelant. Vous pouvez définir la variable MFCR2_CATEGORY avant de démarrer l'appel mfcr2_logdir : Répertoire pour enregistrer les fichiers d'appel. (/var/log/asterisk/mfcr2/directory) mfcr2_call_files : S'il faut enregistrer les appels

- mfcr2_logging : valeurs de journalisation
- cas – bits ABCD pour tx et rx
- mf – Tonalités multifréquences
- stack – sortie verbeuse de la pile de canaux et de contexte
...
Dans ce chapitre, nous apprendrons le protocole Inter-Asterisk eXchange (IAX), y compris ses forces et ses faiblesses. Des détails tels que le mode trunk et l'interconnexion de deux serveurs Asterisk seront également couverts. Toutes les références dans ce document correspondent à la version 2 d'IAX. Le protocole IAX fournit le transport multimédia et la signalisation pour la voix et la vidéo. IAX est très innovant ; il économise de la bande passante en mode trunk et est beaucoup plus simple que SIP lorsque vous devez traverser un NAT. L'utilisation principale d'IAX de nos jours est d'interconnecter les serveurs Asterisk. IAX a été créé principalement pour la voix, mais il peut également accueillir de la vidéo et d'autres flux multimédias. IAX a été inspiré d'autres protocoles VoIP, tels que SIP et MGCP. Au lieu d'utiliser deux protocoles distincts pour la signalisation et les médias, IAX les a unifiés pour créer un protocole unique. IAX n'utilise pas RTP pour le transport multimédia ; au lieu de cela, il intègre les médias dans la même connexion UDP.

> **[Note de la 2e éd. — Statut dans Asterisk 22]** `chan_iax2` est toujours inclus et entièrement pris en charge dans Asterisk 22 LTS, donc tout ce qui se trouve dans ce chapitre reste valide. Cependant, IAX2 est désormais un protocole hérité et voit relativement peu de nouveaux déploiements. L'industrie VoIP a largement convergé vers SIP (via `chan_pjsip` dans Asterisk 22) pour le trunking des fournisseurs et l'interconnexion des serveurs. Le principal argument de vente restant d'IAX2 est sa **traversée NAT à port unique** : toute la signalisation et les médias circulent sur un seul port UDP (4569 par défaut), ce qui simplifie grandement la configuration du pare-feu et du NAT par rapport à SIP + RTP. Si vous construisez un nouveau trunk Asterisk-à-Asterisk et que le NAT n'est pas une préoccupation, les trunks PJSIP sont l'approche moderne recommandée. IAX2 est conservé ici car il reste un choix valide, surtout dans les environnements où un seul port UDP peut être ouvert à travers un pare-feu.
...
- Réduire la bande passante requise pour le transport multimédia et la signalisation
- Fournir une transparence NAT
- Être capable de transmettre les informations du dialplan
- Prendre en charge l'utilisation efficace de la pagination et de l'interphone

IAX est un protocole de signalisation et de média pair-à-pair similaire à SIP sans utiliser RTP. L'approche de base consiste à multiplexer les flux multimédias sur une seule connexion UDP entre deux hôtes. Le plus grand avantage de cette approche est sa simplicité lors de la traversée de connexions via NAT, régulièrement trouvée dans les modems xDSL. IAX utilise un seul port, UDP 4569 par défaut, puis utilise un numéro d'appel avec 15 bits pour multiplexer tous les flux. Le protocole IAX utilise des processus d'enregistrement et d'authentification similaires au protocole SIP. Une description du protocole peut être trouvée sur http://www.ietf.org/internet-drafts/draft-guy-iax-05.txt
...
Asterisk prend en charge les téléphones IP basés sur IAX tels que l'ATCOM et l'ancien ATA de Digium (appelé IAXy) ainsi que les soft-phones tels que Zoiper. Le processus pour les soft-phones, les ATA et les téléphones matériels est similaire. Pour configurer un appareil IAX, vous devez modifier le fichier iax.conf dans /etc/asterisk

```
directory.
```

Nous utiliserons Zoiper (www.zoiper.com) comme exemple. C'est un soft-phone complet et gratuit. Étape 1 : Faites une sauvegarde du fichier iax.conf original en utilisant :
...
Dans les commandes ci-dessus, nous avons défini un ami nommé [2003]. Le contexte est le default (dans les premiers laboratoires, nous utilisons toujours le contexte default pour éviter toute confusion ; ce contexte sera entièrement expliqué au chapitre 9). La ligne « host=dynamic » fournit un enregistrement dynamique de l'adresse IP du téléphone. Étape 3 : Téléchargez et installez Zoiper™ à partir de l'URL suivante : http://www.zoiper.com/ Note : Les URL changent fréquemment. Veuillez recourir à « googler » si vous ne trouvez pas le fichier à cette URL spécifique. Vous pouvez également choisir d'autres soft-phones pour le laboratoire. Étape 4 : Configurez un compte IAX dans le client (clic droit sur l'icône de la barre d'état Zoiper → *Add account* → IAX). Notez que le softphone SipPulse est uniquement SIP et ne peut pas s'enregistrer via IAX2, donc pour les tests IAX, vous avez besoin d'un client qui prend toujours en charge le protocole.
...
> **[Note de la 2e éd.]** Le nombre de fournisseurs VoIP commerciaux compatibles IAX a considérablement diminué depuis Asterisk 16. La plupart des fournisseurs proposent désormais exclusivement des trunks SIP/PJSIP. Avant de choisir un fournisseur IAX, confirmez qu'ils maintiennent activement leur infrastructure IAX. Pour les nouvelles intégrations de fournisseurs, un trunk PJSIP est l'alternative recommandée.
...
Il est très facile de connecter un serveur à un autre. Vous n'aurez pas besoin de les enregistrer car les adresses IP sont déjà connues. Vous devrez créer les pairs et les utilisateurs dans le fichier iax.conf. Toutes les extensions sur le site HQ commencent par 20 suivies de deux chiffres (par exemple, 2000). Dans la branche, toutes les extensions commencent par 22 suivies de deux chiffres (par exemple, 2200). Nous utiliserons le trunk. Vous aurez besoin d'une source de synchronisation DAHDI pour activer cette fonctionnalité. Étape 1 : Modifiez le fichier iax.conf sur le serveur de la branche.

![Connexion de deux serveurs Asterisk avec un trunk IAX : le serveur HQ (192.168.1.1, extensions 20xx) et le serveur de la branche (192.168.1.2, extensions 22xx) se joignent via un seul trunk IAX — aucun enregistrement n'est nécessaire car les deux adresses IP sont fixes et connues.](../images/10-legacy-fig15.png)
...
Étape 2 : Configurez le fichier extensions.conf sur le serveur de la branche
...
Étape 5 : Testez un appel depuis le téléphone 2000 sur le serveur HQ vers le téléphone 2200 sur le serveur de la branche.
...
![Le flux de décision d'authentification IAX pour un appel entrant : Asterisk se branche sur le fait qu'un nom d'utilisateur est fourni, qu'il correspond à une section, que l'adresse IP source est autorisée et que le secret (texte en clair, MD5 ou RSA) correspond — acceptant l'appel avec le contexte et les options de pair de cette section, ou le refusant.](../images/10-legacy-fig16.png)

Lorsqu'Asterisk reçoit une connexion entrante, les informations initiales peuvent inclure un nom d'utilisateur (à partir du champ « username= ») ou non. La connexion entrante a également une adresse IP, qu'Asterisk utilise aussi pour l'authentification. Si un utilisateur est fourni, Asterisk : 1. Recherche dans iax.conf une entrée avec type=user (ou type=friend avec un nom de section correspondant au nom d'utilisateur). S'il ne la trouve pas, Asterisk refuse la connexion. 2. Si l'entrée trouvée a des configurations deny/allow, il compare l'adresse IP de l'appelant pour déterminer s'il faut accepter l'appel ou non en fonction des clauses deny/allow. 3. Il vérifie le mot de passe (secret) en utilisant le texte en clair, md5 ou RSA. 4. Il accepte la connexion et envoie l'appel au contexte spécifié dans la ligne « context= » du fichier iax.conf. Si un nom d'utilisateur n'est pas fourni, Asterisk : 1. Recherche une entrée contenant type=user (ou type=friend) dans le fichier iax.conf sans secret spécifié. Il vérifie également les clauses deny/allow. Si une entrée est trouvée, la connexion est acceptée et le nom de la section est utilisé comme nom d'utilisateur. 2. Recherche une entrée contenant type=user (ou type=friend) dans le fichier iax.conf avec un secret ou une clé RSA spécifiée. Il vérifie les clauses deny/allow. Si une entrée est trouvée, il tente d'authentifier l'appelant en utilisant le secret spécifié ; s'il correspond, il accepte la connexion. Le nom de la section est le nom d'utilisateur. Supposons que votre fichier iax.conf ait les entrées suivantes :
...
permit = <ipaddr>/<netmask> Les règles sont interprétées en séquence, et toutes sont évaluées (ce concept est différent des ACL deny = <ipaddr>/<netmask> généralement trouvées dans les routeurs et les pare-feux). Exemple n°1 permit=0.0.0.0/0.0.0.0 deny=192.168.0.0/255.255.255.0 Refusera tout paquet du réseau 192.168.0.0/24 Exemple n°2 deny=192.168.0.0/255.255.255.0 permit=0.0.0.0/0.0.0.0 Il autorisera tout paquet. La dernière instruction remplace la première.
...
Il est possible d'utiliser IAX avec une authentification forte en utilisant des clés RSA asymétriques. Selon le code source (res_krypto.c), Asterisk utilise des clés RSA avec un algorithme SHA-1 pour les condensés de message au lieu du MD5 plus faible. Voici un guide étape par étape pour configurer deux serveurs en utilisant des clés RSA.

##### Configuration du serveur pour la branche

Étape 1 : Générez les clés RSA sur le serveur de la branche
...
Étape 4 : Modifiez le fichier iax.conf sur le serveur de la branche.
...
Étape 8 : Configurez le fichier extensions.conf sur le serveur de la branche
...
Étape 11 : Testez un appel depuis le téléphone 2000 sur le serveur HQ vers le téléphone 2200 sur le serveur de la branche.
...
- Maxjitterbuffer=<ms> - Généralement inférieur à 100 ms
...
![Asterisk connecté à un fournisseur de services VoIP sur Internet ou un WAN privé, avec des téléphones SIP locaux enregistrés sur le serveur Asterisk](../images/07-sip-and-pjsip-fig07.png)
...
Étape 2 : Configurez le [peer] sur sip.conf Créez une entrée de type peer pour le fournisseur souhaité afin de simplifier la numérotation d'Asterisk.
...
![Connexion de deux serveurs Asterisk utilisant SIP : le serveur A (extensions 4400/4401) et le serveur B (extensions 4500/4501) échangent la signalisation SIP afin que les utilisateurs sur chaque PBX puissent composer l'autre](../images/07-sip-and-pjsip-fig08.png)
...
Ce paramètre permet à une invitation externe d'être traitée sans authentification. Il traite l'appel dans le contexte défini dans la section générale ou dans l'instruction de domaine. Attention : Si vous définissez un contexte dans la section générale avec accès au PSTN, un utilisateur externe peut composer le PSTN via votre PBX. Dans ce cas, vous encourrez des frais. N'autorisez que vos propres extensions dans le contexte défini dans la section générale.

![Connexion à d'autres serveurs SIP par domaine : youdomain.com et yourpartnerdomain.com échangent la signalisation SIP, afin que les utilisateurs tels que lee et bruce puissent appeler chuck et norris en utilisant des URI SIP](../images/07-sip-and-pjsip-fig09.png)
...
- subscribecontext=sip_subscribers : Contexte où chercher les hints
- notifyring=yes : Envoyer SIP NOTIFY sur sonnerie
- notifyhold=yes : Envoyer SIP NOTIFY sur attente
- counteronpeer (renommé depuis limitonpeer pour Asterisk 1.4.x) : Appliquer le compteur uniquement sur le côté pair
...
Étape 2 : Configurez maintenant le soft-phone pour utiliser la présence. Nous vous montrerons comment configurer X-Lite.
...
Étape 3 : Ajoutez le contact aux autres soft-phones. Dans cet exemple, le Xlite est le compte 2000, nous ajouterons donc un contact pour le compte 2001. Séquence : Ouvrez le panneau de droite (panneau de présence dans Xlite)->Cliquez sur Contacts->Ajouter un contact. Remplissez le nom 2001. Affichez en tant que 2001 et n'oubliez pas de cocher la case Afficher la disponibilité de ce contact.

Étape 4 : Appelez maintenant l'extension 2001 et vérifiez l'état du téléphone dans le panneau de droite du soft-phone. Utilisez la commande de console `core show hints` pour voir l'état de présence changer dans le serveur (dans chan_sip hérité, `sip show inuse` montrait combien d'appels vous aviez sur chaque ligne). Sur Asterisk 22, utilisez `pjsip show endpoints` pour inspecter l'état de l'endpoint et du canal. L'état de présence/BLF apparaît dans les contacts ou le panneau BLF du softphone — la façon exacte dont il est affiché dépend du client.
...
À partir de la version 1.4, vous pouvez spécifier différents codes pour la signalisation (SIP), l'audio (RTP) et la vidéo (RTP).
...
![Flux de décision d'authentification chan_sip hérité : Asterisk vérifie l'en-tête From par rapport à sip.conf, tente la section type=user/peer correspondante et les informations d'identification MD5, et revient à insecure=invite ou allowguest avant d'autoriser ou de refuser l'appel](../images/07-sip-and-pjsip-fig10.png)

```
allowguest=yes/no
```

Ce paramètre contrôle si un utilisateur sans pair correspondant peut s'authentifier sans nom et sans secret. Nous avons discuté de ce paramètre dans la section sur la prise en charge des domaines.
...
Cette commande est utilisée lorsqu'Asterisk est connecté à un proxy SIP. Elle crée dynamiquement un pair pour chaque appel. Lorsque cette option est activée, tout UAC peut se connecter au serveur Asterisk. Il est important de limiter la connexion IP au proxy SIP. Le proxy SIP, à son tour, s'occupe du contrôle d'accès. La configuration du pair est basée sur les options générales ainsi que sur le champ d'en-tête « Contact » du paquet SIP. Attention : Utilisez ceci avec une extrême prudence car cela ouvre complètement Asterisk.
...
Attention : N'oubliez pas d'utiliser le paramètre –n ; le retour chariot sera utilisé dans le calcul md5.
...
Ceci termine les appels sans activité RTP même en attente (doit être supérieur à rtptimeout).

### Traversée NAT SIP (sip.conf)

La *théorie* du NAT (les quatre types de NAT, le problème de l'en-tête Contact, les keep-alives et le forçage des médias via le serveur) est au niveau du protocole et est couverte dans le chapitre *SIP & PJSIP en profondeur*. Les paramètres `sip.conf` montrés ici (`nat=`, `qualify=`, `directmedia=`, `externaddr=`, `localnet=`) sont du **chan_sip hérité** et ont été supprimés dans Asterisk 21+. Sur PJSIP, ceux-ci correspondent aux paramètres de transport/endpoint tels que `rewrite_contact=yes`, `force_rport=yes`, `rtp_symmetric=yes`, `direct_media=no`, `external_media_address`, `external_signaling_address` et `local_net=` sur le transport, plus `qualify_frequency=` sur l'AOR.
...
- nat = comedia — Envoyer les médias vers le port sur lequel Asterisk les a reçus, indépendamment de l'endroit où le SDP dit de les envoyer.
...
Lorsque vous mettez l'instruction « nat=force_rport » dans le fichier sip.conf, vous dites à Asterisk d'ignorer l'adresse contenue dans le champ d'en-tête « Contact » de l'en-tête SIP et d'utiliser l'adresse IP source et le port dans l'en-tête IP du paquet et également de renvoyer les médias vers l'adresse d'où ils ont été reçus, en ignorant le contenu de l'en-tête SDP.

```
nat=force_rport,comedia
```

Il est nécessaire de garder le mappage NAT ouvert. Si le NAT expire, Asterisk ne peut pas envoyer d'invitation à l'UAC. L'UAC est capable d'envoyer des appels, mais pas d'en recevoir. L'instruction suivante peut être utilisée pour garder le NAT ouvert.
...
Tous les scénarios précédents supposent que le serveur Asterisk a une adresse Internet externe (valide). Parfois, le serveur Asterisk est implémenté derrière un pare-feu avec NAT. Dans ce cas, il est nécessaire d'effectuer quelques configurations supplémentaires.
...
Étape 1 : Configurez le pare-feu pour rediriger statiquement le port UDP 5060 vers le serveur Asterisk. Étape 2 : Configurez le pare-feu pour rediriger statiquement les ports UDP de 10000 à 20000. Si vous souhaitez restreindre le nombre de ports ouverts, vous pouvez modifier le fichier rtp.conf pour changer la plage de ports RTP. Une autre façon consiste à utiliser un pare-feu intelligent qui prend en charge le protocole SIP pour ouvrir dynamiquement les ports RTP.
...
Le premier paramètre externaddr dit à Asterisk d'inclure l'adresse IP externe à l'intérieur des en-têtes SIP pour les destinations externes. Le deuxième paramètre localnet permet à Asterisk de différencier les adresses externes et internes. En option, vous pouvez utiliser externhost si vous utilisez un DNS dynamique avec une adresse DHCP sur le serveur.
...
4. Pour une facturation précise sur les canaux analogiques, vous devez détecter exactement quand l'extrémité distante répond. Quelle fonctionnalité activez-vous sur Asterisk (et demandez-vous à la TELCO) pour faire cela ?

...
- Implémenter un comportement pour les heures de fermeture dans votre PBX
...
Cette application joue les noms de fichiers donnés (n'incluez pas l'extension). Des options peuvent également être incluses après un symbole pipe. L'option 'skip' provoque l'omission de la lecture du message si le canal n'est pas dans l'état 'up' (c'est-à-dire qu'il n'a pas encore été répondu).
...
### Correspondance pendant la composition

Il s'agit d'un menu de configuration d'entreprise pour recevoir des appels. L'application background lit le context actuel et définit la longueur maximale pour chaque numéro pour toute combinaison possible.
...
Lorsque vous appelez cette entreprise, le message de bienvenue est joué en premier. Après cela, Asterisk attend qu'un chiffre soit composé. Numéro composé Asterisk Action Appelle immédiatement Dial(DAHDI/1) Attend le délai d'attente, puis va à Dial(DAHDI/2) Appelle immédiatement (DAHDI/3) Appelle immédiatement (DAHDI/4) Attend le délai d'attente, puis se déconnecte Appelle immédiatement Dial(DAHDI/5) Appelle immédiatement Dial(DAHDI/6) Se déconnecte immédiatement Il est important d'éviter toute ambiguïté dans les menus. Tout le monde veut obtenir une réponse rapidement. Pour cette raison, vous ne devriez pas utiliser les numéros 2, 21 ou 22.
...
- DBdeltree(<family>) — application qui supprime toute une famille/sous-arbre

L'ancienne application `DBdel()` n'existe plus dans Asterisk 22. Supprimez une seule clé avec la fonction de dialplan `DB_DELETE()` — par exemple `Set(x=${DB_DELETE(family/key)})` ou, en tant qu'opération d'écriture, `Set(DB_DELETE(family/key)=)`. `DBdeltree()` (supprimer toute une famille/sous-arbre) est toujours une application.
...
Dans cet exemple, vous apprendrez à implémenter le transfert d'appel immédiat et le transfert d'appel sur occupation. Nous utiliserons *21* pour programmer le transfert d'appel immédiat et *61* pour programmer le transfert d'appel sur occupation. Pour annuler la programmation, utilisez #21# et #61#, respectivement. Utilisez l'exemple ci-dessus pour peupler la base de données. Familles utilisées :

- CFIM – Call Forward Immediate (Transfert d'appel immédiat)
- CFBS – Call Forward on Busy status (Transfert d'appel sur occupation)
- DND – Do Not Disturb (Ne pas déranger)

Essayez de peupler la base de données en composant :
...
- *41* (Extension pour activer le ne pas déranger)
...
La sous-routine ci-dessus vérifie si la base de données contient les paires clé:valeur correspondant à CFIM, CFBS ou DND, puis les traite de manière appropriée. La sous-routine follow appelle la routine de composition :
...
L'ancienne application `LookupBlacklist()` a été **supprimée** d'Asterisk (elle a disparu avec le mécanisme hérité "saut de priorité+101"). Dans Asterisk 22, vous construisez une liste noire directement avec la fonction `DB_EXISTS()` (qui teste à la fois la présence d'une clé et, lorsqu'elle est trouvée, expose sa valeur dans `${DB_RESULT}`) plus `GotoIf`. Stockez chaque numéro bloqué comme une clé dans une famille `blacklist`, puis vérifiez l'ID de l'appelant en haut de votre context entrant :
...
`DB_EXISTS(blacklist/${CALLERID(num)})` renvoie `1` lorsque le numéro de l'appelant est présent dans la base de données (envoyant l'appel vers le context `blocked`) et `0` sinon, donc l'appel se poursuit vers le `Dial()` normal.
...
Note : Toute valeur peut être associée à la clé. Le test `DB_EXISTS()` recherche la clé, pas la valeur. Pour effacer le numéro de la liste noire, vous pouvez utiliser :

```
CLI>database del blacklist <name/number>
```

## Contexts basés sur le temps

Dans la figure suivante, nous avons un dialplan avec trois contexts. Le context [incoming] est l'endroit où les appels sont généralement reçus. Nous avons inclus quatre lignes qui changent de comportement en fonction de l'heure du système, comme illustré ci-dessous :

```
include => context,<times>,<weekdays>,<mdays>,<months>
```

L'Asterisk moderne (y compris 22) sépare les champs time-include par des **virgules**, pas par des pipes. L'ancienne forme avec pipe (`include => context|times|weekdays|mdays|months`) est analysée comme un nom de context littéral simple et échoue silencieusement à appliquer toute condition temporelle.

Pendant les heures de travail normales, le traitement sera redirigé vers le mainmenu, où il appellera probablement un IVR pour gérer l'appel entrant. Si l'appel a lieu en dehors des heures d'ouverture, il appellera l'extension de sécurité définie dans la variable ${SECURITY}. Si l'extension de sécurité ne répond pas à l'appel, il sera envoyé vers la messagerie vocale de l'opérateur.
...
Dans Asterisk 22, le séparateur de champ est une **virgule**, pas un pipe (la forme avec pipe a été dépréciée dans Asterisk 1.6). Un champ optionnel `timezone` est pris en charge, et chaque étiquette de branche utilise la forme habituelle `[[context,]extension,]priority`.

Cette application peut remplacer le context basé sur le temps et semble plus facile à comprendre et à lire. Vous pouvez spécifier l'heure comme suit :

- <timerange>=<heure>':'<minute>'-'<heure>':'<minute> |"*"
- <daysofweek>=<nomdujour>|<nomdujour>'-'<nomdujour>|"*"
- <nomdujour>="sun"|"mon"|"tue"|"wed"|"thu"|"fri"|"sat"
- <daysofmonth>=<numjour>|<numjour>'-'<numjour> |"*"
- <numjour>=nombre de 1 à 31
- <heure>=nombre de 0 à 23
- <minute>=nombre de 0 à 59
- <months>=<nomdumois>|<nomdumois>'-'<nomdumois>|"*"
...
L'instruction précédente transfère le traitement vers l'extension s dans le context normalhours si l'appel a lieu entre 08h00 et 18h00 du lundi au vendredi.
...
En utilisant l'instruction précédente, l'utilisateur appelle le PBX et — sans nécessiter de mot de passe — reçoit une tonalité. Tout appel utilisant DISA sera traité en utilisant le context `default`. Les arguments de cette application incluent un mot de passe global ou un mot de passe individuel dans un fichier. Si aucun context n'est spécifié, le context `disa` est supposé. Si vous utilisez un fichier de mots de passe, le chemin complet doit être spécifié. Un ID d'appelant peut également être spécifié pour la composition externe DISA. Exemple :

```
exten => s,1,DISA(numeric-passcode,default,"Flavio" <4830258590>)
```

Asterisk 22 utilise des virgules comme séparateurs d'arguments (la forme avec pipe a été dépréciée en 1.6). Le premier argument est soit un code d'accès unique, soit le chemin vers un fichier de codes d'accès, et le context par défaut lorsqu'aucun n'est donné est `disa`.

## Limiter les appels simultanés

La fonction GROUP() vous permet de compter combien de canaux actifs vous avez dans un groupe en même temps. Exemple : Vous avez une succursale à Rio de Janeiro, où les téléphones suivent le motif « _214X ». Cet emplacement est desservi par une ligne louée, avec 64K réservés pour la bande passante vocale. Dans ce cas, le nombre maximal d'appels autorisés est de 2 (G.729, 30r.2K par appel). Pour limiter les appels vers Rio à deux :
...
La messagerie vocale a plusieurs options qui contrôlent son comportement. Pour l'instant, nous nous en tiendrons aux options par défaut et nous nous concentrerons sur la définition de la boîte aux lettres. Après la section `[general]` dans le fichier, vous commencez à configurer les IDs de boîte aux lettres, chacun dans son propre context. Exemple :
...
L'application voicemailmain() est utilisée pour configurer la boîte aux lettres de messagerie vocale. Les utilisateurs peuvent appeler l'application, enregistrer leur message d'accueil et écouter leur messagerie vocale. Pour appeler l'application dans le dialplan, utilisez :
...
Si un appelant appuie sur 0 (zéro) pendant l'annonce, il sera transféré vers l'extension 'o' (out) dans le context actuel de la messagerie vocale. Cela peut être utilisé pour sortir vers l'opérateur. Si pendant l'enregistrement l'appelant appuie sur # ou si la limite de silence expire, l'enregistrement est arrêté et l'appel passe à la priorité suivante. Assurez-vous de gérer l'appel après la lecture de la messagerie vocale, comme indiqué ci-dessous.
...
Pour envoyer la messagerie vocale par e-mail, l'application voicemail utilise l'agent de transfert de messages (MTA), un composant de votre système d'exploitation. Debian utilise Exim comme MTA. L'application qui envoie l'e-mail est définie dans le paramètre 'mailcmd'.
...
Le corps et l'objet de l'e-mail sont construits à partir d'un modèle que vous définissez dans la section `[general]` de `voicemail.conf`. Vous pouvez modifier à la fois le corps et l'objet, mais la limite de taille du message est de 512 octets. Dans le modèle, `\n` insère une nouvelle ligne et `\t` insère une tabulation.

L'exemple `emailsubject` ci-dessous est simple. L'exemple `emailbody` est très proche de la valeur par défaut ; la valeur par défaut affiche juste le CIDNAME lorsqu'il n'est pas nul, sinon le CIDNUM, ou "an unknown caller" lorsque les deux sont nuls.
...
![L'interface web Comedian Mail (`vmail.cgi`) : la connexion à la messagerie vocale web Asterisk — entrez votre boîte aux lettres et votre mot de passe pour lire, enregistrer, transférer ou supprimer la messagerie vocale depuis un navigateur. Il est toujours livré avec Asterisk 22 et est installé avec `make webvmail`.](../images/10-dialplan-advanced-features-img14.png)

### Lab : Notification de message sur le téléphone

Ce lab a été testé en utilisant un softphone SIP. 1. Modifiez `pjsip.conf` et ajoutez `mailboxes=4401` dans la section endpoint pour le périphérique nommé 4401. 2. Modifiez le extensions.conf et créez une extension pour enregistrer une messagerie vocale vers les extensions 4401.
...
Canaux SIP (pjsip.conf) Nous avons choisi la numérotation du dialplan de 2000 à 2099. Deux codecs seront utilisés : G.729 et G.711 ulaw. Le premier sera utilisé pour les téléphones utilisant Asterisk sur Internet ou WAN tandis que le second sera utilisé pour les téléphones utilisant le réseau local. Dans `pjsip.conf`, nous arbitrerons quels périphériques appartiendront à chaque classe de service (restrict, ld, ldi). Pour réduire la vulnérabilité aux attaques par force brute, nous utiliserons les adresses MAC des téléphones comme noms de périphériques. Je vous conseille vivement d'utiliser des mots de passe forts pour éviter les attaques par force brute !

Nous définissons un transport et trois modèles réutilisables — une base endpoint avec les codecs partagés, une authentification userpass et un contact AOR unique — puis nous attachons chaque périphérique aux modèles et ne remplaçons que ce qui diffère (son context de classe de service et ses identifiants). `host=dynamic` devient un AOR contre lequel le téléphone s'enregistre, et `directmedia` devient `direct_media` :
...
Maintenant, commençons à configurer le extensions.conf. Définissez les extensions internes et la composition locale
...
Pour recevoir des appels, utilisez deux contexts. Le premier est pour le fonctionnement pendant les heures normales, où l'appel sera reçu par un standard automatique. Le second est pour les heures de fermeture, où l'appelant recevra un message tel que « vous avez appelé la société XYZ, nos heures normales sont de 08h00 à 18h00 ; si vous connaissez le numéro de l'extension de destination, vous pouvez essayer de le composer maintenant ou raccrocher. » Menus : Heures normales, Heures de fermeture Dans les menus ci-dessous, le système jouera un message avertissant l'appelant que l'entreprise a été contactée en dehors des heures de travail normales, permettant à l'appelant de composer le numéro de l'extension de destination (quelqu'un peut travailler en dehors des heures de travail normales).
...
Menus : Principal et Ventes Pendant les heures de travail normales, l'appel est répondu par un menu de standard automatique, recevant un message tel que « bienvenue chez XYZ Company ; composez 1 pour les ventes, 2 pour le support technique, 3 pour la formation, ou l'extension souhaitée ».
...
Avec toutes ces instructions, la fonctionnalité de votre plan de composition est maintenant prête. Dans la section suivante, nous démontrerons comment faire fonctionner le PBX.

## Résumé

Dans ce chapitre, vous avez appris à recevoir des appels en utilisant un IVR ou un standard automatique. Vous avez étudié le concept d'inclusion de context et implémenté quelques exemples. Les sous-routines ont été utilisées pour éviter la saisie répétitive, et la base de données Asterisk (AstDB, soutenue par SQLite3 dans Asterisk 22) a été utilisée pour les fonctions qui nécessitent un stockage de données (par exemple, transfert d'appel, ne pas déranger, listes noires). Enfin, vous avez appris à implémenter un comportement pour les heures de fermeture et implémenté un dialplan complet en utilisant ces concepts.
...
1. Une inclusion de context dépendante du temps utilise la forme `include => context,<times>,<weekdays>,<mdays>,<months>`. Que fait `include => normalhours,08:00-18:00,mon-fri,*,*` ?
   - A. Exécute les extensions du lundi au vendredi, de 08h00 à 18h00
   - B. Exécute les options tous les jours de tous les mois
...
6. Dans Asterisk 22, la base de données intégrée AstDB est soutenue par :
   - A. Berkeley DB v1
   - B. MySQL
   - C. SQLite3
   - D. PostgreSQL
...
9. Étant donné la syntaxe `Goto([[context,]extension,]priority)`, lesquelles des invocations suivantes de l'application Goto() sont valides ? (cochez tout ce qui s'applique)
   ...
   - D. Goto(priority)
10. Pour supprimer une seule clé d'AstDB dans le dialplan Asterisk 22, vous utilisez :

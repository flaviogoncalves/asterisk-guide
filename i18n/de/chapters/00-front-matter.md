...
> **[author TODO]** Weisen Sie eine neue ISBN für die 2. Ausgabe zu (verwenden Sie nicht die der 1. Ausgabe 9781796396973) und legen Sie das Veröffentlichungsdatum vor dem Druck fest.
...
Dieses Buch wird Sie nicht von alleine zu einem Guru machen – das kann kein Buch –, aber am Ende werden Sie in der Lage sein, eine echte PBX mit erweiterten Funktionen zu erstellen und zu betreiben. Das Buch hat eine Ergänzung – praktische Übungen und einen Online-Kurs – bei **VoIP School Blackbelt** (<https://voip.school>).

## Zielgruppe {.unnumbered}

Dieses Buch richtet sich an Leser, die neu bei Asterisk sind. Ich setze voraus, dass Sie mit Linux vertraut sind – der Shell, einem Texteditor und der grundlegenden Systemadministration. Sie können die Beispiele auf einem Linux-Desktop nachvollziehen, falls das beim Lernen einfacher ist, und eine virtuelle Maschine ist für die Übungen in Ordnung (erwarten Sie eine etwas schlechtere Sprachqualität). Für Produktionssysteme empfehle ich nicht, Asterisk in einer Desktop-Umgebung oder innerhalb einer schwach ausgestatteten VM auszuführen. Etwas Vertrautheit mit IP-Netzwerken, Voice over IP (VoIP) und grundlegenden Telefoniekonzepten ist hilfreich.
...
- **PJSIP ist der einzige SIP-Kanal.** `chan_sip` wurde in Asterisk 21 entfernt und existiert in 22 nicht mehr. Jedes SIP-Beispiel verwendet jetzt PJSIP (`pjsip.conf`); das Material zum alten `sip.conf` wird nur noch als Migrationsreferenz beibehalten.

...
Em diversas situações, pode ser necessário estender os recursos do Asterisk usando aplicações externas. Existem muitas maneiras diferentes pelas quais ele pode ser estendido. Neste capítulo, abordaremos duas das formas clássicas de integrar o Asterisk com outros sistemas: AMI – Asterisk Manager Interface e AGI – Asterisk Gateway Interface. Também veremos o comando asterisk –rx e a aplicação system(). A escolha de qual forma utilizar para integrar com o Asterisk depende da aplicação. Para AGI, a aplicação mais comum é a URA (IVR) conectada a um banco de dados. Para AMI, discadores são a aplicação mais popular. Uma terceira interface, mais moderna — ARI, a Asterisk REST Interface — é abordada separadamente no próximo capítulo.

## Objetivos

Ao final deste capítulo, o leitor deverá ser capaz de:

- Descrever opções de acesso a programas externos
- Usar o comando asterisk –rx para executar um comando de console
- Usar a aplicação system() para chamar programas externos no dialplan
- Explicar o que é AMI e como funciona
- Configurar o arquivo manager.conf e habilitar a AMI
- Executar um comando AMI a partir de um programa PHP
- Explicar o que é o Asterisk manager proxy e como funciona
...
- Linha de comando Linux e Console do Asterisk
- Aplicação System()
- AMI
- AGI
...
Exemplo: Esta aplicação faz um "screen-pop" usando o netbios WindowsPopup.
...
A AMI permite que um programa cliente se conecte a uma instância do Asterisk e emita comandos ou leia eventos através de uma conexão TCP. Integradores de sistemas acharão esses recursos úteis para rastrear estados de canais. A AMI baseia-se no conceito simples de um protocolo de linha usando pares chave:valor sobre TCP. O Asterisk, por si só, não está preparado para lidar com muitas conexões sobre esta interface. Se você tiver muitas conexões com a AMI, considere usar o Asterisk manager proxy.

### Qual linguagem usar para AMI

Selecionar uma linguagem de programação pode ser difícil hoje em dia. Existem simplesmente muitas opções — Java, PHP, Perl, C, C#, Python e várias outras. É possível usar a AMI com qualquer linguagem que suporte uma interface de socket ou telnet. Escolhemos o PHP para este livro devido à sua popularidade.
...
- A primeira linha de um pacote terá a chave “Response” ou “Event” quando vinda do Asterisk
...
- Action: Um pacote enviado de um cliente conectado à AMI solicitando uma ação específica. Existe um conjunto finito de ações disponíveis para os clientes. Os módulos carregados determinam essas ações. Um pacote contém o nome da ação e seus parâmetros.
- Response: A resposta enviada pelo Asterisk à última ação enviada pelo cliente.
- Event: Dados pertencentes a um evento gerado no núcleo do Asterisk ou por um módulo.

Quando um cliente envia pacotes do tipo Action, um parâmetro chamado ActionID é incluído. Como a ordem na qual as respostas são enviadas pelo Asterisk não pode ser prevista, o ActionID é usado para correlacionar ações e respostas. Pacotes de eventos são usados em dois contextos diferentes. Primeiro, eventos informam o cliente sobre mudanças no Asterisk (por exemplo, novos canais criados, canais desconectados ou agentes entrando e saindo de uma fila). Segundo, eventos são usados para transportar respostas a uma ação do cliente.

## Configurando usuários e permissões

Para acessar a AMI, é necessário estabelecer uma conexão TCP ouvindo em uma porta TCP (geralmente 5038). Você precisará configurar o arquivo /etc/asterisk/manager.conf para criar uma conta de usuário e permissões. Existe um conjunto finito de permissões: “read”, “write” ou ambos. Essas permissões são definidas no
...
Para conectar e autenticar na AMI, você precisará enviar um pacote de ação do tipo login com um nome de usuário e conta criados no manager.conf.
...
Exemplo: Conectando-se à AMI usando php
...
Se você precisar saber os parâmetros específicos de um comando, use o manager show command <comando>. Exemplo:
...
Eventos são gerados na interface do gerenciador sempre que algo acontece no Asterisk —
um canal é criado ou muda de estado, dois canais são conectados (bridged) ou desconectados,
um registro muda, um membro de fila é adicionado, e assim por diante. Cada evento é um bloco de
`Key: value` linhas começando com um cabeçalho `Event:`.

O conjunto exato de eventos depende dos módulos carregados e da versão do Asterisk, então,
em vez de reproduzir uma lista que rapidamente se torna obsoleta, consulte o servidor em
execução para obter o conjunto oficial:
...
Por exemplo, a conexão de chamadas (bridging) é relatada através dos eventos `BridgeCreate`, `BridgeEnter`,
`BridgeLeave` e `BridgeDestroy` (`BridgeEnter` é "Gerado quando um canal entra em uma ponte"). Os eventos mais antigos
`Link`/`Unlink` foram removidos no Asterisk 12.

## Asterisk Gateway Interface

A AGI é uma interface de gateway para o Asterisk semelhante à CGI usada por servidores web. Ela permite o uso de linguagens de alto nível como Perl, PHP e Python para estender a funcionalidade do Asterisk. A principal aplicação para CGIs é a construção de URAs. Existem quatro tipos de AGI:
...
- DEADAGI, que dá acesso ao canal mesmo após o hangup(). Geralmente chamada na extensão ‘h’.
...
Você pode mostrar os comandos AGI disponíveis usando o comando `agi show commands` (a saída abaixo é representativa; o Asterisk 22 adiciona alguns comandos adicionais):
...
Neste exemplo, usaremos o php-cli, a versão de linha de comando do PHP. Instale o php-cli se ele ainda não estiver instalado. Siga estes passos para usar scripts AGI em PHP. Passo 1: Todos os scripts AGI estão localizados em /var/lib/asterisk/agi-bin Passo 2: Altere as permissões para permitir a execução.

```
chmod 755 *.php
```

Passo 3: Interface de shell (específico para PHP). As primeiras linhas do script devem ser:
...
Passo 5: Gerenciar a saída do Asterisk. O Asterisk envia o conjunto de informações cada vez que a AGI é chamada.
...
Para chamar uma variável chamada agi_extensions, use $agi[agi_extensions]. Passo 6: Usar AGI de canal. Neste ponto, você pode começar a falar com o Asterisk. Use o comando fputs para enviar comandos para a AGI. Você também pode usar o comando echo.
...
A DeadAGI é usada quando você não tem um canal ativo. Geralmente, você executa a DeadAGI na extensão ´h´.

### FASTAGI

A Fast AGI implementa AGI usando uma porta TCP (4573 por padrão) como canal de Entrada/Saída. O formato da FastAGI é (agi://). Por exemplo:

```
exten => 0800400001, 1, Agi(agi://192.168.0.1)
```

Quando a conexão TCP é perdida ou desconectada, a AGI termina e a conexão TCP é fechada, seguida pela desconexão da chamada. Este recurso é útil para aliviar a carga da CPU do seu servidor Asterisk executando scripts em um servidor externo. Você pode obter mais detalhes sobre a FastAGI no diretório do código-fonte (por favor, veja o arquivo “agi/fastagi-test”). A biblioteca Asterisk-Java fornece uma implementação de servidor FastAGI para Java. Para mais informações, veja https://github.com/asterisk-java/asterisk-java

A ARI, a moderna interface REST/WebSocket, terá seu próprio capítulo a seguir.

## Alterando o código-fonte

O Asterisk é desenvolvido na linguagem C (não C++). Ensinar programação em C está além do escopo deste documento. Se você estiver interessado, encontrará documentação relacionada em https://docs.asterisk.org, que oferece boas dicas sobre como aplicar e criar patches para o Asterisk, bem como documentação da API gerada principalmente pelo software Doxygen. Para aqueles familiarizados com a programação em C, alterar o código-fonte das aplicações pode ser a maneira mais poderosa (e perigosa) de estender o Asterisk.

## Resumo

Neste capítulo, você aprendeu como interagir programas externos com o PBX Asterisk. Começamos com o asterisk –rx passando comandos do shell Linux para o console do Asterisk. Em seguida, aprendemos sobre a aplicação System(), que permite chamar um programa externo a partir do dialplan. A AMI é a interface mais próxima de uma interface CTI comum em PBXs tradicionais. Para chamar uma aplicação a partir do dialplan, usamos a AGI, com uma amostra de seus diferentes tipos: DeadAGI para canais mortos, EAGI para lidar com o streaming de áudio, Fast AGI para usar sockets TCP como interface de entrada/saída, e AGI normal para chamar e processar scripts dentro da mesma caixa do Asterisk. O próximo capítulo é dedicado à ARI, a moderna API REST/WebSocket que dá às aplicações externas controle total dos canais e pontes do Asterisk.
...
1. Qual dos seguintes NÃO é um método de interface para o Asterisk?
   ...
   - D. System()
   - E. External()
2. A AMI permite passar comandos do Asterisk sobre sockets TCP, e esta interface é habilitada por padrão em uma instalação nova do Asterisk.
   - A. Verdadeiro
   - B. Falso
3. A AMI é muito segura, porque sua autenticação usa desafio/resposta MD5.
   - A. Verdadeiro
   - B. Falso
4. A FastAGI permite que o dialplan chame scripts externos em outra máquina sobre sockets TCP (geralmente porta 4573).
   - A. Verdadeiro
   - B. Falso
5. A DeadAGI é usada em canais ativos. Ela pode ser usada em canais DAHDI, mas não em canais SIP ou IAX.
   - A. Verdadeiro
   - B. Falso
6. A AGI suporta apenas PHP como linguagem de script.
   - A. Verdadeiro
   - B. Falso
7. O comando ___ mostra todos os comandos AGI disponíveis.
8. O comando ___ mostra todos os comandos AMI disponíveis.
9. Em um pacote de ação AMI, qual cabeçalho o cliente inclui para que as respostas assíncronas e eventos que retornam do Asterisk possam ser correlacionados com a ação que os disparou?
   - A. `ActionID`
   - B. `Variable`
   - C. `Secret`
   - D. `Event`
10. Qual classe de permissão do manager.conf da AMI um usuário deve ter para executar a ação `Originate` e realizar uma chamada de saída?
    - A. `originate`
    - B. `verbose`
    - C. `log`
    - D. `reporting`

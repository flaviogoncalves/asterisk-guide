...
- Conectar um softphone de navegador (SIP.js) e fazer uma chamada
...
- **Click-to-call** em um site — um visitante chama uma fila ou uma extensão a partir de uma página web.
...
A grande vantagem é o alcance: todo navegador moderno já fala WebRTC. O custo é que o WebRTC é rigoroso — ele *exige* mídia criptografada e um transporte seguro, portanto, há mais para configurar do que em um telefone SIP UDP simples.
...
A sinalização WebRTC é servida pelo servidor HTTP embutido do Asterisk (`res_http_websocket` expõe o caminho `/ws` nele). Os navegadores exigem um WebSocket *seguro*, então habilitamos o TLS. Edite `http.conf`:
...
O navegador se conectará a `wss://your-asterisk:8089/ws`.

### Sobre o certificado

O certificado TLS aqui protege o *WebSocket* (o canal de sinalização). Em um laboratório, um certificado autoassinado é aceitável — você o aceita uma vez no navegador. Em produção, use um certificado real (por exemplo, Let's Encrypt) cujo nome corresponda ao host ao qual o navegador se conecta; caso contrário, o navegador recusará o WebSocket.

> **[Nota da 2ª ed.]** O laboratório fornece `lab/make-certs.sh`, que gera um certificado autoassinado com `CN=localhost`. Para uma implantação pública, documente o fluxo do Let's Encrypt e aponte `tlscertfile`/`tlsprivatekey` para os arquivos emitidos.
...
> **[Nota da 2ª ed.]** O transporte `wss` imprime um endereço de bind `0.0.0.0:5060` mesmo que o WebSocket real seja servido pelo servidor HTTP na porta 8089. Isso é esperado: o transporte `wss` é uma camada fina sobre o `res_http_websocket`; a linha `bind` nele é efetivamente cosmética. Vale uma frase no texto final para que os leitores não fiquem confusos com a porta.
...
`webrtc=yes` é um interruptor de conveniência. É equivalente a definir manualmente todas as opções exigidas pelo WebRTC. Você pode confirmar exatamente o que ele ativou:
...
`allow=opus` é recomendado — Opus é o codec que os navegadores preferem. O Asterisk 22 envia o Opus *passthrough* no core (o módulo `res_format_attr_opus`), o que é suficiente para retransmitir Opus entre duas pontas capazes de Opus sem re-codificação. A *transcodificação* de Opus para outro codec requer o módulo separado `codec_opus`, que o guia oficial do WebRTC lista como opcional, mas altamente recomendado, e que você instala sobre a compilação base; veja a discussão sobre codecs em *Designing a VoIP network*. Mantenha `ulaw` como um fallback para fazer a ponte com pontas não-WebRTC que não falam Opus.
...
> **[Nota da 2ª ed.]** Verifique se devemos recomendar um coturn auto-hospedado para produção (a maioria das implantações reais precisa de TURN). Adicione um breve exemplo de configuração de coturn, se for o caso.
...
Com o navegador registrado, `pjsip show contacts` mostra o contato dinâmico, e uma chamada ilumina o canal:
...
## WebRTC do Asterisk vs um gateway de mídia

O Asterisk pode terminar o WebRTC diretamente, mas nem sempre é a ferramenta certa:

- **Use o WebRTC nativo do Asterisk** quando o navegador for um *telefone* no seu PBX — um agente, uma extensão interna, um click-to-call que cai no seu dialplan. O navegador é apenas mais um endpoint e tudo (filas, correio de voz, IVR) funciona.
...
- **O WebSocket não conecta:** o navegador rejeitou o certificado TLS. Abra `https://host:8089/ws` diretamente e aceite-o, ou instale um certificado confiável.
- **Registra, mas sem áudio:** o ICE falhou — adicione STUN, e TURN se estiver atrás de NAT. Verifique `pjsip set logger on` e observe os candidatos SDP.
- **Áudio unidirecional:** geralmente NAT/ICE em um dos lados, ou um codec sem correspondência comum — garanta `allow=opus,ulaw`.
- **A chamada cai ao atender:** o handshake DTLS falhou; confirme se `dtls_auto_generate_cert` é `Yes` e se o relógio do sistema está correto (certificados são sensíveis ao tempo).

## Laboratório

1. Execute `./lab.sh up`, depois `bash lab/make-certs.sh` e reinicie o Asterisk.
2. Sirva `lab/webrtc/index.html` (`python3 -m http.server` de `lab/webrtc`) e abra-o; aceite o certificado em `https://localhost:8089/ws`.
3. Registre-se como `webrtc-1000` e chame `600` (teste de eco) — você deve ouvir a si mesmo.
...
5. Qual dos itens a seguir o `webrtc=yes` ativa por padrão? (Escolha todos os que se aplicam.)
...
7. Em `rtp.conf`, quais duas configurações apontam o Asterisk para um servidor externo para que ele possa descobrir seu endereço público e retransmitir mídia quando os caminhos diretos falham? (Escolha todos os que se aplicam.)
   - A. `icesupport=yes`
   - B. `stunaddr=`
   - C. `turnaddr=`
   - D. `tlsbindaddr=`
...
9. De acordo com o capítulo, quando você deve optar por um gateway de mídia dedicado (como o Janus) em vez do WebRTC nativo do Asterisk?
   ...
   - B. Quando você precisa escalar muitas sessões de mídia de navegador independentemente do controle de chamadas, fazer encaminhamento seletivo para grandes conferências ou manter o plano de mídia separado do PBX
   - C. Apenas quando o navegador não suporta DTLS
   - D. Quando você quer que o correio de voz e o IVR funcionem para o endpoint do navegador

10. Verdadeiro ou falso: `getUserMedia` (acesso ao microfone) funciona em qualquer página `http://`, portanto, servir o softphone do navegador via HTTPS é opcional.

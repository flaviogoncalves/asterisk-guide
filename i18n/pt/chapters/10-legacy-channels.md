...
A maioria das implementações analógicas usa um par de linhas de cobre chamado tip e ring. Quando um loop é fechado, o telefone recebe o tom de discagem da central telefônica (ou do PBX privado). A sinalização mais usada é a loop-start; outros tipos menos comuns de sinalização incluem ground start, que é usada em vários países. As três categorias de sinalização são:
...
A interface FXO é usada para conectar a uma central telefônica (CO) ou à extensão de outro PBX. Ela se comunica diretamente com uma linha telefônica vinda da PSTN. Outra opção é conectar a interface FXO a um PBX existente, permitindo a comunicação entre o Asterisk e o PBX legado. Conectar o Asterisk a uma porta de PBX e entregar uma extensão remota usando VoIP é frequentemente referido como uma extensão fora das instalações (OPX). Uma interface FXO recebe um tom de discagem. Foreign eXchange Station (FXS) A interface FXS alimenta um telefone analógico, modem ou fax. O FXS fornece o tom de discagem e a energia para um telefone.
...
![Um Xorcom Astribank: um banco de canais USB de montagem em rack de 19 polegadas que expõe dezenas de portas FXS/FXO (aqui uma unidade de 32 portas) sem consumir slots PCI no host.](../images/10-legacy-fig03.png)
...
Ajustar o cancelamento de eco é metade técnico, metade arte. Essas opções ajustam certos parâmetros do Asterisk que afetam a qualidade do áudio nos canais DAHDI. Elas podem ajudar a melhorar a qualidade do áudio em interfaces analógicas.
...
Linhas digitais E1/T1 são uma opção sempre que você precisar implementar um grande número de canais. Um único circuito E1 é capaz de 30 chamadas simultâneas, e você pode ter recursos como discagem direta interna (DID), Caller ID (identificação de chamadas) e sinalização avançada. A linha E1/T1 pode chegar à sua empresa de várias maneiras usando par trançado, fibra e micro-ondas, dependendo do seu país. Linhas digitais são entregues à sua empresa usando UTP, fibra ou micro-ondas. Modems e multiplexadores (MUX) são usados para entregar a linha física. A conexão a uma linha T1 é sempre baseada em um conector RJ45. No entanto, linhas E1 também podem ser provisionadas usando BNC. É muito importante saber o tipo de conector que você vai receber com antecedência, principalmente em linhas E1. Normalmente, todo o equipamento até o RJ45 é fornecido pela operadora (TELCO).
...
Nos EUA, o trunk digital mais comum é o T1, que possui 24 linhas disponíveis; na Europa e América Latina, os trunks E1 possuem 30 linhas. Algumas empresas fornecem um T1/E1 fracionado com menos canais. Sinalização de bit roubado Às vezes, um trunk T1 usa um esquema de bit roubado onde um bit é emprestado para sinalização. Em trunks T1, o canal de dados/voz é transmitido com 56 Kbps em cada timeslot. Como você pode observar, quando você usa o bit roubado, o circuito T1 não perde dois slots para sincronização e sinalização.
...
É importante entender que linhas T1/E1 podem ser entregues usando diferentes tipos de sinalização, tais como:
...
ISDN é frequentemente usada na Europa e nos EUA. É uma rede de voz digital, padronizada pela União Internacional de Telecomunicações (ITU) em 1984. ISDN fornece dois tipos de canais:
...
Às vezes, circuitos E1 usam um esquema de sinalização CAS chamado MFC/R2, que foi definido pela ITU como um padrão conhecido como Q.421/Q441. Isso é frequentemente encontrado na América Latina e Ásia. Várias companhias telefônicas nesses países usam variantes personalizadas de MFC/R2. Portanto, você precisará saber a variação correta do país para fazê-lo funcionar.

### ISDN BRI

Canais usando sinalização ISDN BRI são muito populares na Europa. A maioria das placas ISDN BRI para Asterisk suporta uma interface S/T com capacidades NT e TE. A conexão TE (terminal) é a usada para conectar à operadora ou a outros PBXs configurados como terminação de rede (NT). O NT é usado para conectar telefones e PBXs configurados como TE. ISDN BRI fornece dois canais de dados/voz e um canal de sinalização. Placas ISDN BRI estão disponíveis de vários fornecedores de placas de interface para Asterisk.
...
Existem vários tipos de barramento em seu PC. É muito importante que você tenha a placa certa para seu servidor. A visão geral a seguir descreve as placas usadas com mais frequência:
...
> **[Nota da 2ª ed.]** A Sangoma adquiriu a Digium em 2018. As placas com a marca Digium agora são vendidas e suportadas sob a marca Sangoma. Verifique a disponibilidade atual dos modelos no site da Sangoma (www.sangoma.com), pois alguns SKUs mais antigos podem ter sido descontinuados.
...
Etapa 5: Usando o utilitário dahdi_cfg Este é o output correto do dahdi_cfg para um span E1 fracionado (15 portas) e duas portas FXO.
...
Após configurar os drivers, você pode simplesmente reiniciar o servidor. Se você instalou o DAHDI com make config, você não precisará fazer nada extra. O driver de kernel será carregado e configurado automaticamente. No entanto, às vezes é útil carregar e descarregar os drivers manualmente. Exemplo:
...
> **[Nota da 2ª ed.]** A árvore do Asterisk 1.4 com patch da 1ª edição está obsoleta; para o Asterisk 22, o suporte a MFC/R2 via libopenr2 é integrado na árvore de código-fonte principal, e as etapas abaixo agora usam os repositórios Git atuais em vez do aposentado `svn.digium.com`. Considere condensar essas etapas históricas de compilação para a edição final.
...
Nota: Algumas operadoras não aceitam chamadas sem o caller ID. Por favor, defina o caller ID para um dos números DID atribuídos pela operadora. Em alguns países, esta etapa não é necessária. Etapa 7: Teste a solução: Agora, com uma extensão no contexto from-internal, disque qualquer número e observe o console. Verifique se algum erro está ocorrendo. -- Executing Set("SIP/8564-081ca5d8", "CALLERID(num)=1145678990") in new stack -- Executing Dial("SIP/8564-081ca5d8", "DAHDI/g1/35678899|60|tT") in new stack
...
> **[Nota da 2ª ed. — Status no Asterisk 22]** `chan_iax2` ainda está incluído e totalmente suportado no Asterisk 22 LTS, então tudo neste capítulo permanece válido. No entanto, IAX2 é agora um protocolo legado e vê relativamente pouca nova implantação. A indústria de VoIP convergiu amplamente para o SIP (via `chan_pjsip` no Asterisk 22) tanto para trunking de provedor quanto para interconexão de servidores. O principal ponto de venda restante do IAX2 é sua **travessia NAT de porta única**: toda a sinalização e mídia fluem sobre uma única porta UDP (4569 por padrão), o que simplifica muito a configuração de firewall e NAT em comparação com SIP + RTP. Se você está construindo um novo trunk Asterisk-para-Asterisk e NAT não é uma preocupação, trunks PJSIP são a abordagem moderna recomendada. IAX2 é mantido aqui porque ainda é uma escolha válida, especialmente em ambientes onde apenas uma porta UDP pode ser aberta através de um firewall.
...
É importante entender as convenções de nomeação de canal, pois você usará esses nomes ao especificar um canal no dialplan. O formato de um nome de canal IAX usado para canais de saída é:
...
Canais de saída são vistos no console do Asterisk. IAX2/8590:secret@myserver/8590@default Chama a extensão 8590 no myserver. Usa 8590:secret como o par nome/senha
...
Poucos provedores VoIP suportam IAX. Você pode encontrar facilmente um provedor IAX pesquisando por “IAX providers”. Usar um provedor IAX faz muito sentido, já que o IAX pode economizar muita largura de banda, atravessa NAT facilmente e pode autenticar usando pares de chaves RSA.
...
> **[Nota da 2ª ed.]** O número de provedores VoIP comerciais compatíveis com IAX diminuiu significativamente desde o Asterisk 16. A maioria dos provedores agora oferece trunks SIP/PJSIP exclusivamente. Antes de escolher um provedor IAX, confirme se eles mantêm ativamente sua infraestrutura IAX. Para novas integrações de provedor, um trunk PJSIP é a alternativa recomendada.

#### Conectar a um provedor usando IAX

Etapa 1: Abra uma conta em seu provedor favorito. Seu provedor fornecerá três coisas.
...
Nas instruções descritas acima, você se registrou com seu provedor usando sua conta e senha. No momento em que receber uma chamada, ela será encaminhada para a extensão 2003.
...
É muito fácil conectar um servidor a outro. Você não precisará registrá-los porque os endereços IP já são conhecidos. Você terá que criar os peers e usuários no arquivo iax.conf. Todas as extensões no site HQ começam com 20 seguidas por dois dígitos (por exemplo, 2000). No Branch, todas as extensões começam com 22 seguidas por dois dígitos (por exemplo, 2200). Usaremos o trunk. Você precisará de uma fonte de temporização DAHDI para habilitar esse recurso. Etapa 1: Edite o arquivo iax.conf no servidor Branch.

![Conectando dois servidores Asterisk com um trunk IAX: o servidor HQ (192.168.1.1, extensões 20xx) e o servidor Branch (192.168.1.2, extensões 22xx) alcançam um ao outro sobre um único trunk IAX — nenhum registro é necessário porque ambos os endereços IP são fixos e conhecidos.](../images/10-legacy-fig15.png)
...
Etapa 5: Teste uma chamada do telefone 2000 no servidor HQ para o telefone 2200 no servidor Branch.
...
##### Configurando o servidor para o branch

Etapa 1: Gere as chaves RSA no servidor branch
...
Etapa 4: Edite o arquivo iax.conf no servidor Branch.
...
##### Configurando o servidor para a sede (headquarters)
...
Etapa 11: Teste uma chamada do telefone 2000 no servidor HQ para o telefone 2200 no servidor Branch.
...
Para garantir a compatibilidade com versões mais antigas, você pode precisar desabilitar a rotação de chave usando:
...
### Conectar o Asterisk a um provedor SIP (sip.conf)

O Asterisk é frequentemente usado para conectar a um provedor VoIP SIP. Provedores VoIP geralmente têm taxas melhores para chamadas telefônicas do que provedores tradicionais. Outro ponto interessante e atraente dos provedores VoIP é a possibilidade de comprar números DID em outras cidades — até mesmo em países estrangeiros. Essas são boas razões para usar VoIP para telecomunicações. Nesta seção, você aprenderá como o `chan_sip` legado conectava o Asterisk a um provedor VoIP. Três etapas são necessárias para conectar o Asterisk a um provedor SIP. Testes podem ser conduzidos estabelecendo uma conta com seu provedor favorito. Etapa 1: Registrar-se com um provedor SIP no sip.conf Para conectar a um provedor SIP, você precisará das seguintes informações do provedor:

![Asterisk conectado a um provedor de serviço VoIP sobre a Internet ou uma WAN privada, com telefones SIP locais registrados ao servidor Asterisk](../images/07-sip-and-pjsip-fig07.png)
...
Etapa 2: Configurar o [peer] no sip.conf Crie uma entrada do tipo peer para o provedor desejado para simplificar a discagem do Asterisk.
...
Etapa 3: Criar uma rota para o provedor no dialplan Escolheremos os dígitos 010 como a rota de destino para o provedor. Para discar #610000 dentro do provedor, simplesmente disque 010610000.
...
### Conectar dois servidores Asterisk juntos usando SIP (sip.conf)
...
O comando domain permite que você gerencie mais de um domínio dentro do Asterisk. Se uma chamada vem de um domínio específico, ela é direcionada para um contexto específico.
...
- Altere o modelo de presença de peer-to-peer para presence agent, o que fará com que o soft-phone assine o Asterisk para eventos SIP.
...
É necessário manter o mapeamento NAT aberto. Se o NAT expirar, o Asterisk não pode enviar um convite para o UAC. O UAC é capaz de enviar chamadas, mas não de receber nenhuma. A instrução a seguir pode ser usada para manter o NAT aberto.
...
## Migrar um sistema chan_sip legado para PJSIP
...
| `type=friend` / `type=peer` / `type=user` | um único `type=endpoint` (PJSIP não tem distinção de friend/peer/user) |
...
Trate sua saída como um **ponto de partida**: revise cada objeto gerado, especialmente transportes, configurações de NAT e listas de codec, e teste exaustivamente antes de ir para a produção.
...
1. Em relação às duas interfaces analógicas Foreign eXchange, marque as afirmações corretas (escolha todas as que se aplicam):
   - A. Uma interface FXO conecta-se à central telefônica da rede telefônica pública comutada (PSTN) e obtém o tom de discagem dela.
...
5. O hardware DAHDI é independente do Asterisk: a placa física é configurada no `/etc/dahdi/system.conf`, enquanto o `chan_dahdi.conf` define os canais do Asterisk, não o hardware em si.
   - A. Verdadeiro
   - B. Falso

...
Finalmente, os padrões abertos sobre os quais o VoIP é construído oferecem a liberdade de escolher entre diferentes fornecedores. Esse benefício único torna o cliente o rei, em vez de um subordinado às operadoras (TELCOS) e fabricantes de PBX.

### Integração de Telefonia por Computador (CTI)

A telefonia é muito mais antiga que a computação. Os PBXs de telefonia são baseados em comutação de circuitos, e você geralmente não tem mais do que um computador para supervisão. Com o VoIP, a telefonia é criada desde o início com base em padrões de computação. Isso torna o uso de aplicações de CTI mais barato e mais fácil do que no modelo antigo. Você pode criar rapidamente uma longa lista de aplicações de telefonia baseadas no Asterisk. Você pode desenvolver IVRs, ACDs, CTI, discadores, popups de tela e outras aplicações em uma fração do tempo necessário para PBXs tradicionais.
...
| Link | Ethernet, PPP, Frame Relay… | Link de dados / Física |
...
- **SIP** usa UDP ou TCP na porta 5060 (TLS na 5061) para transportar sinalização. O áudio é transportado separadamente por RTP sobre um intervalo de portas UDP configurável (a amostra `rtp.conf` fornecida pelo Asterisk usa de 10000 a 20000), codificado com um codec como G.711.
...
> **[Nota da 2ª edição]** A figura da 1ª edição aqui mapeava SIP/H.323 na camada de *sessão* do OSI e os codecs na camada de *apresentação*. Esse mapeamento foi uma fonte de controvérsia de longa data — o IETF usa o modelo TCP/IP, no qual o SIP é um protocolo de camada de aplicação — então a figura foi substituída pela tabela acima. Encomende uma figura redesenhada baseada na pilha TCP/IP se um visual for desejado.
...
O IAX é um protocolo aberto originalmente desenvolvido pela Digium (agora Sangoma). O IAX é um protocolo tudo-em-um, pois transporta sinalização e mídia através da mesma porta UDP (4569). Mark Spencer desenvolveu o IAX como um protocolo binário para largura de banda reduzida. A principal força do IAX é seu uso reduzido de largura de banda (ele não usa RTP); também é muito fácil para atravessamento de NAT e firewall, já que usa apenas uma porta UDP (4569). Se um fabricante de PBX tradicional tivesse criado o IAX, provavelmente teria comercializado o protocolo como a "melhor coisa desde o sorvete"; em algumas situações, o IAX em modo trunk pode reduzir o uso de largura de banda de voz em um terço. O IAX2 (versão 2) ainda é fornecido no Asterisk 22 através do módulo `chan_iax2` e continua útil para trunks Asterisk-para-Asterisk, embora seja considerado legado; SIP/PJSIP é preferido para novas implementações. O IAX2 é especificado no [RFC 5456](https://www.rfc-editor.org/rfc/rfc5456) (Informativo).

### MGCP – Media Gateway Control Protocol

O MGCP é um protocolo usado em conjunto com H.323, SIP e IAX. Sua maior vantagem é a escalabilidade. Ele é configurado no agente de chamada em vez dos gateways. Isso simplifica o processo de configuração e permite o gerenciamento centralizado. No entanto, a implementação do Asterisk não está completa, e parece que não muitas pessoas o usam.
...
| IAX2 | RFC 5456 (Informativo) | Trunks Asterisk-para-Asterisk; telefones IAX2; provedores de serviço IAX |
...
> **[Nota da 2ª edição]** Considere atualizar esta tabela para refletir os nomes atuais dos módulos do Asterisk 22 e o status de suporte.

## Peers, Users e Friends

Existem três tipos de clientes SIP e IAX. O primeiro é "user". Users podem fazer chamadas para um servidor Asterisk, mas não podem se conectar para receber chamadas deste servidor. O segundo é um "peer". Você pode fazer chamadas para um peer, mas não receberá chamadas deles. Geralmente, um servidor ou dispositivo exigirá ambos os conceitos ao mesmo tempo. Um "friend" é um atalho para "user" + "peer". Um telefone provavelmente se enquadraria nesta categoria, pois é necessário para fazer e receber chamadas.

![Peer, user e friend do ponto de vista do Asterisk: um "user" faz chamadas para o Asterisk, um "peer" recebe chamadas do Asterisk e um "friend" faz ambos.](../images/06-voip-network-fig03.png)

> **[Nota da 2ª edição]** A distinção peer/user/friend é um conceito `chan_sip` (`sip.conf`). No PJSIP (`pjsip.conf`), isso é substituído pelo objeto **endpoint**, que lida com chamadas de entrada e saída. A separação peer/user não se aplica mais no Asterisk 22.
...
![Pulse Code Modulation (PCM): um sinal analógico de 4000 Hz é amostrado 8000 vezes por segundo (teorema de Nyquist) e codificado em um fluxo de bits digital de 64 Kbps.](../images/06-voip-network-fig04.png)
...
| Resistência à perda de quadros¹ | Nenhuma | 3% | 5% | 3% |
...
² A complexidade refere-se à quantidade, em milhões de instruções por segundo, gasta para codificar e decodificar o codec usando um design de referência em um DSP da Texas Instruments (TMS320C54x). Existe uma relação direta entre a frequência do processador e os MIPS, mas não é possível traçar uma relação precisa entre plataformas de hardware tão diversas. Use esta tabela apenas para comparação.

> **[Nota da 2ª edição]** Refaça esta tabela de comparação para a 2ª edição: adicione Opus (gratuito `codec_opus`) e G.722 (banda larga, 64 Kbps); mantenha G.711 ulaw/alaw como a linha de base PSTN. Para o G.729, observe a realidade do licenciamento — o `codec_g729` da Sangoma é gratuito para download, mas requer uma licença por canal; o `bcg729` de código aberto é uma alternativa.
...
- **Opus:** Recomendado para endpoints modernos, clientes WebRTC e qualquer implementação onde o endpoint o suporte. Taxa de bits adaptável, excelente resiliência à perda de pacotes, disponível gratuitamente via módulo binário `codec_opus` da Sangoma.
...
![Topologia de rede de exemplo (mesma cidade): sede com 120 extensões conecta-se à PSTN via linhas T1, e à filial #1 (30 extensões) e filial #2 (15 extensões) via nuvem Frame-Relay.](../images/06-voip-network-fig06.png)
...
- Para a Sede para Filial #1, são necessárias 11 linhas.
- Para a Sede para Filial #2, são necessárias 8 linhas.
...
Este método é muito comum ao usar gateways VoIP pela Internet. Ao usar uma carga útil maior, você sacrificará a latência em favor da largura de banda reduzida. Você pode alterar a empacotamento RTP anexando o tamanho do quadro ao codec na instrução allow.
...
Os valores permitidos são: Nome Min Max Padrão Incremento g723 gsm ulaw alaw g726 ADPCM SLIN lpc10 g729 speex ilbc
...
5. No modelo TCP/IP (IETF) contra o qual o SIP é realmente definido no RFC 3261, os protocolos de sinalização SIP, H.323 e IAX2 operam na camada ___.
   ...
   - D. Sessão
   - E. Link de dados
6. O SIP é o protocolo mais adotado para telefones IP e é um padrão aberto amplamente definido pelo IETF no RFC 3261.
   - A. Falso
   - B. Verdadeiro
7. O IAX2 transporta tanto sinalização quanto mídia sobre uma única porta UDP, o que o torna eficiente e fácil de atravessar NAT. Qual porta UDP o IAX2 usa?
   ...
   - C. 4569
   - D. 5061
8. O IAX foi originalmente desenvolvido pela Digium (agora Sangoma). Apesar da adoção limitada por fornecedores de telefones, o IAX é excelente quando você precisa (marque todos que se aplicam):
...
9. No modelo legado chan_sip, um "user" pode receber chamadas do Asterisk.
   - A. Falso
   - B. Verdadeiro
...
**Respostas:** 1 — A, B, C, D · 2 — B · 3 — B · 4 — B · 5 — B (Aplicação — SIP é um protocolo de camada de aplicação no modelo TCP/IP que o IETF usa) · 6 — B · 7 — C · 8 — A, C, D · 9 — A · 10 — A, B, C, D

...
![Arquitetura do Asterisk Real Time: arquivos de configuração e tabelas estáticas do banco de dados são carregados quando o Asterisk inicia, enquanto as tabelas de banco de dados em tempo real fornecem configuração dinâmica que é lida sob demanda durante uma chamada.](../images/18-realtime-fig01.png)
...
A seção de configuração estática é onde você armazena o equivalente aos arquivos de configuração no banco de dados. Essas configurações são lidas durante o carregamento do Asterisk. Alguns módulos releem o banco de dados quando você recarrega. Exemplos da configuração estática são:
...
O mapeamento de arquivos estáticos é mais útil para arquivos de configuração que não possuem um equivalente em tempo real por objeto. Para PJSIP, prefira as famílias realtime por objeto (`ps_endpoints`, `ps_aors` e assim por diante) descritas mais adiante neste capítulo, em vez de mapear todo o `pjsip.conf` como um arquivo estático.

Três exemplos são descritos acima. No primeiro, você vincula queues.conf a uma tabela queues no banco de dados asteriskdb. No segundo exemplo, você vincula pjsip.conf à tabela pjsip_conf no banco de dados asteriskdb definido na configuração odbc. No último exemplo, você vincula iax.conf a um diretório LDAP. MyBaseDN é o DN base a ser pesquisado. No exemplo anterior, o aplicativo app_queue.so é carregado enquanto o driver MySQL consulta o banco de dados e obtém as informações necessárias.
...
Um endpoint realtime mínimo consiste então em uma linha em cada uma das três tabelas — por exemplo, endpoint `6010`:
...
O `uniqueid` deve ser único para cada usuário de voicemail e pode ser autoincrement. Ele não precisa ter qualquer relacionamento com a mailbox ou context.

### Construindo um dial plan usando Asterisk Real Time

Você também pode usar o sistema em tempo real para criar o dial plan. O ARA usa a instrução `switch` para incluir as extensões em tempo real no dial plan normal contido no arquivo extensions.conf. A tabela de extensão deve ser parecida com a abaixo:
...
![Construindo um dial plan com Asterisk Real Time: extensions.conf usa uma instrução `switch => realtime` para puxar linhas de extensão (context, exten, priority, app, data) de uma tabela de banco de dados em vez do arquivo de texto.](../images/18-realtime-fig02.png)
...
Neste laboratório, prepararemos o banco de dados para receber parâmetros do Asterisk. Prepararemos apenas as tabelas REALTIME. A configuração estática será deixada para os arquivos de texto de configuração (legal, não é?). A criação de tabelas no MySQL segue abaixo.
...
Passo 3: Crie as tabelas necessárias. Os arquivos de esquema estático legados ainda são fornecidos sob `contrib/realtime/` (por exemplo, `/usr/src/asterisk-22.x/contrib/realtime/mysql`), mas no Asterisk 22 a maneira recomendada e correta em termos de versão para construir as tabelas realtime — especialmente as tabelas PJSIP `ps_*` — são as migrações **Alembic** sob `contrib/ast-db-manage` (veja a seção "Criando o esquema realtime PJSIP com Alembic" acima).
...
O conjunto de migração Alembic `config` constrói as tabelas PJSIP `ps_*` (junto com `voicemail`, `extensions` e os outros esquemas realtime) exatamente com as colunas que a versão do Asterisk em execução espera, para que o esquema sempre corresponda à versão.
...
Você deve ver as tabelas PJSIP `ps_*` (criadas pela migração Alembic `config`), juntamente com as tabelas `voicemail`, `extensions` e outras tabelas realtime:
...
Passo 6: Instale o phpMyAdmin para lidar com tarefas de banco de dados

```
apt-get install phpmyadmin
```

Abaixo estão duas capturas de tela da tela de login do utilitário e da tela de tabelas. Use astdb/supersecret como nome e senha.

> **[Nota da 2ª ed.]** Substitua por uma captura de tela atual de administração de banco de dados (phpMyAdmin/Adminer), ou converta estas etapas para SQL simples (CREATE TABLE/INSERT).
...
Observe as famílias `ps_endpoints`, `ps_aors`, `ps_auths` e `ps_contacts` acima; juntamente com os mapeamentos `sorcery.conf` correspondentes (veja a seção "PJSIP Realtime (Sorcery)"), eles fazem com que o PJSIP leia suas contas do banco de dados. As famílias `voicemail` e `extensions` completam o exemplo.

Passo 2: Teste de extensão Real Time. Usando o phpMyAdmin, crie um novo endpoint `6010` inserindo uma linha em cada uma das tabelas `ps_auths`, `ps_aors` e `ps_endpoints`, então tente registrar este endpoint com um softphone.
...
As configurações de conta restantes estão espalhadas pelos objetos PJSIP. Context, codecs, modo DTMF e tratamento de mídia residem no endpoint; o registro dinâmico reside no AOR:
...
No PJSIP, o modo DTMF RFC 2833 / RFC 4733 é chamado de `rfc4733` (`dtmf_mode=rfc4733`, o padrão). Não existe um sinalizador "dinâmico" separado para registro: um AOR aceita REGISTERs dinâmicos desde que permita pelo menos um contato (`max_contacts` maior que zero), e cada localização registrada é escrita em `ps_contacts`.
...
Use supersecret quando solicitado. Use o phpMyAdmin para incluir uma extensão no banco de dados. Se preferir, use os seguintes comandos na interface do cliente MySQL.
...
Passo 5: Inclua o Asterisk Real Time no dial plan. No contexto `default`:
...
Passo 6: Reconfigure um dos telefones para o nome de usuário `bria`, se você ainda não o fez.

Passo 7: Disque 6007 de um telefone existente; o telefone `bria` deve tocar.

## Resumo

Neste capítulo, você aprendeu que o Asterisk Real Time permite que você coloque suas configurações em um banco de dados. O Asterisk fornece drivers realtime nativos para ODBC (que alcança qualquer banco de dados suportado por UnixODBC, incluindo MySQL/MariaDB e SQLite), MySQL e PostgreSQL, além de um driver realtime LDAP para backends de diretório. A configuração é dividida em estática e em tempo real. A configuração estática substitui os arquivos de configuração, enquanto a configuração em tempo real cria objetos dinâmicos que são carregados apenas quando uma chamada ou outro evento relacionado acontece. Concluímos com um laboratório prático sobre como instalar e configurar o ARA.

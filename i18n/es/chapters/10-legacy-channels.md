# Canales legados: analógicos, TDM e IAX2

En un mundo puramente VoIP en 2026, los tipos de canales en este capítulo son cada vez más raros: la mayoría de las implementaciones nuevas son troncales SIP y endpoints PJSIP sobre Ethernet, sin ningún hardware de telefonía. No obstante, Asterisk 22 sigue siendo totalmente compatible con la mayoría de ellos. La conectividad analógica (FXO/FXS) y digital TDM (E1/T1/ISDN PRI/BRI) se proporciona a través de DAHDI, la pila de controladores desarrollada originalmente por Digium, que fue adquirida por Sangoma en 2018, después de que los controladores Zaptel anteriores fueran renombrados tras una disputa de marca registrada. La conectividad servidor a servidor sobre IAX2 es proporcionada por `chan_iax2`, que todavía se distribuye y recibe soporte, pero que ahora es firmemente un protocolo legado. Este capítulo también recopila el material de **SIP legado**: el antiguo controlador `chan_sip` y su configuración `sip.conf` —eliminados en Asterisk 21 y desaparecidos en Asterisk 22— junto con una guía completa para migrar un sistema `sip.conf` existente a PJSIP. Si usted opera un entorno puramente SIP sobre PJSIP sin tarjetas de telefonía, sin troncales IAX2 y sin `sip.conf` legado que convertir, puede saltarse este capítulo con seguridad.

## Canales analógicos (FXO/FXS)

> **[Nota de la 2.ª ed.]** Actualizar las fechas y el ISBN de la portada para la segunda edición antes de la publicación.

> **[Nota de la 2.ª ed. — contexto de despliegue]** A partir de Asterisk 22, DAHDI y las tarjetas de telefonía analógica siguen siendo totalmente compatibles y DAHDI todavía se compila con los kernels actuales. Sin embargo, la mayoría de los nuevos despliegues son puramente VoIP (troncales SIP, PJSIP). El hardware analógico/TDM es ahora una opción de nicho, que se encuentra principalmente en entornos legados, conectividad PSTN rural o mercados regulados. El contenido a continuación sigue siendo preciso para esos escenarios.

Existen varias formas de conectarse a la red telefónica pública conmutada (PSTN). La mejor manera depende de cómo la compañía telefónica haga disponible esta conexión en su área. La forma más sencilla es utilizar una línea analógica, similar a la que usa en casa. En esta sección, le mostraremos cómo configurar tarjetas analógicas de Sangoma™ (anteriormente Digium™) y Xorcom™.
...
La mayoría de las implementaciones analógicas utilizan un par de líneas de cobre llamadas tip y ring. Cuando se cierra un bucle, el teléfono recibe el tono de marcado del switch de telecomunicaciones (o de la PBX privada). La señalización más utilizada es loop-start; otros tipos de señalización menos comunes incluyen ground start, que se utiliza en varios países. Las tres categorías de señalización son:
...
Las principales señalizaciones de supervisión son on-hook, off-hook y ringing. On-Hook – Cuando un usuario coloca el teléfono en el gancho, la PBX interrumpe y no permite que pase la corriente eléctrica. En este estado, el circuito se denomina on-hook. En esta posición, solo el timbre está activo. Off-Hook – Antes de iniciar una llamada telefónica, el teléfono debe pasar al estado off-hook. Retirar el auricular del gancho cierra el bucle e indica a la PBX que el usuario tiene la intención de realizar una llamada. Al recibir esta indicación, la PBX genera un tono de marcado, indicando al usuario que está lista para aceptar la dirección de destino (es decir, el número de teléfono). Ringing – Cuando un usuario llama a otro teléfono, genera un voltaje hacia el timbre que advierte al otro usuario de que se está recibiendo una llamada. La señalización varía según el país, con diferentes tonos para diferentes países. Puede personalizar los tonos de Asterisk para su país modificando el archivo indications.conf. Por ejemplo:
...
Como en el caso de las antiguas PBX, a menudo se requiere conectar la PBX Asterisk a la PSTN. Aquí le mostraremos cómo hacerlo. Por lo general, tiene tres opciones para las líneas telefónicas.
...
Las interfaces FX son analógicas. El término "Foreign eXchange" se aplica a los troncales de acceso a una oficina central (CO) de la PSTN. Foreign eXchange Office (FXO)

![Asterisk entre un teléfono analógico (FXS) y la línea de la compañía telefónica (FXO): el lado FXS proporciona tono de marcado y timbre al teléfono, mientras que el lado FXO toma el tono de marcado de la oficina central.](../images/10-legacy-fig01.png)

La interfaz FXO se utiliza para conectarse a una oficina central (CO) o a la extensión de otra PBX. Se comunica directamente con una línea telefónica proveniente de la PSTN. Otra opción es conectar la interfaz FXO a una PBX existente, permitiendo la comunicación entre Asterisk y la PBX legada. Conectar Asterisk a un puerto de PBX y entregar una extensión remota usando VoIP a menudo se denomina extensión fuera de las instalaciones (off-premises extension u OPX). Una interfaz FXO recibe un tono de marcado. Foreign eXchange Station (FXS) La interfaz FXS alimenta un teléfono analógico, módem o fax. La FXS proporciona el tono de marcado y la energía para un teléfono.
...
El uso de la señalización kewlstart en Asterisk es casi predeterminado. Kewlstart no es una señalización en sí misma, sino que añade inteligencia al circuito monitoreando lo que sucede en el otro lado. Kewlstart se basa en loop-start. La mayoría de los switches no admiten esta función, que se utiliza para obtener la notificación de colgado.

- Loopstart: Utilizado en la mayoría de las líneas analógicas, permite que el teléfono indique "on-hook" y "off-hook" y que el switch indique "ring" y "no-ring". Esto es probablemente lo que la mayoría de la gente tiene en casa. El nombre proviene del hecho de que la línea siempre está abierta. Cuando usted cierra el bucle, el switch le proporciona un tono de marcado. Una llamada entrante se señala mediante un voltaje de timbre de 100V sobre el par abierto.

![Asterisk operando como una puerta de enlace VoIP: un puerto FXO se conecta a una extensión de PBX legada mientras un Asterisk remoto entrega esa línea a un teléfono analógico sobre IP a través de un puerto FXS (una extensión fuera de las instalaciones, u OPX).](../images/10-legacy-fig02.png)

- Groundstart: Similar a Loopstart. Cuando desea realizar una llamada, un lado de la línea se cortocircuita. Cuando el switch identifica este estado, invierte el voltaje a través del par abierto y luego se cierra el bucle. En consecuencia, la línea primero se ocupa antes de ser ofrecida al llamante.
- Kewlstart: Añade inteligencia a los circuitos, permitiendo el monitoreo del otro lado. Kewlstart incorpora muchas ventajas del loop-start.
...
Antes de elegir el hardware para Asterisk, debe considerar la cantidad de llamadas simultáneas, servicios y codecs que se instalarán y habilitarán. Asterisk es una aplicación intensiva en CPU, por lo que recomendamos una máquina dedicada para Asterisk. La cantidad de tarjetas de interfaz instaladas dentro de la computadora está limitada por la cantidad de ranuras e interrupciones disponibles. Es preferible instalar una sola tarjeta con ocho interfaces de voz que dos tarjetas con cuatro. Otra opción es utilizar un banco de canales USB, como el Xorcom Astribank. Recientemente, algunos fabricantes (por ejemplo, CIANET) han comenzado a producir bancos de canales TDMoE, lo que hace aún más fácil conectar docenas de interfaces analógicas.
...
#### Ejemplo 1: Una instalación con un FXO y un FXS

En este ejemplo, utilizaremos una tarjeta de interfaz de telefonía Sangoma TDM400 (vendida anteriormente como Digium TDM400) con un módulo FXS y un módulo FXO. Los pasos requeridos se enumeran a continuación:
...
Puede verificar la cantidad de pérdidas de interrupción usando la utilidad dahdi_test compilada con las tarjetas DAHDI. Un número por debajo del 99.987% indica posibles problemas.
...
Después de configurar con éxito el hardware, puede proceder a la configuración de Asterisk.
...
language: Activa la internacionalización y configura un idioma. Esta función configurará los mensajes del sistema para un idioma específico. El inglés es el único idioma con prompts completos disponibles a través de la instalación estándar. musiconhold: Selecciona la clase de música en espera.

#### Opciones de Caller ID

Hay muchas opciones de callerid. Algunas se pueden deshabilitar, aunque la mayoría están habilitadas por defecto. usecallerid: Habilita o deshabilita la transmisión del callerid para los canales subsiguientes (Yes/No). Nota: Si su sistema recibe dos timbres antes de responder, intente deshabilitar esta función. Debería responder inmediatamente. hidecallerid: Define si se debe ocultar o no el callerid saliente (Yes/No). callerid: Configura una cadena de callerid para un canal específico. El llamante se puede configurar con asreceived. Esto se utiliza principalmente en interfaces de troncal para indicar el callerid entrante.
...
Asterisk admite llamada en espera en canales FXS. El usuario recibirá un tono de espera si alguien intenta comunicarse con la extensión. Para habilitar la llamada en espera:
...
Ajustar la cancelación de eco es mitad técnica, mitad arte. Estas opciones ajustan ciertos parámetros de Asterisk que afectan la calidad de audio en los canales DAHDI. Pueden ayudar a mejorar la calidad de audio en interfaces analógicas.
...
Estas opciones cambian cómo se registra la información de la llamada en la base de datos de registros de detalle de llamadas (CDR). amaflags: Configura las banderas AMA que afectan la categorización del CDR. Acepta los siguientes valores:
...
Las líneas digitales E1/T1 son una opción siempre que necesite implementar una gran cantidad de canales. Un solo circuito E1 es capaz de 30 llamadas simultáneas, y puede tener funciones como marcación directa entrante (DID), Caller ID (identificación de llamante) y señalización avanzada. La línea E1/T1 puede llegar a su empresa de varias maneras utilizando par trenzado, fibra y microondas, dependiendo de su país. Las líneas digitales se entregan a su empresa utilizando UTP, fibra o microondas. Se utilizan módems y multiplexores (MUX) para entregar la línea física. La conexión a una línea T1 siempre se basa en un conector RJ45. Sin embargo, las líneas E1 también pueden ser aprovisionadas utilizando BNC. Es muy importante saber de antemano el tipo de conector que va a recibir, principalmente en las líneas E1. Por lo general, todo el equipo hasta el RJ45 es proporcionado por la TELCO.

![Cómo se aprovisionan los circuitos E1/T1: la compañía telefónica puede entregar el troncal sobre cobre UTP (módem HDSL para E1, o una conexión de tarjeta directa para T1), sobre fibra óptica a través de un multiplexor óptico, o sobre un enlace de radio de microondas.](../images/10-legacy-fig05.png)
...
La señal analógica se muestrea 8,000 veces por segundo para crear una versión digital de la voz analógica. Esta codificación se conoce como modulación por código de pulso (PCM). En los EE. UU. y Japón, la señal se codifica utilizando la ley mu (en Asterisk, referida como ulaw). En el resto del mundo, la codificación es alaw.

![Modulación por código de pulso (PCM): la señal de voz analógica de 4 kHz se muestrea 8,000 veces por segundo (Nyquist) y se codifica en un flujo de bits digital de 64 Kbps.](../images/10-legacy-fig07.png)

#### Multiplexación por división de tiempo

Las líneas analógicas tienen sentido cuando solo necesita unos pocos canales. Al utilizar la multiplexación por división de tiempo (TDM), es posible meter múltiples canales en una sola conexión de datos. Cuando desea una gran cantidad de circuitos, la compañía telefónica generalmente le proporcionará un troncal digital, que es un circuito de datos en el que la voz se transporta en un formato digital utilizando PCM. Cada intervalo de tiempo (timeslot) utiliza 64 Kbps de ancho de banda para transportar un solo canal de voz.

![Multiplexación por división de tiempo en E1 y T1: una trama E1 transporta 32 intervalos de tiempo a 2048 Kbps (DS0 #0 para sincronización de trama, DS0 #16 para señalización), mientras que una trama T1 transporta 24 intervalos de tiempo a 1544 Kbps usando un bit para sincronización y un esquema de bits robados para señalización.](../images/10-legacy-fig08.png)

En los EE. UU., el troncal digital más común es T1, que tiene 24 líneas disponibles; en Europa y América Latina, los troncales E1 tienen 30 líneas. Algunas empresas proporcionan un T1/E1 fraccional con menos canales. Señalización de bits robados A veces, un troncal T1 utiliza un esquema de bits robados donde se toma prestado un bit para la señalización. En los troncales T1, el canal de datos/voz se transmite con 56 Kbps en cada intervalo de tiempo. Como puede observar, cuando utiliza el bit robado, el circuito T1 no pierde dos ranuras para sincronización y señalización.
...
- Interfaz de velocidad básica (BRI) o Conocida como 2B+D o Dos canales portadores (64K) y un canal de datos (16K) o Utiliza un par de cables de cobre con 148Kbps.
- Interfaz de velocidad primaria (PRI) o Entregada utilizando un troncal T1/E1 o 23B+D para T1s o 30B+D para E1s

A veces, los circuitos E1 utilizan un esquema de señalización CAS llamado MFC/R2, que fue definido por la UIT como un estándar conocido como Q.421/Q441. Esto se encuentra frecuentemente en América Latina y Asia. Varias compañías de telefonía en estos países utilizan variantes personalizadas de MFC/R2. Por lo tanto, necesitará conocer la variación correcta del país para que funcione.

### ISDN BRI

Los canales que utilizan señalización ISDN BRI son muy populares en Europa. La mayoría de las tarjetas ISDN BRI para Asterisk admiten una interfaz S/T con capacidades NT y TE. La conexión TE (terminal) es la que se utiliza para conectarse a la TELCO o a otras PBX configuradas como terminación de red (NT). La NT se utiliza para conectar teléfonos y PBX configuradas como TE. ISDN BRI proporciona dos canales de datos/voz y un canal de señalización. Las tarjetas ISDN BRI están disponibles en varios proveedores de tarjetas de interfaz para Asterisk.
...
Hay varios tipos de bus en su PC. Es muy importante que tenga la tarjeta correcta para su servidor. La siguiente descripción general describe las tarjetas utilizadas con más frecuencia:
...
- PCI Express encontrado en computadoras de escritorio y servidores o Sangoma (anteriormente Digium) TE420, TE220, TE121, AEX2400 y AEX800 o Sangoma A101, A102 y A104

> **[Nota de la 2.ª ed.]** Sangoma adquirió Digium en 2018. Las tarjetas de la marca Digium ahora se venden y reciben soporte bajo la marca Sangoma. Verifique la disponibilidad actual de los modelos en el sitio web de Sangoma (www.sangoma.com), ya que algunos SKUs más antiguos pueden estar descontinuados.
- MiniPCI encontrado en sistemas integrados o OpenVOX A100M(FXO), B100M(ISDN BRI), B200M(ISDN BRI) y B400M(ISDN BRI)
- USB 2.0 encontrado en la mayoría de las PC modernas. Las soluciones basadas en USB permiten una gran densidad de canales analógicos y digitales. Este bus admite 480 Mbps, y cada canal de voz ocupa 64 Kbps. Al utilizar hubs USB, es posible obtener densidades de hasta mil puertos analógicos en un solo puerto. o Xorcom Astribank (FXS, FXO, E1-ISDN, E1-R2)
...
Paso 5: Usando la utilidad dahdi_cfg para configurar el controlador Este es el resultado correcto para dahdi_cfg para un span E1 fraccional (15 puertos) y dos puertos FXO.
...
debug pri span x: Si después de todo todavía tiene problemas, comience a depurar el span pri. Este comando habilita una depuración detallada de las llamadas ISDN. Es un comando importante cuando piensa que algo no es correcto. Puede detectar dígitos marcados incorrectamente y otros problemas. A continuación presentamos el ejemplo de una salida de depuración para una llamada exitosa. Consulte este ejemplo si necesita comparar una llamada fallida con una sin problemas. Un consejo es usar core set verbose=0 para recibir solo los mensajes ISDN q.931.
...
channel: Define el canal o rango de canales. Cada definición de canal heredará las opciones definidas antes de la declaración. Los canales pueden identificarse individualmente o en la misma línea con separación por comas. Los rangos se pueden definir usando "-".
...
language: Activa la internacionalización y configura un idioma. Esta función configurará los mensajes del sistema para un idioma específico. El inglés es el único idioma con prompts completos disponibles desde la instalación estándar. musiconhold: Seleccione la clase de música en espera.
...
pridialplan: Requerido para algunos switches que necesitan una especificación de plan de marcado. Esta opción es ignorada por muchos switches. Las opciones válidas son private, national, international y unknown.
...
overlapdial: La marcación superpuesta (overlap dialing) se utiliza cuando usted pasa dígitos después de que se establece la conexión. Puede usar el modo de numeración por bloques (overlapdial=no) o el modo de dígitos (overlapdial=yes). El modo de bloque es utilizado a menudo por los operadores. signaling: Configura el tipo de señalización para los canales subsiguientes. Estos parámetros deben corresponder a los del archivo chan_dahdi.conf. Las opciones correctas se basan en el canal disponible. Para ISDN puede elegir cinco opciones:

- pri_cpe: Utilizado cuando el dispositivo es un CPE, a veces referido como cliente, usuario o esclavo. Esta es la forma de señalización más simple y utilizada. A veces, cuando intenta conectarse a una PBX privada, la PBX también se ha configurado comúnmente como un CPE. En este caso, use la señalización pri_net en Asterisk.
- pri_net: Utilizado cuando Asterisk está conectado a una PBX privada configurada como un CPE. La señalización a menudo se refiere como host, maestro o red.
- bri_cpe: Utilizado cuando Asterisk está conectado como un CPE a un troncal ISDN BRI
- bri_net: Utilizado cuando Asterisk está conectado a un teléfono ISDN o PBX configurado como terminal (TE).
- bri_cpe_ptmp: Igual que bri_cpe, pero en una arquitectura punto a multipunto.

#### Opciones de CallerID

Hay muchas opciones de Caller ID disponibles. Algunas se pueden deshabilitar, aunque la mayoría están habilitadas por defecto. usecallerid: Habilita o deshabilita la transmisión del Caller ID para los canales subsiguientes (Yes/No). Nota: Si su sistema requiere dos timbres antes de responder, intente deshabilitar esta función para que responda inmediatamente. hidecallerid: Oculta el Caller ID (Yes/No). calleridcallwaiting: Habilita la recepción de Caller ID durante una indicación de llamada en espera (Yes/No). callerid: Configura una cadena de Caller ID para un canal específico. El llamante se puede configurar con “asreceived” en interfaces de troncal para pasar el Caller ID hacia adelante.

```
callerid = "Flavio Eduardo Gonçalves" <48 30258500>
```

Nota: La mayoría de las TELCO exigen que configure su caller ID correcto. Si no pasa el caller ID correcto, no debería poder marcar hacia afuera a través de la TELCO. Por otro lado, podrá recibir llamadas incluso sin configurar el caller ID.
...
Estas opciones cambian la forma en que se registra la información de la llamada en la base de datos de registros de detalle de llamadas (CDR). amaflags: Afecta la categorización del CDR. Acepta estos valores:
...
**Señalización de línea (ITU-T Q.421).** En el intervalo de tiempo 16, cada canal de voz utiliza cuatro bits ABCD para señalar sus estados y el control de llamadas. Los bits C y D rara vez se utilizan. En algunos países, pueden utilizarse para medición (medición por pulsos para facturación). En una conversación normal, tenemos ambos lados trabajando: el llamante y el lado llamado. La señalización desde el lado llamante se denomina señalización de avance (forward), mientras que el lado llamado utiliza señalización de retroceso (backward). Designaremos Af y Bf para la señalización de avance y Ab y Bb para la señalización de retroceso.

| Estado | ABCD avance | ABCD retroceso |
| --- | --- | --- |
| Inactivo/Liberado | 1001 | 1001 |
| Ocupado (Seized) | 0001 | 1001 |
| Ack de ocupación | 0001 | 1101 |
...
MFC/R2 fue definido por la UIT. Desafortunadamente, varios países personalizaron el estándar para sus propias necesidades. Como resultado, surgieron variaciones en los estándares entre países.
...
| Dígito 9 | I-9 |
| Dígito 0 | I-10 |
| Indicador de código de país, se requiere supresor de eco de mitad saliente | I-11 |
| Indicador de código de país, no se requiere supresor de eco | I-12 |
| Indicador de llamada de prueba | I-13 |
| Indicador de código de país, supresor de eco de mitad saliente insertado | I-14 |
| No utilizado | I-15 |

Grupo de señales II (avance):
...
| Abonado u operador sin facilidad de transferencia de avance | II-7 |
| Transmisión de datos | II-8 |
| Abonado con prioridad | II-9 |
| Operador con facilidad de transferencia de avance | II-10 |
| Repuesto | II-11 |
| Repuesto | II-12 |
| Repuesto | II-13 |
| Repuesto | II-14 |
...
Grupo de señales A (retroceso):

| Descripción | Señal de retroceso |
| --- | --- |
...
| Línea de abonado libre, con cobro | B-6 |
...
Nota: Algunas TELCO no aceptan llamadas sin el caller ID. Por favor, establezca el caller ID en uno de los números DID asignados por el operador. En algunos países, este paso no es necesario. Paso 7: Pruebe la solución: Ahora, con una extensión en el contexto from-internal, llame a cualquier número y observe la consola. Verifique si se está produciendo algún error. -- Executing Set("SIP/8564-081ca5d8", "CALLERID(num)=1145678990") in new stack -- Executing Dial("SIP/8564-081ca5d8", "DAHDI/g1/35678899|60|tT") in new stack

#### Depuración de OpenR2

Para detectar errores en las llamadas, puede activar la depuración. Para hacer esto, siga los pasos a continuación. Paso 1: Edite el archivo chan_dahdi.conf y añada las siguientes tres líneas a la configuración:
...
Paso 2: Reinicie el servidor Asterisk Paso 3: Pruebe la llamada y verifique los archivos de llamada en /var/log/asterisk/mfcr2/span1 A continuación se muestra un rastro para una llamada normal. Compárelo con lo que recibe en su llamada.
...
Las opciones están documentadas dentro del archivo chan_dahdi.conf. Algunas de las opciones más importantes se detallan aquí. Parámetros obligatorios: mfcr2_variant, mfcr2_max_ani y mfcr2_max_dnis. mfcr2_variant: Variante del país.
...
mfcr2_mfback_timeout: Este valor merece ser mencionado. A veces, si está llamando a un teléfono celular o cualquier llamada que tarda mucho tiempo en completarse, este parámetro puede agotarse, por lo que a menudo se cambia para un ajuste fino. Si algunas de sus llamadas no se están completando, este es el parámetro que debe cambiar primero. mfcr2_metering_pulse_timeout: Los pulsos son utilizados por algunas variantes R2 para indicar costos mfcr2_allow_collect_calls: En Brasil, el tono II-8 se utiliza para indicar una llamada por cobrar; este parámetro le permite bloquear llamadas por cobrar. mfcr2_double_answer: También se utiliza para evitar llamadas por cobrar cuando se requiere una doble respuesta. Con double_answer=yes usted bloquea realmente las llamadas por cobrar. mfcr2_immediate_accept: Le permite saltarse el uso de señales de grupo B/II e ir directamente al estado aceptado. mfcr2_forced_release: Le permite acelerar la liberación de la llamada; funciona para la variante brasileña.

#### ANI y DNIS

La Identificación Automática de Número (ANI) es el número del llamante. El Servicio de Identificación de Número Marcado (DNIS) es el número llamado o, en otras palabras, el número marcado. Cuando se recibe una llamada, generalmente los últimos cuatro números se pasan a la PBX en un proceso referido como marcación directa entrante (DID). El número ANI es en realidad el Caller ID. ANI tendrá la extensión del llamante al marcar, mientras que DNIS contendrá el destino de la llamada. Es importante que estos parámetros se configuren correctamente. Algunos switches envían solo los últimos cuatro dígitos, mientras que otros envían el número completo.
...
En este capítulo, aprenderemos sobre el protocolo Inter-Asterisk eXchange (IAX), incluyendo sus fortalezas y debilidades. También se cubrirán detalles como el modo troncal y la interconexión de dos servidores Asterisk. Todas las referencias en este documento corresponden a IAX versión 2. El protocolo IAX proporciona transporte de medios y señalización para voz y video. IAX es muy innovador; ahorra ancho de banda en modo troncal y es mucho más simple que SIP cuando necesita atravesar NAT. El uso principal para IAX hoy en día es interconectar servidores Asterisk. IAX fue creado principalmente para voz, pero también puede acomodar video y otros flujos multimedia. IAX se inspiró en otros protocolos VoIP, como SIP y MGCP. En lugar de utilizar dos protocolos separados para señalización y medios, IAX los unificó para hacer un protocolo único. IAX no utiliza RTP para el transporte de medios; en cambio, incrusta los medios en la misma conexión UDP.

> **[Nota de la 2.ª ed. — Estado en Asterisk 22]** `chan_iax2` todavía está incluido y es totalmente compatible en Asterisk 22 LTS, por lo que todo en este capítulo sigue siendo válido. Sin embargo, IAX2 es ahora un protocolo legado y ve relativamente pocos despliegues nuevos. La industria VoIP ha convergido en gran medida en SIP (a través de `chan_pjsip` en Asterisk 22) tanto para troncales de proveedores como para interconexión de servidores. El principal punto de venta restante de IAX2 es su **atravesamiento NAT de puerto único**: toda la señalización y los medios fluyen sobre un solo puerto UDP (4569 por defecto), lo que simplifica enormemente la configuración del firewall y NAT en comparación con SIP + RTP. Si está construyendo un nuevo troncal Asterisk-a-Asterisk y NAT no es una preocupación, los troncales PJSIP son el enfoque moderno recomendado. IAX2 se mantiene aquí porque sigue siendo una opción válida, especialmente en entornos donde solo se puede abrir un puerto UDP a través de un firewall.
...
Asterisk admite teléfonos IP basados en IAX como el ATCOM y el antiguo ATA de Digium (llamado IAXy), así como soft-phones que todavía implementan el protocolo IAX2. El proceso para soft-phones, ATA y teléfonos físicos es similar. Para configurar un dispositivo IAX, necesita editar el archivo iax.conf en /etc/asterisk

```
directory.
```

Utilizaremos un soft-phone compatible con IAX2 como ejemplo. Paso 1: Haga una copia de seguridad del archivo iax.conf original usando:
...
Esta línea afecta la selección de codec. Usar la configuración high permite la selección de un codec de alto ancho de banda y alta calidad como g.711 definido por la palabra clave ulaw. Si mantiene el parámetro predeterminado, no podrá elegir ulaw. En este caso, Asterisk le dará el mensaje “no codec available” para la configuración a continuación.
...
En los comandos anteriores, hemos definido un amigo (friend) llamado [2003]. El contexto es el predeterminado (en los primeros laboratorios siempre usamos el contexto predeterminado para evitar confusiones; este contexto se explicará completamente en el capítulo 9). La línea “host=dynamic” proporciona un registro dinámico de la dirección IP del teléfono. Paso 3: Descargue e instale un soft-phone compatible con IAX2. Puede elegir cualquier soft-phone que todavía admita el protocolo IAX2 para el laboratorio. Paso 4: Configure una cuenta IAX en el cliente (típicamente *Add account* → IAX). Tenga en cuenta que el SipPulse Softphone es solo SIP y no puede registrarse sobre IAX2, por lo que para las pruebas de IAX necesita un cliente que todavía admita el protocolo.
...
Unos pocos proveedores VoIP admiten IAX. Puede encontrar fácilmente un proveedor IAX buscando “IAX providers”. Usar un proveedor IAX tiene mucho sentido ya que IAX puede ahorrar mucho ancho de banda, atraviesa fácilmente NAT y puede autenticarse usando pares de claves RSA.

![El Asterisk de un cliente conectado a un proveedor VoIP sobre un troncal IAX a través de Internet: un solo troncal transporta todas las llamadas hacia y desde el proveedor.](../images/10-legacy-fig14.png)

> **[Nota de la 2.ª ed.]** La cantidad de proveedores VoIP comerciales compatibles con IAX ha disminuido significativamente desde Asterisk 16. La mayoría de los proveedores ahora ofrecen troncales SIP/PJSIP exclusivamente. Antes de elegir un proveedor IAX, confirme que mantienen activamente su infraestructura IAX. Para nuevas integraciones de proveedores, un troncal PJSIP es la alternativa recomendada.
...
Paso 2: Configure el archivo iax.conf para registrar su Asterisk con su proveedor. Añada las siguientes líneas a la sección [general] del archivo.
...
Esto es necesario para la autenticación RSA. Usar la clave pública de su proveedor le permite estar seguro de que la llamada que se está recibiendo es realmente del proveedor verdadero. Si alguien más intenta usar la misma ruta, no podrá autenticarla porque no tienen la clave privada correspondiente. Paso 4: Pruebe la conexión. Para probar la conexión, llame a cualquier número. Algunos proveedores proporcionan una prueba de eco. Para lograr esto, por favor edite el archivo extensions.conf.
...
#### Conectar dos servidores Asterisk a través de un troncal IAX

Es muy fácil conectar un servidor a otro. No necesitará registrarlos porque las direcciones IP ya se conocen. Tendrá que crear los peers y usuarios en el archivo iax.conf. Todas las extensiones en el sitio HQ comienzan con 20 seguidas de dos dígitos (por ejemplo, 2000). En la Sucursal, todas las extensiones comienzan con 22 seguidas de dos dígitos (por ejemplo, 2200). Usaremos el troncal. Necesitará una fuente de temporización DAHDI para habilitar esta función. Paso 1: Edite el archivo iax.conf en el servidor de la Sucursal.

![Conexión de dos servidores Asterisk con un troncal IAX: el servidor HQ (192.168.1.1, extensiones 20xx) y el servidor de la Sucursal (192.168.1.2, extensiones 22xx) se alcanzan entre sí sobre un solo troncal IAX; no se necesita registro porque ambas direcciones IP son fijas y conocidas.](../images/10-legacy-fig15.png)
...
![El flujo de decisión de autenticación IAX para una llamada entrante: Asterisk se ramifica dependiendo de si se proporciona un nombre de usuario, si coincide con una sección, si la IP de origen está permitida y si el secreto (texto plano, MD5 o RSA) coincide, aceptando la llamada con el contexto y las opciones de peer de esa sección, o denegándola.](../images/10-legacy-fig16.png)

Cuando Asterisk recibe una conexión entrante, la información inicial puede incluir un nombre de usuario (del campo “username=”) o no. La conexión entrante también tiene una dirección IP, que Asterisk utiliza para la autenticación también. Si se proporciona un usuario, Asterisk: 1. Busca en iax.conf una entrada con type=user (o type=friend con un nombre de sección que coincida con el nombre de usuario). Si no la encuentra, Asterisk rechaza la conexión. 2. Si la entrada encontrada tiene configuraciones de denegación/permiso (deny/allow), compara la dirección IP del llamante para determinar si aceptar la llamada o no dependiendo de las cláusulas. 3. Verifica la contraseña (secreto) usando texto plano, md5 o RSA. 4. Acepta la conexión y envía la llamada al contexto especificado en la línea “context=” del archivo iax.conf. Si no se proporciona un nombre de usuario, Asterisk: 1. Busca una entrada que contenga type=user (o type=friend) en el archivo iax.conf sin un secreto especificado. También verifica las cláusulas de denegación/permiso. Si se encuentra una entrada, se acepta la conexión y el nombre de la sección se utiliza como el nombre del usuario. 2. Busca una entrada que contenga type=user (o type=friend) en el archivo iax.conf con un secreto o clave RSA especificada. Verifica las cláusulas de denegación/permiso. Si se encuentra una entrada, intenta autenticar al llamante usando el secreto especificado; si coincide, acepta la conexión. El nombre de la sección es el nombre del usuario. Supongamos que su archivo iax.conf tiene las siguientes entradas:
...
Es posible usar IAX con autenticación fuerte usando claves RSA asimétricas. Según el código fuente (res_krypto.c), Asterisk utiliza claves RSA con un algoritmo SHA-1 para resúmenes de mensajes en lugar del MD5 más débil. A continuación, una guía paso a paso para configurar dos servidores usando claves RSA.
...
Cuando se le pregunte, use el nombre de clave branch. Hemos utilizado el parámetro –n para evitar pasar una frase de contraseña cada vez que Asterisk se reinicializa. Si desea mejorar la seguridad, no use el –n e inicie Asterisk con asterisk -i Paso 2: Copie las claves al directorio /var/lib/asterisk/keys
...
Direcciones del servidor bindport = <portnum> Configura el puerto UDP IAX. El valor predeterminado es 4569. bindaddr = <ipaddr> Use 0.0.0.0 para vincular Asterisk a todas las interfaces o especifique la dirección IP de una interfaz específica. Selección de codec bandwidth = [low|medium|high] High = todos los codecs Medium = todos los codecs excepto ulaw y alaw Low = codecs de bajo ancho de banda allow/disallow = Ajuste fino de selección de codec [alaw|ulaw|gsm|g.729| etc.]
...
![El jitter buffer como un tanque de agua: los paquetes llegan irregularmente de la red y llenan el buffer, que luego los libera a una tasa constante para producir un flujo de voz suave. El tamaño del buffer (en ms) intercambia un poco de latencia por un jitter más bajo; la banda de exceso de buffer permite que Asterisk aumente o reduzca el buffer a medida que cambian las condiciones de la red.](../images/10-legacy-fig17.png)

Un jitter pequeño (es decir, por debajo de 20 ms) suele ser imperceptible. Sin embargo, un jitter por encima de este nivel es molesto. La latencia o retardo debe mantenerse por debajo de 150ms. Crear un jitter buffer sacrificará algo de retardo por un jitter más bajo, un concepto conocido como “presupuesto de retardo” (delay-budget). Puede afectar el jitter buffer usando estos parámetros:
...
Este capítulo ha revisado las fortalezas y debilidades del protocolo IAX. Ha demostrado cómo funciona IAX en varios escenarios, como soft-phones y un troncal entre dos servidores Asterisk. El modo troncal le permite ahorrar ancho de banda al transportar más de una llamada en un solo paquete. Finalmente, aprendió comandos de consola que puede usar para verificar el estado y depurar el protocolo.

## SIP legado: chan_sip y sip.conf (eliminado en Asterisk 21+)

> **Legado / histórico:** Todo en esta sección utiliza el antiguo controlador `chan_sip` y su archivo de configuración `sip.conf`. `chan_sip` fue desaprobado durante varias versiones y **eliminado en Asterisk 21**, por lo que **no existe en Asterisk 22**. Ninguno de los ejemplos de `sip.conf` a continuación se ejecutará en un sistema actual; se mantienen aquí solo para documentar cómo funcionaban los despliegues legados y para ayudarle a migrarlos. Para la forma moderna y compatible de hacer cualquiera de estas cosas, consulte la sección *PJSIP: el canal SIP* del capítulo *SIP & PJSIP en profundidad*. La teoría del *protocolo* SIP (métodos, registro, proxy/redirect, SDP, tipos de NAT) es a nivel de protocolo y vive en ese capítulo; lo que sigue es puramente la **configuración** de `chan_sip` eliminada.

En los sistemas legados hasta Asterisk 20, SIP se configuraba en `/etc/asterisk/sip.conf`, que solía ser el segundo archivo más modificado (justo después de `extensions.conf`). Las secciones a continuación muestran cómo `chan_sip` conectaba Asterisk a un proveedor SIP, cómo conectar dos Asterisks juntos usando SIP, soporte de dominio, presencia, opciones de codec/DTMF/QoS, autenticación y NAT, seguido de una guía para migrar todo a PJSIP.

### Conectar Asterisk a un proveedor SIP (sip.conf)

Asterisk se utiliza a menudo para conectarse a un proveedor VoIP SIP. Los proveedores VoIP suelen tener mejores tarifas para llamadas telefónicas que los proveedores tradicionales. Otro punto interesante y atractivo de los proveedores VoIP es la posibilidad de comprar números DID en otras ciudades, incluso en países extranjeros. Estas son buenas razones para usar VoIP para las telecomunicaciones. En esta sección, aprenderá cómo el `chan_sip` legado conectaba Asterisk a un proveedor VoIP. Se requieren tres pasos para conectar Asterisk a un proveedor SIP. Las pruebas se pueden realizar estableciendo una cuenta con su proveedor favorito. Paso 1: Registrarse con un proveedor SIP en sip.conf Para conectarse a un proveedor SIP, necesitará la siguiente información del proveedor:

![Asterisk conectado a un proveedor de servicios VoIP sobre Internet o una WAN privada, con teléfonos SIP locales registrados al servidor Asterisk](../images/07-sip-and-pjsip-fig07.png)
...
La siguiente discusión examina los detalles de las opciones establecidas en el archivo sip.conf para la conexión a un proveedor VoIP.

```
register=>username:password@hostname/4100
```

La instrucción registered en el archivo sip.conf se utiliza para registrarse con un proveedor. La transacción de registro se autentica con el nombre y el secreto. Puede usar una barra diagonal (“/”) para proporcionar una extensión para las llamadas entrantes. Técnicamente hablando, la extensión se colocará en el campo de cabecera “Contact” de la solicitud SIP. El comportamiento de registro puede ser controlado por ciertos parámetros:
...
Para verificar si el registro fue exitoso, el comando de consola legado era `sip show registry`. En Asterisk 22, el comando equivalente es `pjsip show registrations` (registros salientes) y `pjsip show endpoints` para el estado del endpoint.

El parámetro “username” se utiliza en el resumen de autenticación (digest). El resumen se calcula usando nombre de usuario, secreto y reino (realm):

```
username=username
```

Host define la dirección o nombre del proveedor VoIP:

```
host=hostname
```

Los parámetros Fromuser y Fromdomain a veces son necesarios para la autenticación. Estos parámetros se utilizan en el campo de cabecera SIP From:
...
Cuando se conecta a un proveedor VoIP, se requieren credenciales. Después de la invitación inicial, el proveedor le envía un mensaje llamado “407 Proxy Authentication Required”; usted proporciona las credenciales en el mensaje INVITE subsiguiente. Para las llamadas entrantes, su servidor Asterisk solicitará credenciales para el proveedor. Obviamente, el proveedor no tiene una credencial válida para su servidor Asterisk. Cuando usa insecure=invite, le está diciendo a Asterisk que no envíe el “407 Proxy Authentication Required” al proveedor y que acepte las llamadas entrantes. También puede usar insecure=port, invite para hacer coincidir el peer basado en la dirección IP sin hacer coincidir el número de puerto.
...
Este parámetro permite que una invitación externa sea procesada sin autenticación. Procesa la llamada dentro del contexto definido en la sección general o en la declaración de dominio. Advertencia: Si define un contexto en la sección general con acceso a la PSTN, un usuario externo puede marcar a la PSTN a través de su PBX. En este caso, incurrirá en cualquier cargo. Permita solo sus propias extensiones en el contexto definido en la sección general.

![Conexión a otros servidores SIP por dominio: youdomain.com y yourpartnerdomain.com intercambian señalización SIP, por lo que usuarios como lee y bruce pueden llamar a chuck y norris usando URIs SIP](../images/07-sip-and-pjsip-fig09.png)
...
El valor predeterminado es yes. Descomente la línea para no permitir llamadas a dominios externos.

### Configuraciones avanzadas SIP (sip.conf)

Esta sección explica algunos parámetros avanzados del canal SIP legado, como presencia, selección de codec, opciones DTMF y marcado de paquetes QoS. Los **conceptos** (BLF/presencia, negociación de codec, modos DTMF, marcado DSCP) se trasladan a PJSIP, pero los nombres de parámetros de `sip.conf` que se muestran aquí **no** existen en Asterisk 22. En PJSIP, el modo DTMF es `dtmf_mode=` en un endpoint, y los codecs se establecen con `allow=`/`disallow=`.
...
- subscribecontext=sip_subscribers: Contexto donde buscar hints
...
Paso 2: Ahora configure el soft-phone para usar presencia. Le mostraremos cómo configurar el SipPulse Softphone.
...
Paso 3: Agregue el contacto a otros soft-phones. En este ejemplo, el SipPulse Softphone es la cuenta 2000, así que agregaremos un contacto para la cuenta 2001. Secuencia: Abra el panel derecho (panel de presencia en el softphone)->Haga clic en Contacts->Add a contact. Llene el nombre 2001. Display as 2001 y no olvide marcar la casilla Show this contact’s availability.

Paso 4: Ahora llame a la extensión 2001 y verifique el estado del teléfono en el panel derecho del soft-phone. Use el comando de consola `core show hints` para ver el estado de presencia cambiando en el servidor (en chan_sip legado, `sip show inuse` mostraba cuántas llamadas tenía en cada línea). En Asterisk 22, use `pjsip show endpoints` para inspeccionar el estado del endpoint y del canal. El estado de presencia/BLF aparece en los contactos o panel BLF del softphone; exactamente cómo se muestra depende del cliente.

#### Configuración de codec

La configuración de codec es simple y directa. Puede establecer las palabras allow y disallow en la sección [general] o en la sección peer/user. La mejor práctica es estandarizar el codec para evitar la transcodificación, que es intensiva en procesador. Utilice el mismo codec para mensajes y prompts.
...
Finalmente, puede pasar dígitos DTMF dentro de paquetes SIP, en lugar de paquetes RTP. Este método se define en el RFC3265 (eventos de señalización) y RFC2976.
...
QoS es un conjunto de técnicas responsables de la calidad de voz. QoS se implementa de tal manera que reduce el ancho de banda, la latencia y el jitter. Las funciones principales de QoS son la programación de paquetes, la fragmentación y la compresión de cabeceras. QoS se implementa en switches y routers, no por Asterisk en sí. Sin embargo, Asterisk puede ayudar a los routers y switches marcando paquetes para entrega urgente. El marcado se realiza utilizando puntos de código de servicios diferenciados (DSCP) definidos en RFCs 2474 y RFC2475.
...
Cuando el `chan_sip` legado recibía una llamada SIP, seguía las reglas descritas en el siguiente diagrama. Tres parámetros jugaban un papel importante en la autenticación SIP. En Asterisk 22, la autenticación se configura en su lugar con objetos PJSIP `auth` (`type=auth`, `auth_type=userpass`, `username=`, `password=`) referenciados por un endpoint, y el control de acceso IP se realiza con `permit=`/`deny=` en el endpoint o a través de un `acl`.

![Flujo de decisión de autenticación de chan_sip legado: Asterisk verifica la cabecera From contra sip.conf, intenta la sección type=user/peer coincidente y las credenciales MD5, y recurre a insecure=invite o allowguest antes de permitir o denegar la llamada](../images/07-sip-and-pjsip-fig10.png)
...
Cuando usamos insecure=invite, Asterisk no genera el mensaje “407 Proxy Authentication Required”. Sin este mensaje, el usuario puede realizar una llamada sin autenticación. Esto se utiliza a menudo para conectarse a proveedores de servicios VoIP. Las llamadas que provienen del proveedor de servicios VoIP generalmente no están autenticadas.

```
autocreatepeer=yes/no
```

Este comando se utiliza cuando Asterisk está conectado a un proxy SIP. Crea dinámicamente un peer para cada llamada. Cuando esta opción está habilitada, cualquier UAC puede conectarse al servidor Asterisk. Es importante limitar la conexión IP al proxy SIP. El proxy SIP, a su vez, se encarga del control de acceso. La configuración del peer se basa en las opciones generales, así como en el campo de cabecera “Contact” del paquete SIP. Advertencia: Use esto con extrema precaución ya que abre completamente Asterisk.
...
La *teoría* de NAT (los cuatro tipos de NAT, el problema de la cabecera Contact, los keep-alives y forzar los medios a través del servidor) es a nivel de protocolo y se cubre en el capítulo *SIP & PJSIP en profundidad*. Los parámetros de `sip.conf` que se muestran aquí (`nat=`, `qualify=`, `directmedia=`, `externaddr=`, `localnet=`) son de **chan_sip legado** y fueron eliminados en Asterisk 21+. En PJSIP, estos se asignan a configuraciones de transporte/endpoint como `rewrite_contact=yes`, `force_rport=yes`, `rtp_symmetric=yes`, `direct_media=no`, `external_media_address`, `external_signaling_address` y `local_net=` en el transporte, además de `qualify_frequency=` en el AOR.

En chan_sip legado, el parámetro `nat` tenía cinco opciones:

- nat = no — No realizar ningún manejo especial de NAT aparte de RFC3581
- nat = force_rport — Pretender que había un parámetro rport incluso si no lo había
...
Cuando pone la declaración “nat=force_rport” en el archivo sip.conf, le está diciendo a Asterisk que ignore la dirección contenida en el campo de cabecera “Contact” de la cabecera SIP y use la dirección IP de origen y el puerto en la cabecera IP del paquete, y también que envíe los medios de vuelta a la dirección desde donde se recibieron, ignorando el contenido de la cabecera SDP.
...
Paso 3: Configure Asterisk para incluir la dirección externa en los campos de cabecera de los paquetes SIP, incluido el Protocolo de Descripción de Sesión (SDP). Puede lograr esto añadiendo las siguientes dos declaraciones al archivo sip.conf:
...
Puede llamar a un destino SIP legado usando diferentes cadenas de marcado:
...
## Migrar un sistema chan_sip legado a PJSIP

Debido a que `chan_sip` fue eliminado en Asterisk 21 y ya no existe en Asterisk 22, cualquier despliegue de `sip.conf` existente debe migrarse a PJSIP. El mayor cambio conceptual es que un solo `sip.conf` `[peer]` o `[friend]` se divide en varios objetos PJSIP, cada uno con un `type=`: un **endpoint** (configuración de llamada/codec/medios), uno o más objetos **aor** (dónde se puede alcanzar el dispositivo / registro), un objeto **auth** (credenciales) y un **transport** compartido (el socket de escucha, direcciones NAT). La siguiente tabla asigna los conceptos más comunes.

| Concepto de sip.conf legado | Equivalente PJSIP (pjsip.conf) |
| --- | --- |
...
| `host=dynamic` (dispositivo se registra) | `type=aor` con `max_contacts=1`; el dispositivo REGISTER para actualizar su contacto |
...
Una extensión de registro que se veía así en el `sip.conf` legado:
...
Asterisk distribuye un script auxiliar, **`sip_to_pjsip.py`**, que lee un `sip.conf` existente y produce un `pjsip.conf`. Puede ejecutarlo directamente en el directorio /etc/asterisk. La utilidad está en el árbol de fuentes de Asterisk bajo `contrib/scripts/sip_to_pjsip/`, donde `${PATH_TO_ASTERISK_SOURCE}` es la ruta donde se encuentran los archivos fuente de Asterisk (generalmente /usr/src/asterisk-22.x.y/):
...
Trate su salida como un **punto de partida**: revise cada objeto generado, especialmente los transportes, configuraciones NAT y listas de codec, y pruebe exhaustivamente antes de pasar a producción.
...
Aunque la conversión parece correcta, podemos ver que algunos elementos como qualify=yes no se pueden mapear directamente. Para corregirlo, debe agregar a la sección aor el comando qualify_frequency=time en segundos. Ejemplo a continuación.
...
1. Con respecto a las dos interfaces analógicas Foreign eXchange, marque las afirmaciones correctas (elija todas las que correspondan):
   - A. Una interfaz FXO se conecta a la oficina central de la red telefónica pública conmutada (PSTN) y toma el tono de marcado de ella.
   - B. Una interfaz FXS proporciona tono de marcado y energía de timbre a un teléfono, fax o módem analógico estándar.
   - C. Una interfaz FXS es la forma correcta de conectar Asterisk a una línea de compañía telefónica.
   - D. Una interfaz FXO también se puede conectar a un puerto de extensión de una PBX legada.
...
4. Para una facturación precisa en canales analógicos, debe detectar exactamente cuándo contesta el extremo lejano. ¿Qué función activa en Asterisk (y solicita a la TELCO) para hacer esto?
...
5. El hardware DAHDI es independiente de Asterisk: la tarjeta física se configura en `/etc/dahdi/system.conf`, mientras que `chan_dahdi.conf` define los canales de Asterisk, no el hardware en sí.
   - A. Verdadero
   - B. Falso
6. Con respecto a la capacidad y señalización del troncal digital, marque las afirmaciones correctas (elija todas las que correspondan):
   - A. Un troncal E1 transporta 30 canales de voz y un troncal T1 transporta 24.
   - B. Un ISDN PRI utiliza 30B+D en un E1 y 23B+D en un T1.
   - C. ISDN es un ejemplo de señalización CCS, mientras que MFC/R2 es un ejemplo de señalización CAS.
   - D. T1 es el troncal digital más utilizado en Europa y América Latina.
7. ¿Qué utilidad detecta automáticamente las tarjetas DAHDI y genera `/etc/dahdi/system.conf` y `dahdi-channels.conf`?
   - A. dahdi_generator
   - B. dahdi_genconf
   - C. dahdi_cfg
   - D. generate_dahdi
8. Al migrar un `sip.conf` `[friend]` legado a PJSIP, un solo bloque debe dividirse en varios objetos. ¿Qué conjunto de objetos `type=` de PJSIP reemplaza normalmente a un `[friend]` de registro?
   - A. `type=endpoint`, `type=aor` y `type=auth`
   - B. `type=peer` y `type=user`
   - C. `type=sip` solamente
...
9. ¿Cuál es la principal ventaja práctica de usar el modo troncal IAX2 entre dos servidores Asterisk?
   - A. Cifra cada llamada con TLS por defecto
   - B. Transporta varias llamadas bajo una sola cabecera, ahorrando ancho de banda
   - C. Elimina la necesidad de cualquier codec
   - D. Asigna un puerto UDP separado por llamada para una mejor calidad
10. Las claves RSA se pueden usar para la autenticación IAX2. ¿Qué clave debe mantener en secreto y cuál entrega al otro servidor?

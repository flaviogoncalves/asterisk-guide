...
En un mundo puramente VoIP en 2026, los tipos de canales en este capítulo son cada vez más raros: la mayoría de las implementaciones nuevas son troncales SIP y endpoints PJSIP sobre Ethernet, sin hardware de telefonía en absoluto. Asterisk 22, no obstante, todavía los admite casi todos por completo. La conectividad analógica (FXO/FXS) y digital TDM (E1/T1/ISDN PRI/BRI) se proporciona a través de DAHDI: la pila de controladores desarrollada originalmente por Digium, que fue adquirida por Sangoma en 2018, después de que los controladores Zaptel anteriores fueran renombrados tras una disputa de marca registrada. La conectividad de servidor a servidor sobre IAX2 es proporcionada por `chan_iax2`, que todavía se distribuye y admite, pero que ahora es firmemente un protocolo heredado. Este capítulo también recopila el material de **SIP heredado**: el antiguo controlador `chan_sip` y su configuración `sip.conf` —eliminados en Asterisk 21 y desaparecidos en Asterisk 22— junto con una guía completa para migrar un sistema `sip.conf` existente a PJSIP. Si usted está ejecutando una tienda puramente SIP en PJSIP sin tarjetas de telefonía, sin troncales IAX2 y sin `sip.conf` heredado que convertir, puede saltarse este capítulo con seguridad.

## Canales analógicos (FXO/FXS)

> **[Nota de la 2ª ed.]** Actualizar las fechas de la portada/ISBN para la 2ª edición antes de la publicación.

A partir de Asterisk 22, DAHDI y las tarjetas de telefonía analógica siguen siendo totalmente compatibles, y DAHDI todavía se compila con los kernels actuales. La mayoría de las nuevas implementaciones son, sin embargo, puramente VoIP (troncales SIP, PJSIP), por lo que el hardware analógico/TDM es ahora una opción de nicho, que se encuentra principalmente en entornos heredados, conectividad PSTN rural o mercados regulados. Todo lo que aparece a continuación sigue siendo aplicable a esos escenarios.

Existen varias formas de conectarse a la red telefónica pública conmutada (PSTN). La mejor manera depende de cómo la compañía telefónica haga que esta conexión esté disponible en su área. La forma más sencilla es utilizar una línea analógica, similar a la línea que utiliza en casa. En esta sección, le mostraremos cómo configurar tarjetas analógicas de Sangoma™ (anteriormente Digium™) y Xorcom™.
...
#### Ejemplo 1: Una instalación de un FXO, un FXS
...
Ahora tiene que cargar el módulo chan_dahdi y el controlador del kernel de la tarjeta relacionado. Use dahdi_hardware para detectar su tarjeta y el nombre del controlador. Por ejemplo:
...
Puede verificar la cantidad de interrupciones perdidas utilizando la utilidad dahdi_test compilada con las tarjetas DAHDI. Un número por debajo del 99.987% indica posibles problemas.
...
Ajustar la cancelación de eco es mitad técnico, mitad arte. Estas opciones ajustan ciertos parámetros de Asterisk que afectan la calidad de audio en los canales DAHDI. Pueden ayudar a mejorar la calidad de audio en interfaces analógicas.
...
La señal analógica se muestrea 8,000 veces por segundo para crear una versión digital de la voz analógica. Esta codificación se conoce como modulación por código de pulsos (PCM). En los EE. UU. y Japón, la señal se codifica utilizando la ley mu (en Asterisk, denominada ulaw). En el resto del mundo, la codificación es alaw.

![Modulación por código de pulsos (PCM): la señal de voz analógica de 4 kHz se muestrea 8,000 veces por segundo (Nyquist) y se codifica en un flujo digital de bits de 64 Kbps.](../images/10-legacy-fig07.png)
...
En los EE. UU., la troncal digital más común es T1, que tiene 24 líneas disponibles; en Europa y América Latina, las troncales E1 tienen 30 líneas. Algunas empresas proporcionan una T1/E1 fraccionada con menos canales. Señalización de bit robado A veces, una troncal T1 utiliza un esquema de bit robado donde se toma prestado un bit para la señalización. En las troncales T1, el canal de datos/voz se transmite con 56 Kbps en cada intervalo de tiempo. Como puede observar, cuando utiliza el bit robado, el circuito T1 no pierde dos intervalos para la sincronización y la señalización.
...
- Interfaz de velocidad primaria (PRI) o Entregada utilizando una troncal T1/E1 o 23B+D para T1s o 30B+D para E1s
...
Hay varios tipos de bus en su PC. Es muy importante que tenga la tarjeta adecuada para su servidor. La siguiente descripción general describe las tarjetas utilizadas con más frecuencia:
...
Estas familias de tarjetas se originaron en Digium, que Sangoma adquirió en 2018; ahora se venden y admiten bajo la marca Sangoma. Muchos de los SKU más antiguos enumerados aquí han sido descontinuados, así que confirme la disponibilidad del modelo actual en www.sangoma.com antes de comprar.
...
Paso 3: Carga de los controladores del kernel Compruebe qué controlador necesita instalar usando dahdi_hardware.
...
Paso 4: Usando dahdi_test, verifique las interrupciones perdidas Puede verificar la cantidad de interrupciones perdidas utilizando la utilidad dahdi_test compilada con las tarjetas DAHDI. Un número por debajo del 99.987% indica posibles problemas. Encontrará dahdi_test en
...
Paso 5: Usando la utilidad dahdi_cfg para configurar el controlador Esta es la salida correcta de dahdi_cfg para un span E1 fraccionado (15 puertos) y dos puertos FXO.
...
language: Activa la internacionalización y configura un idioma. Esta función configurará los mensajes del sistema para un idioma específico. El inglés es el único idioma con avisos completos disponibles de la instalación estándar. musiconhold: Seleccione la clase de música en espera.
...
**Señalización de línea (ITU-T Q.421).** En el intervalo de tiempo 16, cada canal de voz utiliza cuatro bits ABCD para señalar sus estados y el control de llamadas. Los bits C y D rara vez se utilizan. En algunos países, se pueden utilizar para medición (medición por pulsos para facturación). En una conversación normal, tenemos ambos lados trabajando: el lado que llama y el lado llamado. La señalización desde el lado que llama se denomina señalización de reenvío (forward), mientras que el lado llamado utiliza señalización de retroceso (backward). Designaremos Af y Bf para la señalización de reenvío y Ab y Bb para la señalización de retroceso.
...
| Línea de suscriptor libre, cargo | B-6 |
...
![Un flujo de llamadas MFC/R2 completo entre Asterisk y la TELCO: la señalización de línea (Inactivo, Ocupado, Ack de ocupación, Respuesta, Clearback, Clear Forward) se intercambia en el intervalo de tiempo 16, los dígitos marcados y las señales de retroceso "enviar siguiente dígito" (grupos I/A/B) viajan en banda, y los tonos audibles llegan al suscriptor.](../images/10-legacy-fig11.png)

### Cómo utilizar el controlador libopenr2

El proyecto iniciado por Moises Silva se inspiró en el controlador de canal Unicall escrito por Steve Underwood. La biblioteca OpenR2 es actualmente la solución de software más estable para Asterisk. Con esta solución, podemos utilizar cualquier tarjeta digital compatible con DAHDI. Anteriormente, solo estaban disponibles soluciones propietarias para MFC/R2; una de las mejores que he utilizado es la puesta a disposición por Khomp, www.khomp.com.br. En Asterisk 22, la compatibilidad con MFC/R2 a través de libopenR2 está integrada cuando la biblioteca está presente en el momento de la compilación; no se requiere ningún parche externo. Los pasos a continuación muestran la instalación manual histórica como referencia; en sistemas modernos, instale `libopenr2-dev` desde el gestor de paquetes de su distribución antes de ejecutar `./configure`, luego habilite `chan_dahdi` en `make menuselect`.

Los pasos a continuación compilan openr2 y Asterisk desde sus repositorios Git actuales. Se mantienen como referencia para los sitios que compilan desde el código fuente; en una distribución moderna, generalmente puede omitirlos por completo instalando el paquete `libopenr2-dev` y una compilación empaquetada de Asterisk 22, ya que `chan_dahdi` compila la compatibilidad con R2 directamente contra libopenr2 sin ningún parche externo.
...
Paso 2: Clone la biblioteca openr2 y el código fuente de Asterisk. No se necesita ningún árbol parcheado especial en Asterisk 22: una descarga estándar compila la compatibilidad con R2 siempre que libopenr2 esté presente.
...
**Estado en Asterisk 22.** `chan_iax2` todavía está incluido y es totalmente compatible en Asterisk 22 LTS, por lo que todo en esta sección sigue siendo válido. IAX2 es, sin embargo, un protocolo heredado que ve relativamente poca implementación nueva: la industria ha convergido en gran medida en SIP (a través de `chan_pjsip` en Asterisk 22) tanto para la troncalización de proveedores como para la interconexión de servidores. La principal ventaja restante de IAX2 es su diseño de puerto único: toda la señalización y los medios fluyen sobre un solo puerto UDP (4569 por defecto), lo que simplifica la configuración del firewall y NAT en comparación con SIP más sus flujos RTP separados. Para una nueva troncal de Asterisk a Asterisk donde NAT no es una preocupación, una troncal PJSIP es el enfoque moderno recomendado; IAX2 se cubre aquí porque sigue siendo una opción válida, especialmente donde solo se puede abrir un puerto UDP a través de un firewall.
...
IAX es un protocolo de señalización y medios punto a punto que es similar a SIP sin usar RTP. El enfoque básico es multiplexar los flujos multimedia sobre una sola conexión UDP entre dos hosts. El mayor beneficio de este enfoque es su simplicidad al atravesar conexiones sobre NAT, que se encuentra regularmente en módems xDSL. IAX utiliza un solo puerto, UDP 4569 por defecto, y luego utiliza un número de llamada con 15 bits para multiplexar todos los flujos. El protocolo IAX utiliza procesos de registro y autenticación similares al protocolo SIP. Se puede encontrar una descripción del protocolo en http://www.ietf.org/internet-drafts/draft-guy-iax-05.txt

![El protocolo IAX multiplexa muchas llamadas entre dos endpoints sobre un solo puerto UDP (4569 por defecto), utilizando un número de llamada de 15 bits para mantener los flujos separados, lo que hace que el cruce de NAT sea simple.](../images/10-legacy-fig12.png)
...
Algunos proveedores de VoIP admiten IAX. Puede encontrar fácilmente un proveedor de IAX buscando “IAX providers”. Usar un proveedor de IAX tiene mucho sentido ya que IAX puede ahorrar mucho ancho de banda, atraviesa fácilmente NAT y puede autenticarse usando pares de claves RSA.
...
La cantidad de proveedores de VoIP comerciales compatibles con IAX ha disminuido drásticamente en las últimas versiones de Asterisk; la mayoría de los proveedores ahora ofrecen troncales SIP/PJSIP exclusivamente. Antes de comprometerse con un proveedor de IAX, confirme que mantienen activamente su infraestructura IAX. Para una nueva integración de proveedor, una troncal PJSIP (Capítulo 3) es la alternativa recomendada.
...
Un jitter pequeño (es decir, por debajo de 20 ms) suele ser imperceptible. Sin embargo, un jitter por encima de este nivel es molesto. La latencia o retardo debe mantenerse por debajo de 150ms. Crear un jitter buffer sacrificará algo de retardo por un menor jitter, un concepto conocido como “presupuesto de retardo”. Puede afectar el jitter buffer usando estos parámetros:
...
> **Heredado / histórico:** Todo en esta sección utiliza el antiguo controlador `chan_sip`
> y su archivo de configuración `sip.conf`. `chan_sip` estuvo en desuso durante
> varias versiones y **se eliminó en Asterisk 21**, por lo que **no existe en
> Asterisk 22**. Ninguno de los ejemplos de `sip.conf` a continuación se ejecutará en un sistema
> actual; se mantienen aquí solo para documentar cómo funcionaban las implementaciones heredadas
> y para ayudarle a migrarlas. Para la forma moderna y compatible de hacer cualquiera de estas cosas, consulte
> la sección *PJSIP: el canal SIP* del capítulo *SIP & PJSIP en profundidad*. La
> teoría del *protocolo* SIP (métodos, registro, proxy/redirect, SDP, tipos de NAT)
> es a nivel de protocolo y vive en ese capítulo; lo que sigue es puramente la **configuración**
> de `chan_sip` eliminada.

En los sistemas heredados hasta Asterisk 20, SIP se configuraba en `/etc/asterisk/sip.conf`, que solía ser el segundo archivo más cambiado (justo después de `extensions.conf`). Las secciones a continuación muestran cómo `chan_sip` conectaba Asterisk a un proveedor SIP, cómo conectar dos Asterisk juntos usando SIP, soporte de dominio, presencia, opciones de códec/DTMF/QoS, autenticación y NAT, seguido de una guía para migrar todo a PJSIP.
...
Paso 2: Configurar el [peer] en sip.conf Cree una entrada de tipo peer para el proveedor deseado para simplificar el marcado de Asterisk.
...
El parámetro “username” se utiliza en el resumen de autenticación (digest). El resumen se calcula utilizando nombre de usuario, secreto y reino (realm):
...
Este parámetro habilita las búsquedas DNS SRV en llamadas salientes. Usando este parámetro, es posible marcar llamadas usando nombres SIP basados en dominio.
...
Esta sección explica algunos parámetros avanzados del canal SIP heredado, como presencia, selección de códec, opciones DTMF y marcado de paquetes QoS. Los **conceptos** (BLF/presencia, negociación de códec, modos DTMF, marcado DSCP) se trasladan a PJSIP, pero los nombres de parámetros `sip.conf` que se muestran aquí **no** existen en Asterisk 22. En PJSIP, el modo DTMF es `dtmf_mode=` en un endpoint, y los códecs se establecen con `allow=`/`disallow=`.
...
- subscribecontext=sip_subscribers: Contexto donde buscar sugerencias (hints)
...
Step 2: Ahora configure el soft-phone para usar presencia. Le mostraremos cómo configurar el SipPulse Softphone.
...
Step 3: Añada el contacto a otros soft-phones. En este ejemplo, el SipPulse Softphone es la cuenta 2000, así que añadiremos un contacto para la cuenta 2001. Secuencia: Abra el panel derecho (panel de presencia en el softphone)->Haga clic en Contacts->Add a contact. Rellene el nombre 2001. Muestre como 2001 y no olvide marcar la casilla Show this contact’s availability.
...
Este parámetro controla si un usuario sin un peer correspondiente puede autenticarse sin un nombre y secreto. Discutimos este parámetro en la sección de soporte de dominio.
...
Es necesario mantener abierta la asignación NAT. Si NAT agota el tiempo de espera, Asterisk no puede enviar una invitación al UAC. El UAC puede enviar llamadas, pero no recibir ninguna. La siguiente declaración se puede usar para mantener abierta la NAT.

```
qualify=yes
```

Qualify enviará un paquete SIP usando el método OPTIONS regularmente, lo que ayudará a mantener abierta la NAT. Qualify envía un OPTIONS cada 60 segundos y cada 10 segundos cuando el host no está alcanzable. Puede usar “sip show peers” para ver la latencia de los peers. Si la NAT del usuario es del tipo simétrico, no es posible enviar paquetes de un UAC a otro directamente; en ese caso, tiene que forzar el RTP a través de Asterisk usando:
...
| `host=dynamic` (dispositivo registra) | `type=aor` con `max_contacts=1`; el dispositivo REGISTERs para actualizar su contacto |
...
Asterisk incluye un script de ayuda, **`sip_to_pjsip.py`**, que lee un `sip.conf` existente y produce un `pjsip.conf`. Puede ejecutarlo directamente en el directorio /etc/asterisk. La utilidad se encuentra en el árbol de código fuente de Asterisk bajo `contrib/scripts/sip_to_pjsip/`, donde `${PATH_TO_ASTERISK_SOURCE}` es la ruta donde se encuentran los archivos fuente de Asterisk (generalmente /usr/src/asterisk-22.x.y/):
...
Trate su salida como un **punto de partida**: revise cada objeto generado, especialmente los transportes, la configuración de NAT y las listas de códecs, y pruebe minuciosamente antes de pasar a producción.
...
2. ¿La señalización de supervisión en una línea analógica incluye cuál de las siguientes (elija todas las que correspondan)?
   ...
   - C. Ringing
   - D. DTMF
3. El eco, los chasquidos y el ruido en una tarjeta analógica DAHDI son causados con mayor frecuencia por:
   - A. La forma en que se compiló Asterisk
   - B. Conflictos de interrupción PCI
   - C. Un códec SIP incorrecto
   - D. Un dialplan faltante
4. Para una facturación precisa en canales analógicos, debe detectar exactamente cuándo responde el extremo lejano. ¿Qué función activa en Asterisk (y solicita a la TELCO) para hacer esto?
...
5. El hardware DAHDI es independiente de Asterisk: la tarjeta física se configura en `/etc/dahdi/system.conf`, mientras que `chan_dahdi.conf` define los canales de Asterisk, no el hardware en sí.
   - A. Verdadero
   - B. Falso
6. Con respecto a la capacidad y señalización de la troncal digital, marque las afirmaciones correctas (elija todas las que correspondan):

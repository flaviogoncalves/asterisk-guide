...
En la nueva arquitectura de Real Time, todo el código específico de la base de datos se trasladó a los controladores de canal. El canal solo llama a una rutina genérica que busca en la base de datos. El resultado es un proceso mucho más simple y limpio desde el punto de vista del código fuente. Se accede a la base de datos mediante tres funciones:
...
En Asterisk 22, los endpoints SIP son manejados por la pila **PJSIP** (`res_pjsip`), la cual está construida sobre el modelo de objetos **Sorcery**. Con el asistente `realtime`, Sorcery carga cada objeto PJSIP desde la base de datos bajo demanda, y esos objetos existen entonces como objetos PJSIP configurados ordinarios, no como los peers de realtime desechables que el antiguo controlador SIP descartaba después de cada llamada. Debido a que son objetos reales, el cruce de NAT, qualify y la indicación de mensaje en espera (MWI) funcionan normalmente para los endpoints de realtime. (A Sorcery se le puede indicar adicionalmente que almacene objetos en caché en la memoria mediante un asistente `memory_cache`, pero eso es opcional y separado de la carga en tiempo real). Cuando cambia un objeto en la base de datos, el cambio se detecta en la siguiente búsqueda; no necesita recargar después de cada edición. (El modelo de realtime retirado `chan_sip`, con sus familias `sippeers`/`sipusers`, se cubre solo en el capítulo de *Legacy Channels*).

## Configuración de Asterisk Real Time

Para este laboratorio, asumiremos que ya tiene instalado ODBC desde el capítulo de CDR. ARA se configura en el archivo de texto extconfig.conf, donde se pueden ver fácilmente dos secciones. La primera es la sección de archivos de configuración estática, donde puede sustituir los archivos de configuración de texto por tablas de base de datos. La segunda sección es el motor de configuración de realtime, donde configura las tablas de la base de datos para objetos dinámicos (peers/usuarios). No es inusual usar archivos de texto para la configuración estática y la base de datos para entradas dinámicas. En este caso, la primera sección permanece intacta.
...
![Arquitectura de Asterisk Real Time: los archivos de configuración y las tablas de base de datos estáticas se cargan cuando Asterisk inicia, mientras que las tablas de base de datos de realtime proporcionan una configuración dinámica que se lee bajo demanda durante una llamada.](../images/18-realtime-fig01.png)
...
### Sección de configuración de Real Time

La configuración de tiempo real (segunda parte del archivo extconfig.conf) es donde se configura, actualiza y descarga la pieza de configuración que se cargará en tiempo real. Con el tiempo real, no es necesario recargar las configuraciones. La sintaxis de tiempo real sigue este formato:
...
| Tipo de objeto Sorcery | Tabla de Realtime | Qué contiene |
|---------------------|----------------|---------------|
...
El realtime para PJSIP se habilita en dos lugares. Primero, mapee los tipos de objeto Sorcery a realtime en `extconfig.conf`:
...
Segundo, dígale a Sorcery que use el asistente `realtime` para esos tipos de objeto en `sorcery.conf`. El nombre del mapeo (aquí `res_pjsip`) es el módulo cuyos objetos está reubicando, y el valor de la derecha apunta a la familia que definió en `extconfig.conf`:
...
Puede mezclar objetos estáticos y de realtime. Si omite un tipo de `sorcery.conf`, ese tipo de objeto seguirá leyendo desde `pjsip.conf`. Un patrón común es mantener los transportes estáticos y la configuración global en `pjsip.conf` mientras se almacenan los endpoints, aors, auths y contacts en la base de datos.

### Creación del esquema de realtime PJSIP con Alembic
...
Ahora que hemos configurado el archivo extconfig.conf, creemos las tablas. En términos generales, cada columna de la base de datos coincide con un nombre de opción del archivo de configuración correspondiente. Las tablas PJSIP `ps_*` siguen esta regla: cada columna `ps_endpoints` lleva el nombre de una opción de endpoint `pjsip.conf`, cada columna `ps_auths` el nombre de una opción de auth, y así sucesivamente. Por ejemplo, el endpoint `pjsip.conf` a continuación,
...
Una tabla de voicemail sigue la misma idea. Sus columnas se mapean a los campos `voicemail.conf`:
...
También puede usar el sistema de tiempo real para crear el dial plan. ARA usa la sentencia `switch` para incluir las extensiones de tiempo real en el dial plan normal contenido en el archivo extensions.conf. La tabla de extensiones debería verse como la siguiente:

| context | exten | priority | app | appdata |
|---------|-------|----------|-----|---------|
| from-internal | 4000 | 1 | Dial | PJSIP/4000 |

La familia de realtime `extensions` no cambia en Asterisk 22; solo asegúrese de que la columna `appdata` marque canales PJSIP, por ejemplo `PJSIP/4000`. En el dial plan, tiene que usar el comando `switch` para usar el tiempo real.

![Construcción de un dial plan con Asterisk Real Time: extensions.conf usa una sentencia `switch => realtime` para extraer filas de extensiones (context, exten, priority, app, data) de una tabla de base de datos en lugar del archivo de texto.](../images/18-realtime-fig02.png)
...
En este laboratorio, prepararemos la base de datos para recibir parámetros de Asterisk. Prepararemos solo las tablas de REALTIME. La configuración estática se dejará en los archivos de configuración de texto (genial, ¿verdad?). La creación de tablas en MySQL sigue a continuación.

Paso 1: Entre a la base de datos MySQL como root.
...
Paso 3: Cree las tablas necesarias. Los archivos de esquema estático heredados todavía se incluyen bajo `contrib/realtime/` (por ejemplo, `/usr/src/asterisk-22.x/contrib/realtime/mysql`), pero en Asterisk 22 la forma recomendada y correcta según la versión para construir las tablas de realtime, especialmente las tablas PJSIP `ps_*`, son las migraciones de **Alembic** bajo `contrib/ast-db-manage` (vea la sección "Creación del esquema de realtime PJSIP con Alembic" arriba).
...
El conjunto de migraciones `config` de Alembic construye las tablas PJSIP `ps_*` (junto con `voicemail`, `extensions` y los otros esquemas de realtime) con exactamente las columnas que espera la versión de Asterisk en ejecución, por lo que el esquema siempre coincide con la compilación.
...
Debería ver las tablas PJSIP `ps_*` (creadas por la migración `config` de Alembic), junto con las tablas `voicemail`, `extensions` y otras tablas de realtime:
...
Paso 6: Inspeccione y rellene las tablas desde el cliente MySQL. No necesita una herramienta gráfica como phpMyAdmin; cada paso en este capítulo es SQL simple, copiable y ejecutable desde la línea de comandos `mysql`. Conéctese a la base de datos `astdb` (use `supersecret` cuando se le solicite):
...
Estas tablas fueron creadas por la migración `config` de Alembic, por lo que sus columnas ya coinciden con los nombres de las opciones `pjsip.conf` para la versión de Asterisk en ejecución; solo rellene las columnas que necesite.
...
Paso 2: Prueba de extensión de Real Time. Cree un nuevo endpoint `6010` insertando una fila en cada una de las tablas `ps_auths`, `ps_aors` y `ps_endpoints`, luego intente registrar este endpoint con un softphone. Ejecute el siguiente SQL en el cliente `mysql` (`mysql -u astdb -p astdb`):
...
Las tres filas juntas describen una cuenta SIP. Las configuraciones de cuenta restantes se distribuyen entre los objetos PJSIP: el context, los codecs, el modo DTMF y el manejo de medios residen en el endpoint (las últimas seis columnas arriba); el registro dinámico reside en el AOR. No hay un indicador "dinámico" separado; un AOR acepta REGISTERs dinámicos siempre que `max_contacts` sea mayor que cero, y cada ubicación registrada se escribe en `ps_contacts`.

En PJSIP, el modo DTMF fuera de banda RFC 2833 / RFC 4733 se llama `rfc4733`, y `dtmf_mode=rfc4733` es el valor predeterminado, por lo que la columna `dtmf_mode` arriba es opcional y se muestra solo para mayor claridad.
...
Paso 5: Incluya Asterisk Real Time en el dial plan. En el context `default`:
...
En este capítulo, ha aprendido que Asterisk Real Time le permite poner sus configuraciones en una base de datos. Asterisk incluye controladores de realtime nativos para ODBC (que llega a cualquier base de datos compatible con UnixODBC, incluyendo MySQL/MariaDB y SQLite), MySQL y PostgreSQL, además de un controlador de realtime LDAP para backends de directorio. La configuración se divide en estática y de tiempo real. La configuración estática reemplaza los archivos de configuración, mientras que la configuración de tiempo real crea objetos dinámicos que se cargan solo cuando ocurre una llamada u otro evento relacionado. Concluimos con un laboratorio práctico sobre cómo instalar y configurar ARA.
...
4. En la configuración estática, una vez que los objetos se cargan desde la base de datos, se mantienen en la memoria de Asterisk y solo se actualizan al iniciar o recargar.
   - A. Verdadero
   - B. Falso
5. PJSIP realtime (Sorcery) admite completamente `qualify` y MWI para endpoints de realtime, porque Sorcery los carga como objetos PJSIP configurados ordinarios en lugar de descartarlos después de cada llamada como lo hacían los antiguos peers de realtime SIP.
   - A. Verdadero
   - B. Falso
...
9. La base de datos debe crearse con cada campo que exista en el archivo de configuración.
   - A. Verdadero
   - B. Falso
10. En Asterisk 22, ¿cuál es la forma recomendada y correcta según la versión para crear las tablas de realtime PJSIP (`ps_endpoints`, `ps_aors`, `ps_auths`, `ps_contacts`)?
    ...
    - C. Ejecutar las migraciones `config` de Alembic bajo `contrib/ast-db-manage` (`alembic -c config.ini upgrade head`)

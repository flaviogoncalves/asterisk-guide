...
Voice over IP está creciendo rápidamente en el mercado de la telefonía. El paradigma de la convergencia está cambiando la forma en que nos comunicamos, reduciendo costos y mejorando la manera en que intercambiamos información. La voz es solo el comienzo de una era de comunicación multimedia completa, que incluye voz, video y presencia. En el futuro, no transportaremos a las personas al trabajo, sino el trabajo a las personas, porque es más limpio, rápido y económico. VoIP es solo una parte de esta revolución. Nuestro desafío en este capítulo es diseñar una red VoIP. Para hacerlo, tendremos que comprender conceptos como protocolos de sesión y codecs, así como la forma de dimensionar el número de circuitos y el ancho de banda.
...
La telefonía es mucho más antigua que la informática. Las PBX de telefonía se basan en conmutación de circuitos y, por lo general, no se dispone de más que una computadora para la supervisión. Con VoIP, la telefonía se crea desde cero basándose en estándares informáticos. Esto hace que el uso de aplicaciones de Telefonía e Informática sea más barato y fácil que en el modelo antiguo. Puede crear rápidamente una larga lista de aplicaciones de telefonía basadas en Asterisk. Puede desarrollar IVRs, ACDs, CTI, marcadores, ventanas emergentes en pantalla y otras aplicaciones en una fracción del tiempo requerido para las PBX tradicionales.
...
Los mecanismos de QoS como DiffServ operan en la capa IP para priorizar los paquetes de voz y mejorar la calidad de la llamada. Algunos detalles de los protocolos:

- **SIP** utiliza UDP o TCP en el puerto 5060 (TLS en 5061) para transportar la señalización. El audio se transporta por separado mediante RTP sobre un rango de puertos UDP configurable (la muestra `rtp.conf` incluida en Asterisk usa de 10000 a 20000), codificado con un codec como G.711.
...
- **IAX2** es inusual: multiplexa tanto la señalización como los medios sobre un único puerto UDP (4569), lo que simplifica el cruce de NAT y firewalls.

> **[Nota de la 2ª ed.]** La figura de la 1ª edición aquí mapeaba SIP/H.323 en la capa de *sesión* de OSI y los codecs en la capa de *presentación*. Ese mapeo fue una fuente de controversia de larga data — el IETF utiliza el modelo TCP/IP, en el cual SIP es un protocolo de capa de aplicación — por lo que la figura ha sido reemplazada por la tabla anterior. Encargue una figura redibujada basada en la pila TCP/IP si se desea un visual.
...
SIP es un estándar abierto del Internet Engineering Task Force (IETF), definido en gran medida en RFC 3261. La mayoría de los proveedores de VoIP modernos utilizan SIP; de hecho, se está convirtiendo en el estándar VoIP más popular. La fortaleza de SIP es que es un estándar basado en el IETF. SIP es ligero en comparación con el antiguo H.323. La principal debilidad de SIP es el cruce de NAT, un desafío para la mayoría de los proveedores de VoIP SIP. El IETF no creó SIP pensando en la facturación, sino para comunicaciones abiertas entre pares. La facturación suele ser una preocupación para los proveedores de VoIP.

### IAX – Inter Asterisk eXchange

IAX es un protocolo abierto desarrollado originalmente por Digium (ahora Sangoma). IAX es un protocolo todo en uno, ya que transporta la señalización y los medios a través del mismo puerto UDP (4569). Mark Spencer desarrolló IAX como un protocolo binario para reducir el ancho de banda. La principal fortaleza de IAX es su uso reducido de ancho de banda (no utiliza RTP); también es muy fácil para el cruce de NAT y firewalls ya que utiliza solo un puerto UDP (4569). Si un fabricante de PBX tradicional hubiera creado IAX, probablemente habría comercializado el protocolo como "lo mejor desde el helado"; en algunas situaciones, IAX en modo trunk puede reducir el uso de ancho de banda de voz en un tercio. IAX2 (versión 2) todavía se incluye en Asterisk 22 a través del módulo `chan_iax2` y sigue siendo útil para trunks entre Asterisk, aunque se considera heredado; SIP/PJSIP es preferible para nuevas implementaciones. IAX2 se especifica en [RFC 5456](https://www.rfc-editor.org/rfc/rfc5456) (Informativo).

### MGCP – Media Gateway Control Protocol

MGCP es un protocolo utilizado junto con H.323, SIP e IAX. Su mayor ventaja es la escalabilidad. Se configura en el agente de llamadas en lugar de en los gateways. Esto simplifica el proceso de configuración y permite la gestión centralizada. Sin embargo, la implementación en Asterisk no está completa y parece que no mucha gente lo usa.
...
| Protocolo | Organismo de estandarización | Uso |
|----------|---------------|----------|
| SIP | Estándar IETF | Teléfonos SIP; conexión a proveedores de servicios SIP |
| IAX2 | RFC 5456 (Informativo) | Trunks entre Asterisk; teléfonos IAX2; proveedores de servicios IAX |
| H.323 | Estándar ITU | Teléfonos y gateways H.323 (puede usar un gatekeeper externo, no puede ser uno) |
| MGCP | IETF/ITU | Teléfonos MGCP (sin soporte de proveedor/gateway) |
| SCCP | Propietario de Cisco | Teléfonos Cisco |

> **[Nota de la 2ª ed.]** Considere actualizar esta tabla para reflejar los nombres de los módulos actuales de Asterisk 22 y el estado del soporte.

## Peers, Users y Friends

Existen tres tipos de clientes SIP e IAX. El primero es "user". Los usuarios pueden realizar llamadas a un servidor Asterisk, pero no pueden conectarse para recibir llamadas desde este servidor. El segundo es un "peer". Puede realizar llamadas a un peer, pero no recibirá llamadas de ellos. Por lo general, un servidor o un dispositivo requerirá ambos conceptos al mismo tiempo. Un "friend" es un atajo para "user" + "peer". Un teléfono probablemente caería en esta categoría, ya que es necesario para realizar y recibir llamadas.

![Peer, user y friend desde el punto de vista de Asterisk: un "user" realiza llamadas a Asterisk, un "peer" recibe llamadas de Asterisk y un "friend" hace ambas cosas.](../images/06-voip-network-fig03.png)

> **[Nota de la 2ª ed.]** La distinción peer/user/friend es un concepto de `chan_sip` (`sip.conf`). En PJSIP (`pjsip.conf`), esto es reemplazado por el objeto **endpoint**, que maneja tanto llamadas entrantes como salientes. La separación peer/user ya no se aplica en Asterisk 22.

## Codecs y traducción de codecs

Utilizará un codec para convertir la voz de una onda analógica a una señal digital. Los codecs difieren entre sí en aspectos como la calidad del sonido, la tasa de compresión, el ancho de banda y los requisitos informáticos. Los servicios, teléfonos y gateways suelen admitir varios de estos aspectos. El codec G.729 es muy popular. No es parte de la compilación estándar de Asterisk 22; en cambio, se envía como un módulo adicional externo (`codec_g729`) que descarga desde Digium (ahora Sangoma). La fuente `menuselect` de Asterisk lo enumera con `support_level=external` y señala claramente: "Descargue el codec g729a de Digium. Se debe comprar una licencia para este codec". En otras palabras, el uso legal de G.729 requiere una licencia comprada por canal. (También existe una alternativa de código abierto, `bcg729`).
...
- GSM: 13 Kbps
- iLBC: 13.3 Kbps
- ITU G.711 (ulaw/alaw): 64 Kbps — calidad PSTN estándar; ulaw común en Norteamérica, alaw común en Europa y Latinoamérica
...
|---|---|---|---|---|
| Intervalo de muestreo | 20 ms | 30 ms | 30 ms | RTE/LTP |
| Ancho de banda (Kbps) | 64 | 8 | 13.33 | 13 |
| Costo (por canal) | Gratis | ~USD 10.00 | Gratis | Gratis |
| Resistencia al borrado de tramas¹ | Ninguna | 3% | 5% | 3% |
| Complejidad (MIPS)² | ~0.35 | ~13 | ~18 | ~5 |

¹ La resistencia a la pérdida de paquetes se refiere a la tasa a la que el MOS cae a aproximadamente 0.5 por debajo de la calidad máxima para el codec específico.

² La complejidad se refiere a la cantidad, en millones de instrucciones por segundo, gastada para codificar y decodificar el codec usando un diseño de referencia en un DSP de Texas Instruments (TMS320C54x). Existe una relación directa entre la frecuencia del procesador y los MIPS, pero no es posible establecer una relación precisa entre plataformas de hardware tan diversas. Use esta tabla solo para comparación.

> **[Nota de la 2ª ed.]** Reconstruya esta tabla de comparación para la 2ª edición: agregue Opus (gratis `codec_opus`) y G.722 (banda ancha, 64 Kbps); mantenga G.711 ulaw/alaw como la línea base PSTN. Para G.729, tenga en cuenta la realidad de la licencia: el `codec_g729` de Sangoma es gratuito para descargar pero requiere una licencia por canal; el `bcg729` de código abierto es una alternativa.
...
- **G.722:** Buena opción para banda ancha (voz HD) en LAN/extensiones internas; mismo ancho de banda que G.711 con mejor calidad.
- **Opus:** Recomendado para endpoints modernos, clientes WebRTC y cualquier implementación donde el endpoint lo admita. Tasa de bits adaptativa, excelente resistencia a la pérdida de paquetes, disponible gratuitamente a través del módulo binario `codec_opus` de Sangoma.

## Sobrecarga causada por los encabezados de protocolo

A pesar de que los codecs hacen poco uso del ancho de banda, tenemos que considerar la sobrecarga causada por los encabezados de protocolo como Ethernet, IP, UDP y RTP. Como tal, podríamos decir que el ancho de banda depende de los encabezados utilizados. Si estamos en una red Ethernet, el requisito de ancho de banda es mayor que en una red PPP porque el encabezado PPP es más corto que el de Ethernet. Veamos algunos ejemplos: Destino Ethernet G.729 codificado (20) Encabezado UDP (8) Tipo Ethernet (2) Origen Ethernet Encabezado IP (20) Encabezado RTP (12) Carga útil de voz Suma de comprobación (4) Dirección (6) Dirección (6) Ethernet Codec g.711 (64 Kbps)
...
Un problema principal en el diseño de redes VoIP es dimensionar el número de líneas y el ancho de banda requerido para un destino específico, como una oficina remota o un proveedor de servicios. También es importante dimensionar el número de llamadas simultáneas de Asterisk (parámetro principal para el dimensionamiento de Asterisk).
...
![Topología de red de ejemplo (misma ciudad): la sede con 120 extensiones se conecta a la PSTN a través de líneas T1, y a la sucursal #1 (30 extensiones) y a la sucursal #2 (15 extensiones) a través de una nube Frame-Relay.](../images/06-voip-network-fig06.png)
...
- Número total de líneas = 33 o aproximadamente 2x líneas T1
...
Este método es muy común cuando se utilizan gateways VoIP a través de Internet. Al usar una carga útil más grande, sacrificará la latencia en favor de un ancho de banda reducido. Puede cambiar la paquetización RTP agregando el tamaño de la trama al codec en la instrucción allow.

![Aumento de la carga útil de voz: empaquetar 60 bytes de carga útil g.729 en un paquete (en lugar de 20) amortiza los 58 bytes de encabezados en más voz, reduciendo el ancho de banda a unos 16.05 Kbps por llamada a costa de una mayor latencia.](../images/06-voip-network-fig09.png)
...
2. La convergencia es la integración de voz, datos y video en una sola red; su beneficio principal es la reducción de costos en la implementación y mantenimiento de redes separadas.
   - A. Falso
   - B. Verdadero
3. Asterisk trata cada protocolo VoIP como un canal y puede puentear cualquier tipo de canal con cualquier otro, transcodificando entre codecs cuando sea necesario.
   - A. Falso
   - B. Verdadero
...
6. SIP es el protocolo más adoptado para teléfonos IP y es un estándar abierto definido en gran medida por el IETF en RFC 3261.
   - A. Falso
   - B. Verdadero
7. IAX2 transporta tanto la señalización como los medios sobre un único puerto UDP, lo que lo hace eficiente y fácil de atravesar NAT. ¿Qué puerto UDP utiliza IAX2?
   - A. 5060
   - B. 1720
   - C. 4569
   - D. 5061
8. IAX fue desarrollado originalmente por Digium (ahora Sangoma). A pesar de la adopción limitada por parte de los proveedores de teléfonos, IAX es excelente cuando necesita (marque todos los que correspondan):
   ...
   - D. Modo trunk para combinar muchas llamadas entre Asterisk y amortizar la sobrecarga de encabezado
9. En el modelo heredado chan_sip, un "user" puede recibir llamadas de Asterisk.
   - A. Falso
   - B. Verdadero
10. Con respecto a los codecs en Asterisk 22, marque todas las afirmaciones verdaderas:
    ...
    - D. G.711 u-law es común en Norteamérica, mientras que a-law es común en Europa y Latinoamérica.
    - E. G.729 es ligero y utiliza muy pocos recursos de CPU para codificar y decodificar en comparación con G.711.

**Respuestas:** 1 — A, B, C, D · 2 — B · 3 — B · 4 — B · 5 — B (Aplicación — SIP es un protocolo de capa de aplicación en el modelo TCP/IP que utiliza el IETF) · 6 — B · 7 — C · 8 — A, C, D · 9 — A · 10 — A, B, C, D

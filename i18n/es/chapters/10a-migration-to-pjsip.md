...
- Ejecutar el script de conversión `sip_to_pjsip.py` y revisar su salida de forma crítica
- Traducir los tipos de dispositivos comunes (teléfono que se registra, trunk entrante, registro saliente) de `sip.conf` a `pjsip.conf` manualmente
...
La distinción entre `friend`/`peer`/`user` desaparece por completo — en PJSIP todo es un `endpoint`. Por lo tanto, un único friend `sip.conf` se convierte, típicamente, en tres objetos (`endpoint` + `auth` + `aor`) que comparten un nombre y se apuntan entre sí:
...
Asterisk incluye un script de Python que lee un `sip.conf` existente y escribe un `pjsip.conf`. No se ejecuta como un comando CLI — vive en el **árbol de fuentes de Asterisk**, no en los binarios instalados:
...
Observe en ese fragmento real que `qualify = yes` de un peer `sip.conf` terminó en el bloque *no mapeado* — debido a que PJSIP califica en el **aor** con `qualify_frequency` (segundos), no como un booleano en el dispositivo, el script lo deja para que usted lo configure deliberadamente. Las limitaciones prácticas a planificar:
...
El flujo de trabajo es, por lo tanto: ejecutar el script en un archivo *temporal*, comparar y revisar, integrar las partes buenas en su `pjsip.conf` real, y luego probar exhaustivamente antes de pasar a producción.

## Traducciones lado a lado

Estas son las recetas. `sip.conf` a la izquierda, el equivalente verificado de `pjsip.conf` a la derecha (apilado aquí por el ancho de la página). Cada nombre y valor de opción a la derecha ha sido verificado contra el laboratorio de Asterisk 22 con `config show help res_pjsip ...`.
...
La migración no se detiene en `pjsip.conf`. Dos cosas en el uso diario cambian.
...
La división conceptual es la misma que en el caso de archivos planos: una fila `sippeers` se convierte en *tres* filas en tres tablas (`ps_endpoints` + `ps_aors` + `ps_auths`) que se hacen referencia entre sí por el nombre del endpoint.
...
- **`alwaysauthreject` está integrado ahora — no lo busque.** `chan_sip` necesitaba `alwaysauthreject=yes` para no filtrar qué extensiones existían respondiendo de manera diferente a nombres de usuario incorrectos. PJSIP hace lo seguro por diseño: nunca revela si un endpoint existe. No hay ninguna opción `alwaysauthreject` que establecer. La protección relacionada —limitar a los remitentes no identificados— es el `unidentified_request_count` / `unidentified_request_period` global, activado por defecto.

- **`insecure=invite` no es una opción de PJSIP — use `identify`.** No hay `insecure=` en `pjsip.conf`. La forma de aceptar INVITEs no autenticados de un operador conocido es *identificar el endpoint por IP de origen* con `type=identify` / `match=`. Haga coincidir lo más estrechamente posible (IPs de host específicas, no CIDRs amplios) y respalde esto con un `type=acl` — un trunk con coincidencia de IP sin autenticación es un objetivo para el fraude telefónico.

- **Un transporte por IP/puerto.** No puede vincular dos transportes a la misma IP:puerto, y no puede vincular múltiples transportes TCP o TLS de la misma versión de IP. El script de conversión puede emitir un transporte que colisione con uno que ya tiene — consolide en una única capa de transporte diseñada deliberadamente.
...
1. ¿Por qué un despliegue de Asterisk 22 debe usar PJSIP para SIP?
   ...
   - C. PJSIP es el predeterminado pero `chan_sip` puede cargarse con `modules.conf`
...
4. El script de conversión `sip_to_pjsip.py` es:
   ...
   - B. Un script de Python en el árbol de fuentes de Asterisk bajo `contrib/scripts/sip_to_pjsip/`
   - C. Un módulo compilado cargado en el arranque
...
8. En Asterisk 22, un objeto `auth` debe usar qué `auth_type`, y cuál es el estado de `userpass`?
...
10. En una migración de realtime, la tabla `chan_sip` `sippeers` es reemplazada por qué tablas PJSIP/Sorcery?

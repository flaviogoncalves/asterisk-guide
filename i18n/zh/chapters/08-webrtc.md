...
WebRTC (Web Real-Time Communication) 让 Web 浏览器无需插件和外部 softphone 即可拨打和接听电话——只需要 JavaScript、麦克风以及与 Asterisk 的安全连接。自 Asterisk 11 起，Asterisk 就能够充当 WebRTC 服务器，而自 Asterisk 12 引入 PJSIP 栈（`res_pjsip`）以来，这已成为推荐的实现方式；在 Asterisk 22 中，配置已简化为少数几个易于理解的选项。本章将展示如何将 PJSIP endpoint 转换为浏览器电话，安全媒体路径的工作原理，以及何时应该选择 Asterisk 内置的 WebRTC 支持，何时应该选择专用网关。

本章中的所有内容均已针对本书的 Asterisk 22 实验环境进行了验证；所展示的配置与 `lab/asterisk/etc` 中的相同。
...
- 描述 WebRTC 媒体安全 (DTLS-SRTP) 和 ICE 与普通 SIP 的区别
...
从 Asterisk 的角度来看，WebRTC endpoint 只是另一个 PJSIP endpoint。不同之处在于浏览器如何连接到它以及如何保护媒体安全。典型用途包括：

- 网站上的 **Click-to-call**（点击通话）——访客从网页拨打队列或 extension。
- **基于 Web 的座席**——联络中心座席完全在浏览器中工作，无需安装或更新桌面 softphone。
- 在您自己的 Web 应用程序中 **嵌入通话功能**——例如，SipPulse Web softphone 与 Asterisk 通信。
...
普通的 SIP 电话通过 UDP/TCP 进行信令传输，通常以普通 RTP 承载音频。WebRTC 浏览器客户端在三个重要方面有所不同，Asterisk 必须匹配每一个方面：

- **信令通过 WebSocket 传输。** 浏览器不是通过 UDP 端口 5060 进行 SIP 通信，而是向 Asterisk 的内置 HTTP 服务器打开一个安全 WebSocket (`wss://`)。SIP 消息在 WebSocket 内部传输。
- **媒体始终使用 DTLS-SRTP 加密。** 浏览器拒绝普通 RTP。双方执行 DTLS 握手（通过 SDP 中交换的证书指纹进行身份验证），并从中导出 SRTP 密钥。
- **连接通过 ICE 协商。** 双方不是假设一个可达的 IP 和端口，而是收集候选地址（主机地址、STUN 反射地址、TURN 中继地址）并进行探测，直到找到一个可用的地址。RTP 和 RTCP 通常在单个端口上进行多路复用 (`rtcp_mux`)。
...
此处的 TLS 证书用于保护 *WebSocket*（信令通道）。在实验环境中，自签名证书即可——您只需在浏览器中接受一次。在生产环境中，请使用真实的证书（例如 Let's Encrypt），其名称必须与浏览器连接的主机名匹配，否则浏览器将拒绝该 WebSocket。

对于实验，`lab/make-certs.sh` 使用 `CN=localhost`（加上 `localhost`/`127.0.0.1` SAN）生成自签名证书，并将其写入 `asterisk/etc/keys/`。对于公共部署，请获取真实证书——例如使用 Let's Encrypt：

```
certbot certonly --standalone -d voip.example.com
```

然后将 `http.conf` 指向颁发的文件并重新加载 `res_http_websocket`：
...
确保证书名称与浏览器连接的主机名匹配，并在其过期前续订（certbot 的定时任务会自动执行此操作），否则 WebSocket 将失败。

此证书 **不是** 用于加密媒体的 DTLS 证书——Asterisk 会自动生成该证书，我们稍后会看到。

## 第 2 步 —— WSS 传输

PJSIP 需要一个类型为 `wss` 的传输。将其添加到 `pjsip.conf`：
...
不要被 `wss` 传输显示的 `0.0.0.0:5060` 所误导——WebSocket **不是** 在端口 5060 上提供的。WebRTC 信令由您在第 1 步中配置的 HTTP 服务器提供（`wss` 为端口 8089）。PJSIP `wss` 传输是 `res_http_websocket` 之上的一个薄层，因此为其打印的 `bind` 地址仅供参考，可以忽略；真正重要的是 `http.conf` 中的 `tlsbindaddr` 端口。
...
- `media_encryption: dtls` 和 `dtls_auto_generate_cert: Yes`——媒体是 DTLS-SRTP，且 Asterisk 会自动生成 DTLS 证书，因此您 **不需要** 自己创建。指纹在 SDP (`SHA-256`) 中通告。
...
建议使用 `allow=opus`——Opus 是浏览器首选的 codec。Asterisk 22 在核心中内置了 Opus *透传*（`res_format_attr_opus` 模块），这足以在两个支持 Opus 的端点之间中继 Opus 而无需重新编码。将 Opus *转码* 为其他 codec 需要单独的 `codec_opus` 模块，官方 WebRTC 指南将其列为可选但强烈推荐，您可以在基础构建之上安装它；请参阅 *Designing a VoIP network* 中的 codec 讨论。保留 `ulaw` 作为桥接到无法使用 Opus 的非 WebRTC 端点的后备方案。

## 第 4 步 —— ICE、STUN 和 TURN

在扁平的局域网中，带有主机候选地址的 ICE 就足够了，不需要其他任何东西。在互联网上，您通常需要添加一个 STUN 服务器，以便 Asterisk 和浏览器可以发现它们的公共地址；如果直接媒体通信是不可能的（对称 NAT、限制性防火墙），则需要一个 TURN 服务器。在 `rtp.conf` 中将 Asterisk 指向它们：
...
`turnaddr` 接受一个可选端口（默认 `3478`）；`turnusername` 和 `turnpassword` 用于中继身份验证。STUN 仅帮助对等方 *发现* 其公共地址——当两端都位于对称 NAT 或限制性防火墙之后时，直接媒体通信是不可能的，TURN 中继是使音频流动的唯一途径。

**生产建议：** 公共 STUN 服务器（例如 Google 的）对于地址发现是可以的，但 **不要** 依赖公共 TURN 进行实际流量传输——TURN 会中继您的所有媒体，因此您需要将其置于自己的控制之下。运行您自己的 [coturn](https://github.com/coturn/coturn) 服务器。一个带有长期凭据的最小化 `/etc/turnserver.conf` 如下所示：
...
在浏览器的 `iceServers` 列表中为浏览器提供相同的 TURN 服务器，以便两端都可以进行中继。对于用户位于移动网络或企业防火墙之后的生产环境，自托管的 coturn 实际上是强制性的。
...
浏览器注册后，`pjsip show contacts` 会显示动态联系人，通话会点亮通道：
...
- **使用专用网关（例如 Janus）**：当您需要独立于呼叫控制来扩展大量浏览器会话、为大型会议/流媒体进行选择性转发，或保持媒体平面与 PBX 分离时。网关将 WebRTC 桥接到普通 SIP，然后 Asterisk 看到的是一个普通的 SIP 端。

许多真实系统结合了两者：Asterisk 用于呼叫控制，网关用于浏览器端的媒体扩展。（这就是 SipPulse 自身栈背后的架构。）
...
- **单向音频：** 通常是一侧的 NAT/ICE 问题，或者 codec 没有共同匹配项——确保 `allow=opus,ulaw`。
- **应答时通话掉线：** DTLS 握手失败；确认 `dtls_auto_generate_cert` 为 `Yes` 且系统时钟正确（证书对时间敏感）。
...
WebRTC 将浏览器变成了 Asterisk 的一流 endpoint。配方虽小但很严格：启用带有 TLS 的 HTTP 服务器以便浏览器可以打开安全 WebSocket，添加一个 `wss` PJSIP 传输，并在 endpoint 上设置 `webrtc=yes`——这将开启 DTLS-SRTP（使用自动生成的证书）、ICE、RTP/RTCP 多路复用和 AVPF 配置文件。跨越 NAT 时添加 STUN/TURN，通过 HTTPS 提供您的页面，并将 SIP.js（或 JsSIP）客户端指向 `wss://asterisk:8089/ws`。对于 PBX 上的浏览器电话，Asterisk 原生 WebRTC 是最简单的路径；对于大规模媒体，请将其与网关配对使用。

## 测验

1. WebRTC 浏览器客户端使用哪种传输方式将 SIP 信令传输到 Asterisk？
   - A. 端口 5060 上的普通 UDP
...
6. 填空：WebRTC 通过让双方使用 ________ 框架收集并探测候选地址（主机地址、STUN 反射地址、TURN 中继地址）来协商连接。

7. 在 `rtp.conf` 中，哪两个设置将 Asterisk 指向外部服务器，以便它能够发现其公共地址并在直接路径失败时中继媒体？（多选）
   - A. `icesupport=yes`
   - B. `stunaddr=`
   - C. `turnaddr=`
   - D. `tlsbindaddr=`
...
9. 根据本章，何时应该选择专用媒体网关（例如 Janus）而不是 Asterisk 原生 WebRTC？
   - A. 任何浏览器需要拨打电话时
   - B. 当您需要独立于呼叫控制来扩展大量浏览器媒体会话、为大型会议进行选择性转发，或保持媒体平面与 PBX 分离时
   - C. 仅当浏览器不支持 DTLS 时

...
众所周知，Asterisk 的配置是通过 /etc/asterisk 目录下的多个文本文件来实现的。尽管使用文本文件很方便，但仍存在一些已知的缺点：
...
ARA（即 Asterisk Realtime）由 Anthony Minessale II、Mark Spencer 和 Constantine Filin 创建，旨在实现与 SQL 数据库的透明集成。此外也提供 LDAP 接口。该系统也被称为 Asterisk 外部配置，并在 /etc/asterisk/extconfig.conf 中进行配置。您可以将配置文件映射到数据库中的表（静态配置），也可以使用实时条目来动态创建对象，而无需重新加载 Asterisk。
...
在 Asterisk 22 上，SIP endpoint 由 **PJSIP** 栈（`res_pjsip`）处理，该栈构建在 **Sorcery** 对象模型之上。通过 `realtime` 向导，Sorcery 按需从数据库加载每个 PJSIP 对象，这些对象随后作为普通的已配置 PJSIP 对象存在——而不是像旧的 SIP 驱动程序在每次呼叫后丢弃的那些一次性实时对等体。因为它们是真实的对象，所以 NAT 遍历、qualify 和消息等待指示（MWI）对于实时 endpoint 都能正常工作。（还可以通过 `memory_cache` 向导指示 Sorcery 将对象缓存在内存中，但这属于可选配置，与实时加载是分开的。）当您更改数据库中的对象时，更改会在下一次查找时被获取；您不需要在每次编辑后都进行重新加载。（已弃用的 `chan_sip` 实时模型及其 `sippeers`/`sipusers` 系列仅在 *Legacy Channels* 一章中介绍。）
...
静态配置部分是您在数据库中存储等效配置文件的地方。这些配置在 Asterisk 加载期间被读取。某些模块在您重新加载时会重新读取数据库。静态配置的示例如下：
...
静态文件映射对于那些没有按对象实时等效项的配置文件最有用。对于 PJSIP，请优先使用本章后面描述的按对象实时系列（`ps_endpoints`、`ps_aors`等），而不是将整个 `pjsip.conf` 映射为静态文件。

上面描述了三个示例。在第一个示例中，您将 queues.conf 绑定到 asteriskdb 数据库中的 queues 表。在第二个示例中，您将 pjsip.conf 绑定到 odbc 配置中定义的 asteriskdb 数据库中的 pjsip_conf 表。在最后一个示例中，您将 iax.conf 绑定到 LDAP 目录。MyBaseDN 是要搜索的基准 DN。在前面的示例中，当 MySQL 驱动程序查询数据库并获取所需信息时，应用程序 app_queue.so 会被加载。
...
这里我们有五行配置。在第一行中，您将 PJSIP/Sorcery 系列 `ps_endpoints` 绑定到 asteriskdb 数据库中的 `ps_endpoints` 表。在最后一行中，您将 voicemail 系列绑定到 asteriskdb 数据库中的 test 表。每个 PJSIP 对象类型（endpoint、aor、auth、contact）都有其自己的系列和表；完整集合显示在下方的“PJSIP Realtime (Sorcery)”部分。`voicemail`、`extensions`、`queues` 和 `queue_members` 系列在 Asterisk 22 中仍然有效。

## PJSIP Realtime (Sorcery)

在 Asterisk 22 上，SIP endpoint 由 **PJSIP** 栈（`res_pjsip`）专门处理，该栈构建在 **Sorcery** 对象抽象层之上。PJSIP 不再使用单一的 SIP “peer”，而是将 SIP 账户拆分为多种对象类型，每种类型存储在自己的实时表中：

| Sorcery 对象类型 | 实时表 | 存储内容 |
|---------------------|----------------|---------------|
| endpoint | ps_endpoints | 账户设置（context、codecs、DTMF 等） |
| aor (address of record) | ps_aors | 注册限制和 `qualify` 设置 |
| auth | ps_auths | `username` / `password` 凭据 |
| contact | ps_contacts | 动态注册的位置 |
| domain alias | ps_domain_aliases | endpoint 的备用 SIP 域 |
...
其次，在 `sorcery.conf` 中告知 Sorcery 对这些对象类型使用 `realtime` 向导。映射名称（此处为 `res_pjsip`）是您要重定位其对象的模块，右侧的值指向您在 `extconfig.conf` 中定义的系列：
...
Asterisk 在 `contrib/ast-db-manage` 下提供了所有实时模式的数据库迁移文件。这是创建（和版本升级）PJSIP 表的受支持方式——您不再需要手动编写 `ps_*` 表定义。`config` 迁移集包含 PJSIP/Sorcery 表。
...
这将创建 `ps_endpoints`、`ps_aors`、`ps_auths`、`ps_contacts` 以及其他具有适用于当前运行的 Asterisk 版本正确列的 PJSIP 表。（Alembic 需要 Python 的 `alembic` 包以及 SQLAlchemy 驱动程序，例如用于 MySQL/MariaDB 的 `pymysql` 或用于 PostgreSQL 的 `psycopg2`。）
...
现在我们已经配置了 extconfig.conf 文件，让我们创建表。总的来说，每个数据库列都与相应配置文件中的选项名称相匹配。PJSIP `ps_*` 表遵循此规则：每个 `ps_endpoints` 列都以 `pjsip.conf` endpoint 选项命名，每个 `ps_auths` 列以 auth 选项命名，依此类推。例如，下方的 `pjsip.conf` endpoint，
...
存储为跨三个表的一行。`ps_endpoints` 行保存 `id=4000, context=from-internal, disallow=all, allow=ulaw, auth=4000, aors=4000`；`ps_auths` 行保存 `id=4000, auth_type=userpass, username=4000, password=supersecret`；`ps_aors` 行保存 `id=4000, max_contacts=1`。您只需要填充您实际使用的列——任何留空的列都会回退到选项的默认值。例如，如果您想要 endpoint 上的 `callerid` 参数，请填写 `ps_endpoints` 的 `callerid` 列（列名与 `pjsip.conf` 选项名称相同）。
...
在本实验室中，我们将准备数据库以接收 Asterisk 参数。我们将仅准备 REALTIME 表。静态配置将保留在配置文件文本文件中（很酷，不是吗？）。MySQL 中的表创建如下。

第 1 步：以 root 身份进入 MySQL 数据库。

```
mysql –u root –p
```

第 2 步：登录到在 CDR 实验室中创建的 MySQL 服务器。
...
第 3 步：创建必要的表。旧版静态模式文件仍随 `contrib/realtime/` 提供（例如 `/usr/src/asterisk-22.x/contrib/realtime/mysql`），但在 Asterisk 22 上，构建实时表（尤其是 PJSIP `ps_*` 表）的推荐且版本正确的方法是使用 `contrib/ast-db-manage` 下的 **Alembic** 迁移（请参阅上方的“使用 Alembic 创建 PJSIP 实时模式”部分）。
...
Alembic `config` 迁移集构建 PJSIP `ps_*` 表（以及 `voicemail`、`extensions` 和其他实时模式），其列与当前运行的 Asterisk 版本完全匹配，因此模式始终与构建版本一致。
...
（Alembic 创建的表比这些多——上面的列表显示了与本实验室相关的表。）

第 5 步：数据库已经为 ODBC 配置好（自 CDR 实验室以来），因此这里不需要进一步的 ODBC 设置。

第 6 步：安装 phpMyAdmin 以处理数据库任务

```
apt-get install phpmyadmin
```

下面是该工具登录屏幕和表屏幕的两个截图。使用 astdb/supersecret 作为名称和密码。

> **[2nd-ed note]** 替换为当前的数据库管理截图（phpMyAdmin/Adminer），或将这些步骤转换为纯 SQL（CREATE TABLE/INSERT）。

## 实验室：配置和测试 ARA

在本实验室中，我们将更改 extconfig.conf 配置以反映我们的数据库配置和表。
...
第 2 步：实时 extension 测试。使用 phpMyAdmin，通过在 `ps_auths`、`ps_aors` 和 `ps_endpoints` 的每一个中插入一行来创建一个新的 `6010` endpoint，然后尝试使用软电话注册此 endpoint。
...
其余的账户设置分布在 PJSIP 对象中。Context、codecs、DTMF 模式和媒体处理位于 endpoint 上；动态注册位于 AOR 上：
...
在 PJSIP 中，RFC 2833 / RFC 4733 DTMF 模式命名为 `rfc4733`（`dtmf_mode=rfc4733`，默认值）。没有单独的“动态”注册标志：只要 AOR 允许至少一个 contact（`max_contacts` 大于零），它就会接受动态 REGISTER，并且每个注册的位置都会写入 `ps_contacts`。
...
当被询问时使用 supersecret。使用 phpMyAdmin 在数据库中包含一个 extension。如果您愿意，也可以在 MySQL 客户端界面中使用以下命令。
...
第 6 步：如果尚未完成，请将其中一部手机重新配置为用户名 `bria`。

第 7 步：从现有手机拨打 6007；`bria` 手机应该会响铃。

## 总结

在本章中，您已经了解到 Asterisk Real Time 允许您将配置放入数据库中。Asterisk 为 ODBC（支持任何 UnixODBC 支持的数据库，包括 MySQL/MariaDB 和 SQLite）、MySQL 和 PostgreSQL 提供了原生的实时驱动程序，此外还为目录后端提供了 LDAP 实时驱动程序。配置分为静态和实时。静态配置替换了配置文件，而实时配置创建了仅在呼叫或其他相关事件发生时才加载的动态对象。我们最后通过一个关于如何安装和配置 ARA 的实践实验室结束了本章。
...
3. `extconfig.conf` 文件配置了 Realtime 使用的表。它有两个不同的部分（勾选两个）：
   ...
   - C. 出局路由
   - D. IP 地址和数据库端口
4. 在静态配置中，对象从数据库加载后，它们会保留在 Asterisk 的内存中，仅在启动或重新加载时刷新。
   - A. True
   - B. False
...
9. 数据库必须使用配置文件中存在的每个字段来创建。
   - A. True
   - B. False
10. 在 Asterisk 22 上，创建 PJSIP 实时表（`ps_endpoints`、`ps_aors`、`ps_auths`、`ps_contacts`）的推荐且版本正确的方法是什么？
    - A. 手动编写每个 `ps_*` 表的 `CREATE TABLE` 语句
    - B. 从 `contrib/realtime/` 导入旧版 `mysql_config.sql`
    - C. 运行 `contrib/ast-db-manage`（`alembic -c config.ini upgrade head`）下的 Alembic `config` 迁移

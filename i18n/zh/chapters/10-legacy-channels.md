...
在 2026 年的纯 VoIP 世界中，本章介绍的通道类型已日益罕见：大多数新部署都是基于以太网的 SIP trunk 和 PJSIP endpoint，完全不涉及任何电话硬件。尽管如此，Asterisk 22 仍然全面支持其中的大多数。模拟 (FXO/FXS) 和数字 TDM (E1/T1/ISDN PRI/BRI) 连接通过 DAHDI 提供——这是由 Digium 最初开发的驱动程序栈，Digium 在 2018 年被 Sangoma 收购，此前早期的 Zaptel 驱动程序在商标纠纷后被更名。通过 IAX2 进行的服务器间连接由 `chan_iax2` 提供，它仍然在发布和支持中，但现在已明确属于传统协议。本章还收集了 **传统 SIP** 的资料：旧的 `chan_sip` 驱动程序及其 `sip.conf` 配置——已在 Asterisk 21 中移除，并在 Asterisk 22 中消失——以及一份将现有 `sip.conf` 系统迁移到 PJSIP 的完整指南。如果您正在运行一个基于 PJSIP 的纯 SIP 环境，且没有电话卡、没有 IAX2 trunk，也没有需要转换的传统 `sip.conf`，则可以安全地跳过本章。

## 模拟通道 (FXO/FXS)

> **[第二版注]** 在出版前更新第二版的前言日期/ISBN。

> **[第二版注 — 部署背景]** 截至 Asterisk 22，DAHDI 和模拟电话卡仍然得到全面支持，且 DAHDI 仍可在当前内核上构建。然而，大多数新部署都是纯 VoIP（SIP trunk，PJSIP）。模拟/TDM 硬件现在是一种小众选择，主要存在于传统环境、农村 PSTN 连接或受监管的市场中。以下内容对于这些场景仍然准确。

连接公共交换电话网 (PSTN) 有多种方式。最佳方式取决于电话公司在您所在地区提供连接的方式。最简单的方法是使用模拟线路，类似于您在家中使用的线路。在本节中，我们将向您展示如何配置来自 Sangoma™（前身为 Digium™）和 Xorcom™ 的模拟卡。
...
主要的监控信令包括挂机 (on-hook)、摘机 (off-hook) 和振铃 (ringing)。挂机 (On-Hook) – 当用户将电话放回挂钩上时，PBX 会中断并禁止电流通过。在这种状态下，电路被称为挂机。在此位置，只有振铃器处于活动状态。摘机 (Off-Hook) – 在开始通话之前，电话需要进入摘机状态。从挂钩上取下听筒会闭合回路，并向 PBX 指示用户打算拨打电话。收到此指示后，PBX 会产生拨号音，向用户指示它已准备好接收目标地址（即电话号码）。振铃 (Ringing) – 当用户呼叫另一部电话时，它会向振铃器产生电压，提醒对方有来电。信令因国家而异，不同国家有不同的音调。您可以通过修改 indications.conf 文件来根据您所在的国家个性化 Asterisk 音调。例如：
...
与旧式 PBX 的情况一样，通常需要将 Asterisk PBX 连接到 PSTN。在这里，我们将向您展示如何做到这一点。通常您有三种电话线路选项。

- 模拟：家庭和小型企业最常见的形式，通常由一对金属铜线提供。
- 数字：在需要多条线路时使用。数字线路通常由 CSU/DSU 或光纤多路复用器提供。最终用户连接器通常是 RJ45。在一些国家，E1 线路使用两个同轴 BNC 连接器提供；在这种情况下，您将需要一个平衡-不平衡转换器 (balun) 来将 RJ45 插孔连接到电话板。
- SIP：此选项是最近开发的。电话线路使用带有 SIP 信令的数据连接 (VoIP) 提供。这是一个与 Asterisk 一起使用的好选择，因为您不需要购买电话卡。电话呼叫将直接传送到以太网端口。另一个优点是，您可以通过避免编解码器转码来释放 CPU 资源。

### 模拟 FXS、FXO 和 E&M 接口

有多种类型的模拟接口可用。理解这些接口之间的差异对于学习如何连接到电话网络以及其他 PBX 至关重要。在这里，我们将向您展示 E&M 接口。虽然它目前不适用于 Asterisk，并且已被多家供应商停产，但您可能会发现带有这种接口的路由器和 PBX，因此最好了解您正在处理的内容。

#### 外汇 (FX) 接口

FX 接口是模拟的。术语“外汇 (Foreign eXchange)”应用于访问 PSTN 中心局 (CO) 的中继线。外汇局 (FXO)

![Asterisk 位于模拟电话 (FXS) 和电信线路 (FXO) 之间：FXS 侧为电话提供拨号音和振铃，而 FXO 侧从中心局获取拨号音。](../images/10-legacy-fig01.png)

FXO 接口用于连接到中心局 (CO) 或其他 PBX 的分机。它直接与来自 PSTN 的电话线通信。另一个选项是将 FXO 接口连接到现有的 PBX，允许 Asterisk 和传统 PBX 之间的通信。将 Asterisk 连接到 PBX 端口并使用 VoIP 提供远程分机通常被称为异地分机 (OPX)。FXO 接口接收拨号音。外汇站 (FXS) FXS 接口为模拟电话、调制解调器或传真机供电。FXS 为电话提供拨号音和电源。
...
在 Asterisk 中使用 kewlstart 信令几乎是默认设置。Kewlstart 本身不是信令，但通过监控另一侧发生的情况增加了电路的智能性。Kewlstart 基于环路启动。大多数交换机不支持此功能，该功能用于获取挂断通知。

- 环路启动 (Loopstart)：用于大多数模拟线路，它允许电话指示“挂机”和“摘机”，并允许交换机指示“振铃”和“无振铃”。这可能是大多数人家里有的。该名称源于线路始终处于打开状态的事实。当您闭合回路时，交换机会为您提供拨号音。来电通过打开的线对上的 100V 振铃电压发出信号。

![Asterisk 作为 VoIP 网关运行：FXO 端口连接到传统 PBX 分机，而远程 Asterisk 通过 FXS 端口通过 IP 将该线路传送到模拟电话（异地分机，或 OPX）。](../images/10-legacy-fig02.png)

- 接地启动 (Groundstart)：类似于环路启动。当您想拨打电话时，线路的一侧会短路。当交换机识别出此状态时，它会通过打开的线对反转电压，然后闭合回路。因此，线路在提供给呼叫者之前首先被占用。
- 挂机监控 (Kewlstart)：增加了电路的智能性，允许监控另一侧。Kewlstart 结合了环路启动的许多优点。
...
在为 Asterisk 选择硬件之前，您应该考虑将要安装和启用的并发呼叫数、服务和编解码器。Asterisk 是一个 CPU 密集型应用程序，这就是为什么我们建议为 Asterisk 使用专用机器。安装在计算机内的接口卡数量受可用插槽和中断数量的限制。安装一张带有 8 个语音接口的卡比安装两张带有 4 个接口的卡更可取。另一个选择是使用 USB 通道库，例如 Xorcom Astribank。最近，一些制造商（例如 CIANET）已经开始生产 TDMoE 通道库，使得连接数十个模拟接口变得更加容易。
...
在执行 `dahdi_genconf` 之前，配置 `genconf_parameters` 文件（通常称为 `gen_parameters.conf`）非常重要：

![Sangoma/Digium TDM404P 模拟卡：最多四个 FXS 或 FXO 模块插入编号端口，带有可选的硬件回声消除子卡和用于 FXS 模块的专用 12V 电源连接器。](../images/10-legacy-fig04.png)
...
警告：您必须至少为通道配置回声消除算法。base_exten 参数定义了 FXS 分机的基本拨号方案。在这种情况下，第一个 FXS 通道将接收分机号 4000，第二个接收 4001，依此类推。创建线路 (context_lines) 和中继 (context_phones) 的上下文非常重要。生成文件后，您应该在 `/etc/asterisk/chan_dahdi.conf` 文件中包含 `/etc/asterisk/dahdi-channels.conf` 文件：
...
DAHDI 有一个不寻常的驱动程序加载系统。首先配置 /etc/dahdi/system.conf，然后使用 dahdi_cfg 将这些配置应用到 DAHDI 驱动程序。在这种情况下，dahdi_cfg 用于为 FX 接口配置信令。要查看结果，您可以将“-vvvvv”附加到命令以获取详细信息。
...
这听起来很奇怪，但在配置 /etc/dahdi/system.conf 后，您已经配置了卡本身。DAHDI 可用于其他目的，如路由和 SS7。要将其与 Asterisk 一起使用，您必须配置 Asterisk DAHDI 通道。Asterisk 中的每个通道都必须定义；SIP/PJSIP 通道在 pjsip.conf 中定义（注意：chan_sip 和 sip.conf 已在 Asterisk 21 中移除），而 TDM 通道在 chan_dahdi.conf 中定义。这创建了要在您的拨号方案中使用的逻辑 TDM 通道。
...
chan_dahdi.conf 文件中有多个选项可用。描述所有选项会很枯燥且适得其反；相反，我们将专注于主要选项组，以便于理解。
...
group：允许将通道作为组处理。如果您拨打组号而不是通道号，则使用第一个可用的通道。如果通道是电话，当您呼叫组时，所有电话将同时振铃。使用逗号，您可以为同一通道指定多个组。
...
有许多 callerid 选项。有些可以禁用，尽管大多数默认启用。usecallerid：启用或禁用后续通道的 callerid 传输（Yes/No）。注意：如果您的系统在应答前收到两次振铃，请尝试禁用此功能。它应该立即应答。hidecallerid：定义是否隐藏传出的 callerid（Yes/No）。callerid：为特定通道配置 callerid 字符串。呼叫者可以配置为 asreceived。这主要用于中继接口以指示传入的 callerid。
...
调整回声消除既是技术活，也是艺术活。这些选项调整影响 DAHDI 通道中音频质量的某些 Asterisk 参数。它们可以帮助提高模拟接口中的音频质量。
...
大多数回声消除算法通过生成接收信号的多个副本来操作，其中每个副本都延迟特定的时间量。滤波器的抽头 (taps) 数量决定了需要消除的回声延迟大小。然后调整这些延迟的副本并从接收信号中减去。诀窍是仅调整延迟信号以消除回声，而不使用过多的 CPU 周期。从用户的角度来看，选择合适的回声消除算法很重要。默认是 MG2；但是，还有另外两个选项：来自 Sangoma（前身为 Digium）的高性能回声消除 (HPEC) 和由 David Rowe 开发的开源回声消除 (OSLEC)。

> **[第二版注]** OSLEC 项目页面 (http://www.rowetel.com/ucasterisk/oslec.html) 可能不再是最新的；在引用它之前，请验证现代内核的可用性和内核集成状态。要更改回声消除算法，请更改 /etc/dahdi/system.conf 中的参数 echo_can。例如：
...
这些项目用于获取有关呼叫进度的信息。在公共接口中，检测呼叫进度并确定它是已应答还是忙碌可能很有用。忙音检测是高度实验性的，并受特定参数调节。
...
这些参数（上文）指定接口是否将尝试检测忙音、成功检测需要多少个音调以及忙音模式是什么。忙音检测在很大程度上是实验性的，可以在 Makefile 中更改一些附加参数。要检测呼叫的应答（这对于精确计费至关重要），可以使用极性反转来发出精确的应答时间信号。如果您计划对呼叫收费或只是希望进行精确的计费比较，这一点很重要。通常您必须联系电话公司请求此服务。

```
answeronpolarityswitch=yes
```

在一些国家，也可以使用极性反转来检测呼叫挂断。
...
这些选项用于连接到 FXS 接口的电话。直接连接到 DAHDI 接口的模拟电话所提供的所有功能均由 Asterisk 控制。Adsi (模拟显示服务接口)：这是一组电信标准，被一些电信公司用于提供购票等服务。cancallforward：启用或禁用呼叫转移（*72 启用，*73 禁用）。calleridcallwaiting：启用在呼叫等待指示期间接收的 callerid（Yes/No）。immediate：在立即模式下，通道不会提供拨号音，而是立即跳转到定义上下文中的“s”分机。这用于创建热线。threewaycalling：启用或禁用三方会议。mailbox：警告用户有可用的语音信箱消息。它可以是声音信号或视觉指示器（如果电话支持此功能）。参数是邮箱号码。callgroup：对电话进行分组以拨打或接听。pickupgroup：用于呼叫接听的电话组。
...
> **[第二版注]** 截至 Asterisk 22，DAHDI 和 libpri 仍然得到全面支持，但 TDM 数字中继 (E1/T1/ISDN PRI) 在新部署中正日益被 SIP 中继取代。本章在需要 TDM 连接的地方仍然完全适用；绿地环境的读者可能更喜欢 SIP 中继（第 3 章）以获得类似的通道密度。

数字通道非常普遍，因此如果您想专注于大型客户，则需要学习如何实现这些通道。当通道数量较多——通常超过 8 个——时，使用 T1/E1/J1 等数字接口相当普遍。T1 在美国非常普遍，而 E1 在欧洲普遍，J1 在日本普遍。这些类型的通道允许良好的电路密度——每个 T1 通道 24 个，E1 通道 30 个。在拉丁美洲、中国和非洲，通常使用一种称为 MFC/R2 的随路信令 (CAS) 通道类型。本章将研究如何使用库 OpenR2 实现 MFC/R2。在美国和欧洲，综合业务数字网 (ISDN) PRI 是最常见的信令。本章还将讨论 ISDN 基群速率接口 (BRI)，它在欧洲的中端应用中非常普遍。本书中的所有示例都集中在 DAHDI 通道上。有些卡是使用专有通道实现的，因此请与您的制造商联系，以获取有关如何配置特定卡的更多详细信息。
...
每当您需要实现大量通道时，数字线路 E1/T1 都是一种选择。单个 E1 电路能够进行 30 次并发呼叫，并且您可以拥有直接拨入 (DID)、来电显示 (caller identification) 和高级信令等功能。E1/T1 线路可能通过双绞线、光纤和微波以多种方式到达您的公司，具体取决于您所在的国家。数字线路使用 UTP、光纤或微波传送到您的公司。调制解调器和多路复用器 (MUX) 用于传送物理线路。与 T1 线路的连接始终基于 RJ45 连接器。但是，E1 线路也可以使用 BNC 提供。提前了解您将要接收的连接器类型非常重要，主要是在 E1 线路中。通常，直到 RJ45 的所有设备都由电信公司提供。

![E1/T1 电路的配置方式：电信公司可以通过 UTP 铜线（用于 E1 的 HDSL 调制解调器，或用于 T1 的直接卡连接）、通过光多路复用器的光纤或通过微波无线电链路提供中继。](../images/10-legacy-fig05.png)

![UTP 还是 BNC？大多数数字卡使用 RJ45 (UTP) 连接器，但一些 E1 线路是在双 BNC 同轴电缆上提供的，在这种情况下，需要一个平衡-不平衡转换器来使同轴线对适应卡的 RJ45 插孔。](../images/10-legacy-fig06.png)
...
当您只需要几个通道时，模拟线路是有意义的。当使用时分复用 (TDM) 时，可以将多个通道填充到单个数据连接中。当您需要大量电路时，电话公司通常会为您提供数字中继，这是一个数据电路，其中语音以数字格式使用 PCM 传输。每个时隙使用 64 Kbps 的带宽来传输单个语音通道。

![E1 和 T1 中的时分复用：E1 帧以 2048 Kbps 承载 32 个时隙（DS0 #0 用于帧同步，DS0 #16 用于信令），而 T1 帧以 1544 Kbps 承载 24 个时隙，使用一位用于同步，并使用一种窃取位方案进行信令。](../images/10-legacy-fig08.png)

在美国，最常见的数字中继是 T1，它有 24 个可用线路；在欧洲和拉丁美洲，E1 中继有 30 个线路。一些公司提供通道较少的部分 T1/E1。窃取位信令有时 T1 中继使用窃取位方案，其中一位被借用于信令。在 T1 中继上，数据/语音通道在每个时隙上以 56 Kbps 传输。正如您所观察到的，当您使用窃取位时，T1 电路不会因为同步和信令而丢失两个时隙。

#### T1/E1 线路代码

T1 和 E1 实际上是数据电路，并且具有确定比特解释方式的数据编码。对于 E1，最常见的线路代码是第 1 层的 HDB3 和第 2 层的 CCS。了解数字中继配置方式的最简单方法是向电信公司询问此信息。您将需要此信息来配置文件 /etc/dahdi/system.conf。
...
- 基群速率接口 (PRI) o 使用 T1/E1 中继提供 o T1 为 23B+D o E1 为 30B+D

有时，E1 电路使用称为 MFC/R2 的 CAS 信令方案，该方案由 ITU 定义为称为 Q.421/Q441 的标准。这在拉丁美洲和亚洲很常见。这些国家的几家电话公司使用 MFC/R2 的定制变体。因此，您需要了解正确的国家变体才能使其工作。

### ISDN BRI

使用 ISDN BRI 信令的通道在欧洲非常流行。大多数用于 Asterisk 的 ISDN BRI 卡支持带有 NT 和 TE 功能的 S/T 接口。TE（终端）连接用于连接到电信公司或配置为网络终端 (NT) 的其他 PBX。NT 用于连接配置为 TE 的电话和 PBX。ISDN BRI 提供两个数据/语音通道和一个信令通道。ISDN BRI 卡可从多家用于 Asterisk 的接口卡供应商处获得。
...
您的 PC 上有几种类型的总线。为您的服务器配备正确的卡非常重要。以下概述列出了最常用的卡：
...
> **[第二版注]** Sangoma 于 2018 年收购了 Digium。Digium 品牌的卡现在以 Sangoma 品牌销售和支持。请在 Sangoma 网站 (www.sangoma.com) 上验证当前型号的可用性，因为一些旧的 SKU 可能已停产。
...
硬件回声消除减少了主机 CPU 的负载。对于具有多个 E1 接口的卡，硬件回声消除可以帮助减轻处理器的负担。新的增强型软件回声消除器（如 OSLEC）正在减少对硬件回声消除器的需求。要在硬件和软件回声消除器之间进行选择，您应该考虑服务器中可用的处理能力和 E1 电路的数量。使用 OSLEC 时，回声消除过程每个语音通道最多可以使用 9 MIPS（每秒百万条指令），具有 128 个抽头幅度（参考：Xorcom Ltd.）。如果您考虑每条指令 1 个 CPU 周期（基于处理器和软件实现本身，这并不总是正确的），我们谈论的是四个 E1 的 1.080 Ghz。
...
实现 R2 对于安装名为 OpenR2 (www.libopenr2.org) 的库（由 Moises Silva 开发）以及在安装前修补 Asterisk 是必要的——这是一个在本章后面显示的简单过程。该库已经通过了多项测试，并在我们的几位客户中投入生产。在我看来，如果可用，ISDN 始终是最佳选择。一些提供商可以访问 7 号信令系统 (SS7)，这是电话公司之间可用的 CCS 信令。SS7 有专有和开源解决方案可用。库 libss7 用于支持 Asterisk 上的 SS7。
...
另一个选项是手动配置接口。下面是一些 DAHDI 通道配置示例。
...
第 1 步：TE205P 安装。在安装 TE205P 之前，了解 TE205P 和 TE210P 卡之间的区别非常重要。TE210P 卡使用由 3.3 伏供电的 64 位总线，几乎只存在于服务器主板中。如果您指定此接口卡，请务必小心；确保您的硬件支持 64 位、3.3V 总线。TE205P 卡使用 5V PCI，这在台式计算机中很常见。我们在此示例中选择了带有两个跨度的 TE205P 接口卡，因为它更容易将其减少为单跨度卡或将其扩展为四跨度卡。这些卡现在以 Sangoma 品牌（前身为 Digium）销售。
...
第 3 步：加载内核驱动程序使用 dahdi_hardware 检查您需要安装的驱动程序。
...
第 4 步：使用 dahdi_test，检查丢失的中断您可以使用随 DAHDI 卡编译的 dahdi_test 实用程序验证中断丢失的数量。低于 99.987% 的数字表示可能存在问题。您将在以下位置找到 dahdi_test
...
第 5 步：使用 dahdi_cfg 实用程序这是 dahdi_cfg 对于一个部分 E1 (15 端口) 跨度和两个 FXO 端口的正确输出。
...
对于点对多点 BRI，请使用 signaling=bri_cpe_ptmp。目前，NT 模式不支持 BRI 点对多点。
...
有时事情第一次无法正常工作。让我们检查一些用于故障排除 DAHDI 的资源。第 1 步：检查操作系统是否识别了该卡。Sangoma/Digium 卡通常被识别为 ISDN 调制解调器。
...
警报指示端口存在问题：红色警报：无法与远程交换机保持同步。这通常是一个物理问题，例如线路代码或帧不匹配。黄色警报：信号表明远程交换机处于红色警报状态。这表明远程交换机没有接收到您的传输。蓝色警报：在所有时隙上接收所有未成帧的 1；dahdi_tool 目前无法检测到蓝色警报。环回：端口处于本地或远程环回状态
...
debug pri span x：如果一切完成后仍然有问题，请开始调试 pri 跨度。此命令启用 ISDN 呼叫的详细调试。当您认为某些内容不正确时，这是一个重要的命令。您可以检测拨错的数字和其他问题。下面我们提供了一个成功呼叫的调试输出示例。如果您需要将不成功的呼叫与没有问题的呼叫进行比较，请参考此示例。一个提示是使用 core set verbose=0 仅接收 ISDN q.931 消息。
...
文件 chan_dahdi.conf 中有多个选项可用。描述所有选项会很枯燥且适得其反。在这里，我们将详细说明可用的主要选项组，以便更好地理解。
...
group：允许将通道视为一个组。如果您拨打组号而不是通道号，则使用第一个可用的通道。如果通道是电话，当您呼叫组时，所有电话将同时振铃。使用逗号，您可以为同一通道指定多个组。
...
pridialplan：某些需要拨号方案规范的交换机需要此项。此选项被许多交换机忽略。有效选项是 private、national、international 和 unknown。
...
overlapdial：当您在连接建立后传递数字时，使用重叠拨号。您可以使用块模式编号 (overlapdial=no) 或数字模式 (overlapdial=yes)。块模式通常由运营商使用。signaling：为后续通道配置信令类型。这些参数应与 chan_dahdi.conf 文件中的参数相对应。正确的选择基于可用的通道。对于 ISDN，您可以选择五个选项：

- pri_cpe：当设备是 CPE 时使用，有时称为客户端、用户或从属。这是最简单且最常用的信令形式。有时，当您尝试连接到专用 PBX 时，PBX 也通常被配置为 CPE。在这种情况下，请在 Asterisk 中使用 pri_net 信令。
...
有许多 Caller ID 选项可用。有些可以禁用，尽管大多数默认启用。usecallerid：启用或禁用后续通道的 Caller ID 传输（Yes/No）。注意：如果您的系统在应答前需要两次振铃，请尝试禁用此功能，以便它立即应答。hidecallerid：隐藏 Caller ID（Yes/No）。calleridcallwaiting：启用在呼叫等待指示期间接收 Caller ID（Yes/No）。callerid：为特定通道配置 Caller ID 字符串。呼叫者可以在中继接口中配置为“asreceived”以向前传递 Caller ID。

```
callerid = "Flavio Eduardo Gonçalves" <48 30258500>
```

注意：大多数电信公司要求您配置正确的来电显示。如果您没有传递正确的来电显示，您将无法通过电信公司拨出。另一方面，即使不配置来电显示，您也可以接听电话。

#### 音频质量选项

这些选项调整影响 DAHDI 通道中音频质量的某些 Asterisk 参数。echocancel：禁用或启用回声消除。您应该保持此功能启用。它接受“yes”或抽头数量。解释：回声消除是如何工作的？大多数回声消除算法通过生成接收信号的多个副本来操作，每个副本都延迟一个小间隔。这个小流被称为“抽头”。抽头数量决定了可以消除的回声延迟。这些副本被延迟、调整并从原始信号中减去。诀窍是将延迟信号精确调整到消除回声所需的程度。echocancelwhenbridged：在纯 TDM 呼叫期间启用或禁用回声消除器。这通常是不必要的。rxgain：调整音频接收增益以增加或减少接收音量（-100% 到 100%）。txgain：调整音频传输增益以增加或减少传输音量（-100% 到 100%）。示例：
...
MFC/R2 由 ITU 定义。不幸的是，几个国家根据自己的需要定制了该标准。结果，各国标准之间出现了差异。
...
以下序列说明了从 Asterisk 分机到 PSTN 中终端的呼叫。PSTN 挂断呼叫并结束通信。

![Asterisk 和电信公司之间的完整 MFC/R2 呼叫流：线路信令（空闲、占用、占用确认、应答、清除后向、清除前向）在时隙 16 中交换，拨打的数字和后向“发送下一个数字”信号（组 I/A/B）带内传输，可听音调到达用户。](../images/10-legacy-fig11.png)

### 如何使用驱动程序 libopenr2

由 Moises Silva 发起的项目受到了 Steve Underwood 编写的 Unicall 通道驱动程序的启发。OpenR2 库目前是 Asterisk 最稳定的软件解决方案。通过此解决方案，我们可以使用任何与 DAHDI 兼容的数字卡。以前，MFC/R2 只有专有解决方案可用，我使用过的最好的解决方案之一是 Khomp 提供的，www.khomp.com.br。在 Asterisk 22 中，当库在编译时存在时，通过 libopenR2 的 MFC/R2 支持是内置的——不需要外部补丁。以下步骤显示了历史手动安装以供参考；在现代系统上，在运行 `./configure` 之前从您的发行版包管理器安装 `libopenr2-dev`，然后在 `make menuselect` 中启用 `chan_dahdi`。

> **[第二版注]** 第一版中打补丁的 Asterisk 1.4 树已过时；对于 Asterisk 22，通过 libopenr2 的 MFC/R2 支持已集成到主源代码树中，以下步骤现在使用当前的 Git 存储库，而不是已停用的 `svn.digium.com`。考虑为最终版本精简这些历史构建步骤。

第 1 步：检查您要安装的 Asterisk 版本的补丁。

```
apt-get install git
```

第 2 步：下载安装了补丁的修改后的 Asterisk 代码。

> **[第二版注]** 原来的 Asterisk 1.4 SVN 补丁树已在下面被当前的 Git 存储库取代。在 Asterisk 22 上，MFC/R2 补丁是不必要的——`chan_dahdi` 直接针对 libopenr2 构建 R2 支持——因此您只需要 openr2 库加上正常的 Asterisk 构建。
...
第 5 步：运行命令 dahdi_cfg 将更改应用到驱动程序：
...
第 6 步：更改文件 extensions.conf 中的拨号方案
...
注意：一些电信公司不接受没有来电显示的呼叫。请将来电显示设置为运营商分配的 DID 号码之一。在一些国家，不需要此步骤。第 7 步：测试解决方案：现在，使用上下文 from-internal 中的分机，拨打任何号码并观察控制台。检查是否有任何错误发生。 -- Executing Set("SIP/8564-081ca5d8", "CALLERID(num)=1145678990") in new stack -- Executing Dial("SIP/8564-081ca5d8", "DAHDI/g1/35678899|60|tT") in new stack
...
第 2 步：重新启动 Asterisk 服务器第 3 步：测试呼叫并检查 /var/log/asterisk/mfcr2/span1 处的呼叫文件。下面是正常呼叫的跟踪。将其与您在呼叫中收到的内容进行比较。
...
mfcr2_mfback_timeout：此值值得一提。有时，如果您正在呼叫手机或任何需要很长时间才能完成的呼叫，此参数可能会超时，因此它通常被更改以进行微调。如果您的某些呼叫未完成，这是您首先应该更改的参数。mfcr2_metering_pulse_timeout：脉冲被一些 R2 变体用于指示成本 mfcr2_allow_collect_calls：在巴西，音调 II-8 用于指示对方付费呼叫；此参数允许您阻止对方付费呼叫。mfcr2_double_answer：也用于在需要双重应答时避免对方付费呼叫。使用 double_answer=yes，您实际上阻止了对方付费呼叫。mfcr2_immediate_accept：允许您跳过 B/II 组信号的使用并直接进入接受状态。mfcr2_forced_release：允许您加快呼叫释放；适用于巴西变体。

#### ANI 和 DNIS

自动号码识别 (ANI) 是呼叫者的号码。拨号号码识别服务 (DNIS) 是被呼叫的号码，或者换句话说，是拨打的号码。当收到呼叫时，通常最后四位数字会传递给 PBX，这个过程称为直接拨入 (DID)。ANI 号码实际上是来电显示。拨号时，ANI 将拥有呼叫者的分机，而 DNIS 将包含呼叫目的地。正确配置这些参数非常重要。一些交换机只发送最后四位数字，而另一些则发送完整号码。
...
在本章中，我们将了解 Inter-Asterisk eXchange (IAX) 协议，包括其优点和缺点。中继模式和两个 Asterisk 服务器的互连等细节也将被涵盖。本文档中的所有参考资料均对应 IAX 版本 2。IAX 协议为语音和视频提供媒体传输和信令。IAX 非常创新；它在中继模式下节省带宽，并且在您需要遍历 NAT 时比 SIP 简单得多。IAX 目前的主要用途是互连 Asterisk 服务器。IAX 最初是为语音创建的，但它也可以容纳视频和其他多媒体流。IAX 的灵感来自其他 VoIP 协议，如 SIP 和 MGCP。IAX 没有使用两个独立的协议进行信令和媒体传输，而是将它们统一起来，形成了一个独特的协议。IAX 不使用 RTP 进行媒体传输；相反，它将媒体嵌入到同一个 UDP 连接中。

> **[第二版注 — Asterisk 22 中的状态]** `chan_iax2` 在 Asterisk 22 LTS 中仍然包含并得到全面支持，因此本章中的所有内容仍然有效。然而，IAX2 现在是一个传统协议，几乎没有新的部署。VoIP 行业已在很大程度上收敛于 SIP（通过 Asterisk 22 中的 `chan_pjsip`），用于提供商中继和服务器互连。IAX2 剩下的主要卖点是其 **单端口 NAT 遍历**：所有信令和媒体流都通过单个 UDP 端口（默认 4569）流动，这与 SIP + RTP 相比，极大地简化了防火墙和 NAT 配置。如果您正在构建一个新的 Asterisk 到 Asterisk 中继且不担心 NAT，PJSIP 中继是推荐的现代方法。IAX2 保留在这里是因为它仍然是一个有效的选择，特别是在只能通过防火墙打开一个 UDP 端口的环境中。
...
IAX 是一种点对点信令和媒体协议，类似于不使用 RTP 的 SIP。基本方法是在两个主机之间的单个 UDP 连接上多路复用多媒体流。这种方法最大的好处是它在遍历 NAT（通常在 xDSL 调制解调器中发现）上的连接时的简单性。IAX 使用单个端口（默认 UDP 4569），然后使用带有 15 位的呼叫号码来多路复用所有流。IAX 协议使用类似于 SIP 协议的注册和认证过程。该协议的描述可以在 http://www.ietf.org/internet-drafts/draft-guy-iax-05.txt 找到

![IAX 协议通过单个 UDP 端口（默认 4569）多路复用两个端点之间的许多呼叫，使用 15 位呼叫号码来保持流分开——这使得 NAT 遍历变得简单。](../images/10-legacy-fig12.png)

### 带宽使用

VoIP 网络中使用的带宽受多种因素影响；编解码器和协议头是最重要的。IAX 协议有一个令人惊讶的功能，称为中继模式，通过该模式，它使用单个报头多路复用多个呼叫。通过使用 Asterisk 带宽计算器，您将看到 IAX 中继如何通过多个呼叫为您节省高达 80% 的流量。
...
了解通道命名约定很重要，因为当您在拨号方案中指定通道时，您将使用这些名称。用于出站通道的 IAX 通道名称格式为：

```
IAX/[<user>[:<secret>]@]<peer>[:<portno>][/<exten>[@<context>][/<options>]
```

<user> 远程对等体上的 UserID，或在 iax.conf 中配置的客户端名称 <secret> 密码。或者，它可以是 RSA 密钥的文件名，不带尾随扩展名（.key 或 .pub），并用方括号括起来 <peer> 要连接的服务器名称 <portno> 连接的端口号 <exten> 远程 Asterisk 服务器中的分机 <context> 远程 Asterisk 服务器中的上下文 <options> 可用的唯一选项是 ‘a’，意思是 ‘请求自动应答’

#### 出站通道示例：

出站通道在 Asterisk 控制台中可见。IAX2/8590:secret@myserver/8590@default 呼叫 myserver 中的 8590 分机。它使用 8590:secret 作为名称/密码对

IAX2/iaxphone 呼叫 "iaxphone" IAX2/judy:[judyrsa]@somewhere.com 使用 judy 作为用户名和 RSA 密钥进行认证来呼叫 somewhere.com
...
<username> 用户名（如果已知） <host> 连接的主机 <callno> 本地呼叫号码传入通道示例：IAX2[flavio@8.8.30.34]/10 来自 IP 地址 8.8.30.34 的呼叫号码 10，使用 flavio 作为用户。IAX2[8.8.30.50]/11 来自 IP 地址 8.8.30.50 的呼叫号码 11。
...
我试图保留示例文件的默认（非注释）行。修改了以下参数：

```
bandwidth=high
```

此行影响编解码器选择。使用 high 设置允许选择高带宽和高质量的编解码器，例如由 ulaw 关键字定义的 g.711。如果您保持默认参数，您将无法选择 ulaw。在这种情况下，Asterisk 将为您提供下述配置的“no codec available”消息。
...
在上述命令中，我们禁用了所有编解码器并仅启用了 ulaw。在局域网中，大多数人更喜欢使用 ulaw，因为它不是处理器密集型的，并且节省了 CPU 周期。即使使用更多带宽，此编解码器也是首选，因为在局域网中，您通常拥有 100 兆位以太网甚至千兆位。使用 ulaw 的语音呼叫从您的网络使用近 100 kbps 的带宽，这对当今的高速局域网来说是非常轻的使用。在广域网或互联网网络中，您通常会禁用 ulaw，用语音压缩换取一些可用的 CPU 周期，以实现更好的带宽使用。编解码器 gsm、g729 和 ilbc 也提供了良好的压缩系数。
...
在上述命令中，我们定义了一个名为 [2003] 的朋友。上下文是 default（在最初的实验室中，我们总是使用 default 上下文以避免混淆；此上下文将在第 9 章中全面解释）。“host=dynamic”行提供了电话 IP 地址的动态注册。第 3 步：下载并安装支持 IAX2 的软电话。您可以选择任何仍然支持 IAX2 协议的软电话进行实验室练习。第 4 步：在客户端中配置 IAX 帐户（通常是 *Add account* → IAX）。请注意，SipPulse Softphone 仅支持 SIP，无法通过 IAX2 注册，因此对于 IAX 测试，您需要一个仍然支持该协议的客户端。
...
现在，您可以在第 3 章中创建的 SIP 电话和实验室中创建的 IAX 电话之间拨打电话。

#### 使用 IAX 连接到 VoIP 提供商

很少有 VoIP 提供商支持 IAX。您可以通过搜索“IAX providers”轻松找到 IAX 提供商。使用 IAX 提供商非常有意义，因为 IAX 可以节省大量带宽，轻松遍历 NAT，并且可以使用 RSA 密钥对进行认证。

![客户的 Asterisk 通过互联网上的 IAX 中继连接到 VoIP 提供商：单个中继承载往返提供商的所有呼叫。](../images/10-legacy-fig14.png)

> **[第二版注]** 自 Asterisk 16 以来，支持 IAX 的商业 VoIP 提供商数量已显著减少。大多数提供商现在只提供 SIP/PJSIP 中继。在选择 IAX 提供商之前，请确认他们积极维护其 IAX 基础设施。对于新的提供商集成，PJSIP 中继是推荐的替代方案。
...
这是 RSA 认证所必需的。使用来自提供商的公钥可以让您确定收到的呼叫确实来自真正的提供商。如果其他人尝试使用相同的路径，他们将无法对其进行认证，因为他们没有相应的私钥。第 4 步：尝试连接。要测试连接，请拨打任何号码。一些供应商提供回声测试。为此，请编辑文件 extensions.conf。
...
将一台服务器连接到另一台服务器非常容易。您不需要注册它们，因为 IP 地址是已知的。您必须在 iax.conf 文件中创建对等体和用户。总部站点的所有分机都以 20 开头，后跟两位数字（例如 2000）。在分支机构，所有分机都以 22 开头，后跟两位数字（例如 2200）。我们将使用中继。您将需要一个 DAHDI 定时源来启用此功能。第 1 步：编辑分支服务器中的 iax.conf 文件。

![通过 IAX 中继连接两台 Asterisk 服务器：总部服务器（192.168.1.1，分机 20xx）和分支服务器（192.168.1.2，分机 22xx）通过单个 IAX 中继相互连接——不需要注册，因为两个 IP 地址都是固定的且已知的。](../images/10-legacy-fig15.png)
...
第 2 步：配置分支服务器中的文件 extensions.conf
...
第 5 步：测试从总部服务器中的 2000 电话到分支服务器中的 2200 电话的呼叫。

### IAX 认证

现在让我们从实际的角度分析 IAX 认证过程，以帮助您为每个特定需求选择最佳方法。

#### 传入连接

![传入呼叫的 IAX 认证决策流：Asterisk 根据是否提供用户名、它是否匹配部分、源 IP 是否允许以及密码（明文、MD5 或 RSA）是否匹配进行分支——接受带有该部分上下文和对等体选项的呼叫，或拒绝它。](../images/10-legacy-fig16.png)

当 Asterisk 收到传入连接时，初始信息可以包括用户名（来自字段“username=”）或不包括。传入连接也有一个 IP 地址，Asterisk 也使用该地址进行认证。如果提供了用户，Asterisk：1. 在 iax.conf 中搜索 type=user（或 type=friend，且部分名称与用户名匹配）的条目。如果找不到，Asterisk 将拒绝连接。2. 如果找到的条目具有 deny/allow 配置，它会比较呼叫者的 IP 地址，以根据 deny/allow 子句确定是否接受呼叫。3. 它使用明文、md5 或 RSA 检查密码 (secret)。4. 它接受连接并将呼叫发送到 iax.conf 文件中“context=”行中指定的上下文。如果未提供用户名，Asterisk：1. 在 iax.conf 文件中搜索包含 type=user（或 type=friend）且未指定密码的条目。它也检查 deny/allow 子句。如果找到条目，则接受连接，并将部分名称用作用户的名称。2. 在 iax.conf 文件中搜索包含 type=user（或 type=friend）且指定了密码或 RSA 密钥的条目。它检查 deny/allow 子句。如果找到条目，它会尝试使用指定的密码认证呼叫者；如果匹配，则接受连接。部分名称是用户的名称。假设您的 iax.conf 文件具有以下条目：
...
Asterisk 将尝试仅使用 iax.conf 文件中的相应条目来认证呼叫。如果指定了任何其他名称，呼叫将被拒绝。如果未指定用户，Asterisk 将尝试将连接认证为 guest。但是，如果 guest 不存在，它将尝试任何其他具有匹配密码的连接。换句话说，如果您的 iax.conf 文件中没有 guest 部分，恶意用户可以通过不指定用户名来尝试猜测任何匹配的密码。IP 地址的 deny/allow 限制也适用。避免密码猜测的一个好方法是使用 RSA 认证。另一种方法是限制允许呼叫的 IP 地址。

#### IP 地址限制

permit = <ipaddr>/<netmask> 规则按顺序解释，并且全部进行评估（此概念与路由器和防火墙中通常发现的 ACL 不同）。deny = <ipaddr>/<netmask> 示例 #1 permit=0.0.0.0/0.0.0.0 deny=192.168.0.0/255.255.255.0 将拒绝来自 192.168.0.0/24 网络的所有数据包 示例 #2 deny=192.168.0.0/255.255.255.0 permit=0.0.0.0/0.0.0.0 它将允许任何数据包。最后一条指令取代第一条。
...
第 11 步：测试从总部服务器中的 2000 电话到分支服务器中的 2200 电话的呼叫。

### iax.conf 文件配置

文件 iax.conf 有多个参数；逐个讨论每个参数会很枯燥且适得其反。所有参数以及描述都可以在示例文件中找到。在 wiki www.voip-info.org 中，您将找到有关每个参数的详细信息。在这里，我们将展示一些用于配置常规部分、对等体和用户的重要参数。

#### [General] 部分

服务器地址 bindport = <portnum> 配置 IAX UDP 端口。默认是 4569。bindaddr = <ipaddr> 使用 0.0.0.0 将 Asterisk 绑定到所有接口，或指定特定接口的 IP 地址。编解码器选择 bandwidth = [low|medium|high] High = 所有编解码器 Medium = 除 ulaw 和 alaw 外的所有编解码器 Low = 低带宽编解码器 allow/disallow = 编解码器选择微调 [alaw|ulaw|gsm|g.729| 等]

### 抖动缓冲区

抖动是数据包之间的延迟变化。它是影响语音质量的最重要因素。抖动缓冲区用于补偿延迟变化。它牺牲延迟以换取更低的抖动。您可以将抖动缓冲区与水箱进行类比。两者都可以以不规则的间隔接收数据包或水，但最终会提供规则的流动。

![抖动缓冲区作为水箱：数据包从网络不规则地到达并填充缓冲区，然后缓冲区以稳定的速率释放它们以产生平滑的语音流。缓冲区大小（以毫秒为单位）以一点延迟换取更低的抖动；过量缓冲区带让 Asterisk 随着网络条件的变化增加或减少缓冲区。](../images/10-legacy-fig17.png)

小的抖动（即低于 20 毫秒）通常是无法察觉的。但是，高于此水平的抖动是令人讨厌的。延迟应保持在 150 毫秒以下。创建抖动缓冲区将牺牲一些延迟以换取更低的抖动——这被称为“延迟预算”的概念。您可以使用以下参数影响抖动缓冲区：
...
IAX 支持使用对称密钥、128 位分组密码（称为 AES，高级加密标准）进行呼叫加密。激活 IAX 中继之间的加密非常简单。在文件 iax.conf 中使用：
...
以下是一些用于 Asterisk 的最重要的故障排除控制台命令。
...
本章回顾了 IAX 协议的优点和缺点。它演示了 IAX 在多种场景下的工作方式，例如软电话和两台 Asterisk 服务器之间的中继。中继模式允许您通过在单个数据包中承载多个呼叫来节省带宽。最后，您学习了可用于检查状态和调试协议的控制台命令。

## 传统 SIP：chan_sip 和 sip.conf（在 Asterisk 21+ 中移除）

> **传统 / 历史：** 本节中的所有内容都使用旧的 `chan_sip` 驱动程序及其 `sip.conf` 配置文件。`chan_sip` 在多个版本中被弃用，并 **在 Asterisk 21 中移除**，因此它 **在 Asterisk 22 中不存在**。以下 `sip.conf` 示例都无法在当前系统上运行——它们保留在这里仅用于记录传统部署的工作方式并帮助您迁移它们。对于执行任何这些操作的现代、受支持的方式，请参阅 *SIP & PJSIP in depth* 章节中的 *PJSIP: the SIP channel* 部分。SIP *协议* 理论（方法、注册、代理/重定向、SDP、NAT 类型）是协议级的，存在于该章节中；以下纯粹是已移除的 `chan_sip` **配置**。

在 Asterisk 20 之前的传统系统上，SIP 在 `/etc/asterisk/sip.conf` 中配置，它曾经是更改次数第二多的文件（仅次于 `extensions.conf`）。以下部分展示了 `chan_sip` 如何将 Asterisk 连接到 SIP 提供商、如何使用 SIP 将两台 Asterisk 连接在一起、域支持、状态、编解码器/DTMF/QoS 选项、认证和 NAT——随后是将其全部迁移到 PJSIP 的指南。

### 将 Asterisk 连接到 SIP 提供商 (sip.conf)

Asterisk 通常用于连接到 SIP VoIP 提供商。VoIP 提供商通常比传统提供商拥有更好的通话费率。VoIP 提供商的另一个有趣且吸引人的点是可以在其他城市——甚至在外国——购买 DID 号码的可能性。这些是使用 VoIP 进行电信的好理由。在本节中，您将学习旧的 `chan_sip` 如何将 Asterisk 连接到 VoIP 提供商。将 Asterisk 连接到 SIP 提供商需要三个步骤。测试可以通过与您最喜欢的提供商建立帐户来进行。第 1 步：在 sip.conf 中向 SIP 提供商注册要连接到 SIP 提供商，您将需要提供商提供的以下信息：

![Asterisk 通过互联网或专用 WAN 连接到 VoIP 服务提供商，本地 SIP 电话注册到 Asterisk 服务器](../images/07-sip-and-pjsip-fig07.png)

- 用户名
- 密码和远程密码（使用 secret 认证入站请求，使用 remotesecret 认证出站请求）
- 主机名
- 域
- 允许的编解码器
...
第 2 步：在 sip.conf 上配置 [peer] 创建一个对等体类型的条目到所需的提供商，以简化 Asterisk 的拨号。
...
第 3 步：在拨号方案中创建到提供商的路由我们将选择数字 010 作为到提供商的目的地路由。要在提供商内部拨打 #610000，只需拨打 010610000。
...
以下讨论检查了 sip.conf 文件中为连接到 VoIP 提供商而设置的选项的详细信息。

```
register=>username:password@hostname/4100
```

sip.conf 文件中的注册指令用于向提供商注册。注册事务使用名称和密码进行认证。您可以使用斜杠 (“/”) 为传入呼叫提供分机。从技术上讲，分机将放置在 SIP 请求的“Contact”头字段中。注册行为可以由某些参数控制：
...
要检查注册是否成功，旧的控制台命令是 `sip show registry`。在 Asterisk 22 上，等效命令是 `pjsip show registrations`（出站注册）和 `pjsip show endpoints`（端点状态）。

“username”参数用于认证摘要。摘要是使用用户名、密码和领域计算的：
...
参数 Fromuser 和 Fromdomain 有时是认证所必需的。这些参数用于 SIP From 头字段：
...
当您连接到 VoIP 提供商时，需要凭据。在初始邀请后，提供商会向您发送一条名为“407 Proxy Authentication Required”的消息；您在随后的 INVITE 消息中提供凭据。对于传入呼叫，您的 Asterisk 服务器将询问提供商的凭据。显然，提供商没有您的 Asterisk 服务器的有效凭据。当您使用 insecure=invite 时，您是在告诉 Asterisk 不要向提供商发送“407 Proxy Authentication Required”并接受传入呼叫。您还可以使用 insecure=port, invite 根据 IP 地址匹配对等体，而不匹配端口号。
...
您可以使用 SIP 将两台 Asterisk 盒子互连。在继续此配置之前，注意拨号方案很重要。用户通常希望以最小的努力连接其他 PBX。这里的想法是仅使用分机号来连接到另一个 PBX。第 1 步：编辑服务器 A 中的 sip.conf 文件：
...
此参数允许在不进行认证的情况下处理外部邀请。它在常规部分或域语句中定义的上下文中处理呼叫。警告：如果您在常规部分中定义了一个可以访问 PSTN 的上下文，外部用户可以通过您的 PBX 拨打 PSTN。在这种情况下，您将承担任何费用。仅允许您自己的分机在常规部分中定义的上下文中。
...
SIP 状态在 Asterisk 中部分实现。Asterisk 支持 SUBSCRIBE 和 NOTIFY 用户等请求，具体取决于通道的状态。Asterisk 不支持 SIP 方法 PUBLISH。换句话说，您可以订阅通道的状态（忙碌、空闲和振铃），但不能发布“离开”或“请勿打扰”等信息。状态最常见的场景是忙灯场 (BLF)，其中您模拟 KS 系统的行为，为每个分机和中继配备灯。用于状态的 SIP 参数：
...
- counteronpeer（从 Asterisk 1.4.x 的 limitonpeer 重命名）：仅在对等体侧应用计数器
- callcounter=yes：在设备中启用呼叫计数器。
- busylevel=1：考虑设备忙碌的呼叫数量阈值。
...
- 将状态模型从点对点更改为状态代理，这将使软电话订阅 Asterisk 的 SIP 事件。

第 3 步：将联系人添加到其他软电话。在此示例中，SipPulse Softphone 是帐户 2000，因此我们将为帐户 2001 添加一个联系人。序列：打开右侧面板（软电话中的状态面板）->点击联系人->添加联系人。填写名称 2001。显示为 2001，别忘了勾选显示此联系人的可用性框。
...
在某些情况下，您会将数字传递给语音信箱或交互式语音应答 (IVR) 等应用程序。正确传递 DTMF 很重要。传递 DTMF 的最简单方法称为 inband。它在 sip.conf 文件的 [general] 或对等体/用户部分中设置。当您设置 dtmfmode=inband 时，DTMF 音调作为音频通道中的声音生成。此方法的主要问题是，当您使用 g729 等编解码器压缩音频通道时，声音会失真，并且 DTMF 音调无法正确识别。如果您计划使用 dtmfmode=inband，请使用 g.711 编解码器（ulaw 和 alaw）。
...
最后，您可以将 DTMF 数字传递到 SIP 数据包中，而不是 RTP 数据包中。此方法在 RFC3265（信令事件）和 RFC2976 中定义。
...
当旧的 `chan_sip` 收到 SIP 呼叫时，它遵循下图中描述的规则。三个参数在 SIP 认证中起重要作用。在 Asterisk 22 上，认证改为使用 PJSIP `auth` 对象（`type=auth`、`auth_type=userpass`、`username=`、`password=`）配置，由端点引用，IP 访问控制通过端点上的 `permit=`/`deny=` 或通过 `acl` 完成。

![传统 chan_sip 认证决策流：Asterisk 根据 sip.conf 检查 From 头，尝试匹配 type=user/peer 部分和 MD5 凭据，并回退到 insecure=invite 或 allowguest，然后再允许或拒绝呼叫](../images/07-sip-and-pjsip-fig10.png)

```
allowguest=yes/no
```

此参数控制没有相应对等体的用户是否可以在没有名称和密码的情况下进行认证。我们在域支持部分讨论了此参数。

```
insecure=invite,port
```

当我们使用 insecure=invite 时，Asterisk 不会生成消息“407 Proxy Authentication Required”。没有此消息，用户可以在不进行认证的情况下拨打电话。这通常用于连接到 VoIP 服务提供商。来自 VoIP 服务提供商的呼叫通常不进行认证。

```
autocreatepeer=yes/no
```

此命令用于 Asterisk 连接到 SIP 代理时。它为每个呼叫动态创建一个对等体。启用此选项后，任何 UAC 都可以连接到 Asterisk 服务器。将 IP 连接限制为 SIP 代理很重要。SIP 代理反过来负责访问控制。对等体配置基于常规选项以及 SIP 数据包的“Contact”头字段。警告：请极其谨慎地使用此功能，因为它完全打开了 Asterisk。

```
secret=secret, remotesecret=secret
```

此参数配置用于认证的密码，使用 secret 进行入站请求，使用 remotesecret 进行出站请求。如果您不想在文本文件中显示密码，可以使用 md5secret 来包含哈希而不是密码。要生成 MD5 密码，您可以使用：
...
NAT *理论*（四种 NAT 类型、Contact 头问题、保持连接以及强制媒体通过服务器）是协议级的，在 *SIP & PJSIP in depth* 章节中涵盖。此处显示的 `sip.conf` 参数（`nat=`、`qualify=`、`directmedia=`、`externaddr=`、`localnet=`）是 **传统 chan_sip**，已在 Asterisk 21+ 中移除。在 PJSIP 上，这些映射到传输/端点设置，例如传输上的 `rewrite_contact=yes`、`force_rport=yes`、`rtp_symmetric=yes`、`direct_media=no`、`external_media_address`、`external_signaling_address` 和 `local_net=`，以及 AOR 上的 `qualify_frequency=`。
...
当您在 sip.conf 文件中放入语句“nat=force_rport”时，您是在告诉 Asterisk 忽略 SIP 头中“Contact”头字段包含的地址，并使用数据包 IP 头中的源 IP 地址和端口，并且还将媒体发送回接收它的地址，忽略 SDP 头的内容。
...
Qualify 将定期使用 OPTIONS 方法发送 SIP 数据包，这将有助于保持 NAT 打开。Qualify 每 60 秒发送一次 OPTIONS，当主机不可达时每 10 秒发送一次。您可以使用“sip show peers”查看对等体的延迟。如果用户的 NAT 是对称类型的，则无法直接从一个 UAC 向另一个 UAC 发送数据包；在这种情况下，您必须使用以下命令强制 RTP 通过 Asterisk：

```
directmedia=no
```

#### Asterisk 位于 NAT 之后 (sip.conf)

所有以前的场景都假设 Asterisk 服务器具有外部（有效）互联网地址。有时 Asterisk 服务器是在带有 NAT 的防火墙后面实现的。在这种情况下，有必要进行一些额外的配置。

![Asterisk 位于 NAT 之后：防火墙将公共地址 200.180.4.168 映射到内部 Asterisk 服务器 (192.168.1.100)，转发 UDP 5060 上的 SIP 和 rtp.conf 中定义的 UDP 10000–20000 上的 RTP 范围](../images/07-sip-and-pjsip-fig13.png)

第 1 步：配置防火墙以将 UDP 端口 5060 静态重定向到 Asterisk 服务器。第 2 步：配置防火墙以将 UDP 端口从 10000 静态重定向到 20000。如果您想限制打开的端口数量，可以编辑 rtp.conf 文件以更改 RTP 端口范围。另一种方法是使用支持 SIP 协议的智能防火墙来动态打开 RTP 端口。
...
因为 `chan_sip` 已在 Asterisk 21 中移除，并且在 Asterisk 22 中消失，所以任何现有的 `sip.conf` 部署都必须迁移到 PJSIP。最大的概念转变是单个 `sip.conf` `[peer]` 或 `[friend]` 被拆分为多个 PJSIP 对象，每个对象都有一个 `type=`：一个 **endpoint**（呼叫/编解码器/媒体设置），一个或多个 **aor** 对象（设备可达/注册的位置），一个 **auth** 对象（凭据），以及一个共享的 **transport**（监听套接字，NAT 地址）。下表映射了最常见的概念。
...
| `disallow=all` / `allow=ulaw` | 端点上的 `disallow=all` / `allow=ulaw`（相同语法） |
...
将其输出视为一个 **起点**：审查每个生成的对象，特别是传输、NAT 设置和编解码器列表，并在投入生产之前进行彻底测试。
...
虽然转换看起来还可以，但我们可以看到 qualify=yes 等一些元素无法直接映射。要修复，您必须在 aor 部分添加命令 qualify_frequency=time（以秒为单位）。示例如下。
...
完整的 PJSIP 配置在 *SIP & PJSIP in depth* 章节中涵盖，docs.asterisk.org 上的官方文档对该通道有全面的覆盖。在我们 voip.school 的配套实验室中，实验室 5 让您练习刚刚学到的内容。
...
4. 为了在模拟通道上进行精确计费，您必须准确检测远端何时应答。您在 Asterisk 上激活（并向电信公司请求）什么功能来执行此操作？
   ...
   - C. 极性反转
   - D. 拨号音生成
5. DAHDI 硬件独立于 Asterisk：物理卡在 `/etc/dahdi/system.conf` 中配置，而 `chan_dahdi.conf` 定义了 Asterisk 通道，而不是硬件本身。
   - A. 正确
   - B. 错误
6. 关于数字中继容量和信令，标记正确的陈述（选择所有适用项）：
   - A. E1 中继承载 30 个语音通道，T1 中继承载 24 个。
...
10. RSA 密钥可用于 IAX2 认证。您必须保密哪个密钥，并将哪个密钥交给另一台服务器？
    - A. 对公钥保密；共享私钥
    - B. 对私钥保密；共享公钥
    - C. 对共享密钥保密；共享私钥
    - D. 两个密钥都必须共享

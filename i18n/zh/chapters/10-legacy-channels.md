...
在 2026 年的纯 VoIP 世界中，本章介绍的通道类型已日益罕见：大多数新部署都是基于以太网的 SIP trunk 和 PJSIP endpoint，完全不涉及任何电话硬件。尽管如此，Asterisk 22 仍然全面支持其中的大多数。模拟 (FXO/FXS) 和数字 TDM (E1/T1/ISDN PRI/BRI) 连接通过 DAHDI 提供——这是由 Digium 最初开发的驱动程序栈，Digium 于 2018 年被 Sangoma 收购，此前早期的 Zaptel 驱动程序在商标纠纷后被更名。服务器间的 IAX2 连接由 `chan_iax2` 提供，它虽然仍在发布和支持中，但现在已明确属于传统协议。本章还收集了 **传统 SIP** 的资料：旧的 `chan_sip` 驱动程序及其 `sip.conf` 配置（已在 Asterisk 21 中移除，并在 Asterisk 22 中彻底消失），以及一份将现有 `sip.conf` 系统迁移到 PJSIP 的完整指南。如果您运行的是基于 PJSIP 的纯 SIP 环境，且没有电话卡、没有 IAX2 trunk，也没有需要转换的传统 `sip.conf`，则可以安全地跳过本章。

## 模拟通道 (FXO/FXS)

> **[第 2 版注]** 在出版前更新第 2 版的前言日期/ISBN。

> **[第 2 版注 — 部署背景]** 截至 Asterisk 22，DAHDI 和模拟电话卡仍然得到全面支持，且 DAHDI 仍可针对当前内核进行构建。然而，大多数新部署都是纯 VoIP（SIP trunk，PJSIP）。模拟/TDM 硬件现在是一种小众选择，主要存在于传统环境、农村 PSTN 连接或受监管的市场中。以下内容对于这些场景仍然准确。
...
大多数模拟实现使用一对名为 tip 和 ring 的铜线。当回路闭合时，电话会从电信交换机（或专用 PBX）接收拨号音。最常用的信令是环路启动 (loop-start)；其他不太常见的信令包括接地启动 (ground start)，这在一些国家使用。这三类信令是：
...
主要的监控信令包括挂机 (on-hook)、摘机 (off-hook) 和振铃 (ringing)。挂机 (On-Hook) – 当用户将电话放回挂机状态时，PBX 会中断并禁止电流通过。在这种状态下，电路被称为挂机。在此位置，只有振铃器处于活动状态。摘机 (Off-Hook) – 在开始通话之前，电话需要进入摘机状态。从挂机状态取下听筒会闭合回路，并向 PBX 指示用户打算拨打电话。收到此指示后，PBX 会产生拨号音，向用户指示它已准备好接收目的地址（即电话号码）。振铃 (Ringing) – 当用户呼叫另一部电话时，它会向振铃器产生电压，提醒对方有来电。信令因国家/地区而异，不同国家有不同的音调。您可以通过修改 indications.conf 文件来根据您所在的国家/地区个性化 Asterisk 音调。例如：
...
- 数字：在需要多条线路时使用。数字线路通常由 CSU/DSU 或光纤多路复用器提供。最终用户连接器通常是 RJ45。在某些国家/地区，E1 线路使用两个同轴 BNC 连接器提供；在这种情况下，您需要一个平衡-不平衡转换器 (balun) 将 RJ45 插孔连接到电话板。
- SIP：此选项是最近开发的。电话线路使用带有 SIP 信令的数据连接 (VoIP) 提供。这是与 Asterisk 一起使用的好选择，因为您不需要购买电话卡。电话呼叫将直接传送到以太网端口。另一个优点是，您可以通过避免编解码器转码来释放 CPU 资源。
...
FX 接口是模拟的。术语“外汇 (Foreign eXchange)”适用于访问 PSTN 中心局 (CO) 的中继线。外汇局 (FXO)

![Asterisk 位于模拟电话 (FXS) 和电信线路 (FXO) 之间：FXS 侧为电话提供拨号音和振铃，而 FXO 侧从中心局获取拨号音。](../images/10-legacy-fig01.png)

FXO 接口用于连接到中心局 (CO) 或其他 PBX 的分机。它直接与来自 PSTN 的电话线通信。另一个选择是将 FXO 接口连接到现有的 PBX，从而允许 Asterisk 和传统 PBX 之间的通信。将 Asterisk 连接到 PBX 端口并使用 VoIP 提供远程分机通常被称为异地分机 (OPX)。FXO 接口接收拨号音。外汇站 (FXS) FXS 接口为模拟电话、调制解调器或传真机供电。FXS 为电话提供拨号音和电源。
...
在 Asterisk 中使用 kewlstart 信令几乎是默认设置。Kewlstart 本身不是信令，但通过监控另一侧发生的情况为电路增加了智能。Kewlstart 基于环路启动。大多数交换机不支持此功能，该功能用于获取挂机通知。

- 环路启动 (Loopstart)：用于大多数模拟线路，它允许电话指示“挂机”和“摘机”，并允许交换机指示“振铃”和“无振铃”。这可能是大多数人家里拥有的。这个名字来源于线路始终处于打开状态的事实。当您闭合回路时，交换机会为您提供拨号音。来电通过开路对上的 100V 振铃电压发出信号。

![Asterisk 作为 VoIP 网关运行：FXO 端口连接到传统 PBX 分机，而远程 Asterisk 通过 FXS 端口（异地分机或 OPX）通过 IP 将该线路传送到模拟电话。](../images/10-legacy-fig02.png)

- 接地启动 (Groundstart)：类似于环路启动。当您想拨打电话时，线路的一侧会短路。当交换机识别出此状态时，它会通过开路对反转电压，然后闭合回路。因此，线路在提供给呼叫者之前首先被占用。
- 酷启动 (Kewlstart)：为电路增加了智能，允许监控另一侧。Kewlstart 结合了环路启动的许多优点。
...
在为 Asterisk 选择硬件之前，您应该考虑将要安装和启用的并发呼叫、服务和编解码器的数量。Asterisk 是一个 CPU 密集型应用程序，这就是为什么我们建议为 Asterisk 使用专用机器。计算机内安装的接口卡数量受可用插槽和中断数量的限制。安装一张带有 8 个语音接口的卡比安装两张带有 4 个接口的卡更可取。另一个选择是使用 USB 通道库，例如 Xorcom Astribank。最近，一些制造商（例如 CIANET）开始生产 TDMoE 通道库，使得连接数十个模拟接口变得更加容易。
...
在此示例中，我们将使用带有 1 个 FXS 和 1 个 FXO 模块的 Sangoma TDM400 电话接口卡（前身为 Digium TDM400）。所需步骤如下：

1. 安装模拟卡 FXS、FXO 或两者。
2. 配置文件 `/etc/dahdi/system.conf`（前身为 `/etc/zaptel.conf`）。
...
警告：您必须至少为通道配置回声消除算法。base_exten 参数定义了 FXS 分机的基本拨号方案。在这种情况下，第一个 FXS 通道将接收分机号码 4000，第二个接收 4001，依此类推。创建线路 (context_phones) 和中继线 (context_lines) 的上下文非常重要。生成文件后，您应该在 `/etc/asterisk/chan_dahdi.conf` 文件中包含文件 `/etc/asterisk/dahdi-channels.conf`：

```
#include dahdi-channels.conf
```

注意：模拟信令有点令人困惑；它总是与卡相反。FXS 卡使用 FXO 发送信令，而 FXO 卡使用 FXS 发送信令。Asterisk 与这些设备对话，就好像它在对面一样。
...
一个重要的实用程序是 dahdi_test，它用于验证 DAHDI 卡中的中断丢失。音频质量问题通常与中断冲突有关。要验证您的 DAHDI 卡是否未与其他卡共享中断，请使用以下命令：
...
听起来很奇怪，但在配置 /etc/dahdi/system.conf 后，您已经配置了卡本身。DAHDI 可用于其他目的，如路由和 SS7。要将其与 Asterisk 一起使用，您必须配置 Asterisk DAHDI 通道。Asterisk 中的每个通道都必须定义；SIP/PJSIP 通道在 pjsip.conf 中定义（注意：chan_sip 和 sip.conf 已在 Asterisk 21 中移除），而 TDM 通道在 chan_dahdi.conf 中定义。这创建了要在您的拨号方案中使用的逻辑 TDM 通道。
...
group：允许将通道作为组处理。如果您拨打组号而不是通道号，则使用第一个可用的通道。如果通道是电话，当您呼叫一个组时，所有电话将同时振铃。使用逗号，您可以为同一通道指定多个组。
...
有许多 callerid 选项。有些可以禁用，尽管大多数默认情况下是启用的。usecallerid：启用或禁用后续通道的 callerid 传输（是/否）。注意：如果您的系统在应答前收到两次振铃，请尝试禁用此功能。它应该立即应答。hidecallerid：定义是否隐藏传出的 callerid（是/否）。callerid：为特定通道配置 callerid 字符串。呼叫者可以配置为 asreceived。这主要用于中继接口以指示传入的 callerid。
...
调整回声消除既是技术活，也是艺术活。这些选项调整影响 DAHDI 通道中音频质量的某些 Asterisk 参数。它们可以帮助提高模拟接口的音频质量。

#### fxotune 实用程序

fxotune 是一个用于微调 FXO 模块某些参数的实用程序。需要此微调来调整由混合电路引起的阻抗失配。该实用程序有三种操作模式：
...
重要的是要了解您必须在启动 Asterisk 之前在系统加载中插入指令 fxotune –s：
...
大多数回声消除算法通过生成接收信号的多个副本来运行，其中每个副本都延迟特定的时间量。滤波器的抽头 (taps) 数量决定了需要消除的回声延迟的大小。然后调整这些延迟的副本并从接收到的信号中减去。诀窍是仅调整延迟信号以消除回声，而不使用过多的 CPU 周期。从用户的角度来看，选择合适的回声消除算法很重要。默认是 MG2；但是，还有两个其他选项可用：来自 Sangoma（前身为 Digium）的高性能回声消除 (HPEC) 和由 David Rowe 开发的开源回声消除 (OSLEC)。

> **[第 2 版注]** OSLEC 项目页面 (http://www.rowetel.com/ucasterisk/oslec.html) 可能不再是最新的；在引用它之前，请验证现代内核的可用性和内核集成状态。要更改回声消除算法，请更改 /etc/dahdi/system.conf 中的参数 echo_can。例如：
...
echocancel：禁用或启用回声消除。您应该保持此功能启用。它接受“yes”或抽头数量。解释：回声消除是如何工作的？大多数回声消除算法通过生成接收信号的多个副本来运行，每个副本都延迟一小段时间。这个小流被称为“抽头”。抽头数量决定了可以消除的回声延迟。这些副本被延迟、调整并从原始信号中减去。诀窍是将延迟信号精确调整到消除回声所需的程度。echocancelwhenbridged：在纯 TDM 呼叫期间启用或禁用回声消除器。这通常是不必要的。rxgain：调整音频接收增益以增加或减少接收音量（-100% 到 100%）。txgain：调整音频传输增益以增加或减少传输音量（-100% 到 100%）。例如：
...
这些参数（上文）指定接口是否将尝试检测忙音、成功检测将使用多少个音调以及忙音模式是什么。忙音检测在很大程度上是实验性的，可以在 Makefile 中更改一些附加参数。要检测呼叫的应答（这对于精确计费至关重要），可以使用极性反转来发出精确应答时间的信号。如果您计划对呼叫收费，或者只是希望进行精确的计费以进行比较，这一点很重要。通常，您必须联系电话公司以请求此服务。

```
answeronpolarityswitch=yes
```

在某些国家/地区，也可以使用极性反转来检测呼叫挂断。
...
这些选项用于连接到 FXS 接口的电话。直接连接到 DAHDI 接口的模拟电话所提供的所有功能均由 Asterisk 控制。Adsi (模拟显示服务接口)：这是一组电信标准，被一些电信公司用于提供购票等服务。cancallforward：启用或禁用呼叫转移（*72 启用，*73 禁用）。calleridcallwaiting：启用在呼叫等待指示期间接收的 callerid（是/否）。immediate：在立即模式下，通道不会提供拨号音，而是立即跳转到定义上下文中的“s”分机。这用于创建热线。threewaycalling：启用或禁用三方会议。mailbox：警告用户有关可用的语音邮件消息。它可以是声音信号或视觉指示器（如果电话支持此功能）。参数是邮箱号码。callgroup：对电话进行分组以拨打或接听。pickupgroup：用于呼叫代接的电话组。
...
每当您需要实现大量通道时，数字线路 E1/T1 都是一种选择。单个 E1 电路能够进行 30 次并发呼叫，并且您可以拥有诸如直接拨入 (DID)、来电显示 (caller identification) 和高级信令等功能。E1/T1 线路可能会通过双绞线、光纤和微波等多种方式到达您的公司，具体取决于您所在的国家/地区。数字线路使用 UTP、光纤或微波传送到您的公司。调制解调器和多路复用器 (MUX) 用于传送物理线路。与 T1 线路的连接始终基于 RJ45 连接器。但是，E1 线路也可以使用 BNC 提供。提前了解您将要接收的连接器类型非常重要，主要是在 E1 线路中。通常，直到 RJ45 的所有设备都由电信公司提供。

![E1/T1 电路的配置方式：电信公司可以通过 UTP 铜线（用于 E1 的 HDSL 调制解调器，或用于 T1 的直接卡连接）、通过光多路复用器的光纤或通过微波无线电链路提供中继线。](../images/10-legacy-fig05.png)

![UTP 还是 BNC？大多数数字卡使用 RJ45 (UTP) 连接器，但一些 E1 线路是在双 BNC 同轴电缆上提供的，在这种情况下，需要一个平衡-不平衡转换器来使同轴对适应卡的 RJ45 插孔。](../images/10-legacy-fig06.png)

#### 语音是如何转换为比特的？

模拟信号每秒采样 8,000 次，以创建模拟语音的数字版本。这种编码称为脉冲编码调制 (PCM)。在美国和日本，信号使用 law 编码（在 Asterisk 中称为 ulaw）。在世界其他地区，编码是 alaw。

![脉冲编码调制 (PCM)：4 kHz 模拟语音信号每秒采样 8,000 次 (奈奎斯特)，并编码为 64 Kbps 的数字比特流。](../images/10-legacy-fig07.png)

#### 时分复用

当您只需要几个通道时，模拟线路是有意义的。使用时分复用 (TDM) 时，可以将多个通道塞入单个数据连接中。当您需要大量电路时，电话公司通常会为您提供数字中继线，这是一种数据电路，其中语音以使用 PCM 的数字格式传输。每个时隙使用 64 Kbps 的带宽来传输单个语音通道。

![E1 和 T1 中的时分复用：E1 帧以 2048 Kbps 承载 32 个时隙（DS0 #0 用于帧同步，DS0 #16 用于信令），而 T1 帧以 1544 Kbps 承载 24 个时隙，使用一位用于同步，并使用一种窃位方案进行信令。](../images/10-legacy-fig08.png)

在美国，最常见的数字中继线是 T1，它有 24 条可用线路；在欧洲和拉丁美洲，E1 中继线有 30 条线路。一些公司提供通道较少的零碎 T1/E1。窃位信令 (Robbed bit signaling) 有时 T1 中继线使用窃位方案，其中一位被借用于信令。在 T1 中继线上，数据/语音通道在每个时隙上以 56 Kbps 传输。正如您可能观察到的，当您使用窃位时，T1 电路不会因为同步和信令而丢失两个时隙。

#### T1/E1 线路编码

T1 和 E1 实际上是数据电路，并且具有确定比特解释方式的数据编码。对于 E1，最常见的线路编码是第 1 层的 HDB3 和第 2 层的 CCS。了解数字中继线配置方式的最简单方法是询问电信公司有关此信息。您将需要此信息来配置文件 /etc/dahdi/system.conf。
...
> **[第 2 版注]** Sangoma 于 2018 年收购了 Digium。Digium 品牌的卡现在以 Sangoma 品牌销售和支持。请在 Sangoma 网站 (www.sangoma.com) 上验证当前型号的可用性，因为某些旧 SKU 可能已停产。
- MiniPCI，存在于嵌入式系统中 o OpenVOX A100M(FXO), B100M(ISDN BRI), B200M(ISDN BRI), 和 B400M(ISDN BRI)
- USB 2.0，存在于大多数现代 PC 中。基于 USB 的解决方案允许高密度的模拟和数字通道。此总线支持 480 Mbps，每个语音通道占用 64 Kbps。使用 USB 集线器，可以在单个端口中获得高达一千个模拟端口的密度。o Xorcom Astribank (FXS, FXO, E1-ISDN, E1-R2)
- 以太网。以太网最大的优势是允许卡由多台服务器连接。高可用性解决方案通常是这些设备的核心应用。该解决方案的优势在于使用没有空闲 PCI 插槽的服务器或刀片服务器。o Redfone FoneBridge (最多四个 E1 电路)

### 使用硬件回声消除

硬件回声消除减少了主机 CPU 的负载。对于具有多个 E1 接口的卡，硬件回声消除可以帮助减轻处理器的压力。诸如 OSLEC 之类的新型增强型软件回声消除器正在减少对硬件回声消除器的需求。要在硬件和软件回声消除器之间进行选择，您应该考虑服务器中可用的处理能力和 E1 电路的数量。使用 OSLEC 时，回声消除过程每个语音通道最多可使用 9 MIPS（每秒百万条指令），具有 128 个抽头的幅度（参考：Xorcom Ltd.）。如果您考虑每条指令 1 个 CPU 周期（基于处理器和软件实现本身，这并不总是正确的），我们谈论的是四个 E1 的 1.080 Ghz。

#### 信令类型

选择信令类型（例如，T1 CAS、T1 PRI、E1 CAS R2 或 E1 CAS ISDN）并非易事。这实际上取决于您所在地区可用的内容以及价格。公共信道信令 (CCS) 通常优于随路信令 (CAS)。但是，它通常不可用。在美国，您通常可以选择，因为大多数电信公司为普通用户提供 T1 CAS，为高级用户（例如呼叫中心）提供 T1 PRI。在拉丁美洲，E1 CAS R2 很普遍，但在一些城市可以使用 ISDN PRI。
...
实现 R2 需要安装一个名为 OpenR2 (www.libopenr2.org) 的库（由 Moises Silva 开发），并在安装前修补 Asterisk——本章稍后将展示一个简单的过程。该库已通过多项测试，并在我们的几位客户中投入生产。在我看来，如果可用，ISDN 始终是最佳选择。一些提供商可以访问 7 号信令系统 (SS7)，这是电话公司之间可用的 CCS 信令。SS7 有专有和开源解决方案可用。库 libss7 用于支持 Asterisk 上的 SS7。
...
有两种配置 DAHDI 通道的方法。第一种是手动配置，完全控制所有参数。第二种是使用实用程序 dahdi_genconf 来检测和配置卡。
...
第 1 步：TE205P 安装。在安装 TE205P 之前，了解 TE205P 和 TE210P 卡之间的区别非常重要。TE210P 卡使用由 3.3 伏供电的 64 位总线，几乎只存在于服务器的主板中。如果您指定此接口卡，请务必小心；确保您的硬件支持 64 位、3.3V 总线。TE205P 卡使用 5V PCI，这在台式计算机中很常见。我们在此示例中选择了带有两个跨度的 TE205P 接口卡，因为它更容易将其减少为单跨度卡或扩展为四跨度卡。这些卡现在以 Sangoma 品牌（前身为 Digium）销售。
...
第 6 步：将 DAHDI 配置到文件 /etc/asterisk/chan_dahdi.conf 中 示例 #1 (2xT1)
...
对于点对多点 BRI，使用 signaling=bri_cpe_ptmp。目前，NT 模式下不支持 BRI 点对多点。
...
有时事情第一次无法正常工作。让我们检查一些用于 DAHDI 故障排除的资源。第 1 步：检查操作系统是否识别该卡。Sangoma/Digium 卡通常被识别为 ISDN 调制解调器。
...
警报指示端口存在问题：红色警报：无法与远程交换机保持同步。这通常是一个物理问题，例如线路编码或帧格式不匹配。黄色警报：信号表明远程交换机处于红色警报状态。这表明远程交换机未收到您的传输。蓝色警报：在所有时隙上接收所有未成帧的 1；dahdi_tool 目前无法检测到蓝色警报。环回：端口处于本地或远程环回状态
...
overlapdial：当您在连接建立后传递数字时，使用重叠拨号。您可以使用块模式编号 (overlapdial=no) 或数字模式 (overlapdial=yes)。块模式通常由运营商使用。signaling：配置后续通道的信令类型。这些参数应对应于 chan_dahdi.conf 文件中的参数。正确的选择基于可用的通道。对于 ISDN，您可以选择五个选项：

- pri_cpe：当设备是 CPE 时使用，有时称为客户端、用户或从属。这是最简单且最常用的信令形式。有时，当您尝试连接到专用 PBX 时，PBX 也通常被配置为 CPE。在这种情况下，请在 Asterisk 中使用 pri_net 信令。
- pri_net：当 Asterisk 连接到配置为 CPE 的专用 PBX 时使用。信令通常被称为主机、主控或网络。
...
有许多 Caller ID 选项可用。有些可以禁用，尽管大多数默认情况下是启用的。usecallerid：启用或禁用后续通道的 Caller ID 传输（是/否）。注意：如果您的系统在应答前需要两次振铃，请尝试禁用此功能，以便它立即应答。hidecallerid：隐藏 Caller ID（是/否）。calleridcallwaiting：启用在呼叫等待指示期间接收 Caller ID（是/否）。callerid：为特定通道配置 Caller ID 字符串。呼叫者可以在中继接口中配置为“asreceived”以向前传递 Caller ID。

```
callerid = "Flavio Eduardo Gonçalves" <48 30258500>
```

注意：大多数电信公司要求您配置正确的 caller ID。如果您不传递正确的 caller ID，您将无法通过电信公司拨出。另一方面，即使不配置 caller ID，您也可以接听电话。

#### 音频质量选项

这些选项调整影响 DAHDI 通道中音频质量的某些 Asterisk 参数。echocancel：禁用或启用回声消除。您应该保持此功能启用。它接受“yes”或抽头数量。解释：回声消除是如何工作的？大多数回声消除算法通过生成接收信号的多个副本来运行，每个副本都延迟一小段时间。这个小流被称为“抽头”。抽头数量决定了可以消除的回声延迟。这些副本被延迟、调整并从原始信号中减去。诀窍是将延迟信号精确调整到消除回声所需的程度。echocancelwhenbridged：在纯 TDM 呼叫期间启用或禁用回声消除器。这通常是不必要的。rxgain：调整音频接收增益以增加或减少接收音量（-100% 到 100%）。txgain：调整音频传输增益以增加或减少传输音量（-100% 到 100%）。示例：
...
用于发出 MFC/R2 信令的卡与用于发出 ISDN 信令的卡相同。可以使用名为 libopenR2 (www.libopenr2.com) 的库在 DAHDI 通道上使用 MFC/R2。此库不是 1.6.2 之前版本的 Asterisk 的一部分。
...
以下序列说明了从 Asterisk 分机发起到 PSTN 中终端的呼叫。PSTN 丢弃呼叫并结束通信。

![Asterisk 和电信公司之间的完整 MFC/R2 呼叫流程：线路信令（空闲、占用、占用确认、应答、清除后向、清除前向）在时隙 16 中交换，拨打的数字和后向“发送下一个数字”信号（I/A/B 组）带内传输，可听音调到达用户。](../images/10-legacy-fig11.png)
...
> **[第 2 版注]** 第 1 版中的补丁 Asterisk 1.4 树已过时；对于 Asterisk 22，通过 libopenr2 的 MFC/R2 支持已集成在主源代码树中，以下步骤现在使用当前的 Git 存储库，而不是已停用的 `svn.digium.com`。考虑为最终版本精简这些历史构建步骤。
...
> **[第 2 版注]** 原来的 Asterisk 1.4 SVN 补丁树已在下面被当前的 Git 存储库替换。在 Asterisk 22 上，MFC/R2 补丁是不必要的——`chan_dahdi` 直接针对 libopenr2 构建 R2 支持——因此您只需要 openr2 库加上正常的 Asterisk 构建。
...
注意：一些电信公司不接受没有 caller ID 的呼叫。请将 caller ID 设置为运营商分配的 DID 号码之一。在一些国家，不需要此步骤。第 7 步：测试解决方案：现在，在上下文 from-internal 中使用分机，拨打任何号码并观察控制台。检查是否有任何错误发生。-- Executing Set("SIP/8564-081ca5d8", "CALLERID(num)=1145678990") in new stack -- Executing Dial("SIP/8564-081ca5d8", "DAHDI/g1/35678899|60|tT") in new stack
...
选项在文件 chan_dahdi.conf 中有记录。这里详细说明了一些最重要的选项。强制参数：mfcr2_variant、mfcr2_max_ani 和 mfcr2_max_dnis。mfcr2_variant：国家变体。
...
mfcr2_max_ani：要请求的最大 ANI 数字量 mfcr2_max_dnis：要请求的最大 DNIS 数字量 mfcr2_get_ani_first：是否在 DNIS 之前获取 ANI（某些电信公司要求）mfcr2_category：呼叫者类别。您可以在开始呼叫之前设置变量 MFCR2_CATEGORY mfcr2_logdir：记录呼叫文件的目录。(/var/log/asterisk/mfcr2/directory) mfcr2_call_files：是否记录呼叫
...
mfcr2_mfback_timeout：此值值得一提。有时，如果您正在呼叫手机或任何需要很长时间才能完成的呼叫，此参数可能会超时，因此它通常会被更改以进行微调。如果您的某些呼叫未完成，这是您应该首先更改的参数。mfcr2_metering_pulse_timeout：脉冲被一些 R2 变体用于指示成本 mfcr2_allow_collect_calls：在巴西，音调 II-8 用于指示对方付费呼叫；此参数允许您阻止对方付费呼叫。mfcr2_double_answer：也用于在需要双重应答时避免对方付费呼叫。使用 double_answer=yes，您实际上阻止了对方付费呼叫。mfcr2_immediate_accept：允许您跳过 B/II 组信号的使用，直接进入接受状态。mfcr2_forced_release：允许您加快呼叫的释放；适用于巴西变体。

#### ANI 和 DNIS

自动号码识别 (ANI) 是呼叫者的号码。拨入号码识别服务 (DNIS) 是被呼叫的号码，或者换句话说，是拨打的号码。当收到呼叫时，通常最后四位数字会以称为直接拨入 (DID) 的过程传递给 PBX。ANI 号码实际上是 Caller ID。拨号时，ANI 将拥有呼叫者的分机，而 DNIS 将包含呼叫目的地。正确配置这些参数非常重要。一些交换机只发送最后四位数字，而另一些则发送完整号码。
...
在本章中，我们将了解 Inter-Asterisk eXchange (IAX) 协议，包括其优势和劣势。还将涵盖中继模式和两个 Asterisk 服务器的互连等详细信息。本文档中的所有引用均对应于 IAX 版本 2。IAX 协议为语音和视频提供媒体传输和信令。IAX 非常创新；它在中继模式下节省带宽，并且在需要遍历 NAT 时比 SIP 简单得多。IAX 目前的主要用途是互连 Asterisk 服务器。IAX 最初是为语音创建的，但它也可以容纳视频和其他多媒体流。IAX 受其他 VoIP 协议（如 SIP 和 MGCP）的启发。IAX 没有使用两个单独的协议进行信令和媒体传输，而是将它们统一起来，形成了一个独特的协议。IAX 不使用 RTP 进行媒体传输；相反，它将媒体嵌入到同一个 UDP 连接中。

> **[第 2 版注 — Asterisk 22 中的状态]** `chan_iax2` 在 Asterisk 22 LTS 中仍然包含并得到全面支持，因此本章中的所有内容仍然有效。然而，IAX2 现在是一个传统协议，几乎没有新的部署。VoIP 行业已在很大程度上收敛于 SIP（通过 Asterisk 22 中的 `chan_pjsip`），用于提供商中继和服务器互连。IAX2 的主要剩余卖点是其 **单端口 NAT 遍历**：所有信令和媒体都在单个 UDP 端口（默认 4569）上流动，与 SIP + RTP 相比，这极大地简化了防火墙和 NAT 配置。如果您正在构建一个新的 Asterisk 到 Asterisk 中继线且不担心 NAT，则 PJSIP 中继线是推荐的现代方法。IAX2 保留在这里是因为它仍然是一个有效的选择，特别是在只能通过防火墙打开一个 UDP 端口的环境中。
...
IAX 是一种对等信令和媒体协议，类似于不使用 RTP 的 SIP。基本方法是在两个主机之间的单个 UDP 连接上多路复用多媒体流。这种方法最大的好处是当遍历 NAT 上的连接时（通常在 xDSL 调制解调器中发现），其简单性。IAX 使用单个端口（默认 UDP 4569），然后使用带有 15 位的呼叫号码来多路复用所有流。IAX 协议使用类似于 SIP 协议的注册和身份验证过程。可以在 http://www.ietf.org/internet-drafts/draft-guy-iax-05.txt 找到该协议的描述。

![IAX 协议通过单个 UDP 端口（默认 4569）多路复用两个端点之间的许多呼叫，使用 15 位呼叫号码来保持流分开——这使得 NAT 遍历变得简单。](../images/10-legacy-fig12.png)

### 带宽使用

VoIP 网络中使用的带宽受多种因素影响；编解码器和协议头是最重要的。IAX 协议有一个令人惊讶的功能，称为中继模式，通过它可以使用单个报头多路复用多个呼叫。通过使用 Asterisk 带宽计算器，您将看到 IAX 中继线如何通过多个呼叫为您节省高达 80% 的流量。
...
了解通道命名约定很重要，因为在拨号方案中指定通道时将使用这些名称。用于出站通道的 IAX 通道名称格式为：

```
IAX/[<user>[:<secret>]@]<peer>[:<portno>][/<exten>[@<context>][/<options>]
```

<user> 远程对等体上的 UserID，或在 iax.conf 中配置的客户端名称 <secret> 密码。或者，它可以是 RSA 密钥的文件名，不带尾随扩展名（.key 或 .pub），并用方括号括起来 <peer> 要连接的服务器名称 <portno> 连接的端口号 <exten> 远程 Asterisk 服务器中的分机 <context> 远程 Asterisk 服务器中的上下文 <options> 唯一可用的选项是 ‘a’，意思是 ‘请求自动应答’
...
<username> 如果已知，则为用户名 <host> 连接的主机 <callno> 本地呼叫号码 传入通道示例：IAX2[flavio@8.8.30.34]/10 来自 IP 地址 8.8.30.34 的呼叫号码 10，使用 flavio 作为用户。IAX2[8.8.30.50]/11 来自 IP 地址 8.8.30.50 的呼叫号码 11。
...
Asterisk 支持基于 IAX 的 IP 电话，例如 ATCOM 和 Digium 的旧 ATA（称为 IAXy），以及软电话，例如 Zoiper。软电话、ATA 和硬电话的过程类似。要配置 IAX 设备，您需要编辑 /etc/asterisk 中的 iax.conf 文件

```
directory.
```

我们将以 Zoiper (www.zoiper.com) 为例。它是一个功能齐全且免费的软电话。第 1 步：备份原始 iax.conf 文件，使用：
...
在上述命令中，我们禁用了所有编解码器并仅启用了 ulaw。在局域网中，大多数人更喜欢使用 ulaw，因为它不是处理器密集型的，并且节省了 CPU 周期。即使使用更多带宽，此编解码器也是可取的，因为在局域网中，您通常拥有 100 兆位以太网甚至千兆位。使用 ulaw 的语音呼叫从您的网络中使用了近 100 kbps 的带宽，这对当今的高速局域网来说是非常轻的使用。在广域网或互联网网络中，您通常会禁用 ulaw，用一些可用的 CPU 周期换取语音压缩，以实现更好的带宽使用。编解码器 gsm、g729 和 ilbc 也提供了良好的压缩因子。
...
在上述命令中，我们定义了一个名为 [2003] 的朋友。上下文是 default（在最初的实验室中，我们总是使用 default 上下文以避免混淆；此上下文将在第 9 章中全面解释）。“host=dynamic”行提供了电话 IP 地址的动态注册。第 3 步：从以下 URL 下载并安装 Zoiper™：http://www.zoiper.com/ 注意：URL 经常更改。如果您无法在此特定 URL 找到文件，请尝试“谷歌搜索”。您也可以为实验室选择其他软电话。第 4 步：在客户端中配置 IAX 帐户（右键单击 Zoiper 托盘图标 → *Add account* → IAX）。请注意，SipPulse Softphone 仅支持 SIP，无法通过 IAX2 注册，因此对于 IAX 测试，您需要一个仍然支持该协议的客户端。
...
> **[第 2 版注]** 自 Asterisk 16 以来，支持 IAX 的商业 VoIP 提供商数量已显著减少。大多数提供商现在仅提供 SIP/PJSIP 中继线。在选择 IAX 提供商之前，请确认他们积极维护其 IAX 基础设施。对于新的提供商集成，PJSIP 中继线是推荐的替代方案。
...
第 2 步：配置 iax.conf 文件以向您的提供商注册您的 Asterisk。将以下行添加到文件的 [general] 部分。
...
这是 RSA 身份验证所必需的。使用来自提供商的公钥允许您确保收到的呼叫确实来自真正的提供商。如果其他人尝试使用相同的路径，他们将无法对其进行身份验证，因为他们没有相应的私钥。第 4 步：尝试连接。要测试连接，请拨打任何号码。一些供应商提供回声测试。要完成此操作，请编辑文件 extensions.conf。
...
将一台服务器连接到另一台非常容易。您不需要注册它们，因为 IP 地址是已知的。您必须在 iax.conf 文件中创建对等体和用户。总部站点的所有分机都以 20 开头，后跟两位数字（例如 2000）。在分支机构，所有分机都以 22 开头，后跟两位数字（例如 2200）。我们将使用中继线。您将需要一个 DAHDI 定时源来启用此功能。第 1 步：编辑分支服务器中的 iax.conf 文件。

![使用 IAX 中继线连接两台 Asterisk 服务器：总部服务器（192.168.1.1，分机 20xx）和分支服务器（192.168.1.2，分机 22xx）通过单个 IAX 中继线相互到达——不需要注册，因为两个 IP 地址都是固定的且已知的。](../images/10-legacy-fig15.png)
...
现在让我们从实际角度分析 IAX 身份验证过程，以帮助您为每个特定要求选择最佳方法。
...
当 Asterisk 收到传入连接时，初始信息可以包括用户名（来自字段“username=”）或不包括。传入连接也有一个 IP 地址，Asterisk 也使用它进行身份验证。如果提供了用户，Asterisk：1. 在 iax.conf 中搜索 type=user（或 type=friend，且部分名称与用户名匹配）的条目。如果找不到，Asterisk 将拒绝连接。2. 如果找到的条目有 deny/allow 配置，它会将呼叫者的 IP 地址与 deny/allow 子句进行比较，以确定是否接受呼叫。3. 它使用明文、md5 或 RSA 检查密码（secret）。4. 它接受连接并将呼叫发送到 iax.conf 文件中“context=”行中指定的上下文。如果未提供用户名，Asterisk：1. 在 iax.conf 文件中搜索包含 type=user（或 type=friend）且未指定 secret 的条目。它也检查 deny/allow 子句。如果找到条目，则接受连接，并将部分名称用作用户名。2. 在 iax.conf 文件中搜索包含 type=user（或 type=friend）且指定了 secret 或 RSA 密钥的条目。它检查 deny/allow 子句。如果找到条目，它会尝试使用指定的 secret 验证呼叫者；如果匹配，则接受连接。部分名称是用户名。假设您的 iax.conf 文件有以下条目：
...
Asterisk 将仅尝试使用 iax.conf 文件中的相应条目来验证呼叫。如果指定了任何其他名称，呼叫将被拒绝。如果未指定用户，Asterisk 将尝试将连接验证为 guest。但是，如果 guest 不存在，它将尝试任何其他具有匹配 secret 的连接。换句话说，如果您在 iax.conf 文件中没有 guest 部分，恶意用户可以通过不指定用户名来尝试猜测任何匹配的 secret。IP 地址的 deny/allow 限制也适用。避免 secret 猜测的一个好方法是使用 RSA 身份验证。另一种方法是限制允许呼叫的 IP 地址。
...
可以使用带有非对称 RSA 密钥的强身份验证来使用 IAX。根据源代码 (res_krypto.c)，Asterisk 使用带有 SHA-1 算法的 RSA 密钥进行消息摘要，而不是较弱的 MD5。以下是使用 RSA 密钥设置两台服务器的分步指南。
...
当被问及时，使用密钥名称 branch。我们使用了参数 –n 以避免在 Asterisk 重新初始化时传递密码。如果您想提高安全性，请不要使用 –n 并使用 asterisk -i 启动 Asterisk 第 2 步：将密钥复制到目录 /var/lib/asterisk/keys
...
当被问及时，使用密钥名称 hq。第 2 步：将密钥复制到目录 /var/lib/asterisk/keys
...
抖动是数据包之间的延迟变化。它是影响语音质量的最重要因素。抖动缓冲区用于补偿延迟变化。它牺牲延迟以换取更低的抖动。您可以将抖动缓冲区与水箱进行类比。两者都可以以不规则的间隔接收数据包或水，但最终会提供稳定的流量。

![抖动缓冲区作为水箱：数据包从网络不规则地到达并填充缓冲区，然后缓冲区以稳定的速率释放它们以产生平滑的语音流。缓冲区大小（以毫秒为单位）以一点延迟换取更低的抖动；过量缓冲区带允许 Asterisk 随着网络条件的变化增加或减少缓冲区。](../images/10-legacy-fig17.png)

小的抖动（即低于 20 毫秒）通常是难以察觉的。但是，高于此级别的抖动是令人讨厌的。延迟应保持在 150 毫秒以下。创建抖动缓冲区将牺牲一些延迟以换取更低的抖动——这被称为“延迟预算”的概念。您可以使用以下参数影响抖动缓冲区：
...
下面的参数标记服务类型字段中的 IP 数据包。路由器可以读取此标签，从而优先处理流量。Asterisk 为此字段使用 DSCP 代码 (RFC 2474)。允许的值为 CS0、CS1、CS2、CS3、CS4、CS5、CS6、CS7、AF11、AF12、AF13、AF21、AF22、AF23、AF31、AF32、AF33、AF41、AF42、AF43 和 ef（即加速转发）。
...
IAX 支持使用对称密钥、128 位分组密码（称为 AES，高级加密标准）进行呼叫加密。激活 IAX 中继线之间的加密非常简单。在文件 iax.conf 中使用：
...
> **传统/历史：** 本节中的所有内容都使用旧的 `chan_sip` 驱动程序及其 `sip.conf` 配置文件。`chan_sip` 在几个版本中被弃用，并 **在 Asterisk 21 中移除**，因此它 **在 Asterisk 22 中不存在**。下面 `sip.conf` 的示例都不会在当前系统上运行——它们保留在这里仅用于记录传统部署的工作方式并帮助您迁移它们。对于执行任何这些操作的现代、受支持的方式，请参阅 *SIP & PJSIP in depth* 章节中的 *PJSIP: the SIP channel* 部分。SIP *协议* 理论（方法、注册、代理/重定向、SDP、NAT 类型）是协议级的，存在于该章节中；以下纯粹是已移除的 `chan_sip` **配置**。

在 Asterisk 20 之前的传统系统上，SIP 是在 `/etc/asterisk/sip.conf` 中配置的，它曾经是更改次数第二多的文件（仅次于 `extensions.conf`）。以下部分展示了 `chan_sip` 如何将 Asterisk 连接到 SIP 提供商，如何使用 SIP 将两个 Asterisk 连接在一起，域支持、状态、编解码器/DTMF/QoS 选项、身份验证和 NAT——随后是将其全部迁移到 PJSIP 的指南。

### 将 Asterisk 连接到 SIP 提供商 (sip.conf)

Asterisk 通常用于连接到 SIP VoIP 提供商。与传统提供商相比，VoIP 提供商通常拥有更好的电话呼叫费率。VoIP 提供商的另一个有趣且吸引人的点是可以在其他城市——甚至在外国——购买 DID 号码。这些是使用 VoIP 进行电信的好理由。在本节中，您将学习传统 `chan_sip` 如何将 Asterisk 连接到 VoIP 提供商。将 Asterisk 连接到 SIP 提供商需要三个步骤。可以通过在您最喜欢的提供商处建立帐户来进行测试。第 1 步：在 sip.conf 中向 SIP 提供商注册 要连接到 SIP 提供商，您将需要来自提供商的以下信息：
...
第 3 步：在拨号方案中创建到提供商的路由 我们将选择数字 010 作为到提供商的路由目的地。要在提供商内部拨打 #610000，只需拨打 010610000。
...
sip.conf 文件中的注册指令用于向提供商注册。注册事务使用名称和密码进行验证。您可以使用斜杠 (“/”) 为传入呼叫提供分机。从技术上讲，分机将被放置在 SIP 请求的“Contact”头字段中。注册行为可以由某些参数控制：
...
当您连接到 VoIP 提供商时，需要凭据。初始 invite 后，提供商会向您发送一条称为“407 Proxy Authentication Required”的消息；您在随后的 INVITE 消息中提供凭据。对于传入呼叫，您的 Asterisk 服务器将要求提供商提供凭据。显然，提供商没有您的 Asterisk 服务器的有效凭据。当您使用 insecure=invite 时，您是在告诉 Asterisk 不要向提供商发送“407 Proxy Authentication Required”，并接受传入呼叫。您还可以使用 insecure=port, invite 根据 IP 地址匹配对等体，而不匹配端口号。
...
您可以使用 SIP 互连两台 Asterisk 盒子。在继续此配置之前，注意拨号方案非常重要。用户通常希望以最小的努力连接其他 PBX。这里的想法是仅使用分机号码连接到另一个 PBX。第 1 步：编辑服务器 A 中的 sip.conf 文件：
...
此参数允许在未经身份验证的情况下处理外部 invite。它在常规部分或域语句中定义的上下文中处理呼叫。警告：如果您在常规部分中定义了一个可以访问 PSTN 的上下文，则外部用户可以通过您的 PBX 拨打 PSTN。在这种情况下，您将承担任何费用。仅允许您自己的分机在常规部分定义的上下文中。
...
domain 命令允许您在 Asterisk 中处理多个域。如果呼叫来自特定域，它将被定向到特定上下文。
...
本节解释了传统 SIP 通道的一些高级参数，例如状态、编解码器选择、DTMF 选项和 QoS 数据包标记。这些 **概念**（BLF/状态、编解码器协商、DTMF 模式、DSCP 标记）延续到 PJSIP，但此处显示的 `sip.conf` 参数名称在 Asterisk 22 中 **不存在**。在 PJSIP 上，DTMF 模式是端点上的 `dtmf_mode=`，编解码器通过 `allow=`/`disallow=` 设置。

#### SIP 状态 (Presence)

SIP 状态在 Asterisk 中是部分实现的。Asterisk 支持 SUBSCRIBE 和 NOTIFY 用户等请求，具体取决于通道的状态。Asterisk 不支持 SIP 方法 PUBLISH。换句话说，您可以订阅通道的状态（忙碌、空闲和振铃），但不能发布诸如“离开”或“请勿打扰”之类的信息。状态最常见的场景是忙灯场 (BLF)，其中您模拟具有每个分机和中继线灯的 KS 系统的行为。用于状态的 SIP 参数：

- allowsubscribe=yes：允许 SIP 订阅方法
- subscribecontext=sip_subscribers：查找提示的上下文
- notifyring=yes：发送 SIP NOTIFY 振铃
- notifyhold=yes：发送 SIP NOTIFY 保持
...
第 2 步：现在配置软电话以使用状态。我们将向您展示如何配置 X-Lite。

- 序列：右键单击->SIP 帐户设置->属性->状态
- 将状态模型从对等更改为状态代理，这将使软电话订阅 Asterisk 以获取 SIP 事件。

第 3 步：将联系人添加到其他软电话。在此示例中，Xlite 是帐户 2000，因此我们将为帐户 2001 添加一个联系人。序列：打开右侧面板（Xlite 中的状态面板）->单击联系人->添加联系人。填写名称 2001。显示为 2001，不要忘记选中框“显示此联系人的可用性”。
...
QoS 是一组负责语音质量的技术。QoS 的实现旨在减少带宽、延迟和抖动。QoS 是在交换机和路由器中实现的，而不是由 Asterisk 本身实现的。但是，Asterisk 可以通过标记数据包以进行快速交付来帮助路由器和交换机。标记是使用 RFC 2474 和 RFC2475 中定义的区分服务代码点 (DSCP) 完成的。
...
当传统 `chan_sip` 收到 SIP 呼叫时，它遵循下图中描述的规则。三个参数在 SIP 身份验证中发挥了重要作用。在 Asterisk 22 上，身份验证改为使用 PJSIP `auth` 对象（`type=auth`、`auth_type=userpass`、`username=`、`password=`）配置，这些对象由端点引用，IP 访问控制在端点上通过 `permit=`/`deny=` 完成，或通过 `acl` 完成。

![传统 chan_sip 身份验证决策流程：Asterisk 根据 sip.conf 检查 From 头，尝试匹配 type=user/peer 部分和 MD5 凭据，并回退到 insecure=invite 或 allowguest，然后再允许或拒绝呼叫](../images/07-sip-and-pjsip-fig10.png)

```
allowguest=yes/no
```

此参数控制没有相应对等体的用户是否可以在没有名称和密码的情况下进行身份验证。我们在域支持部分讨论了这个参数。

```
insecure=invite,port
```

当我们使用 insecure=invite 时，Asterisk 不会生成消息“407 Proxy Authentication Required”。没有此消息，用户可以在未经身份验证的情况下拨打电话。这通常用于连接到 VoIP 服务提供商。来自 VoIP 服务提供商的呼叫通常未经身份验证。
...
此参数配置身份验证的密码，使用 secret 进行入站请求，使用 remotesecret 进行出站请求。如果您不想在文本文件中显示密码，可以使用 md5secret 包含哈希而不是密码。要生成 MD5 密码，您可以使用：
...
NAT *理论*（四种 NAT 类型、Contact 头问题、保持活动和通过服务器强制媒体）是协议级的，在 *SIP & PJSIP in depth* 章节中介绍。此处显示的 `sip.conf` 参数（`nat=`、`qualify=`、`directmedia=`、`externaddr=`、`localnet=`）是 **传统 chan_sip**，已在 Asterisk 21+ 中移除。在 PJSIP 上，这些映射到传输/端点设置，例如传输上的 `rewrite_contact=yes`、`force_rport=yes`、`rtp_symmetric=yes`、`direct_media=no`、`external_media_address`、`external_signaling_address` 和 `local_net=`，以及 AOR 上的 `qualify_frequency=`。

在传统 chan_sip 中，参数 `nat` 有五个选项：

- nat = no — 不做除 RFC3581 之外的特殊 NAT 处理
- nat = force_rport — 假装有 rport 参数，即使没有
- nat = comedia — 将媒体发送到 Asterisk 从中接收它的端口，无论 SDP 说发送到哪里
- nat = auto_force_rport — 如果 Asterisk 检测到 NAT，则设置 force_rport 选项（默认）
- nat = auto_comedia — 如果 Asterisk 检测到 NAT，则设置 comedia 选项

当您在 sip.conf 文件中放入语句“nat=force_rport”时，您是在告诉 Asterisk 忽略 SIP 头中“Contact”头字段中包含的地址，并使用数据包 IP 头中的源 IP 地址和端口，并且还将媒体发送回从中接收它的地址，忽略 SDP 头的内容。

```
nat=force_rport,comedia
```

有必要保持 NAT 映射打开。如果 NAT 超时，Asterisk 无法向 UAC 发送 invite。UAC 可以发送呼叫，但不能接收呼叫。以下语句可用于保持 NAT 打开。

```
qualify=yes
```

Qualify 将定期使用 OPTIONS 方法发送 SIP 数据包，这将有助于保持 NAT 打开。Qualify 每 60 秒发送一次 OPTIONS，当主机不可达时每 10 秒发送一次。您可以使用“sip show peers”查看对等体的延迟。如果用户的 NAT 是对称类型的，则无法直接从一个 UAC 向另一个发送数据包；在这种情况下，您必须使用以下命令强制 RTP 通过 Asterisk：
...
第 1 步：配置防火墙以将 UDP 端口 5060 静态重定向到 Asterisk 服务器。第 2 步：配置防火墙以将 UDP 端口从 10000 到 20000 静态重定向。如果您想限制打开的端口数量，可以编辑 rtp.conf 文件以更改 RTP 端口范围。另一种方法是使用支持 SIP 协议的智能防火墙来动态打开 RTP 端口。
...
下面显示的 `SIP/...` 拨号字符串技术是已移除的 chan_sip 驱动程序。在 Asterisk 22 上，改用 `PJSIP/...` 技术——例如 `Dial(PJSIP/2000)` 或 `Dial(PJSIP/${EXTEN}@provider)`。形式和含义在其他方面是类似的。
...
因为 `chan_sip` 已在 Asterisk 21 中移除，并且在 Asterisk 22 中消失，所以任何现有的 `sip.conf` 部署都必须迁移到 PJSIP。最大的概念转变是单个 `sip.conf` `[peer]` 或 `[friend]` 被拆分为几个 PJSIP 对象，每个对象都有一个 `type=`：一个 **endpoint**（呼叫/编解码器/媒体设置），一个或多个 **aor** 对象（可以到达设备的地方/注册），一个 **auth** 对象（凭据），以及一个共享的 **transport**（监听套接字，NAT 地址）。下表映射了最常见的概念。
...
Asterisk 附带一个辅助脚本 **`sip_to_pjsip.py`**，它读取现有的 `sip.conf` 并生成 `pjsip.conf`。您可以直接在 /etc/asterisk 目录中运行它。该实用程序位于 Asterisk 源代码树下的 `contrib/scripts/sip_to_pjsip/` 中，其中 `${PATH_TO_ASTERISK_SOURCE}` 是找到 Asterisk 源文件的路径（通常为 /usr/src/asterisk-22.x.y/）：
...
完整的 PJSIP 配置在 *SIP & PJSIP in depth* 章节中介绍，docs.asterisk.org 上的官方文档对该通道有完整的覆盖。在我们的 voip.school 配套实验室中，实验室 5 让您练习刚刚学到的内容。

## 测验

1. 关于两个模拟外汇接口，标记正确的陈述（选择所有适用的）：
...
5. DAHDI 硬件独立于 Asterisk：物理卡在 `/etc/dahdi/system.conf` 中配置，而 `chan_dahdi.conf` 定义 Asterisk 通道，而不是硬件本身。
   - A. 正确
   - B. 错误
6. 关于数字中继线容量和信令，标记正确的陈述（选择所有适用的）：
...
7. 哪个实用程序自动检测 DAHDI 卡并生成 `/etc/dahdi/system.conf` 和 `dahdi-channels.conf`？
   ...
   - C. dahdi_cfg
   - D. generate_dahdi
8. 当将传统 `sip.conf` `[friend]` 迁移到 PJSIP 时，单个块必须拆分为几个对象。哪一组 PJSIP `type=` 对象通常替换一个注册的 `[friend]`？

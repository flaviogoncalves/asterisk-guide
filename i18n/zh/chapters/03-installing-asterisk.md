...
在第一章中，我们了解了 Asterisk 在电话环境中的作用。在本章中，我们将介绍如何下载和安装 Asterisk。在开始之前，学习如何编译和安装它至关重要。对于传统的 Microsoft™ Windows™ 用户来说，编译过程可能看起来很奇怪，但这在 Linux™ 环境中非常普遍。编译 Asterisk 时，可以获得针对您的硬件优化的代码，这也是我们将在此处执行的操作。Asterisk 可以在多种操作系统上运行，但我们选择简化流程，仅从其中一种开始：Linux。我们选择 Debian 作为 Linux™ 发行版，因为其依赖项易于安装，且该发行版稳定、占用资源少。如果您想使用其他发行版，请相应地更改依赖项名称。

本版针对 **Asterisk 22 LTS**（于 2024-10-16 发布；全面支持至 2028-10-16，安全修复至 2029-10-16）。Asterisk 22 是当前的长期支持版本。请注意，Digium 已于 2018 年被 **Sangoma** 收购，Asterisk 现在由 Sangoma 赞助——本章中提到的“Digium”均指代用于传统硬件的旧品牌。
...
Asterisk 运行不需要太高端的硬件，但针对您的需求选择最佳硬件有一些技巧。在选择硬件时，您应考虑以下主要因素：

- 注册用户总数。定义您需要支持的每秒注册次数。
- 并发通话总数。定义您需要在 Asterisk 服务器的网络适配器和网桥中处理的网络会话数量。
...
Asterisk 服务器的主要组件是网络适配器。建议使用优质的服务器网络适配器。当您需要支持 g.729 和 iLBC 等高复杂度 codec 以及回声消除时，CPU 非常重要。您可以选择使用专用 DSP，Sangoma（前身为 Digium）提供了一款名为 TC400B 的 DSP 卡，能够支持 120 路 g729 并发通话。最佳实践是选择知名制造商生产的全新服务器级计算机。要准确了解特定机器可以支持多少并发通话或多少注册用户，您应该使用 SIPP (http://sipp.sourceforge.net) 等压力测试工具对该硬件进行测试。一些硬件制造商（如 Xorcom，http://www.xorcom.com）会在其网站上发布测试结果。注意：某些 Asterisk 应用程序（如 ConfBridge 和 music on hold）需要内部定时源。在现代 Linux 上，这由内置的 `res_timing_timerfd` 模块自动提供——不需要电话硬件。（旧的 `dahdi_dummy` 软件定时器已不存在；其功能在 DAHDI Linux 2.3.0 中已合并到主要的 `dahdi` 内核模块中。）您可以使用 CLI 命令 `timing test` 确认活动的定时器。

### 硬件配置

Asterisk 硬件不需要很复杂。您不需要昂贵的显卡或大量外设。关于硬件配置的一些建议：
...
Asterisk 最初是为在 Linux 上运行而开发的。但是，它也可以在 BSD Unix 或 macOS 上运行。如果您是 Asterisk 新手，请先尝试使用 Linux，因为它要容易得多。Asterisk 官方目标是 RHEL 系列（CentOS/RHEL/Fedora）、Ubuntu 和 Debian。目前不错的实用选择是 **Debian 12**、**Ubuntu 22.04 LTS / 24.04 LTS** 和 **Rocky Linux 9 / AlmaLinux 9** —— CentOS Linux 已停止维护，因此在 RHEL 系列系统上请优先选择 Rocky 或 AlmaLinux。对于本书，我将使用 Ubuntu 24.04 LTS。从下面的官方发布目录下载最新的 24.04 点发布服务器镜像（确切的文件名包含当前点发布版本，例如 `ubuntu-24.04.4-live-server-amd64.iso`）：
...
安装 Asterisk 后，我们将立即安装后续编译 Asterisk 和 DAHDI 驱动程序所需的软件包。首先，我们将向 Debian 指明从何处下载软件包。这是通过使用 apt-setup 实用程序完成的。第 1 步：在虚拟机中安装 Ubuntu 24.04 LTS Server（使用 64 位镜像；本书中使用的发行版均为 64 位，尽管 Asterisk 本身仍支持 32 位 x86）。我们在此培训中使用了 VirtualBox。您可以从 https://releases.ubuntu.com/24.04 下载镜像。Linux 安装超出了本培训的范围。Linux 基础知识是本培训的前提条件。

## 为 Asterisk 安装 Linux

像往常一样安装 Linux，不要安装图形用户界面。同时安装并配置电子邮件服务器。我们需要电子邮件服务器 (exim4) 在本书后面发送语音信箱通知。注意：此安装将格式化您的 PC。您的所有磁盘数据都将被擦除。请确保在开始之前备份所有数据。第 1 步：将 CD 放入 CD-ROM 驱动器并启动您的 PC。大多数问题都非常简单。

## 安装依赖项

要安装 Asterisk 和 DAHDI，您必须安装许多软件依赖项。在 Asterisk 22 中，推荐的方法是使用随源代码树一起提供的脚本，该脚本知道每个受支持发行版的正确软件包名称。下载并解压 Asterisk 源代码（请参阅下文“编译 Asterisk”）后，运行：
...
请注意，Asterisk 源代码现在托管在 Git 上，因此不再需要 `subversion`，现代 Debian/Ubuntu 附带的是 `libncurses-dev` 而不是带版本的 `libncurses5-dev`。优先使用 `./contrib/scripts/install_prereq install` 而不是手动维护的列表，因为该脚本始终跟踪您发行版的正确软件包名称。

### DAHDI

DAHDI (Digium/Sangoma Asterisk Hardware Device Interface) 是用于模拟和数字卡的驱动程序架构。如果您计划使用模拟或数字接口，在安装 Asterisk 之前安装 DAHDI 非常重要。DAHDI 仍然存在于模拟/数字电话卡中，但越来越小众——大多数现代部署都是纯 VoIP，可以完全跳过本节。仅当您有物理电话接口硬件时才安装 DAHDI。使用以下命令获取源代码文件：
...
make install-config DAHDI 已配置完毕。如果您有任何 DAHDI 硬件，现在建议您编辑 /etc/dahdi/modules，以便仅加载此系统中安装的 DAHDI 硬件的支持。默认情况下，所有 DAHDI 硬件的支持都会在 DAHDI 启动时加载。我认为您系统上的 DAHDI 硬件是：usb:004/002 xpp_usb- e4e4:1150 Astribank-multi no-firmware 此屏幕（上方）要求您更改 /etc/dahdi/modules 文件，以仅加载特定配置所需的驱动程序并显示检测到的硬件。编辑 /etc/dahdi/modules 文件并仅加载所需的硬件。在我的例子中，我使用的是一台带有 Xorcom Astribank 6FXS 和 2FXO 的测试机器。文件如下所示。
...
根据经验，您应该使用具有所需功能的版本。Asterisk 遵循交替使用 LTS（长期支持）和标准版本的发布模型。在本版编写时，**Asterisk 22 是当前的 LTS 版本**（2024 年 10 月发布；最新的点发布版本是 22.10.0），这使其成为目前最佳的选择。Asterisk 20 是上一个 LTS 版本，而版本 16（第一版中使用）已停止维护。对于生产系统，请始终选择 LTS 版本。
...
使用 make menuselect 仅安装必要的模块。在 Asterisk 22 中，SIP 通道是 **chan_pjsip**（默认构建）；旧的 **chan_sip** 已在 Asterisk 21 中移除，不再存在。Opus *透传* 开箱即用（树内 `res_format_attr_opus` 模块处理 SDP 协商），但 **codec_opus** 转码模块仍然是来自 Sangoma/Digium 的外部闭源二进制文件——在 menuselect 中选择它会从 Digium 的服务器下载它。该二进制文件是免费的。有关详细信息，请参阅下文“使用 menuselect 选择模块”。
...
`make install` 安装二进制文件和模块，`make samples` 将示例配置文件写入 `/etc/asterisk`，`make config` 为您检测到的发行版安装 SysV init 启动脚本（例如 Debian/Ubuntu 上的 `/etc/init.d/asterisk`），`ldconfig` 刷新共享库缓存。源代码树中在 `contrib/systemd/asterisk.service` 处也提供了一个 systemd 单元，但 `make config` 不会自动安装它——如果您更喜欢在 systemd 下运行 Asterisk，请自行将其复制到位（见下文）。
...
在现代 Linux 发行版（Debian 12、Ubuntu 22.04/24.04、Rocky/AlmaLinux 9）上，系统服务管理器是 **systemd**。Asterisk 在源代码树的 `contrib/systemd/asterisk.service` 处提供了一个 systemd 单元；将其复制到 `/etc/systemd/system/asterisk.service` 并运行 `systemctl daemon-reload`。安装完成后，在生产环境中运行 Asterisk 的推荐方法是通过 `systemctl`：
...
在旧系统上，Asterisk 是通过旧版 SysV init 脚本 (`/etc/init.d/asterisk`) 和 **safe_asterisk** 包装器启动的，如果 Asterisk 崩溃，它会自动重启。使用 systemd，自动重启由单元文件的 `Restart=` 指令处理，因此通常不再需要 `safe_asterisk`。旧版 init/`safe_asterisk` 方法仍然有效，但在基于 systemd 的发行版上已弃用。

### Asterisk 运行时选项

Asterisk 启动过程非常简单。如果 Asterisk 在没有任何参数的情况下运行，它将作为守护进程启动。
...
Asterisk 安装在多个目录中，可以在 asterisk.conf 文件中进行修改。出于培训目的，我会将 verbose 从 3 更改为 15，对于生产环境，请保持为 3。选项 max_calls 和 max_load 是保护系统免受过载的好选项。
...
如果 Asterisk 安装在生产环境中，您应该注意系统设计。服务器必须以电话系统优先于其他系统进程的方式进行优化。Asterisk 不应与处理器密集型软件（如 X-Windows）一起运行。如果您需要运行 CPU 密集型进程（例如大型数据库），请使用单独的服务器。总的来说，Asterisk 对硬件性能变化很敏感。因此，尝试在不需要超过 40% CPU 利用率的硬件环境中使用 Asterisk。

### 网络提示

如果您计划使用 IP 电话，请务必注意您的网络。语音协议非常好，并且能够抵抗延迟甚至抖动；但是，如果您使用配置不当的局域网，语音质量将会受到影响。只有在交换机和路由器中使用服务质量 (QoS) 才能保证良好的语音质量。局域网中的语音往往很好，但即使在 LAN 环境中，如果您有 10 Mbps 的集线器且冲突过多，最终也会导致语音失真或质量低劣。遵循以下建议以确保尽可能好的语音质量：

- 如果可能或经济上可行，请使用端到端 QoS。通过端到端 QoS，语音质量是完美的。没有借口！
- 避免在生产环境中使用 10/100 Mbps 集线器进行语音传输。冲突可能会给网络带来抖动。首选全双工 10/100 Mbps，因为不会发生冲突。
- 使用 VLAN 分隔语音网络中不必要的广播。您不希望病毒通过 ARP 广播破坏您的语音网络。
- 教育用户了解语音网络的预期。如果没有 QoS，不要声称语音会像大多数情况下那样完美。通常会达到类似于手机的语音质量。使用高质量的电话，因为固件和硬件设计方面的问题很常见。

## 总结

在本章中，您了解了最低硬件要求以及如何下载、安装和编译 Asterisk。出于安全原因，Asterisk 应以非 root 用户身份执行。在启动生产环境之前，您应该检查您的网络环境。

## 测验

1. 在 Asterisk 22 中，哪个通道驱动程序提供 SIP 支持，旧的 `chan_sip` 发生了什么？
   - A. `chan_sip` 仍然是默认设置；`chan_pjsip` 是可选的。
...
6. 如果您没有 TDM 接口卡，您仍然拥有用于同步的内部定时源，由 Linux 上的 `res_timing_timerfd` 模块提供。此定时由诸如 ________ 和 ________ 之类的应用程序使用。
...
**答案：** 1 — B · 2 — B · 3 — A · 4 — B · 5 — 从解压的 Asterisk 源代码树中运行 `./contrib/scripts/install_prereq install` · 6 — ConfBridge 和 Music on Hold · 7 — A · 8 — `/etc/asterisk` · 9 — `make samples` · 10 — 安全性（如果 Asterisk 被入侵，可限制损害）

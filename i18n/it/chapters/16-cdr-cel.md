...
Asterisk, come altre piattaforme di telefonia, consente la fatturazione delle chiamate telefoniche. Esistono diversi programmi sul mercato in grado di importare i record generati dai PBX. Tali record vengono utilizzati, tra le altre cose, per verificare la correttezza dell'importo in fattura e per scopi statistici.
...
È possibile modificare il formato CSV modificando il file cdr_custom.conf.
...
- cdr_mysql – Database MySQL (**deprecato**; utilizzare cdr_adaptive_odbc + driver ODBC MySQL)
- cdr_freetds – Database Sybase e MSSQL
...
La registrazione dei CDR viene eseguita per tutti i moduli attivi caricati nel file /etc/asterisk/modules.conf. Se il parametro autoload=yes è impostato, tutti i moduli vengono caricati. Per verificare quali cdr_drivers sono attualmente caricati nel sistema, utilizzare il comando seguente:
...
Se si osserva lo screenshot sopra, sono in esecuzione almeno cdr_adaptive_odbc, cdr_csv, cdr_custom, cdr_manager, cdr_odbc e cdr_sqlite3_custom. Negli ultimi anni, dopo alcune Astricon, è diventato chiaro per me che il team di Asterisk predilige ODBC. È l'unico driver che supporta il connection pooling. Il connection pooling è un grande vantaggio in termini di prestazioni perché non è necessario aprire una nuova connessione per ogni operazione. Questo capitolo era stato precedentemente scritto utilizzando cdr_mysql. Per questa edizione sono passato a cdr_adaptive_odbc, pur sapendo che è un po' più complesso da configurare. La scelta di cdr_adaptive_odbc ci consente anche di personalizzare il CDR. È sufficiente impostare una nuova variabile CDR nel dialplan e aggiungere la colonna al database. Set(CDR(jitter)=
...
> **[Nota 2ª ed.]** I passaggi originali erano destinati a Ubuntu 18.04 e MySQL Connector/ODBC 8.0.14. I passaggi seguenti sono aggiornati per Ubuntu 22.04 LTS; regolare le versioni dei pacchetti e gli URL di download per corrispondere alla versione corrente di MySQL Connector/ODBC da dev.mysql.com.

Mi pento sempre di pubblicare istruzioni dettagliate nel libro. A volte cambiano prima ancora che il libro venga pubblicato. Le versioni cambiano, i moduli cambiano, quindi cercate di adattare il comando qui alla vostra situazione. Il più delle volte, piccole modifiche sono sufficienti per riprodurre l'installazione. Prestate attenzione ai passaggi, anche gli utenti Linux esperti troveranno difficile installare i driver ODBC.
...
Passaggio 4: Scaricare il connettore ODBC MySQL da Oracle. Controllare il proprio sistema operativo utilizzando: `lsb_release -a`. Per Ubuntu 22.04 (x86_64), visitare https://dev.mysql.com/downloads/connector/odbc/ e scegliere la versione 8.x o 9.x corrente per Ubuntu 22.04.

> **[Nota 2ª ed.]** Verificare l'URL di download esatto e il nome del file su dev.mysql.com; il numero di versione e il suffisso Ubuntu differiranno dalla 1ª edizione. L'esempio seguente utilizza una versione segnaposto.
...
Passaggio 6 - Configurare il connettore ODBC, modificare il file /etc/odbc.ini per creare il DSN (Data Source Name)
...
Per favore, non procedere con la configurazione di Asterisk se non è possibile visualizzare il risultato del comando isql.
...
> **[Nota 2ª ed.]** Il testo originale utilizzava l'applicazione `NoCDR()`. `NoCDR` è stata deprecata e successivamente rimossa in Asterisk 21; utilizzare invece `Set(CDR_PROP(disable)=1)`.
...
> **[Nota 2ª ed.]** Il testo originale faceva riferimento a `cdr_addon_mysql` e `cdr_mysql.conf`. Il modulo `cdr_mysql` è deprecato in Asterisk 22; il percorso consigliato è `cdr_adaptive_odbc` con un driver ODBC MySQL, che supporta i campi utente nativamente tramite la mappatura adattiva delle colonne.
...
- a – Imposta il codice conto come password.
- d – Interpreta il parametro come una chiave del database Asterisk
- r – Rimuove la chiave dopo un'autenticazione riuscita (solo con l'opzione 'd')
- j – Salta alla priorità n+101 per autenticazione non valida
...
Se viene specificata una casella vocale, solo la password della casella vocale sarà considerata valida. Se la casella vocale non è specificata, verrà impostata una variabile di canale AUTH_MAILBOX con la casella vocale autenticata. Se l'opzione 's' (silent) è impostata, non verrà eseguito alcun prompt. Esempio: (Chiamate internazionali)
...
I record CDR forniscono una riga di riepilogo per chiamata. Per un tracciamento degli eventi più dettagliato — come le transizioni di stato dei singoli canali, gli eventi di ingresso/uscita dal bridge e le tratte di trasferimento assistito — Asterisk 22 include **Channel Event Logging (CEL)**, configurato tramite `/etc/asterisk/cel.conf` e archiviato tramite backend come `cel_odbc` o `cel_custom`.
...
> **[Nota 2ª ed.]** Considerare l'aggiunta di una breve sottosezione CEL o un riferimento a un capitolo avanzato sulla fatturazione se il programma lo prevede. Il pattern di configurazione `cel.conf` rispecchia `cdr.conf`.
...
6. La differenza tra `Set(CDR_PROP(disable)=1)` e `ResetCDR()` è che disabilitare il CDR impedisce la scrittura di qualsiasi record, mentre `ResetCDR()` reimposta (azzera) il record corrente. (L'applicazione `NoCDR()` che in precedenza disabilitava i CDR è stata rimossa in Asterisk 21.)
   - A. Falso
   - B. Vero
...
8. I tre metodi di autenticazione disponibili per l'applicazione Authenticate() sono:
   - A. Password
   - B. File di password
   - C. Database Asterisk (dbput e dbget)

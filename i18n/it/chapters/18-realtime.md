...
- Maggiore utilizzo di memoria per un elevato volume di utenti
...
ARA, o Asterisk Realtime, come è noto, è stato creato da Anthony Minessale II, Mark Spencer e Constantine Filin ed è stato progettato per consentire un'integrazione trasparente con database SQL. È disponibile anche un'interfaccia LDAP. Questo sistema è noto anche come Asterisk External Configuration ed è configurato in /etc/asterisk/extconfig.conf. È possibile mappare i file di configurazione su tabelle in un database (configurazione statica) e voci in tempo reale per la creazione dinamica di oggetti senza la necessità di ricaricare Asterisk.

## Obiettivi

Al termine di questo capitolo, il lettore dovrebbe essere in grado di:

- Comprendere vantaggi e limitazioni di Asterisk Real Time.
...
La sezione di configurazione statica è dove memorizzi l'equivalente dei file di configurazione nel database. Queste configurazioni vengono lette durante il caricamento di Asterisk. Alcuni moduli rileggono il database quando effettui un reload. Esempi di configurazione statica sono:
...
Sopra sono descritti tre esempi. Nel primo, colleghi queues.conf a una tabella queues nel database asteriskdb. Nel secondo esempio, colleghi pjsip.conf alla tabella pjsip_conf nel database asteriskdb definito nella configurazione odbc. Nell'ultimo esempio, colleghi iax.conf a una directory LDAP. MyBaseDN è il DN di base da ricercare. Nell'esempio precedente, l'applicazione app_queue.so viene caricata mentre il driver MySQL interroga il database e ottiene le informazioni richieste.
...
Qui abbiamo cinque righe di configurazione. Nella prima riga, colleghi la famiglia PJSIP/Sorcery `ps_endpoints` a una tabella `ps_endpoints` nel database asteriskdb. Nell'ultima, colleghi la famiglia voicemail alla tabella test nel database asteriskdb. Ogni tipo di oggetto PJSIP (endpoint, aor, auth, contact) ottiene la propria famiglia e tabella; l'insieme completo è mostrato nella sezione "PJSIP Realtime (Sorcery)" qui sotto. Le famiglie `voicemail`, `extensions`, `queues` e `queue_members` sono ancora valide in Asterisk 22.

## PJSIP Realtime (Sorcery)

Su Asterisk 22, gli endpoint SIP sono gestiti esclusivamente dallo stack **PJSIP** (`res_pjsip`), che è costruito sul layer di astrazione degli oggetti **Sorcery**. Piuttosto che un singolo "peer" SIP, PJSIP divide un account SIP in diversi tipi di oggetti, ognuno memorizzato nella propria tabella realtime:
...
Asterisk fornisce migrazioni di database per tutti i suoi schemi realtime sotto `contrib/ast-db-manage`. Questo è il modo supportato per creare (e aggiornare la versione) delle tabelle PJSIP — non scriverai più a mano le definizioni delle tabelle `ps_*`. Il set di migrazione `config` contiene le tabelle PJSIP/Sorcery.
...
Dopo aver inserito le righe non c'è nulla da ricaricare — il successivo REGISTER/INVITE estrae gli oggetti dal database. Puoi confermare ciò che il realtime ha restituito con:
...
Una tabella voicemail segue la stessa idea. Le sue colonne mappano i campi `voicemail.conf`:

| uniqueid | mailbox | context | password | email | fullname |
|----------|---------|---------|----------|-------|----------|
| 1 | 4000 | default | 4000 | john@doe.com | John Doe |

Il `uniqueid` dovrebbe essere univoco per ogni utente voicemail e può essere autoincrement. Non deve avere alcuna relazione con la mailbox o il context.

### Costruire un dial plan utilizzando Asterisk Real Time

Puoi anche utilizzare il sistema real-time per creare il dial plan. ARA utilizza l'istruzione `switch` per includere le estensioni real-time nel normale dial plan contenuto nel file extensions.conf. La tabella delle estensioni dovrebbe apparire come quella qui sotto:

| context | exten | priority | app | appdata |
|---------|-------|----------|-----|---------|
| from-internal | 4000 | 1 | Dial | PJSIP/4000 |

La famiglia realtime `extensions` è invariata in Asterisk 22; assicurati solo che la colonna `appdata` componga i canali PJSIP, ad esempio `PJSIP/4000`. Nel dial plan, devi usare il comando `switch` per utilizzare il real time.
...
Il set di migrazione Alembic `config` costruisce le tabelle PJSIP `ps_*` (insieme a `voicemail`, `extensions` e gli altri schemi realtime) con esattamente le colonne che la versione di Asterisk in esecuzione si aspetta, quindi lo schema corrisponde sempre alla build.
...
Dovresti vedere le tabelle PJSIP `ps_*` (create dalla migrazione Alembic `config`), insieme alle tabelle `voicemail`, `extensions` e altre tabelle realtime:
...
Passaggio 6: Installa phpMyAdmin per gestire le attività del database

```
apt-get install phpmyadmin
```

Di seguito sono riportati due screenshot della schermata di accesso dell'utility e della schermata della tabella. Usa astdb/supersecret come nome utente e password.

> **[Nota 2a ed.]** Sostituisci con uno screenshot attuale dell'amministrazione del database (phpMyAdmin/Adminer), o converti questi passaggi in SQL semplice (CREATE TABLE/INSERT).
...
Passaggio 2: Test dell'estensione Real Time. Usando phpMyAdmin, crea un nuovo endpoint `6010` inserendo una riga in ciascuna di `ps_auths`, `ps_aors` e `ps_endpoints`, quindi prova a registrare questo endpoint con un softphone.
...
Le restanti impostazioni dell'account sono distribuite tra gli oggetti PJSIP. Context, codec, modalità DTMF e gestione dei media risiedono sull'endpoint; la registrazione dinamica risiede sull'AOR:
...
In PJSIP la modalità DTMF RFC 2833 / RFC 4733 è chiamata `rfc4733` (`dtmf_mode=rfc4733`, il valore predefinito). Non esiste un flag "dinamico" separato per la registrazione: un AOR accetta REGISTER dinamici finché consente almeno un contatto (`max_contacts` maggiore di zero), e ogni posizione registrata viene scritta su `ps_contacts`.

Passaggio 3: Prova a registrare il nuovo telefono con un softphone utilizzando il nome utente `6010` e la password `supersecret`. Conferma la registrazione sulla CLI di Asterisk:
...
Usa supersecret quando richiesto. Usa phpMyAdmin per includere un'estensione nel database. Se preferisci, usa invece i seguenti comandi nell'interfaccia client MySQL.
...
Passaggio 6: Riconfigura uno dei telefoni sul nome utente `bria`, se non l'hai già fatto.

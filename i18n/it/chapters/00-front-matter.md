...
Il concetto di PBX open source è rivoluzionario. Per decenni, la telefonia è stata dominata da una manciata di aziende che vendevano costosi sistemi proprietari. Asterisk ha restituito quel potere nelle mani degli utenti: funzionalità che un tempo erano economicamente fuori portata — CTI (computer-telephony integration), IVR (interactive voice response), ACD (automatic call distribution), voicemail e molto altro — sono ora disponibili per chiunque abbia una macchina Linux e la volontà di imparare.
...
Questo libro è destinato ai lettori che sono nuovi ad Asterisk. Do per scontato che tu abbia familiarità con Linux — la shell, un editor di testo e l'amministrazione di base del sistema. Puoi seguire il percorso su un desktop Linux se è più facile durante l'apprendimento, e una macchina virtuale va bene per i laboratori (aspettati una qualità vocale leggermente inferiore). Per i sistemi di produzione non consiglio di eseguire Asterisk su un ambiente desktop o all'interno di una VM con poche risorse. Una certa familiarità con le reti IP, Voice over IP (VoIP) e i concetti base di telefonia sarà utile.
...
- **Gestione Sangoma.** Digium è stata acquisita da Sangoma nel 2018; il progetto è ora sviluppato e sponsorizzato da Sangoma, e il testo riflette questo aspetto in tutto il libro.
...
- **Una nuova struttura.** Il libro è ora organizzato in quattro parti: Foundations, Channels & Connectivity, Dialplan & Call Features e Integration & Operations.
...
Cerco con impegno di trovare ed eliminare gli errori, ma alcuni sfuggono sempre. Se trovi qualcosa di sbagliato, per favore fammelo sapere e interverrò.

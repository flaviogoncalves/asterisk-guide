...
> **[Nota 2a ed.]** Assegnare un **nuovo ISBN** per la seconda edizione — non riutilizzare l'ISBN della prima edizione (9781796396973). Impostare qui anche la data di pubblicazione.
...
Questo libro è destinato a lettori che si avvicinano per la prima volta ad Asterisk. Do per scontato che tu abbia familiarità con Linux — la shell, un editor di testo e l'amministrazione di base del sistema. Puoi seguire il percorso su un desktop Linux se è più facile durante l'apprendimento, e una macchina virtuale va bene per i laboratori (aspettati una qualità vocale leggermente inferiore). Per i sistemi di produzione, non consiglio di eseguire Asterisk su un ambiente desktop o all'interno di una VM con poche risorse. Una certa familiarità con le reti IP, Voice over IP (VoIP) e i concetti base di telefonia sarà utile.
...
- **PJSIP è l'unico canale SIP.** `chan_sip` è stato rimosso in Asterisk 21 e non esiste nella 22. Ogni esempio SIP ora utilizza PJSIP (`pjsip.conf`); il materiale legacy `sip.conf` è mantenuto solo come riferimento per la migrazione.
...
- **Funzionalità modernizzate.** ConfBridge sostituisce la vecchia conferenza MeetMe, ARI viene introdotto insieme ad AMI/AGI, PJSIP Realtime (Sorcery) è trattato, e i capitoli su installazione, sicurezza e CDR sono stati aggiornati.
...
Flavio E. Gonçalves è nato nel 1966 in Brasile. Ha nutrito un forte interesse per i computer fin da quando ha ottenuto il suo primo PC nel 1983 e ha conseguito una laurea in ingegneria nel 1989 con specializzazione in progettazione e produzione assistita da computer. È CEO di SipPulse in Brasile, un'azienda dedicata a softswitch, session border controller e PBX multitenant.

Nel corso della sua carriera ha ottenuto una lunga lista di certificazioni: Novell MCNE/MCNI, Microsoft MCSE/MCT, Cisco CCSP/CCNP/CCDP e Asterisk dCAP tra queste. Ha iniziato a scrivere di software open source perché crede che il modo strutturato in cui le certificazioni insegnavano il materiale sia un ottimo modo per imparare, e ha attinto a più di 25 anni di esperienza nell'insegnamento per scrivere basandosi su come le persone imparano realmente, piuttosto che solo da un punto di vista tecnico.
...
Cerco con impegno di trovare ed eliminare gli errori, ma alcuni sfuggono sempre. Se trovi qualcosa di sbagliato, fammelo sapere e interverrò.

Questo libro è utilizzato anche come materiale didattico. Se desideri utilizzarlo nel tuo centro di formazione, o seguire il corso online e i laboratori associati, visita **VoIP School Blackbelt** su <https://voip.school> o invia un'email a <flavio@voip.school>.

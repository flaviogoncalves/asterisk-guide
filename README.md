...
- **`asterisk-guide-pt.*` / `asterisk-guide-es.*`** — Portuguese (pt-BR) and Spanish editions,
  machine-translated with Gemini and built the same way (figures stay in English).
...
> locally with `BOOK_LANG=pt bash book/build.sh clean`.

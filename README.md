...
The book is **free to download** in **9 languages**. Tagged releases (`v*`) attach:

- **`asterisk-guide-sponsored[-<lang>].pdf` and `.epub`** — the free, **sponsored** editions.
  Every language ships **both** formats: English plus **pt** (Portuguese), **es** (Spanish),
  **fr** (French), **de** (German), **it** (Italian), **hi** (Hindi), **zh** (Simplified
  Chinese), **ja** (Japanese). Sponsored by **[SipPulse](https://www.sippulse.com)** — the SIP
  authority behind **200+ operators** — plus per-lab sponsors; the sponsor pages and slogan are
  translated into each language. Figures stay in English; Hindi/Chinese/Japanese use Noto
  Devanagari/CJK fonts.
...
> Translations are generated by `book/translate/translate.py` (cheapest Gemini model, cached by
> content hash). CI builds them when the `GEMINI_API_KEY` repository secret is set. Build one
> locally with `BOOK_LANG=pt bash book/build.sh` (sponsored PDF + EPUB).

...
Asterisk was initially developed to run on Linux. However, it can also run on BSD Unix or macOS. If you are new to Asterisk, try using Linux first since it is much easier. Several Linux distributions are officially supported and tested with Asterisk 22; choose one for your system. The currently supported distributions are **Debian 12**, **Ubuntu 22.04 LTS / 24.04 LTS**, and **Rocky Linux 9 / AlmaLinux 9**. Note that CentOS is effectively end-of-life — on RHEL-family systems use Rocky Linux or AlmaLinux 9 instead. For this book I will use Ubuntu 24.04 LTS. You can download the distribution from the address below:
